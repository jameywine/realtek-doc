 ...
 *
 * $Revision: 80927 $
 * $Date: 2017-07-31 19:49:19 +0800 (Mon, 31 Jul 2017) $
 *
...
diag_acl_field_t diag_acl_field_list[] =
{
    ...
    {USER_FIELD_VACL_ROUTING_HIT,           "vacl-routing",         "VACL routing action hit"},
#endif
    ...
    {USER_FIELD_IGR_UNIT,                   "igr-unit",             "ingress unit id"},
    ...
    {USER_FIELD_EGR_UNIT,                   "egr-unit",             "egress unit id"},
...
#ifdef CMD_ACL_SET_ENTRY_PHASE_PHASE_ENTRY_INDEX_ACTION_COPY_UNIT_UNITID_PORT_PORT
/*
 * acl set entry phase <UINT:phase> entry <UINT:index> action copy unit <UINT:unitId> port <UINT:port>
 */
cparser_result_t
cparser_cmd_acl_set_entry_phase_phase_entry_index_action_copy_unit_unitId_port_port(
    cparser_context_t *context,
    uint32_t *phase_ptr,
    uint32_t *index_ptr,
    uint32_t *unitId_ptr,
    uint32_t *port_ptr)
{
    ...
    switch (phase)
    {
        ...
        case ACL_PHASE_VACL:
            ...
            action.vact.fwd_data.fwd_type = ACL_ACTION_FWD_COPY_TO_PORTID;
            action.vact.fwd_data.unit = *unitId_ptr;
            action.vact.fwd_data.fwd_info = *port_ptr;
            break;
        case ACL_PHASE_IACL:
            action.iact.fwd_data.flags &= ~RTK_ACL_FWD_FLAG_TRUNK;
            action.iact.fwd_data.fwd_type = ACL_ACTION_FWD_COPY_TO_PORTID;
            action.iact.fwd_data.unit = *unitId_ptr;
            action.iact.fwd_data.fwd_info = *port_ptr;
            break;
        #endif  /* (defined(CONFIG_SDK_RTL9300) || defined(CONFIG_SDK_RTL9310)) */
        default:
...
#ifdef CMD_ACL_SET_ENTRY_PHASE_PHASE_ENTRY_INDEX_ACTION_REDIRECT_UNIT_UNITID_PORT_PORT
/*
 * acl set entry phase <UINT:phase> entry <UINT:index> action redirect unit <UINT:unitId> port <UINT:port>
 */
cparser_result_t
cparser_cmd_acl_set_entry_phase_phase_entry_index_action_redirect_unit_unitId_port_port(
    cparser_context_t *context,
    uint32_t *phase_ptr,
    uint32_t *index_ptr,
    uint32_t *unitId_ptr,
...
#ifdef CMD_ACL_SET_ENTRY_PHASE_PHASE_ENTRY_INDEX_ACTION_TUNNEL_STATE_DISABLE_ENABLE
/*
 * acl set entry phase <UINT:phase> entry <UINT:index> action tunnel state ( disable | enable )
 */
cparser_result_t
cparser_cmd_acl_set_entry_phase_phase_entry_index_action_tunnel_state_disable_enable(
    cparser_context_t *context,
    uint32_t *phase_ptr,
    uint32_t *index_ptr)
{
    ...
    action.vact.tnl_en = enable;

    DIAG_UTIL_ERR_CHK(rtk_acl_ruleAction_set(unit, phase, *index_ptr, &action), ret);

    return CPARSER_OK;
}   /* end of cparser_cmd_acl_set_entry_phase_phase_entry_index_action_tunnel_state_disable_enable */
#endif

#ifdef CMD_ACL_SET_ENTRY_PHASE_PHASE_ENTRY_INDEX_ACTION_TUNNEL_IP_CAPWAP_TNLIDX
/*
 * acl set entry phase <UINT:phase> entry <UINT:index> action tunnel ( ip | capwap ) <UINT:tnlIdx>
 */
cparser_result_t
cparser_cmd_acl_set_entry_phase_phase_entry_index_action_tunnel_ip_capwap_tnlIdx(
    cparser_context_t *context,
    uint32_t *phase_ptr,
    uint32_t *index_ptr,
    uint32_t *tnlIdx_ptr)
{
    ...
    if ('i' == TOKEN_CHAR(9, 0))
        action.vact.tnl_data.type = RTK_ACL_TNL_IP;
    else
        action.vact.tnl_data.type = RTK_ACL_TNL_CAPWAP;

    action.vact.tnl_data.idx = *tnlIdx_ptr;

    DIAG_UTIL_ERR_CHK(rtk_acl_ruleAction_set(unit, phase, *index_ptr, &action), ret);

    return CPARSER_OK;
}   /* end of cparser_cmd_acl_set_entry_phase_phase_entry_index_action_tunnel_ip_capwap_tnlIdx */
#endif
...
_diag_acl_vAct_show(uint32 unit, rtk_acl_vAct_t *action)
{
            ...
            DIAG_OM_GET_FAMILYID(RTL9310_FAMILY_ID))
    {
        ...
        if (ENABLED == action->fwd_en)
        {
            ...
            switch (action->fwd_data.fwd_type)
            {
                ...
                case ACL_ACTION_FWD_COPY_TO_PORTID:
                    ...
                    {
                        diag_util_mprintf("copy unit %d port:%d\n",
                                action->fwd_data.unit, action->fwd_data.fwd_info);
                    }
                    break;
                case ACL_ACTION_FWD_COPY_TO_PORTMASK:
                    diag_util_mprintf("copy multi index:%d\n", action->fwd_data.fwd_info);
                ...
                case ACL_ACTION_FWD_REDIRECT_TO_PORTID:
                    ...
                    {
                        diag_util_mprintf("redirect unit %d port:%d\n",
                                action->fwd_data.unit, action->fwd_data.fwd_info);
                    }
                    break;
                case ACL_ACTION_FWD_REDIRECT_TO_PORTMASK:
       ...
       /* tag status action */
        ...
        if (DIAG_OM_GET_FAMILYID(RTL9310_FAMILY_ID))
        {
            diag_util_mprintf("\tTunnel action state: ");
            if (ENABLED == action->tnl_en)
            {
                diag_util_mprintf("%s\n", DIAG_STR_ENABLE);
                if (RTK_ACL_TNL_IP == action->tnl_data.type)
                {
                    diag_util_mprintf("\t\tIP tunnel\n");
                }
                else
                {
                    diag_util_mprintf("\t\tCAPWAP tunnel\n");
                }

                diag_util_mprintf("\t\ttunnel index: %d\n", action->tnl_data.idx);
            }   /* end of if (ENABLED == action->tnl_en) */
            else
                diag_util_mprintf("%s\n", DIAG_STR_DISABLE);
        }
        #endif
                                        ...
                                        diag_acl_dumpEntInfo_t *pAclDumpEntInfo)
{
    ...
    osal_memset(field_data, 0, sizeof(field_data));
    osal_memset(field_mask, 0, sizeof(field_mask));
    ret = rtk_acl_ruleEntryField_get(unit, phase, entry_idx, pEntry_buffer,
                USER_FIELD_TEMPLATE_ID, field_data, field_mask);
    if (ret != RT_ERR_OK)
    {
        diag_util_printf("rtk_acl_ruleEntryField_get() fail!\n");
        DIAG_ERR_PRINT(ret);
        goto EXIT;
    }

    template_id = (uint32)field_data[0];
    ...
    if (pAclDumpEntInfo->disp_hdr == FALSE)
    {
        ...
        for (i = 0; i < template_info.field_num; i++)
        {
            diag_util_printf("f%-*d", (width-1), (template_info.field_num-i-1));
        }
        diag_util_mprintf("\n");
    ...
    for (i = 0; i < template_info.field_num; i++)
    {
        str = diag_pie_templateAbbreviateName_get(template_info.field[(template_info.field_num-i-1)]);
        if (strlen(str) >= width)
        {
            strBuf[0] = '!';
