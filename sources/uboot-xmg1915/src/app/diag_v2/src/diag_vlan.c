...
#ifdef CMD_VLAN_SET_PROFILE_ENTRY_INDEX_SA_LEARNING_STATE_DISABLE_ENABLE
/*
    ...
    uint32_t *vid_ptr)
{
    ...
    if(DIAG_OM_GET_FAMILYID(RTL9300_FAMILY_ID) || DIAG_OM_GET_FAMILYID(RTL9310_FAMILY_ID))
    {
...
#if 0   /* should use MCAST group commands to get the info */
        char macStr[32];
        uint32 cnt = 3;
        uint32 i = 0;
        ...
        /* Dump bssid list */
        if (RT_ERR_OK == (ret = rtk_vlan_bssid_get(unit, *vid_ptr, CAPWAP_MCAST_BSSID_ARRAY_SIZE_MAX, bssid, &cnt)) )
        {
            for (i = 0; i < cnt ; ++i)
            {
                osal_memset(&macStr, 0, sizeof(macStr));
                DIAG_UTIL_MAC2STR(macStr, bssid[i].octet);
                DIAG_UTIL_MPRINTF("  Capwap bssid :%s\n", macStr);
            }
        }
        else
        {
    ...
    uint32_t *index_ptr)
{
    ...
    if (DIAG_OM_GET_FAMILYID(RTL9300_FAMILY_ID) || DIAG_OM_GET_FAMILYID(RTL9310_FAMILY_ID))
    {
        ...
        DIAG_UTIL_MPRINTF("Priority                  : %u\n", entry.pri_new);
  #if defined(CONFIG_SDK_RTL9310)
        DIAG_UTIL_MPRINTF("Capwap tunnel state       : %s\n", entry.capwap_ts ? DIAG_STR_ENABLE : DIAG_STR_DISABLE);
        osal_memset(macStr, 0, sizeof(macStr));
        DIAG_UTIL_MAC2STR(macStr, entry.bssid.octet);
...
#ifdef CMD_VLAN_GET_VLAN_CONVERSION_EGRESS_RANGE_CHECK_SET_SET_ID_ENTRY_ENTRY_INDEX_VID
/*
 * vlan get vlan-conversion egress range-check set <UINT:set_id> entry <UINT:entry_index> vid
 */
cparser_result_t
cparser_cmd_vlan_get_vlan_conversion_egress_range_check_set_set_id_entry_entry_index_vid(
    cparser_context_t *context,
    uint32_t *set_id_ptr,
    uint32_t *entry_index_ptr)
{
...
#ifdef CMD_VLAN_SET_VLAN_CONVERSION_EGRESS_RANGE_CHECK_SET_SET_ID_ENTRY_ENTRY_INDEX_VID_INNER_OUTER_LOWER_UPPER
/*
 * vlan set vlan-conversion egress range-check set <UINT:set_id> entry <UINT:entry_index> vid ( inner | outer ) <UINT:lower> <UINT:upper>
 */
cparser_result_t
cparser_cmd_vlan_set_vlan_conversion_egress_range_check_set_set_id_entry_entry_index_vid_inner_outer_lower_upper(
    cparser_context_t *context,
    uint32_t *set_id_ptr,
    uint32_t *entry_index_ptr,
    uint32_t *lower_ptr,
...
#ifdef CMD_VLAN_SET_MAC_BASED_VLAN_ENTRY_INDEX_CAPWAP_TUNNEL_STATE_DISABLE_ENABLE
/*
 * vlan set mac-based-vlan entry <UINT:index> capwap-tunnel state ( disable | enable )
 */
cparser_result_t
cparser_cmd_vlan_set_mac_based_vlan_entry_index_capwap_tunnel_state_disable_enable(
    cparser_context_t *context,
    uint32_t *index_ptr)
{
    uint32 unit;
    int32 ret = RT_ERR_FAILED;
    rtk_vlan_macVlanEntry_t vm;

    DIAG_UTIL_FUNC_INIT(unit);
    osal_memset(&vm, 0, sizeof(vm));
    DIAG_UTIL_ERR_CHK(rtk_vlan_macBasedVlanEntry_get(unit, *index_ptr, &vm), ret);
    DIAG_UTIL_PARSE_STATE(7, vm.capwap_ts);
    DIAG_UTIL_ERR_CHK(rtk_vlan_macBasedVlanEntry_set(unit, *index_ptr, &vm), ret);
    DIAG_UTIL_MPRINTF("mac vlan entry %u tunnel state set to %s\n", *index_ptr, text_state[vm.capwap_ts]);

    return CPARSER_OK;
}   /* end of cparser_cmd_vlan_set_mac_based_vlan_entry_index_capwap_tunnel_state_disable_enable */
#endif

#ifdef CMD_VLAN_SET_MAC_BASED_VLAN_ENTRY_INDEX_CAPWAP_TUNNEL_BSSID_BSSID
/*
 * vlan set mac-based-vlan entry <UINT:index> capwap-tunnel bssid <MACADDR:bssid>
 */
cparser_result_t
cparser_cmd_vlan_set_mac_based_vlan_entry_index_capwap_tunnel_bssid_bssid(
    cparser_context_t *context,
    uint32_t *index_ptr,
    cparser_macaddr_t *bssid_ptr)
{
    ...
    osal_memset(&vm, 0, sizeof(vm));
    DIAG_UTIL_ERR_CHK(rtk_vlan_macBasedVlanEntry_get(unit, *index_ptr, &vm), ret);
    osal_memcpy((void*)&vm.bssid, (void*)bssid_ptr, ETHER_ADDR_LEN);
    DIAG_UTIL_ERR_CHK(rtk_vlan_macBasedVlanEntry_set(unit, *index_ptr, &vm), ret);
    DIAG_UTIL_MPRINTF("mac vlan entry %u bssid set to %02x:%02x:%02x:%02x:%02x:%02x\n",
        *index_ptr, bssid_ptr->octet[0], bssid_ptr->octet[1], bssid_ptr->octet[2],
        bssid_ptr->octet[3], bssid_ptr->octet[4], bssid_ptr->octet[5]);

    return CPARSER_OK;
}   /* end of cparser_cmd_vlan_set_mac_based_vlan_entry_index_capwap_tunnel_bssid_bssid */
#endif

#ifdef CMD_VLAN_SET_IP_SUBNET_BASED_VLAN_ENTRY_INDEX_CAPWAP_TUNNEL_STATE_DISABLE_ENABLE
/*
 * vlan set ip-subnet-based-vlan entry <UINT:index> capwap-tunnel state ( disable | enable )
 */
cparser_result_t
cparser_cmd_vlan_set_ip_subnet_based_vlan_entry_index_capwap_tunnel_state_disable_enable(
    cparser_context_t *context,
    uint32_t *index_ptr)
{
    uint32 unit;
    ...
    DIAG_UTIL_ERR_CHK(rtk_vlan_ipSubnetBasedVlanEntry_get(unit, *index_ptr, &entry), ret);
    DIAG_UTIL_PARSE_STATE(7, entry.capwap_ts);
    DIAG_UTIL_ERR_CHK(rtk_vlan_ipSubnetBasedVlanEntry_set(unit, *index_ptr, &entry), ret);
    DIAG_UTIL_MPRINTF("ip subnet vlan entry %u tunnel state set to %s\n", *index_ptr, text_state[entry.capwap_ts]);

    return CPARSER_OK;
}   /* end of cparser_cmd_vlan_set_ip_subnet_based_vlan_entry_index_capwap_tunnel_state_disable_enable */
#endif

#ifdef CMD_VLAN_SET_IP_SUBNET_BASED_VLAN_ENTRY_INDEX_CAPWAP_TUNNEL_BSSID_BSSID
/*
 * vlan set ip-subnet-based-vlan entry <UINT:index> capwap-tunnel bssid <MACADDR:bssid>
 */
cparser_result_t
cparser_cmd_vlan_set_ip_subnet_based_vlan_entry_index_capwap_tunnel_bssid_bssid(
    cparser_context_t *context,
    uint32_t *index_ptr,
    cparser_macaddr_t *bssid_ptr)
{
    ...
    osal_memset(&entry, 0, sizeof(entry));
    DIAG_UTIL_ERR_CHK(rtk_vlan_ipSubnetBasedVlanEntry_get(unit, *index_ptr, &entry), ret);
    osal_memcpy((void*)&entry.bssid, (void*)bssid_ptr, ETHER_ADDR_LEN);
    DIAG_UTIL_ERR_CHK(rtk_vlan_ipSubnetBasedVlanEntry_set(unit, *index_ptr, &entry), ret);
    DIAG_UTIL_MPRINTF("mac vlan entry %u bssid set to %02x:%02x:%02x:%02x:%02x:%02x\n",
        *index_ptr, bssid_ptr->octet[0], bssid_ptr->octet[1], bssid_ptr->octet[2],
        bssid_ptr->octet[3], bssid_ptr->octet[4], bssid_ptr->octet[5]);
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_vlan_set_ip_subnet_based_vlan_entry_index_capwap_tunnel_bssid_bssid */
#endif
...
#ifdef CMD_VLAN_ADD_VLAN_TABLE_VID_VID_BSSID_BSSID
/*
 * vlan add vlan-table vid <UINT:vid> bssid <MACADDR:bssid>
 */
cparser_result_t
cparser_cmd_vlan_add_vlan_table_vid_vid_bssid_bssid(
    cparser_context_t *context,
    uint32_t *vid_ptr,
    cparser_macaddr_t *bssid_ptr)
{
    ...
    DIAG_UTIL_FUNC_INIT(unit);
    DIAG_UTIL_ERR_CHK(rtk_vlan_bssid_add(unit, *vid_ptr, (rtk_mac_t*)bssid_ptr), ret);
    osal_memset(&macStr, 0, sizeof(macStr));
    DIAG_UTIL_MAC2STR(macStr, bssid_ptr->octet);
    DIAG_UTIL_MPRINTF("vid %u bssid set to %s\n", *vid_ptr, macStr);

    return CPARSER_OK;
}   /* end of cparser_cmd_vlan_add_vlan_table_vid_vid_bssid_bssid */
#endif

#ifdef CMD_VLAN_DEL_VLAN_TABLE_VID_VID_BSSID_BSSID
/*
 * vlan del vlan-table vid <UINT:vid> bssid <MACADDR:bssid>
 */
cparser_result_t
cparser_cmd_vlan_del_vlan_table_vid_vid_bssid_bssid(
    cparser_context_t *context,
    uint32_t *vid_ptr,
    cparser_macaddr_t *bssid_ptr)
{
    ...
    DIAG_UTIL_FUNC_INIT(unit);
    DIAG_UTIL_ERR_CHK(rtk_vlan_bssid_del(unit, *vid_ptr, (rtk_mac_t*)bssid_ptr), ret);
    osal_memset(&macStr, 0, sizeof(macStr));
    DIAG_UTIL_MAC2STR(macStr, bssid_ptr->octet);
    DIAG_UTIL_MPRINTF("Delete vid %u bssid %s\n", *vid_ptr, macStr);

    return CPARSER_OK;
}   /* end of cparser_cmd_vlan_del_vlan_table_vid_vid_bssid_bssid */
#endif
