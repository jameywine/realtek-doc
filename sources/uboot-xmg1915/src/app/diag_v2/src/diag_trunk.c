...
cparser_result_t cparser_cmd_trunk_dump_trunk_group_all(cparser_context_t *context)
{
    ...
    for (trunk_index = min_trk_gid; trunk_index <= max_trk_gid; trunk_index++)
    {
        ...
        {
            len = snprintf((char *)pBuf, bufSz, (TRUE == isFirst)? "%d:%d": ", %d:%d", trk_egr_ports.egr_port[i].unit, trk_egr_ports.egr_port[i].port);
            isFirst = FALSE;
    ...
    char **ports_ptr)
{
    ...
    DIAG_UTIL_ERR_CHK(rtk_stack_unit_get(unit, &myUnit), ret);
    unitPort.unit = myUnit;
...
#ifdef CMD_TRUNK_SET_EGRESS_PORT_TRUNK_GROUP_TRUNK_ID_PORT_UNIT_UNIT_PORTS_NONE
/*
 * trunk set egress-port trunk-group <UINT:trunk_id> port ( <UNIT_PORT_LIST:unit_ports> | none )
 */
cparser_result_t
cparser_cmd_trunk_set_egress_port_trunk_group_trunk_id_port_unit_ports_none(
    cparser_context_t *context,
    uint32_t *trunk_id_ptr,
    char **unit_ports_ptr)
{
    ...
    if ('n' != TOKEN_CHAR(6,0))
    {
        diag_util_str2unitPorts(*unit_ports_ptr, &trk_egr_ports);
    }

    DIAG_UTIL_ERR_CHK(rtk_trunk_egrPort_set(unit, trk_gid, &trk_egr_ports), ret);
    return CPARSER_OK;
}   /* end of cparser_cmd_trunk_set_egress_port_trunk_group_trunk_id_num_num_ports_port_unit_ports_none */
#endif
...
#ifdef CMD_TRUNK_GET_SRC_PORT_MAPPING_UNIT_UNIT_ID_PORT_PORTS_NONE
/*
 * trunk get src-port-mapping unit <UINT:unit_id> port ( <PORT_LIST:ports> | none )
 */
cparser_result_t
cparser_cmd_trunk_get_src_port_mapping_unit_unit_id_port_ports_none(
    cparser_context_t *context,
    ...
    char **ports_ptr)
{
    ...
    {
        unit_port.unit = *unit_id_ptr;
        unit_port.port = port;
        DIAG_UTIL_ERR_CHK(rtk_trunk_srcPortMap_get(unit, unit_port, &is_trk_mbr, &trk_gid), ret);
...
#ifdef CMD_TRUNK_SET_SRC_PORT_MAPPING_UNIT_UNIT_ID_PORT_PORTS_NONE_TRK_GROUP_TRUNK_ID_NONE
/*
 * trunk set src-port-mapping unit <UINT:unit_id> port ( <PORT_LIST:ports> | none ) trk_group ( <UINT:trunk_id> | none )
 */
cparser_result_t
cparser_cmd_trunk_set_src_port_mapping_unit_unit_id_port_ports_none_trk_group_trunk_id_none(
    cparser_context_t *context,
    uint32_t *unit_id_ptr,
    char **ports_ptr,
    uint32_t *trunk_id_ptr)
{
    ...
    DIAG_UTIL_PORTMASK_SCAN(portlist, port)
    {
        ...
        DIAG_UTIL_ERR_CHK(rtk_trunk_srcPortMap_set(unit, unit_port, is_trk_mbr, trk_gid), ret);
    }
