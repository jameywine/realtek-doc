 ...
 *
 * $Revision: 81481 $
 * $Date: 2017-08-18 11:51:24 +0800 (Fri, 18 Aug 2017) $
 *
...
cparser_result_t cparser_cmd_l2_table_get_entry_index_index(cparser_context_t *context, uint32_t *index_ptr)
{
    ...
    if (FLOW_TYPE_UNICAST == entry.entry_type)
    {
        ...
        {
            diag_util_mprintf(" Index | VID | MAC Address       | SPA  | SaBlock  | DaBlock  | Static  | Nexthop  | Suspend | Agg VID | Age\n");
            diag_util_mprintf("-------+-----+-------------------+------+----------+----------+---------+----------+---------+---------+-----\n");
            diag_util_mprintf("%6d |%4d | %02X:%02X:%02X:%02X:%02X:%02X | %4d | %8d | %8d | %7d | %8d | %7d |   %4d  |  %2d\n",
                                *index_ptr,
                                l2_data.vid,
        ...
        {
            sprintf(buffer, "%d", l2_data.unit_id);
            diag_util_mprintf(" Index | VID | MAC Address       | TRK | Unit | SPA |SBlk|DBlk|Sttc| NH |Sus|    AVID    |  MAC Index | Age\n");
            diag_util_mprintf("-------+-----+-------------------+-----+------+-----+----+----+----+----+---+------------+------------+-----\n");
            if(l2_data.flags&RTK_L2_UCAST_FLAG_NEXTHOP)
            {
                diag_util_mprintf("%6d |%4d | %02X:%02X:%02X:%02X:%02X:%02X | %3d | %4s | %3d |%4d|%4d|%4d| %2d |%3d| %s| %11d|  %2d\n",
                            *index_ptr,
                            ...
                            (l2_data.flags&RTK_L2_UCAST_FLAG_TRUNK_PORT) ? (l2_data.unit_id==1 ? "ntfy" : " - ") : buffer,
        ...
        {
            sprintf(buffer, "%d", l2_data.unit_id);
            diag_util_mprintf(" Index | VID | MAC Address       | TRK | Unit | SPA |SBlk|DBlk|Sttc| NH |Sus|Tag| APri | AVID | ECID     | Age\n");
            diag_util_mprintf("-------+-----+-------------------+-----+------+-----+----+----+----+----+---+---+------+------+----------+-----\n");
            diag_util_mprintf("%6d |%4d | %02X:%02X:%02X:%02X:%02X:%02X | %3d | %4s | %3d |%4d|%4d|%4d| %2d |%3d|%3d| %4d | %4d | 0x%06x |  %2d\n",
                            *index_ptr,
                            l2_data.vid,
                            l2_data.mac.octet[0],l2_data.mac.octet[1],l2_data.mac.octet[2],l2_data.mac.octet[3],l2_data.mac.octet[4],l2_data.mac.octet[5],
                            (l2_data.flags&RTK_L2_UCAST_FLAG_TRUNK_PORT) ? 1 : 0,
    ...
    uint32_t *index_ptr)
{
    ...
    if(l2_entry.entry_type  == FLOW_TYPE_UNICAST)
    {
        ...
        if (DIAG_OM_GET_FAMILYID(RTL9300_FAMILY_ID))
        {
            ...
            if(l2_entry.unicast.flags&RTK_L2_UCAST_FLAG_NEXTHOP)
            {
                diag_util_mprintf("%6d |%4d | %02X:%02X:%02X:%02X:%02X:%02X | %3d | %4s | %3d |%4d|%4d|%4d| %2d |%3d| %s| %11d|  %2d\n",
                            ...
                            (l2_entry.unicast.flags&RTK_L2_UCAST_FLAG_TRUNK_PORT) ? (l2_entry.unicast.unit_id==1 ? "ntfy" : " - ") : buffer,
        ...
        if (DIAG_OM_GET_FAMILYID(RTL9310_FAMILY_ID))
        {
            ...
            sprintf(buffer, "%d", l2_entry.unicast.unit_id);
            diag_util_mprintf("%6d |%4d | %02X:%02X:%02X:%02X:%02X:%02X | %3d | %4s | %3d |%4d|%4d|%4d| %2d |%3d|%3d| %4d | %4d | 0x%06x |  %2d\n",
                l2_entry.unicast.l2_idx,
                l2_entry.unicast.vid,
                l2_entry.unicast.mac.octet[0],l2_entry.unicast.mac.octet[1],l2_entry.unicast.mac.octet[2],l2_entry.unicast.mac.octet[3],l2_entry.unicast.mac.octet[4],l2_entry.unicast.mac.octet[5],
    ...
    uint32_t *id_ptr)
{
    ...
    if ('p' == TOKEN_CHAR(4, 0))
    {
        ...
        if (DIAG_OM_GET_FAMILYID(RTL9310_FAMILY_ID) || DIAG_OM_GET_FAMILYID(RTL9300_FAMILY_ID))
            DIAG_UTIL_ERR_CHK(rtk_stack_unit_get(unit, &config.unit_id), ret);
#endif
...
#if defined(CONFIG_SDK_RTL9310) || defined(CONFIG_SDK_RTL9300)
        if (DIAG_OM_GET_FAMILYID(RTL9310_FAMILY_ID) || DIAG_OM_GET_FAMILYID(RTL9300_FAMILY_ID))
            DIAG_UTIL_ERR_CHK(rtk_stack_unit_get(unit, &config.replacingUnit_id), ret);
#endif
    ...
    uint32_t *id_ptr)
{
    ...
    if ('p' == TOKEN_CHAR(5, 0))
    {
        if (((DIAG_OM_GET_FAMILYID(RTL8380_FAMILY_ID) || DIAG_OM_GET_FAMILYID(RTL8330_FAMILY_ID) || DIAG_OM_GET_FAMILYID(RTL9300_FAMILY_ID)) && *id_ptr == 0x1f) ||
            ((DIAG_OM_GET_FAMILYID(RTL8390_FAMILY_ID) || DIAG_OM_GET_FAMILYID(RTL8350_FAMILY_ID) || DIAG_OM_GET_FAMILYID(RTL9310_FAMILY_ID)) && *id_ptr == 0x31f))
...
#if defined(CONFIG_SDK_RTL9310) || defined(CONFIG_SDK_RTL9300)
    if (DIAG_OM_GET_FAMILYID(RTL9310_FAMILY_ID) || DIAG_OM_GET_FAMILYID(RTL9300_FAMILY_ID))
        DIAG_UTIL_ERR_CHK(rtk_stack_unit_get(unit, &l2_uAddr.unit_id), ret);
#endif
...
#ifdef CMD_L2_TABLE_ADD_SET_MAC_UCAST_VID_MAC_UNIT_UNIT_PORT_TRUNK_ID_SA_BLOCK_DA_BLOCK_STATIC_NEXTHOP_SUSPEND_AGED
/*
l2-table ( add | set ) mac-ucast <UINT:vid> <MACADDR:mac> unit <UINT:unit> ( port | trunk ) <UINT:id>  { sa-block } { da-block } { static } { nexthop } { suspend } { aged }
*/
cparser_result_t cparser_cmd_l2_table_add_set_mac_ucast_vid_mac_unit_unit_port_trunk_id_sa_block_da_block_static_nexthop_suspend_aged(cparser_context_t *context,
    uint32_t *vid_ptr,
    cparser_macaddr_t *mac_ptr,
    uint32_t *unit_ptr,
    uint32_t *id_ptr)
    ...
    cparser_macaddr_t *mac_ptr)
{
    ...
    if (DIAG_OM_GET_FAMILYID(RTL8390_FAMILY_ID) || DIAG_OM_GET_FAMILYID(RTL8350_FAMILY_ID) ||
        DIAG_OM_GET_FAMILYID(RTL8380_FAMILY_ID) || DIAG_OM_GET_FAMILYID(RTL8330_FAMILY_ID))
    {
        diag_util_mprintf(" Index | VID | MAC Address       | SPA  | SaBlock  | DaBlock  | Static  | Nexthop  | Suspend | Aggreate VID\n");
        diag_util_mprintf("-------+-----+-------------------+------+----------+----------+---------+----------+---------+-------------\n");
        diag_util_mprintf("%6d | %3d | %02X:%02X:%02X:%02X:%02X:%02X | %4d | %8d | %8d | %7d | %8d | %7d | %4d\n",
                        l2_data.l2_idx,
                        l2_data.vid,
                        l2_data.mac.octet[0],l2_data.mac.octet[1],l2_data.mac.octet[2],l2_data.mac.octet[3],l2_data.mac.octet[4],l2_data.mac.octet[5],
                 l2_data.port, (l2_data.flags&RTK_L2_UCAST_FLAG_SA_BLOCK)?1:0, (l2_data.flags&RTK_L2_UCAST_FLAG_DA_BLOCK)?1:0,
                ...
                {
                    sprintf(buffer, "%d", l2_data.unit_id);
                    diag_util_mprintf(" Index | VID | MAC Address       | TRK | Unit |  SPA  | SaBlock  | DaBlock  | Static  | Nexthop  | Suspend | Aggreate VID | MAC Index |\n");
                    diag_util_mprintf("-------+------+------------------+------+------+------+----------+----------+---------+----------+---------+---------+-------------\n");
                    if(l2_data.flags&RTK_L2_UCAST_FLAG_NEXTHOP)
                    {
        ...
        {
            sprintf(buffer, "%d", l2_data.unit_id);
            diag_util_mprintf(" Index | VID | MAC Address       |TRK|Unit|SPA|SBlk|DBlk|Sttc|NH|Sus|Tag|APri|AVID|Tnl|ECID/Idx| Age\n");
            diag_util_mprintf("-------+-----+-------------------+---+----+---+----+----+----+--+---+---+----+----+---+--------+-----\n");
...
#if defined(CONFIG_SDK_RTL9310) || defined(CONFIG_SDK_RTL9300)
    if (DIAG_OM_GET_FAMILYID(RTL9310_FAMILY_ID) || DIAG_OM_GET_FAMILYID(RTL9300_FAMILY_ID))
        DIAG_UTIL_ERR_CHK(rtk_stack_unit_get(unit, &l2_data.unit_id), ret);
#endif
...
cparser_result_t cparser_cmd_l2_table_dump_mac_ucast_dynamic_static_nexthop_all(cparser_context_t *context)
{
    ...
    while (1)
    {
        ...
        {
            sprintf(buffer, "%d", l2_data.unit_id);
            ret = diag_util_mprintf("%6d |%4d | %02X:%02X:%02X:%02X:%02X:%02X | %d |%4s|%3d|%4d|%4d|%4d|%2d|%3d|%3d|%4d|%4d| %d |0x%06X|  %2d\n",
                l2_data.l2_idx,
                l2_data.vid,
                l2_data.mac.octet[0],l2_data.mac.octet[1],l2_data.mac.octet[2],l2_data.mac.octet[3],l2_data.mac.octet[4],l2_data.mac.octet[5],
                (l2_data.flags&RTK_L2_UCAST_FLAG_TRUNK_PORT) ? 1 : 0,
        ...
        if ('p' == TOKEN_CHAR(3, 0))
        {
            DIAG_UTIL_PORTMASK_SCAN(portlist, port)
            {
                ...
                if (DIAG_OM_GET_FAMILYID(RTL8390_FAMILY_ID) || DIAG_OM_GET_FAMILYID(RTL8350_FAMILY_ID) ||
                    DIAG_OM_GET_FAMILYID(RTL8380_FAMILY_ID) || DIAG_OM_GET_FAMILYID(RTL8330_FAMILY_ID))
                {
                    diag_util_mprintf("%6d | %4d | %02X:%02X:%02X:%02X:%02X:%02X | %4d | %8d | %8d | %7d | %8d | %7d | %4d\n",
                                    l2_data.l2_idx,
                                    l2_data.vid,
                                    l2_data.mac.octet[0],l2_data.mac.octet[1],l2_data.mac.octet[2],l2_data.mac.octet[3],l2_data.mac.octet[4],l2_data.mac.octet[5],
                             l2_data.port, (l2_data.flags&RTK_L2_UCAST_FLAG_SA_BLOCK)?1:0, (l2_data.flags&RTK_L2_UCAST_FLAG_DA_BLOCK)?1:0,
...
#ifdef CMD_L2_TABLE_SET_FLUSH_FLUSH_ENTRY_UNIT_UNIT_PORT_TRUNK_ID_DYNAMIC_ONLY_INCLUDE_STATIC
/*
 * l2-table set flush flush-entry unit <UINT:unit> ( port | trunk ) <UINT:id> ( dynamic-only | include-static )
 */
cparser_result_t
cparser_cmd_l2_table_set_flush_flush_entry_unit_unit_port_trunk_id_dynamic_only_include_static(
    cparser_context_t *context,
    uint32_t *unit_ptr,
    uint32_t *id_ptr)
{
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_l2_table_set_flush_flush_entry_unit_unit_port_trunk_id_dynamic_only_include_static */
#endif

#ifdef CMD_L2_TABLE_SET_FLUSH_FLUSH_ENTRY_UNIT_UNIT_PORT_TRUNK_ID_VID_VID_DYNAMIC_ONLY_INCLUDE_STATIC
/*
 * l2-table set flush flush-entry unit <UINT:unit> ( port | trunk ) <UINT:id> vid <UINT:vid> ( dynamic-only | include-static )
 */
cparser_result_t
cparser_cmd_l2_table_set_flush_flush_entry_unit_unit_port_trunk_id_vid_vid_dynamic_only_include_static(
    cparser_context_t *context,
    uint32_t *unit_ptr,
    uint32_t *id_ptr,
    uint32_t *vid_ptr)
{
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_l2_table_set_flush_flush_entry_unit_unit_port_trunk_id_vid_vid_dynamic_only_include_static */
#endif

#ifdef CMD_L2_TABLE_SET_FLUSH_REPLACE_ENTRY_REPLACE_UNIT_REPLACE_UNIT_REPLACE_PORT_REPLACE_TRUNK_REPLACE_ID_DYNAMIC_ONLY_INCLUDE_STATIC
/*
 * l2-table set flush replace-entry replace-unit <UINT:replace_unit> ( replace-port | replace-trunk ) <UINT:replace_id> ( dynamic-only | include-static )
 */
cparser_result_t
cparser_cmd_l2_table_set_flush_replace_entry_replace_unit_replace_unit_replace_port_replace_trunk_replace_id_dynamic_only_include_static(
    cparser_context_t *context,
    uint32_t *replace_unit_ptr,
    uint32_t *replace_id_ptr)
{
    uint32                          unit = 0;
    int32                            ret = RT_ERR_FAILED;
    ...
    if ('p' == TOKEN_CHAR(6, 8))
    {
        config.replacingUnit_id = *replace_unit_ptr;
        config.replacePortOrTrunk = ENABLED;
        config.replacingPort = *replace_id_ptr;
    }
    else if ('t' == TOKEN_CHAR(6, 8))
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_l2_table_set_flush_replace_entry_replace_unit_replace_unit_replace_port_replace_trunk_replace_id_dynamic_only_include_static */
#endif

#ifdef CMD_L2_TABLE_SET_FLUSH_REPLACE_ENTRY_UNIT_UNIT_PORT_TRUNK_ID_REPLACE_UNIT_REPLACE_UNIT_REPLACE_PORT_REPLACE_TRUNK_REPLACE_ID_DYNAMIC_ONLY_INCLUDE_STATIC
/*
 * l2-table set flush replace-entry unit <UINT:unit> ( port | trunk ) <UINT:id> replace-unit <UINT:replace_unit> ( replace-port | replace-trunk ) <UINT:replace_id>  ( dynamic-only | include-static )
 */
cparser_result_t
cparser_cmd_l2_table_set_flush_replace_entry_unit_unit_port_trunk_id_replace_unit_replace_unit_replace_port_replace_trunk_replace_id_dynamic_only_include_static(
    cparser_context_t *context,
    uint32_t *unit_ptr,
    uint32_t *id_ptr,
    uint32_t *replace_unit_ptr,
    uint32_t *replace_id_ptr)
{
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_l2_table_set_flush_replace_entry_unit_unit_port_trunk_id_replace_unit_replace_unit_replace_port_replace_trunk_replace_id_dynamic_only_include_static */
#endif

#ifdef CMD_L2_TABLE_SET_FLUSH_REPLACE_ENTRY_VID_VID_REPLACE_UNIT_REPLACE_UNIT_REPLACE_PORT_REPLACE_TRUNK_REPLACE_ID_DYNAMIC_ONLY_INCLUDE_STATIC
/*
 * l2-table set flush replace-entry vid <UINT:vid> replace-unit <UINT:replace_unit> ( replace-port | replace-trunk ) <UINT:replace_id> ( dynamic-only | include-static )
 */
cparser_result_t
cparser_cmd_l2_table_set_flush_replace_entry_vid_vid_replace_unit_replace_unit_replace_port_replace_trunk_replace_id_dynamic_only_include_static(
    cparser_context_t *context,
    uint32_t *vid_ptr,
    uint32_t *replace_unit_ptr,
    uint32_t *replace_id_ptr)
{
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_l2_table_set_flush_replace_entry_vid_vid_replace_unit_replace_unit_replace_port_replace_trunk_replace_id_dynamic_only_include_static */
#endif

#ifdef CMD_L2_TABLE_SET_FLUSH_REPLACE_ENTRY_UNIT_UNIT_PORT_TRUNK_ID_VID_VID_REPLACE_UNIT_REPLACE_UNIT_REPLACE_PORT_REPLACE_TRUNK_REPLACE_ID_DYNAMIC_ONLY_INCLUDE_STATIC
/*
 * l2-table set flush replace-entry unit <UINT:unit> ( port | trunk ) <UINT:id> vid <UINT:vid> replace-unit <UINT:replace_unit> ( replace-port | replace-trunk ) <UINT:replace_id> ( dynamic-only | include-static )
 */
cparser_result_t
cparser_cmd_l2_table_set_flush_replace_entry_unit_unit_port_trunk_id_vid_vid_replace_unit_replace_unit_replace_port_replace_trunk_replace_id_dynamic_only_include_static(
    cparser_context_t *context,
    uint32_t *unit_ptr,
    uint32_t *id_ptr,
    uint32_t *vid_ptr,
    ...
    uint32_t *replace_id_ptr)
{
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_l2_table_set_flush_replace_entry_unit_unit_port_trunk_id_vid_vid_replace_unit_replace_unit_replace_port_replace_trunk_replace_id_dynamic_only_include_static */
#endif

#ifdef CMD_L2_TABLE_SET_FLUSH_CLEAR_NEXTHOP_UNIT_UNIT_PORT_TRUNK_ID
/*
 * l2-table set flush clear-nexthop unit <UINT:unit> ( port | trunk ) <UINT:id>
 */
cparser_result_t
cparser_cmd_l2_table_set_flush_clear_nexthop_unit_unit_port_trunk_id(
    cparser_context_t *context,
    uint32_t *unit_ptr,
    uint32_t *id_ptr)
{
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_l2_table_set_flush_clear_nexthop_unit_unit_port_trunk_id */
#endif

#ifdef CMD_L2_TABLE_SET_FLUSH_CLEAR_NEXTHOP_UNIT_UNIT_PORT_TRUNK_ID_VID_VID
/*
 * l2-table set flush clear-nexthop unit <UINT:unit> ( port | trunk ) <UINT:id> vid <UINT:vid>
 */
cparser_result_t
cparser_cmd_l2_table_set_flush_clear_nexthop_unit_unit_port_trunk_id_vid_vid(
    cparser_context_t *context,
    uint32_t *unit_ptr,
    uint32_t *id_ptr,
    uint32_t *vid_ptr)
{
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_l2_table_set_flush_clear_nexthop_unit_unit_port_trunk_id_vid_vid */
#endif

#ifdef CMD_L2_TABLE_SET_FLUSH_CLEAR_AGGVID_UNIT_UNIT_PORT_TRUNK_ID
/*
 * l2-table set flush clear-aggVid unit <UINT:unit> ( port | trunk ) <UINT:id>
 */
cparser_result_t
cparser_cmd_l2_table_set_flush_clear_aggVid_unit_unit_port_trunk_id(
    cparser_context_t *context,
    uint32_t *unit_ptr,
    uint32_t *id_ptr)
{
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_l2_table_set_flush_clear_aggVid_unit_unit_port_trunk_id */
#endif

#ifdef CMD_L2_TABLE_SET_FLUSH_CLEAR_AGGVID_UNIT_UNIT_PORT_TRUNK_ID_VID_VID
/*
 * l2-table set flush clear-aggVid unit <UINT:unit> ( port | trunk ) <UINT:id> vid <UINT:vid>
 */
cparser_result_t
cparser_cmd_l2_table_set_flush_clear_aggVid_unit_unit_port_trunk_id_vid_vid(
    cparser_context_t *context,
    uint32_t *unit_ptr,
    uint32_t *id_ptr,
    uint32_t *vid_ptr)
{
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_l2_table_set_flush_clear_aggVid_unit_unit_port_trunk_id_vid_vid */
#endif

#ifdef CMD_L2_TABLE_SET_FLUSH_CLEAR_AGGVID_UNIT_UNIT_PORT_TRUNK_ID_AGG_VID_AGG_VID
/*
 * l2-table set flush clear-aggVid unit <UINT:unit> ( port | trunk ) <UINT:id> agg-vid <UINT:agg_vid>
 */
cparser_result_t
cparser_cmd_l2_table_set_flush_clear_aggVid_unit_unit_port_trunk_id_agg_vid_agg_vid(
    cparser_context_t *context,
    uint32_t *unit_ptr,
    uint32_t *id_ptr,
    uint32_t *agg_vid_ptr)
{
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_l2_table_set_flush_clear_aggVid_unit_unit_port_trunk_id_agg_vid_agg_vid */
#endif

#ifdef CMD_L2_TABLE_SET_FLUSH_CLEAR_AGGVID_UNIT_UNIT_PORT_TRUNK_ID_VID_VID_AGG_VID_AGG_VID
/*
 * l2-table set flush clear-aggVid unit <UINT:unit> ( port | trunk ) <UINT:id> vid <UINT:vid> agg-vid <UINT:agg_vid>
 */
cparser_result_t
cparser_cmd_l2_table_set_flush_clear_aggVid_unit_unit_port_trunk_id_vid_vid_agg_vid_agg_vid(
    cparser_context_t *context,
    uint32_t *unit_ptr,
    uint32_t *id_ptr,
    uint32_t *vid_ptr,
    uint32_t *agg_vid_ptr)
{
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_l2_table_set_flush_clear_aggVid_unit_unit_port_trunk_id_vid_vid_agg_vid_agg_vid */
#endif

#ifdef CMD_L2_TABLE_DUMP_MAC_UCAST_UNIT_UNIT_PORT_TRUNK_PORT_TRUNK_ALL_DYNAMIC_STATIC_NEXTHOP_ALL
/*
 * l2-table dump mac-ucast unit <UINT:unit> ( port | trunk ) ( <MASK_LIST:port_trunk> | all ) ( dynamic | static |  nexthop | all )
 */
cparser_result_t
cparser_cmd_l2_table_dump_mac_ucast_unit_unit_port_trunk_port_trunk_all_dynamic_static_nexthop_all(
    cparser_context_t *context,
    uint32_t *unit_ptr,
    char **port_trunk_ptr)
{
    ...
    if (DIAG_OM_GET_FAMILYID(RTL9310_FAMILY_ID))
    {
        diag_util_mprintf(" Index | VID  | MAC Address       | TRK | Unit | SPA  | SaBlock  | DaBlock  | Static | Nexthop | Suspend | Aggreate Pri | Aggreate VID\n");
        diag_util_mprintf("-------+------+-------------------+-----+------+------+----------+----------+--------+---------+---------+--------------+-------------\n");
    }
#endif
    ...
    while (1)
    {
        ...
        if ('p' == TOKEN_CHAR(3, 0))
        {
            DIAG_UTIL_PORTMASK_SCAN(portlist, port)
            {
                if((l2_data.flags&RTK_L2_UCAST_FLAG_TRUNK_PORT) || l2_data.unit_id != *unit_ptr || l2_data.port!= port)
                {
                    continue;
                }
                ...
                {
                    sprintf(buffer, "%d", l2_data.unit_id);
                    diag_util_mprintf("%6d | %4d | %02X:%02X:%02X:%02X:%02X:%02X | %3s | %4d | %4d | %8d | %8d | %6d | %7d | %7d |     %4d     |     %4d\n",
                                    l2_data.l2_idx,
                                    l2_data.vid,
                                    l2_data.mac.octet[0],l2_data.mac.octet[1],l2_data.mac.octet[2],l2_data.mac.octet[3],l2_data.mac.octet[4],l2_data.mac.octet[5],
                                    (l2_data.flags&RTK_L2_UCAST_FLAG_TRUNK_PORT) ? 1 : 0,
    ...
    diag_util_mprintf("\nTotal Number Of Entries : %d\n", total_entry);
    return CPARSER_OK;
}   /* end of cparser_cmd_l2_table_dump_mac_ucast_unit_unit_port_trunk_port_trunk_all_dynamic_static_nexthop_all */
#endif

#ifdef CMD_L2_TABLE_ADD_MAC_MCAST_VID_MAC_ECID_ECID_PORT_PORT_ALL
/*
 * l2-table add mac-mcast <UINT:vid> <MACADDR:mac> ecid <UINT:ecid> port ( <PORT_LIST:port> | all )
 */
cparser_result_t
cparser_cmd_l2_table_add_mac_mcast_vid_mac_ecid_ecid_port_port_all(
    cparser_context_t *context,
    uint32_t *vid_ptr,
    cparser_macaddr_t *mac_ptr,
    uint32_t *ecid_ptr,
    char **port_ptr)
{
    ...
    RT_PARAM_CHK((NULL == vid_ptr), CPARSER_ERR_INVALID_PARAMS);
    RT_PARAM_CHK((NULL == mac_ptr), CPARSER_ERR_INVALID_PARAMS);
    RT_PARAM_CHK((NULL == ecid_ptr), CPARSER_ERR_INVALID_PARAMS);

    osal_memset(&mcast_data, 0, sizeof(rtk_l2_mcastAddr_t));
    osal_memset(&pmsk_data, 0, sizeof(rtk_bpe_pmskEntry_t));


    if ((FALSE == diag_util_isBcastMacAddr(mac_ptr->octet)) &&
        (FALSE == diag_util_isMcastMacAddr(mac_ptr->octet)))
    {
        diag_util_printf("Unicast MAC address is not allowed to configure.\n");
        return CPARSER_NOT_OK;
    }

    pmsk_data.port_type = 0;
    DIAG_UTIL_ERR_CHK(DIAG_UTIL_EXTRACT_PORTLIST(portlist, 8), ret);
    pmsk_data.portmask = portlist.portmask;
    pmsk_data.fwdIndex = -1; /* auto allocate portmask idx */
    osal_memcpy(&(pmsk_data.ecid), ecid_ptr, sizeof(pmsk_data.ecid));

    mcast_data.rvid = *vid_ptr;
    osal_memcpy(mcast_data.mac.octet, mac_ptr->octet, ETHER_ADDR_LEN);


    DIAG_UTIL_ERR_CHK(rtk_l2_mcastAddrEcidPmsk_add(unit, &mcast_data, &pmsk_data), ret);

    return CPARSER_OK;
}   /* end of cparser_cmd_l2_table_add_mac_mcast_vid_mac_ecid_ecid_port_port_all */
#endif

#ifdef CMD_L2_TABLE_ADD_MAC_MCAST_VID_MAC_ECID_ECID_PORT_PORT_ALL_OFFSET
/*
 * l2-table add mac-mcast <UINT:vid> <MACADDR:mac> ecid <UINT:ecid> port ( <PORT_LIST:port> | all ) offset
 */
cparser_result_t
cparser_cmd_l2_table_add_mac_mcast_vid_mac_ecid_ecid_port_port_all_offset(
    cparser_context_t *context,
    uint32_t *vid_ptr,
    cparser_macaddr_t *mac_ptr,
    uint32_t *ecid_ptr,
    char **port_ptr)
{
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_l2_table_add_mac_mcast_vid_mac_ecid_ecid_port_port_all_offset */
#endif


#ifdef CMD_L2_TABLE_DEL_MAC_MCAST_VID_MAC_ECID_ECID
/*
 * l2-table del mac-mcast <UINT:vid> <MACADDR:mac> ecid <UINT:ecid>
 */
cparser_result_t
cparser_cmd_l2_table_del_mac_mcast_vid_mac_ecid_ecid(
    cparser_context_t *context,
    uint32_t *vid_ptr,
    cparser_macaddr_t *mac_ptr,
    uint32_t *ecid_ptr)
{
    ...
    DIAG_UTIL_PARAM_RANGE_CHK((NULL == vid_ptr), CPARSER_ERR_INVALID_PARAMS);
    DIAG_UTIL_PARAM_RANGE_CHK((NULL == mac_ptr), CPARSER_ERR_INVALID_PARAMS);
    DIAG_UTIL_PARAM_RANGE_CHK((NULL == ecid_ptr), CPARSER_ERR_INVALID_PARAMS);
    osal_memset(&mcast_data, 0, sizeof(rtk_l2_mcastAddr_t));
    osal_memset(&pmsk_data, 0, sizeof(rtk_bpe_pmskEntry_t));
    ...
    DIAG_UTIL_ERR_CHK(rtk_l2_mcastAddrEcidPmsk_del(unit, mcast_data, &pmsk_data), ret);
    return CPARSER_OK;
}   /* end of cparser_cmd_l2_table_del_mac_mcast_vid_mac_ecid_ecid */
#endif

#ifdef CMD_L2_TABLE_GET_MAC_MCAST_VID_MAC_ECID_ECID
/*
 * l2-table get mac-mcast <UINT:vid> <MACADDR:mac> ecid <UINT:ecid>
 */
cparser_result_t
cparser_cmd_l2_table_get_mac_mcast_vid_mac_ecid_ecid(
    cparser_context_t *context,
    uint32_t *vid_ptr,
    cparser_macaddr_t *mac_ptr,
    uint32_t *ecid_ptr)
{
    ...
    if ((ret = rtk_l2_mcastAddrEcidPmsk_get(unit, &mcast_data, &pmsk_data)) != RT_ERR_OK)
    {
        if(ret == RT_ERR_L2_ENTRY_NOTFOUND || ret == RT_ERR_ENTRY_NOTFOUND)
        {
            diag_util_printf("Entry is not exist\n");
    ...
    diag_util_mprintf(" Index | VID  | MAC address       | Ecid     | Port\n");
    diag_util_mprintf("-------+------+-------------------+----------+-------------------------\n");

    diag_util_lPortMask2str(port_list, &pmsk_data.portmask);

    diag_util_mprintf("%6d | %4d | %02X:%02X:%02X:%02X:%02X:%02X | 0x%06x | %s \n",
                mcast_data.l2_idx,
                mcast_data.rvid,
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_l2_table_get_mac_mcast_vid_mac_ecid_ecid */
#endif

#ifdef CMD_L2_TABLE_DUMP_MAC_MCAST_VID_MAC_ECID
/*
 * l2-table dump mac-mcast <UINT:vid> <MACADDR:mac> ecid
 */
cparser_result_t
cparser_cmd_l2_table_dump_mac_mcast_vid_mac_ecid(
    cparser_context_t *context,
    uint32_t *vid_ptr,
    cparser_macaddr_t *mac_ptr)
{
    ...
    while (1)
    {
        if ((ret = rtk_l2_mcastAddrEcidPmskNextValid_get(unit, &mcast_data, &pmsk_data)) != RT_ERR_OK)
        {
            if(ret == RT_ERR_L2_ENTRY_NOTFOUND || ret == RT_ERR_ENTRY_NOTFOUND)
            {
                if (total_entry == 0)
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_l2_table_dump_mac_mcast_vid_mac_ecid */
#endif
...
#define MAX_HASH_BUCKET_NUM 8
    ...
    if (ret_cnt && contain_ucst)
    {
        ...
        if (DIAG_OM_GET_FAMILYID(RTL9300_FAMILY_ID))
        {
            diag_util_mprintf(" Index | VID | MAC Address       | TRK | Unit | SPA |SBlk|DBlk|Sttc| NH |Sus| Agg VID | MAC Index \n");
            diag_util_mprintf("-------+-----+-------------------+-----+------+-----+----+----+----+----+---+---------+------------\n");
        }
        if (DIAG_OM_GET_FAMILYID(RTL9310_FAMILY_ID))
        {
    ...
    for (i = 0; i < ret_cnt; i++)
    {
        if (output_l2_entry[i].entry_type == FLOW_TYPE_UNICAST)
        {
            ...
            if (DIAG_OM_GET_FAMILYID(RTL8390_FAMILY_ID) || DIAG_OM_GET_FAMILYID(RTL8350_FAMILY_ID) ||
                DIAG_OM_GET_FAMILYID(RTL8380_FAMILY_ID) || DIAG_OM_GET_FAMILYID(RTL8330_FAMILY_ID))
            {
                diag_util_mprintf("%6d |%4d | %02X:%02X:%02X:%02X:%02X:%02X | %3d | %4d | %4d | %6d | %7d | %7d |   %4d  |  %2d\n",
            ...
            if (DIAG_OM_GET_FAMILYID(RTL9310_FAMILY_ID))
            {
                sprintf(buffer, "%d", output_l2_entry[i].unicast.unit_id);
                diag_util_mprintf("%6d |%4d | %02X:%02X:%02X:%02X:%02X:%02X | %3d | %4s | %3d |%4d|%4d|%4d| %2d |%3d|%3d| %4d | %4d | 0x%06x |  %2d\n",
                                output_l2_entry[i].unicast.l2_idx,
                                output_l2_entry[i].unicast.vid,
                                output_l2_entry[i].unicast.mac.octet[0],output_l2_entry[i].unicast.mac.octet[1],output_l2_entry[i].unicast.mac.octet[2],output_l2_entry[i].unicast.mac.octet[3],output_l2_entry[i].unicast.mac.octet[4],output_l2_entry[i].unicast.mac.octet[5],
                                ...
                                output_l2_entry[i].unicast.agg_vid,
                                output_l2_entry[i].unicast.ecid,
