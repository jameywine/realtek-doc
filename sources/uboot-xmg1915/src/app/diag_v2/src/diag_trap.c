 ...
 *
 * $Revision: 79948 $
 * $Date: 2017-06-22 18:58:09 +0800 (Thu, 22 Jun 2017) $
 *
...
/*
 * trap set rma l2-user-define <UINT:index> <MACADDR:mac_min> <MACADDR:mac_max>
 */
...
#ifdef CMD_TRAP_SET_REASON_IP_RSVD_ADDR_INGR_VLAN_FILTER_CFI_IVC_INVALID_SA_MAC_CONSTRAINT_NEW_SA_PMV_FORBID_STTC_PMV_DYNM_PMV_HASH_FULL_ATTACK_ACL_MIRROR_CAPWAP_QUEUE_ID_QUEUE_ID
/*
 * trap set reason ( ip-rsvd-addr | ingr-vlan-filter | cfi | ivc | invalid-sa | mac-constraint | new-sa | pmv-forbid | sttc-pmv | dynm-pmv | hash-full | attack | acl | mirror | capwap ) queue-id <UINT:queue_id>
 */
cparser_result_t cparser_cmd_trap_set_reason_ip_rsvd_addr_ingr_vlan_filter_cfi_ivc_invalid_sa_mac_constraint_new_sa_pmv_forbid_sttc_pmv_dynm_pmv_hash_full_attack_acl_mirror_capwap_queue_id_queue_id(cparser_context_t *context,
    uint32_t *queue_id_ptr)
{
    uint32 unit = 0;
    int32  ret = RT_ERR_FAILED;
...
#ifdef CMD_TRAP_SET_REASON_CFM_ETHDM_IP6_ND_CAPWAP_TT_EXCPT_INVALID_CAPWAP_HDR_MPLS_EXCEPT_OF_HIT_OF_LOOKUP_MISS_TT_HIT_QUEUE_ID_QUEUE_ID
/*
 * trap set reason ( cfm | ethdm | ip6-nd | capwap-tt-excpt | invalid-capwap-hdr | mpls-except | of-hit | of-lookup-miss | tt-hit ) queue-id <UINT:queue_id>
 */
cparser_result_t cparser_cmd_trap_set_reason_cfm_ethdm_ip6_nd_capwap_tt_excpt_invalid_capwap_hdr_mpls_except_of_hit_of_lookup_miss_tt_hit_queue_id_queue_id(cparser_context_t *context,
    uint32_t *queue_id_ptr)
{
    uint32 unit = 0;
    int32  ret = RT_ERR_FAILED;
    ...
    else if ('i' == TOKEN_CHAR(3, 0))
    {
        if ('p' == TOKEN_CHAR(3, 1))
            type = TRAP_Q_IPV6ND;
        else if ('n' == TOKEN_CHAR(3, 1))
            type = TRAP_Q_INVLD_CAPWAP_HDR;
        else
        {
            diag_util_printf("User config: Error!\n");
            return CPARSER_NOT_OK;
    ...
    for (type = TRAP_Q_ARP_REQ_REP_GRTS; type < TRAP_Q_END; type++)
    {
        ...
        switch (type)
        {
            ...
            case TRAP_Q_CAPWAP:
                ...
                break;
            case TRAP_Q_CAPWAP_TT_EXCPT:
                diag_util_mprintf("CAPWAP Tunnel Terminate Exception Trap Queue ID : %u\n", qId);
                break;
            case TRAP_Q_INVLD_CAPWAP_HDR:
                diag_util_mprintf("Invalid CAPWAP Header Trap Queue ID      : %u\n", qId);
                break;
