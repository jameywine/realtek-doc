 ...
 *
 * $Revision: 81260 $
 * $Date: 2017-08-15 11:41:50 +0800 (Tue, 15 Aug 2017) $
 *
...
#ifdef CMD_PORT_GET_ISOLATION_SRC_UNIT_UNIT_SRC_PORT_SRC_PORTS
/*
 * port get isolation src-unit <UINT:unit> src-port <PORT_LIST:src_ports>
 */
cparser_result_t cparser_cmd_port_get_isolation_src_unit_unit_src_port_src_ports(cparser_context_t *context,
    uint32_t *unit_ptr,
    char **src_ports_ptr)
{
    uint32          unit = 0;
    ...
    {
        osal_memset(&portmask, 0, sizeof(rtk_portmask_t));
        DIAG_UTIL_ERR_CHK(rtk_port_isolationExt_get(unit, *unit_ptr, port, &portmask), ret);
        osal_memset(port_list, 0, DIAG_UTIL_PORT_MASK_STRING_LEN * sizeof(uint8));
        diag_util_lPortMask2str(port_list, &portmask);
        diag_util_mprintf("Source Device %2d  Port %2d : Isolation Port list %s\n", *unit_ptr, port, port_list[0] == '\0' ? "is NULL":port_list);
    }
    ...
    char **ports_ptr)
{
    ...
    for (option_num = 6; option_num < TOKEN_NUM; option_num++)
    {
        ...
        else if (!strncmp("flow-control", str, len))
        {
...
#ifdef CMD_PORT_SET_ISOLATION_SRC_UNIT_UNIT_SRC_PORT_SRC_PORTS_DST_PORT_DST_PORTS_ALL
/*
 * port set isolation src-unit <UINT:unit> src-port <PORT_LIST:src_ports> dst-port ( <PORT_LIST:dst_ports> | all )
 */
cparser_result_t cparser_cmd_port_set_isolation_src_unit_unit_src_port_src_ports_dst_port_dst_ports_all(cparser_context_t *context,
    uint32_t *unit_ptr,
    char **src_ports_ptr,
    char **dst_ports_ptr)
{
    ...
    DIAG_UTIL_PORTMASK_SCAN(portlist, port)
    {
        DIAG_UTIL_ERR_CHK(rtk_port_isolationExt_set(unit, *unit_ptr, port, &targetPortlist.portmask), ret);
    }
...
#ifdef CMD_PORT_SET_10G_MEDIA_PORT_PORTS_ALL_FIBER10G_FIBER1G_FIBER100M_DAC50CM_DAC100CM_DAC300CM_NONE
/*
 * port set 10g-media port ( <PORT_LIST:ports> | all ) ( fiber10g | fiber1g | fiber100m | dac50cm | dac100cm | dac300cm | dac500cm  | none )
 */
cparser_result_t
cparser_cmd_port_set_10g_media_port_ports_all_fiber10g_fiber1g_fiber100m_dac50cm_dac100cm_dac300cm_dac500cm_none(
    cparser_context_t *context,
    char **ports_ptr)
{
    diag_portlist_t     portlist;
