...
int32
switch_phy_is_8295res(uint32 unit, uint32 port, rtk_switch_devInfo_t *pDevInfo)
{
    int32       ret;
    uint32      mdxMacId;
    uint32      data;

    if (pDevInfo->chipId == RTL8396M_CHIP_ID)
    {
        if (port == 24)
            mdxMacId = 25;
        else if (port == 36)
    ...
    ret = rtk_port_phyReg_get(unit, mdxMacId, 0, 3, &data);
    if (ret != RT_ERR_OK)
        return FALSE;

    if (data == 0x0)
    {
        ret = rtk_port_phyReg_get(unit, mdxMacId, 0, 17, &data);
        if (ret != RT_ERR_OK)
            return FALSE;
        if ((data & 0xF) == 0x4)    /* BOND_MODE_SEL */
            return TRUE;
    }
    return FALSE;
}
...
int32
switch_phy_is_8214qfes(uint32 unit, uint32 port, rtk_switch_devInfo_t *pDevInfo)
{
    uint32      mdxMacId;
    int32       ret;
    uint32      data;

    ret = rtk_port_phyReg_get(unit, port, 0, 3, &data);
    if (ret != RT_ERR_OK)
        return FALSE;
    ...
    if (data == 0x0)
    {
        ...
        if (ret != RT_ERR_OK)
            return FALSE;
        if ((data & 0xF) == 0x1) /* BOND_MODE_SEL */
            return TRUE;
    }
    return FALSE;
}
#endif /* CONFIG_SDK_RTL8214QF */
#endif/* CMD_SWITCH_GET_PROBE_INFORMATION */
...
#ifdef CMD_SWITCH_GET_PAUSE_TAG_ACTION
/*
 * switch get pause-tag action
 */
cparser_result_t
cparser_cmd_switch_get_pause_tag_action(
    cparser_context_t *context)
{
    rtk_action_t    act;
    uint32          unit;
    ...
    DIAG_UTIL_ERR_CHK(rtk_switch_tagPauseAction_get(unit, &act), ret);

    diag_util_mprintf("The pause frame with tag action: ");
    if (ACTION_DROP == act)
    {
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_switch_get_pause_tag_action */
#endif

#ifdef CMD_SWITCH_SET_PAUSE_TAG_ACTION_DROP_FORWARD
/*
 * switch set pause-tag action ( drop | forward )
 */
cparser_result_t
cparser_cmd_switch_set_pause_tag_action_drop_forward(
    cparser_context_t *context)
{
    rtk_action_t    act;
    uint32          unit;
    ...
    DIAG_UTIL_ERR_CHK(rtk_switch_tagPauseAction_set(unit, act), ret);

    return CPARSER_OK;
}   /* end of cparser_cmd_switch_set_pause_tag_action_drop_forward */
#endif
...
#ifdef CMD_SWITCH_GET_HALF_CONSECUTIVE_RETRY_STATE
/*
 * switch get half-consecutive-retry state
 */
cparser_result_t
cparser_cmd_switch_get_half_consecutive_retry_state(
    cparser_context_t *context)
{
    uint32  unit;
    int32   ret;
    ...
    DIAG_UTIL_ERR_CHK(rtk_switch_halfConsecutiveRetryEnable_get(unit, &state), ret);
...
#ifdef CMD_SWITCH_SET_HALF_CONSECUTIVE_RETRY_STATE_DISABLE_ENABLE
/*
 * switch set half-consecutive-retry state ( disable | enable )
 */
cparser_result_t
cparser_cmd_switch_set_half_consecutive_retry_state_disable_enable(
    cparser_context_t *context)
{
    uint32 unit;
    int32  ret = RT_ERR_FAILED;
    ...
    DIAG_UTIL_ERR_CHK(rtk_switch_halfConsecutiveRetryEnable_set(unit, state), ret);
    return CPARSER_OK;
