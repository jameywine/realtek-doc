 ...
 *
 * $Revision: 81276 $
 * $Date: 2017-08-15 16:25:07 +0800 (Tue, 15 Aug 2017) $
 *
    ...
    uint32_t *index_ptr)
{
    ...
    switch (*index_ptr)
    {
        ...
        case RTK_DBG_CNTR_L2_UC_MC_BRIDGE:
            diag_util_mprintf("Debug Counter %2d - L2 Unicast/Multicast Bridge : %u \n", *index_ptr, cntr);
            break;
        case RTK_DBG_CNTR_IP4_IP6_MC_BRIDGE:
            diag_util_mprintf("Debug Counter %2d - IPv4/IPv6 Multicast Bridge : %u \n", *index_ptr, cntr);
            break;
        ...
        case RTK_DBG_CNTR_XX_RMA:
            diag_util_mprintf("Debug Counter %2d - RMA : %u \n", *index_ptr, cntr);
            break;
        case RTK_DBG_CNTR_L3_IPUC_NON_IP:
            diag_util_mprintf("Debug Counter %2d - L3 IPUC Non-IP : %u \n", *index_ptr, cntr);
            break;
        ...
        case RTK_DBG_CNTR_MAC_PAUSE_FRAME:
            diag_util_mprintf("Debug Counter %2d - MAC Frame (Pause frame and unknown opcoce) : %u \n", *index_ptr, cntr);
            break;
        case RTK_DBG_CNTR_MAC_RX_DROP:
...
/*
 * diag dump sc reg <UINT:mdx_macId> ( daclong | s18431 )
 */

cparser_result_t
cparser_cmd_diag_dump_sc_reg_mdx_macId_daclong_8431(cparser_context_t *context, uint32_t *mdx_macId_ptr)
{
#if defined(CONFIG_SDK_RTL8295) || defined(CONFIG_SDK_RTL8295R) || defined(CONFIG_SDK_RTL8214QF)
    int32   ret = RT_ERR_FAILED;
    uint32  unit = 0;
...
/*
 *  diag set 8390 10g-media port ( <PORT_LIST:ports> | all ) ( fiber10g | fiber1g | fiber100m | none )
 */
cparser_result_t
cparser_cmd_diag_set_8390_10g_media_port_ports_all_fiber10g_fiber1g_fiber100m_none(cparser_context_t *context,
    char **ports_ptr)
{
#if defined(CONFIG_SDK_RTL8390)
    diag_portlist_t     portlist;
    ...
    else if (!strcmp(TOKEN_STR(tIdx), "fiber100m"))
    {
    ...
    uint32_t *frameNum_ptr)
{
    ...
    for (i = 0; i < 64; i++)
        for (j = 0; j < 64; j++)
            for (k = 0; k < *frameNum_ptr; k++)
                diag_util_printf("[%d, %d : %d]\n", i, j, 64*i + j * 20 + k*10);
