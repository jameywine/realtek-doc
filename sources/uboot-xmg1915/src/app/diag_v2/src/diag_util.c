 ...
 *
 * $Revision: 81282 $
 * $Date: 2017-08-15 17:18:40 +0800 (Tue, 15 Aug 2017) $
 *
...
#define _parse_err_return_with_pUnitPorts()  \
    do { \
        osal_memset(pUnitPort, 0, sizeof(*pUnitPort)); \
        return RT_ERR_FAILED; \
    } while (0)
...
int32
diag_util_str2unitPort(char *str, rtk_unit_port_t *pUnit_port)
{
    uint8    *ptr = NULL;


    if ((NULL == str) || (NULL == pUnit_port))
    {
        return RT_ERR_FAILED;
    }
    ...
    ptr = (uint8 *)strtok((char *)str, ":");
    _s2m_atoi(pUnit_port->unit, ptr, '\0');
    ptr = (uint8 *)strtok(NULL, ":");
    _s2m_atoi(pUnit_port->port, ptr, '\0');

    return RT_ERR_OK;
} /* end of diag_util_str2unitPortMask */

int32
diag_util_str2unitPorts(char *str, rtk_trk_egrPort_t *pUnitPort)
{
    uint32   i = 0, unit = 0;
    uint32   num = 0;
    uint32   num_end = 0;
    ...
    while (NULL != ptr)
    {
        if (isdigit((int)*ptr))
        {
            ...
            if (NULL == p)
            {   /* not correct format */
                _parse_err_return_with_pUnitPorts();
            }
            ...
            {   /* number only */
                ...
                pUnitPort->egr_port[pUnitPort->num_ports].unit = unit;
                pUnitPort->egr_port[pUnitPort->num_ports].port = num;
                pUnitPort->num_ports ++;
            }
            else
            {   /* number-number */
                ...
                if (num_end > num)
                {
                    ...
                    {
                        pUnitPort->egr_port[pUnitPort->num_ports].unit = unit;
                        pUnitPort->egr_port[pUnitPort->num_ports].port = i;
                        pUnitPort->num_ports ++;
                    }
                }
                else
