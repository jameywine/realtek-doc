 ...
 *
 * $Revision: 81058 $
 * $Date: 2017-08-07 17:38:04 +0800 (Mon, 07 Aug 2017) $
 *
        ...
        if (type == OF_INS_ACT_TYPE_END || type == OF_INS_ACT_TYPE_WRITEACT)
        {
            ...
            switch(igr->wa_data.output_data.type)
            {
                ...
                case OF_OUTPUT_TYPE_TUNNEL:
                    DIAG_UTIL_MPRINTF("\t\t\ttunnel_idx: %u\n", igr->wa_data.output_data.tunnel_idx);
                    break;
                default:
                    break;
            }
    ...
    if (type == OF_INS_ACT_TYPE_END || type == OF_INS_ACT_TYPE_WRITEACT)
    {
        ...
        switch(entry.ins.wa_data.output_data.type)
        {
            case OF_IGR_FT1_ACT_OUTPUT_TYPE_PHY_PORT:
                DIAG_UTIL_MPRINTF("\t\t\tunit: %u\n", entry.ins.wa_data.output_data.unit);
            ...
            case OF_IGR_FT1_ACT_OUTPUT_TYPE_MULT_EGR_PORT:
                ...
                break;
            case OF_OUTPUT_TYPE_TUNNEL:
                DIAG_UTIL_MPRINTF("\t\t\ttunnel_idx: %u\n", entry.ins.wa_data.output_data.tunnel_idx);
                break;
            default:
                break;
        }
    ...
    if (type == OF_INS_ACT_TYPE_END || type == OF_INS_ACT_TYPE_WRITEACT)
    {
        ...
        switch(ins.wa_data.output_data.type)
        {
            ...
            case OF_OUTPUT_TYPE_FAILOVER:
                ...
                break;
            case OF_OUTPUT_TYPE_TUNNEL:
                DIAG_UTIL_MPRINTF("\t\t\ttunnel_idx: %u\n", ins.wa_data.output_data.tunnel_idx);
                break;
            default:
                break;
        }
...
/*
 * openflow set entry <UINT:phase> <UINT:index> instruction write-action output ( phy-port | phy-port-src-port-filter ) <UINT:port> { <UINT:uid> }
 */
cparser_result_t
cparser_cmd_openflow_set_entry_phase_index_instruction_write_action_output_phy_port_phy_port_src_port_filter_port_uid(
    cparser_context_t *context,
    uint32_t *phase_ptr,
    uint32_t *index_ptr,
    uint32_t *port_ptr,
    uint32_t *uid_ptr)
{
    ...
    if ( uid_ptr == NULL )
    {
        IGR_SET_EGR_RET(phase, igrwa, output_data.unit, 0);
    }
    else
    {
        IGR_SET_EGR_RET(phase, igrwa, output_data.unit, *uid_ptr);
    }
    DIAG_UTIL_ERR_CHK(rtk_of_flowEntryInstruction_set(unit, phase, *index_ptr, &ins), ret);
    DUMP_INSTRUCTION(phase, ins, OF_INS_ACT_TYPE_WRITEACT, *index_ptr);
    return CPARSER_OK;
}   /* end of cparser_cmd_openflow_set_entry_phase_index_instruction_write_action_output_phy_port_phy_port_src_port_filter_port_uid */
#endif
...
#ifdef CMD_OPENFLOW_SET_ENTRY_PHASE_INDEX_INSTRUCTION_WRITE_ACTION_OUTPUT_TUNNEL_TID
/*
 * openflow set entry <UINT:phase> <UINT:index> instruction write-action output tunnel <UINT:tid>
 */
cparser_result_t
cparser_cmd_openflow_set_entry_phase_index_instruction_write_action_output_tunnel_tid(
    cparser_context_t *context,
    uint32_t *phase_ptr,
    uint32_t *index_ptr,
    uint32_t *tid_ptr)
{
    ...
    IGR_SET_EGR_RET(phase, igrwa, output_data.type, type);
    IGR_SET_EGR_RET(phase, igrwa, output_data.tunnel_idx, *tid_ptr);
    DIAG_UTIL_ERR_CHK(rtk_of_flowEntryInstruction_set(unit, phase, *index_ptr, &ins), ret);
    DUMP_INSTRUCTION(phase, ins, OF_INS_ACT_TYPE_WRITEACT, *index_ptr);

    return CPARSER_OK;
}   /* end of cparser_cmd_openflow_set_entry_phase_index_instruction_write_action_output_tunnel_tid */
#endif
...
#ifdef CMD_OPENFLOW_GET_FLOWTABLE_SELECTOR_BLOCK_ID
/*
 * openflow get flowtable selector <UINT:block_id>
 */
cparser_result_t
cparser_cmd_openflow_get_flowtable_selector_block_id(
    cparser_context_t *context,
    uint32_t *block_id_ptr)
{
    ...
    DIAG_UTIL_ERR_CHK(rtk_of_ftTemplateSelector_get(unit, *block_id_ptr, &idx), ret);
...
#ifdef CMD_OPENFLOW_SET_FLOWTABLE_SELECTOR_BLOCK_ID_TEMPLATE_INDEX0_TEMPLATE_INDEX
/*
 * openflow set flowtable selector <UINT:block_id> template-index0 <UINT:template_index>
 */
cparser_result_t
cparser_cmd_openflow_set_flowtable_selector_block_id_template_index0_template_index(
    cparser_context_t *context,
    uint32_t *block_id_ptr,
    uint32_t *template_index_ptr)
{
    ...
    DIAG_UTIL_ERR_CHK(rtk_of_ftTemplateSelector_set(unit, *block_id_ptr, idx), ret);
    DIAG_UTIL_MPRINTF("Block id %u template-index0 set to template %u\n", *block_id_ptr, *template_index_ptr);

    return CPARSER_OK;
}   /* end of cparser_cmd_openflow_set_flowtable_selector_block_id_template_index0_template_index */
#endif

#ifdef CMD_OPENFLOW_SET_FLOWTABLE_SELECTOR_BLOCK_ID_TEMPLATE_INDEX1_TEMPLATE_INDEX
/*
 * openflow set flowtable selector <UINT:block_id> template-index1 <UINT:template_index>
 */
cparser_result_t
cparser_cmd_openflow_set_flowtable_selector_block_id_template_index1_template_index(
    cparser_context_t *context,
    uint32_t *block_id_ptr,
    uint32_t *template_index_ptr)
{
    ...
    DIAG_UTIL_ERR_CHK(rtk_of_ftTemplateSelector_set(unit, *block_id_ptr, idx), ret);
    DIAG_UTIL_MPRINTF("Block id %u template-index1 set to template %u\n", *block_id_ptr, *template_index_ptr);
...
#ifdef CMD_OPENFLOW_SET_ENTRY_PHASE_INDEX_COUNTER_MODE_PACKET_AND_BYTE_COUNTER_PACKET_COUNTER_AND_THRESHOLD_BYTE_COUNTER_AND_THRESHOLD
/*
 * openflow set entry <UINT:phase> <UINT:index> counter mode ( packet-and-byte-counter |  packet-counter-and-threshold | byte-counter-and-threshold)
 */
cparser_result_t
cparser_cmd_openflow_set_entry_phase_index_counter_mode_packet_and_byte_counter_packet_counter_and_threshold_byte_counter_and_threshold(
    cparser_context_t *context,
    uint32_t *phase_ptr,
    uint32_t *index_ptr)
{
...
/*
 * openflow set l2-entry type ( smac | dmac ) <MACADDR:mac> ( <UINT:metadata> | none ) instruction write-action output phy-port-src-port-filter <UINT:port> { <UINT:uid> }
 */
cparser_result_t
cparser_cmd_openflow_set_l2_entry_type_smac_dmac_mac_metadata_none_instruction_write_action_output_phy_port_src_port_filter_port_uid(
    cparser_context_t *context,
    cparser_macaddr_t *mac_ptr,
    uint32_t *metadata_ptr,
    uint32_t *port_ptr,
    uint32_t *uid_ptr)
{
    ...
    if (uid_ptr)
        entry.ins.wa_data.output_data.unit = *uid_ptr;
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_openflow_set_l2_entry_type_smac_dmac_mac_metadata_none_instruction_write_action_output_phy_port_src_port_filter_port_uid */
#endif
...
/*
 * openflow set l2-entry type ( vid-smac | vid-dmac ) <UINT:vid> <MACADDR:mac> ( <UINT:metadata> | none ) instruction write-action output phy-port-src-port-filter <UINT:port> { <UINT:uid> }
 */
cparser_result_t
cparser_cmd_openflow_set_l2_entry_type_vid_smac_vid_dmac_vid_mac_metadata_none_instruction_write_action_output_phy_port_src_port_filter_port_uid(
    cparser_context_t *context,
    uint32_t *vid_ptr,
    cparser_macaddr_t *mac_ptr,
    uint32_t *metadata_ptr,
    ...
    uint32_t *uid_ptr)
{
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_openflow_set_l2_entry_type_vid_smac_vid_dmac_vid_mac_metadata_none_instruction_write_action_output_phy_port_src_port_filter_port_uid */
#endif
...
/*
 * openflow set l2-entry type smac-dmac <MACADDR:smac> <MACADDR:dmac> ( <UINT:metadata> | none ) instruction write-action output phy-port-src-port-filter <UINT:port> { <UINT:uid> }
 */
cparser_result_t
cparser_cmd_openflow_set_l2_entry_type_smac_dmac_smac_dmac_metadata_none_instruction_write_action_output_phy_port_src_port_filter_port_uid(
    cparser_context_t *context,
    cparser_macaddr_t *smac_ptr,
    cparser_macaddr_t *dmac_ptr,
    uint32_t *metadata_ptr,
    ...
    uint32_t *uid_ptr)
{
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_openflow_set_l2_entry_type_smac_dmac_smac_dmac_metadata_none_instruction_write_action_output_phy_port_src_port_filter_port_uid */
#endif
...
/*
 * openflow set l2-entry type ( 5-tuple-ipproto | 5-tuple-vid ) <IPV4ADDR:sip> <IPV4ADDR:dip> <UINT:l4_sport> <UINT:l4_dport> <UINT:ipproto_vid> instruction write-action output phy-port-src-port-filter <UINT:port> { <UINT:uid> }
 */
cparser_result_t
cparser_cmd_openflow_set_l2_entry_type_5_tuple_ipproto_5_tuple_vid_sip_dip_l4_sport_l4_dport_ipproto_vid_instruction_write_action_output_phy_port_src_port_filter_port_uid(
    cparser_context_t *context,
    uint32_t *sip_ptr,
    uint32_t *dip_ptr,
    uint32_t *l4_sport_ptr,
    ...
    uint32_t *uid_ptr)
{
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_openflow_set_l2_entry_type_5_tuple_ipproto_5_tuple_vid_sip_dip_l4_sport_l4_dport_ipproto_vid_instruction_write_action_output_phy_port_src_port_filter_port_uid */
#endif
...
#ifdef CMD_OPENFLOW_SET_L2_ENTRY_TYPE_SMAC_DMAC_MAC_METADATA_NONE_INSTRUCTION_WRITE_ACTION_OUTPUT_TUNNEL_TID
/*
 * openflow set l2-entry type ( smac | dmac ) <MACADDR:mac> ( <UINT:metadata> | none ) instruction write-action output tunnel <UINT:tid>
 */
cparser_result_t
cparser_cmd_openflow_set_l2_entry_type_smac_dmac_mac_metadata_none_instruction_write_action_output_tunnel_tid(
    cparser_context_t *context,
    cparser_macaddr_t *mac_ptr,
    uint32_t *metadata_ptr,
    uint32_t *tid_ptr)
{
    ...
    entry.ins.wa_data.output_data.tunnel_idx = *tid_ptr;
    entry.flags |= RTK_OF_FLAG_FLOWENTRY_REPLACE;
    DIAG_UTIL_ERR_CHK(rtk_of_l2FlowEntry_add(unit, &entry), ret);
    DUMP_L2_INSTRUCTION(entry, OF_INS_ACT_TYPE_WRITEACT);

    return CPARSER_OK;
}   /* end of cparser_cmd_openflow_set_l2_entry_type_smac_dmac_mac_metadata_none_instruction_write_action_output_tunnel_tid */
#endif

#ifdef CMD_OPENFLOW_SET_L2_ENTRY_TYPE_VID_SMAC_VID_DMAC_VID_MAC_METADATA_NONE_INSTRUCTION_WRITE_ACTION_OUTPUT_TUNNEL_TID
/*
 * openflow set l2-entry type ( vid-smac | vid-dmac ) <UINT:vid> ( <UINT:metadata> | none ) instruction write-action output tunnel <UINT:tid>
 */
cparser_result_t
cparser_cmd_openflow_set_l2_entry_type_vid_smac_vid_dmac_vid_mac_metadata_none_instruction_write_action_output_tunnel_tid(
    cparser_context_t *context,
    uint32_t *vid_ptr,
    cparser_macaddr_t *mac_ptr,
    uint32_t *metadata_ptr,
    uint32_t *tid_ptr)
{
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_openflow_set_l2_entry_type_vid_smac_vid_dmac_vid_metadata_none_instruction_write_action_output_tunnel_tid */
#endif

#ifdef CMD_OPENFLOW_SET_L2_ENTRY_TYPE_SMAC_DMAC_SMAC_DMAC_METADATA_NONE_INSTRUCTION_WRITE_ACTION_OUTPUT_TUNNEL_TID
/*
 * openflow set l2-entry type smac-dmac <MACADDR:smac> <MACADDR:dmac> ( <UINT:metadata> | none ) instruction write-action output tunnel <UINT:tid>
 */
cparser_result_t
cparser_cmd_openflow_set_l2_entry_type_smac_dmac_smac_dmac_metadata_none_instruction_write_action_output_tunnel_tid(
    cparser_context_t *context,
    cparser_macaddr_t *smac_ptr,
    cparser_macaddr_t *dmac_ptr,
    uint32_t *metadata_ptr,
    uint32_t *tid_ptr)
{
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_openflow_set_l2_entry_type_smac_dmac_smac_dmac_metadata_none_instruction_write_action_output_tunnel_tid */
#endif

#ifdef CMD_OPENFLOW_SET_L2_ENTRY_TYPE_5_TUPLE_IPPROTO_5_TUPLE_VID_SIP_DIP_L4_SPORT_L4_DPORT_IPPROTO_VID_INSTRUCTION_WRITE_ACTION_OUTPUT_TUNNEL_TID
/*
 * openflow set l2-entry type ( 5-tuple-ipproto | 5-tuple-vid ) <IPV4ADDR:sip> <IPV4ADDR:dip> <UINT:l4_sport> <UINT:l4_dport> <UINT:ipproto_vid> instruction write-action output tunnel <UINT:tid>
 */
cparser_result_t
cparser_cmd_openflow_set_l2_entry_type_5_tuple_ipproto_5_tuple_vid_sip_dip_l4_sport_l4_dport_ipproto_vid_instruction_write_action_output_tunnel_tid(
    cparser_context_t *context,
    uint32_t *sip_ptr,
    uint32_t *dip_ptr,
    uint32_t *l4_sport_ptr,
    ...
    uint32_t *tid_ptr)
{
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_openflow_set_l2_entry_type_5_tuple_ipproto_5_tuple_vid_sip_dip_l4_sport_l4_dport_ipproto_vid_instruction_write_action_output_tunnel_tid */
#endif
...
/*
 * openflow add l3-cam-entry <UINT:index> instruction write-action output ( phy-port | phy-port-src-port-filter ) <UINT:port> { <UINT:uid> }
 */
cparser_result_t
cparser_cmd_openflow_add_l3_cam_entry_index_instruction_write_action_output_phy_port_phy_port_src_port_filter_port_uid(
    cparser_context_t *context,
    uint32_t *index_ptr,
    uint32_t *port_ptr,
    uint32_t *uid_ptr)
{
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_openflow_add_l3_cam_entry_index_instruction_write_action_output_phy_port_phy_port_src_port_filter_port_uid */
#endif
...
#ifdef CMD_OPENFLOW_ADD_L3_CAM_ENTRY_INDEX_INSTRUCTION_WRITE_ACTION_OUTPUT_FAILOVER_TUNNEL_TID
/*
 * openflow add l3-cam-entry <UINT:index> instruction write-action output failover tunnel <UINT:tid>
 */
cparser_result_t
cparser_cmd_openflow_add_l3_cam_entry_index_instruction_write_action_output_failover_tunnel_tid(
    cparser_context_t *context,
    uint32_t *index_ptr,
    uint32_t *tid_ptr)
{
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_openflow_add_l3_cam_entry_index_instruction_write_action_output_failover_tunnel_tid */
#endif
    ...
    uint32_t *entry_num_ptr)
{
    ...
    DIAG_UTIL_MPRINTF("Move %u l3 cam entry from %u to %u\n", *entry_num_ptr, *start_ptr, *end_ptr);
...
/*
 * openflow set l3-hash-entry type ( sip | dip ) <IPV4ADDR:ip> ( <UINT:metadata> | none ) instrutcion write-action output ( phy-port | phy-port-src-port-filter ) <UINT:port> { <UINT:uid> }
 */
cparser_result_t
cparser_cmd_openflow_set_l3_hash_entry_type_sip_dip_ip_metadata_none_instrutcion_write_action_output_phy_port_phy_port_src_port_filter_port_uid(
    cparser_context_t *context,
    uint32_t *ip_ptr,
    uint32_t *metadata_ptr,
    uint32_t *port_ptr,
    uint32_t *uid_ptr)
{
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_openflow_set_l3_hash_entry_type_sip_dip_ip_metadata_none_instrutcion_write_action_output_phy_port_phy_port_src_port_filter_port_uid */
#endif
...
/*
 * openflow set l3-hash-entry type sip-dip <IPV4ADDR:sip> <IPV4ADDR:dip> ( <UINT:metadata> | none ) instrutcion write-action output ( phy-port | phy-port-src-port-filter ) <UINT:port> { <UINT:uid> }
 */
cparser_result_t
cparser_cmd_openflow_set_l3_hash_entry_type_sip_dip_sip_dip_metadata_none_instrutcion_write_action_output_phy_port_phy_port_src_port_filter_port_uid(
    cparser_context_t *context,
    uint32_t *sip_ptr,
    uint32_t *dip_ptr,
    uint32_t *metadata_ptr,
    ...
    uint32_t *uid_ptr)
{
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_openflow_set_l3_hash_entry_type_sip_dip_sip_dip_metadata_none_instrutcion_write_action_output_phy_port_phy_port_src_port_filter_port_uid */
#endif
...
/*
 * openflow set l3-hash-entry type ( sip6 | dip6 ) <IPV6ADDR:ip> ( <UINT:metadata> | none ) instrutcion write-action output ( phy-port | phy-port-src-port-filter ) <UINT:port> { <UINT:uid> }
 */
cparser_result_t
cparser_cmd_openflow_set_l3_hash_entry_type_sip6_dip6_ip_metadata_none_instrutcion_write_action_output_phy_port_phy_port_src_port_filter_port_uid(
    cparser_context_t *context,
    char **ip_ptr,
    uint32_t *metadata_ptr,
    uint32_t *port_ptr,
    uint32_t *uid_ptr)
{
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_openflow_set_l3_hash_entry_type_sip6_dip6_ip_metadata_none_instrutcion_write_action_output_phy_port_phy_port_src_port_filter_port_uid */
#endif
...
/*
 * openflow set l3-hash-entry type sip6-dip6 <IPV6ADDR:sip> <IPV6ADDR:dip> ( <UINT:metadata> | none ) instrutcion write-action output ( phy-port | phy-port-src-port-filter ) <UINT:port> { <UINT:uid> }
 */
cparser_result_t
cparser_cmd_openflow_set_l3_hash_entry_type_sip6_dip6_sip_dip_metadata_none_instrutcion_write_action_output_phy_port_phy_port_src_port_filter_port_uid(
    cparser_context_t *context,
    char **sip_ptr,
    char **dip_ptr,
    uint32_t *metadata_ptr,
    ...
    uint32_t *uid_ptr)
{
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_openflow_set_l3_hash_entry_type_sip6_dip6_sip_dip_metadata_none_instrutcion_write_action_output_phy_port_phy_port_src_port_filter_port_uid */
#endif
...
#ifdef CMD_OPENFLOW_SET_L3_HASH_ENTRY_TYPE_SIP_DIP_IP_METADATA_NONE_INSTRUTCION_WRITE_ACTION_OUTPUT_TUNNEL_TID
/*
 * openflow set l3-hash-entry type ( sip | dip ) <IPV4ADDR:ip> ( <UINT:metadata> | none ) instrutcion write-action output tunnel <UINT:tid>
 */
cparser_result_t
cparser_cmd_openflow_set_l3_hash_entry_type_sip_dip_ip_metadata_none_instrutcion_write_action_output_tunnel_tid(
    cparser_context_t *context,
    uint32_t *ip_ptr,
    uint32_t *metadata_ptr,
    uint32_t *tid_ptr)
{
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_openflow_set_l3_hash_entry_type_sip_dip_ip_metadata_none_instrutcion_write_action_output_tunnel_tid */
#endif

#ifdef CMD_OPENFLOW_SET_L3_HASH_ENTRY_TYPE_SIP_DIP_SIP_DIP_METADATA_NONE_INSTRUTCION_WRITE_ACTION_OUTPUT_TUNNEL_TID
/*
 * openflow set l3-hash-entry type sip-dip <IPV4ADDR:sip> <IPV4ADDR:dip> ( <UINT:metadata> | none ) instrutcion write-action output tunnel <UINT:tid>
 */
cparser_result_t
cparser_cmd_openflow_set_l3_hash_entry_type_sip_dip_sip_dip_metadata_none_instrutcion_write_action_output_tunnel_tid(
    cparser_context_t *context,
    uint32_t *sip_ptr,
    uint32_t *dip_ptr,
    uint32_t *metadata_ptr,
    uint32_t *tid_ptr)
{
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_openflow_set_l3_hash_entry_type_sip_dip_sip_dip_metadata_none_instrutcion_write_action_output_tunnel_tid */
#endif

#ifdef CMD_OPENFLOW_SET_L3_HASH_ENTRY_TYPE_SIP6_DIP6_IP_METADATA_NONE_INSTRUTCION_WRITE_ACTION_OUTPUT_TUNNEL_TID
/*
 * openflow set l3-hash-entry type ( sip6 | dip6 ) <IPV6ADDR:ip> ( <UINT:metadata> | none ) instrutcion write-action output tunnel <UINT:tid>
 */
cparser_result_t
cparser_cmd_openflow_set_l3_hash_entry_type_sip6_dip6_ip_metadata_none_instrutcion_write_action_output_tunnel_tid(
    cparser_context_t *context,
    char **ip_ptr,
    uint32_t *metadata_ptr,
    uint32_t *tid_ptr)
{
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_openflow_set_l3_hash_entry_type_sip6_dip6_ip_metadata_none_instrutcion_write_action_output_tunnel_tid */
#endif

#ifdef CMD_OPENFLOW_SET_L3_HASH_ENTRY_TYPE_SIP6_DIP6_SIP_DIP_METADATA_NONE_INSTRUTCION_WRITE_ACTION_OUTPUT_TUNNEL_TID
/*
 * openflow set l3-hash-entry type sip6-dip6 <IPV6ADDR:sip> <IPV6ADDR:dip> ( <UINT:metadata> | none ) instrutcion write-action output tunnel <UINT:tid>
 */
cparser_result_t
cparser_cmd_openflow_set_l3_hash_entry_type_sip6_dip6_sip_dip_metadata_none_instrutcion_write_action_output_tunnel_tid(
    cparser_context_t *context,
    char **sip_ptr,
    char **dip_ptr,
    uint32_t *metadata_ptr,
    uint32_t *tid_ptr)
{
    ...
    return CPARSER_OK;
}   /* end of cparser_cmd_openflow_set_l3_hash_entry_type_sip6_dip6_sip_dip_metadata_none_instrutcion_write_action_output_tunnel_tid */
#endif
...
/*
 * openflow set group action-bucket <UINT:index> action output ( phy-port | phy-port-src-port-filter ) <UINT:port> { <UINT:uid> }
 */
cparser_result_t
cparser_cmd_openflow_set_group_action_bucket_index_action_output_phy_port_phy_port_src_port_filter_port_uid(
    cparser_context_t *context,
    uint32_t *index_ptr,
    uint32_t *port_ptr,
    uint32_t *uid_ptr)
{
    ...
    if (uid_ptr)
        ab.output_data.unit = *uid_ptr;
    DIAG_UTIL_ERR_CHK(rtk_of_actionBucket_set(unit, *index_ptr, &ab), ret);
    DUMP_ACTION_BUCKET(ab, *index_ptr);

    return CPARSER_OK;
}   /* end of cparser_cmd_openflow_set_group_action_bucket_index_action_output_phy_port_phy_port_src_port_filter_port_uid */
#endif
...
#ifdef CMD_OPENFLOW_SET_GROUP_ACTION_BUCKET_INDEX_ACTION_OUTPUT_TUNNEL_TID
/*
 * openflow set group action-bucket <UINT:index> action output tunnel <UINT:tid>
 */
cparser_result_t
cparser_cmd_openflow_set_group_action_bucket_index_action_output_tunnel_tid(
    cparser_context_t *context,
    uint32_t *index_ptr,
    uint32_t *tid_ptr)
{
    ...
    ab.output_data.tunnel_idx = *tid_ptr;
    DIAG_UTIL_ERR_CHK(rtk_of_actionBucket_set(unit, *index_ptr, &ab), ret);
    DUMP_ACTION_BUCKET(ab, *index_ptr);

    return CPARSER_OK;
}   /* end of cparser_cmd_openflow_set_group_action_bucket_index_action_output_tunnel_tid */
#endif
