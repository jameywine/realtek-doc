 ...
 *
 * $Revision: 79931 $
 * $Date: 2017-06-22 09:48:15 +0800 (Thu, 22 Jun 2017) $
 *
...
#ifdef CMD_IPMCAST_SET_ADDR_VRF_ID_VRF_ID_SIP6_SIP_DIP6_DIP_INTF_VLAN_ID_RPF_STATE_ENABLE_DISABLE
/*
 * ipmcast set addr vrf-id <UINT:vrf_id> sip6 <IPV6ADDR:sip> dip6 <IPV6ADDR:dip> ( intf | vlan ) <UINT:id> rpf state ( enable | disable )
 */
cparser_result_t
cparser_cmd_ipmcast_set_addr_vrf_id_vrf_id_sip6_sip_dip6_dip_intf_vlan_id_rpf_state_enable_disable(
    cparser_context_t *context,
    uint32_t *vrf_id_ptr,
    char **sip_ptr,
    char **dip_ptr,
    ...
    uint32_t *end_index_ptr)
{
    ...
    do
    {
        ...
        {
            entryNum++;
            DUMP_IPMC_ADDR_LH(ipmcAddr, base);
        }
    } while (base >= 0);
