 ...
 *
 * $Revision: 81343 $
 * $Date: 2017-08-16 17:38:30 +0800 (Wed, 16 Aug 2017) $
 *
...
#ifdef CMD_BANDWIDTH_GET_INGRESS_FIXED_BANDWIDTH_PORT_PORTS_ALL_QUEUE_QUEUE_ID_STATE
/*
 * bandwidth get ingress fixed-bandwidth port ( <PORT_LIST:ports> | all ) queue <UINT:queue_id> state
 */
cparser_result_t cparser_cmd_bandwidth_get_ingress_fixed_bandwidth_port_ports_all_queue_queue_id_state(cparser_context_t *context,
    char **ports_ptr,
    uint32_t *queue_id_ptr)
{
    uint32      unit = 0;
    ...
    DIAG_UTIL_PORTMASK_SCAN(portlist, port)
    {
        diag_util_mprintf("Ingress Fixed Bandwidth Ability\n");
        DIAG_UTIL_ERR_CHK(rtk_rate_portIgrQueueFixedBwCtrlEnable_get(unit, port, *queue_id_ptr, &enable), ret);
        if (ENABLED == enable)
        {
            diag_util_mprintf("Port %u Queue %u : Enable\n", port, *queue_id_ptr);
        }
...
#ifdef CMD_BANDWIDTH_SET_INGRESS_FIXED_BANDWIDTH_PORT_PORTS_ALL_QUEUE_QUEUE_ID_STATE_DISABLE_ENABLE
/*
 * bandwidth set ingress fixed-bandwidth port ( <PORT_LIST:ports> | all ) queue <UINT:queue_id> state ( disable | enable )
 */
cparser_result_t cparser_cmd_bandwidth_set_ingress_fixed_bandwidth_port_ports_all_queue_queue_id_state_disable_enable(cparser_context_t *context,
    char **ports_ptr,
    uint32_t *queue_id_ptr)
{
    uint32      unit = 0;
    ...
    DIAG_UTIL_PARSE_STATE(9, enable);

    DIAG_UTIL_PORTMASK_SCAN(portlist, port)
    {
        DIAG_UTIL_ERR_CHK(rtk_rate_portIgrQueueFixedBwCtrlEnable_set(unit, port, *queue_id_ptr, enable), ret);
    }
