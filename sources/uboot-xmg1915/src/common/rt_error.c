 ...
 *
 * $Revision: 74787 $
 * $Date: 2016-12-28 19:05:26 +0800 (Wed, 28 Dec 2016) $
 *
...
const uint8 *rt_error_numToStr(int32 err_num)
{
    switch (err_num)
    {
        ...
        /* CAPWAP */
        case RT_ERR_CAPWAP_NO_EMPTY_ENTRY:
            return (uint8 *)"No empty entry";
        case RT_ERR_CAPWAP_ENTRY_EXIST:
            return (uint8 *)"entry already exist";
        case RT_ERR_CAPWAP_BSSIDLIST_TBL_FULL:
            return (uint8 *)"BSSID-List table full";
        case RT_ERR_CAPWAP_WIRELESS_FWD_NOT_EXIST:
            return (uint8 *)"This multicast entry doesn't contain wireless tunnel";
        case RT_ERR_CAPWAP_BSSID_NOT_EXIST:
            return (uint8 *)"BSSID not exist";

        /*Stacking*/
        case RT_ERR_STACK_ID:
            return (uint8 *)"Invalid stacking ID";
