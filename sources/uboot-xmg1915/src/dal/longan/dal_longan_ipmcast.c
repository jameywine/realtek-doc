 ...
 *
 * $Revision: 81353 $
 * $Date: 2017-08-16 19:40:18 +0800 (Wed, 16 Aug 2017) $
 *
...
#define DEFLT_LONGAN_IPMC_MAC_IP_ERR_ACT    RTK_IPMC_ACT_FORWARD
#define DEFLT_LONGAN_IPMC_EXT_HEAD_ACT      RTK_IPMC_ACT_DROP
...
static uint32 _actIpmcRouteCtrlBadSip[] = {   /* LONGAN_L3_IPMC_ROUTE_CTRLr, LONGAN_BAD_SIP_ACTf */
    ...
    RTK_IPMC_ACT_FORWARD,          /*tmac-base bridge*/
    };
...
static uint32 _actIpmcRouteCtrlZeroSip[] = {  /* LONGAN_L3_IPMC_ROUTE_CTRLr, LONGAN_ZERO_SIP_ACTf */
    ...
    RTK_IPMC_ACT_TRAP2CPU,         /*trap + mac-base bridge*/
    RTK_IPMC_ACT_TRAP2MASTERCPU,   /*trap + mac-base bridge*/
    RTK_IPMC_ACT_FORWARD,          /*mac-base bridge*/
    };
...
static inline int32 _dal_longan_ipmc_mtu_get(uint32 unit, uint32 hw_index, dal_longan_ipmc_mtu_info_t *pMtu)
{
    ...
    RT_ERR_HDL(_dal_longan_l3_hostEntry_get(unit, hw_index, &hostEntry, \
    ...
    uint32 reallocMtu)
{
    ...
    if (l2fwdIdx < 0)
        pEntry->l2_en = DISABLED;
    else
    {
        pEntry->l2_en = ENABLED;
        pEntry->l2_mc_pmsk_idx = l2fwdIdx;
    }

    if (l3oilIdx < 0)
    {
        pEntry->l3_en = DISABLED;
        pEntry->oil_idx_valid = FALSE;
    }
    else
    {
        pEntry->l3_en = ENABLED;
...
_dal_longan_ipmc_hostHash0_ret(uint32 unit, rtk_ipmc_addr_t *pIpmcAddr, uint32 move_dip_0_8)
{
     ...
     */
    if (IS_L3_ECO_VER(unit)) /*old hash alg0*/
    {
        if (pIpmcAddr->ipmc_flags & RTK_IPMC_FLAG_IPV6)
        {
            if (pIpmcAddr->ipmc_flags & RTK_IPMC_FLAG_SIP)
            ...
            /* source IPv6 address */
            hashRow[0] = 0x0 | \
                          HASH_BIT_EXTRACT(sipv6.octet[0], 6, 2);      /* SIP [127:126] */
            hashRow[1] = (HASH_BIT_EXTRACT(sipv6.octet[0], 0, 6) << 3) | \
                          HASH_BIT_EXTRACT(sipv6.octet[1], 5, 3);     /* SIP [125:117] */
            hashRow[2] = (HASH_BIT_EXTRACT(sipv6.octet[1], 0, 5) << 4) | \
                          HASH_BIT_EXTRACT(sipv6.octet[2], 4, 4);     /* SIP [116:108] */
            hashRow[3] = (HASH_BIT_EXTRACT(sipv6.octet[2], 0, 4) << 5) | \
                          HASH_BIT_EXTRACT(sipv6.octet[3], 3, 5);     /* SIP [107:99] */
            hashRow[4] = (HASH_BIT_EXTRACT(sipv6.octet[3], 0, 3) << 6) | \
                          HASH_BIT_EXTRACT(sipv6.octet[4], 2, 6);     /* SIP [99:90] */
            hashRow[5] = (HASH_BIT_EXTRACT(sipv6.octet[4], 0, 2) << 7) | \
                          HASH_BIT_EXTRACT(sipv6.octet[5], 1, 7);     /* SIP [89:81] */
            hashRow[6] = (HASH_BIT_EXTRACT(sipv6.octet[5], 0, 1) << 8) | \
                          HASH_BIT_EXTRACT(sipv6.octet[6], 0, 8);     /* SIP [80:72] */
            hashRow[7] = (HASH_BIT_EXTRACT(sipv6.octet[7], 0, 8) << 1) | \
                          HASH_BIT_EXTRACT(sipv6.octet[8], 7, 1);     /* SIP [71:63] */
            hashRow[8] = (HASH_BIT_EXTRACT(sipv6.octet[8], 0, 7) << 2) | \
                          HASH_BIT_EXTRACT(sipv6.octet[9], 6, 2);     /* SIP [62:54] */
            hashRow[9] = (HASH_BIT_EXTRACT(sipv6.octet[9], 0, 6) << 3) | \
                          HASH_BIT_EXTRACT(sipv6.octet[10], 5, 3);    /* SIP [53:45] */
            hashRow[10] = (HASH_BIT_EXTRACT(sipv6.octet[10], 0, 5) << 4) | \
                           HASH_BIT_EXTRACT(sipv6.octet[11], 4, 4);   /* SIP [44:36] */
            hashRow[11] = (HASH_BIT_EXTRACT(sipv6.octet[11], 0, 4) << 5) | \
                           HASH_BIT_EXTRACT(sipv6.octet[12], 3, 5);   /* SIP [35:27] */
            hashRow[12] = (HASH_BIT_EXTRACT(sipv6.octet[12], 0, 3) << 6) | \
                           HASH_BIT_EXTRACT(sipv6.octet[13], 2, 6);   /* SIP [26:18] */
            hashRow[13] = (HASH_BIT_EXTRACT(sipv6.octet[13], 0, 2) << 7) | \
                           HASH_BIT_EXTRACT(sipv6.octet[14], 1, 7);   /* SIP [17:9] */
            hashRow[14] = (HASH_BIT_EXTRACT(sipv6.octet[14], 0, 1) << 8) | \
                           HASH_BIT_EXTRACT(sipv6.octet[15], 0, 8);   /* SIP [8:0] */
            /* destination IPv6 address */
            hashRow[15] = 0x0 | \
                          HASH_BIT_EXTRACT(pIpmcAddr->dst_ip_addr.ipv6.octet[0], 6, 2);      /* DIP [127:126] */
            hashRow[16] = (HASH_BIT_EXTRACT(pIpmcAddr->dst_ip_addr.ipv6.octet[0], 0, 6) << 3) | \
                          HASH_BIT_EXTRACT(pIpmcAddr->dst_ip_addr.ipv6.octet[1], 5, 3);     /* DIP [125:117] */
            hashRow[17] = (HASH_BIT_EXTRACT(pIpmcAddr->dst_ip_addr.ipv6.octet[1], 0, 5) << 4) | \
                          HASH_BIT_EXTRACT(pIpmcAddr->dst_ip_addr.ipv6.octet[2], 4, 4);     /* DIP [116:108] */
            hashRow[18] = (HASH_BIT_EXTRACT(pIpmcAddr->dst_ip_addr.ipv6.octet[2], 0, 4) << 5) | \
                          HASH_BIT_EXTRACT(pIpmcAddr->dst_ip_addr.ipv6.octet[3], 3, 5);     /* DIP [107:99] */
            hashRow[19] = (HASH_BIT_EXTRACT(pIpmcAddr->dst_ip_addr.ipv6.octet[3], 0, 3) << 6) | \
                          HASH_BIT_EXTRACT(pIpmcAddr->dst_ip_addr.ipv6.octet[4], 2, 6);     /* DIP [99:90] */
            hashRow[20] = (HASH_BIT_EXTRACT(pIpmcAddr->dst_ip_addr.ipv6.octet[4], 0, 2) << 7) | \
                          HASH_BIT_EXTRACT(pIpmcAddr->dst_ip_addr.ipv6.octet[5], 1, 7);     /* DIP [89:81] */
            hashRow[21] = (HASH_BIT_EXTRACT(pIpmcAddr->dst_ip_addr.ipv6.octet[5], 0, 1) << 8) | \
                          HASH_BIT_EXTRACT(pIpmcAddr->dst_ip_addr.ipv6.octet[6], 0, 8);     /* DIP [80:72] */
            hashRow[22] = (HASH_BIT_EXTRACT(pIpmcAddr->dst_ip_addr.ipv6.octet[7], 0, 8) << 1) | \
                          HASH_BIT_EXTRACT(pIpmcAddr->dst_ip_addr.ipv6.octet[8], 7, 1);     /* DIP [71:63] */
            hashRow[23] = (HASH_BIT_EXTRACT(pIpmcAddr->dst_ip_addr.ipv6.octet[8], 0, 7) << 2) | \
                          HASH_BIT_EXTRACT(pIpmcAddr->dst_ip_addr.ipv6.octet[9], 6, 2);     /* DIP [62:54] */
            hashRow[24] = (HASH_BIT_EXTRACT(pIpmcAddr->dst_ip_addr.ipv6.octet[9], 0, 6) << 3) | \
                          HASH_BIT_EXTRACT(pIpmcAddr->dst_ip_addr.ipv6.octet[10], 5, 3);    /* DIP [53:45] */
            hashRow[25] = (HASH_BIT_EXTRACT(pIpmcAddr->dst_ip_addr.ipv6.octet[10], 0, 5) << 4) | \
                           HASH_BIT_EXTRACT(pIpmcAddr->dst_ip_addr.ipv6.octet[11], 4, 4);   /* DIP [44:36] */
            hashRow[26] = (HASH_BIT_EXTRACT(pIpmcAddr->dst_ip_addr.ipv6.octet[11], 0, 4) << 5) | \
                           HASH_BIT_EXTRACT(pIpmcAddr->dst_ip_addr.ipv6.octet[12], 3, 5);   /* DIP [35:27] */
            hashRow[27] = (HASH_BIT_EXTRACT(pIpmcAddr->dst_ip_addr.ipv6.octet[12], 0, 3) << 6) | \
                           HASH_BIT_EXTRACT(pIpmcAddr->dst_ip_addr.ipv6.octet[13], 2, 6);   /* DIP [26:18] */
            hashRow[28] = (HASH_BIT_EXTRACT(pIpmcAddr->dst_ip_addr.ipv6.octet[13], 0, 2) << 7) | \
                           HASH_BIT_EXTRACT(pIpmcAddr->dst_ip_addr.ipv6.octet[14], 1, 7);   /* DIP [17:9] */
            hashRow[29] = (HASH_BIT_EXTRACT(pIpmcAddr->dst_ip_addr.ipv6.octet[14], 0, 1) << 8) | \
                           HASH_BIT_EXTRACT(pIpmcAddr->dst_ip_addr.ipv6.octet[15], 0, 8);   /* DIP [8:0] */
        }
        else  /*IPV4*/
        {
            if (pIpmcAddr->ipmc_flags & RTK_IPMC_FLAG_SIP)
            ...
            /* source IPv4 address */
            hashRow[11] = HASH_BIT_EXTRACT(sipv4, 27, 5);
            hashRow[12] = HASH_BIT_EXTRACT(sipv4, 18, 9);
            hashRow[13] = HASH_BIT_EXTRACT(sipv4, 9, 9);
            hashRow[14] = HASH_BIT_EXTRACT(sipv4, 0, 9);
    ...
    if (IS_L3_ECO_VER(unit))
    {
        ...
        hashIdx = \
            ...
            (((hashRow[13] & 0x01F) << 4) | ((hashRow[13] & 0x1E0) >> 5)) ^ hashRow[12] ^ \
                                                                            hashRow[14] ^ \
            (((hashRow[16] & 0x01F) << 4) | ((hashRow[16] & 0x1E0) >> 5)) ^ hashRow[15] ^ \
            (((hashRow[18] & 0x01F) << 4) | ((hashRow[18] & 0x1E0) >> 5)) ^ hashRow[17] ^ \
            (((hashRow[20] & 0x01F) << 4) | ((hashRow[20] & 0x1E0) >> 5)) ^ hashRow[19] ^ \
            (((hashRow[22] & 0x01F) << 4) | ((hashRow[22] & 0x1E0) >> 5)) ^ hashRow[21] ^ \
            (((hashRow[24] & 0x01F) << 4) | ((hashRow[24] & 0x1E0) >> 5)) ^ hashRow[23] ^ \
            (((hashRow[26] & 0x01F) << 4) | ((hashRow[26] & 0x1E0) >> 5)) ^ hashRow[25] ^ \
            (((hashRow[28] & 0x01F) << 4) | ((hashRow[28] & 0x1E0) >> 5)) ^ hashRow[27] ^ \
                                                                            hashRow[29] ^ \
            ((hashRow[30] & 0x07) << 4) ^ hashRow[31] ;
    }
    else    /*old hash alg1*/
    {
        if (pIpmcAddr->ipmc_flags & RTK_IPMC_FLAG_IPV6)
...
//only vidHashSel == 1
static int32
_dal_longan_ipmc_dip_get(uint32 unit, uint32 hw_entry_idx, rtk_ipmc_addr_t *pIpmcAddr)
{
    int32 ret = RT_ERR_OK;
    ...
    if (DAL_LONGAN_IPMC_L3TBL_HASH_KEY_SIP_GIP_VID == _pIpmcDb[unit]->l3TblHashKey)
    {
        ...
        hashIdx.hashIdx_of_alg[0] = _dal_longan_ipmc_hostHash0_ret(unit, pIpmcAddr, TRUE);
        hashIdx.hashIdx_of_alg[1] = _dal_longan_ipmc_hostHash1_ret(unit, pIpmcAddr, TRUE);
        ...
        if (pIpmcAddr->ipmc_flags & RTK_IPMC_FLAG_IPV6)
        {
            pIpmcAddr->dst_ip_addr.ipv6.octet[15] = dip_0_8;
            pIpmcAddr->dst_ip_addr.ipv6.octet[14] = (pIpmcAddr->dst_ip_addr.ipv6.octet[14] & 0xfe) | (dip_0_8 >> 8);
        }
        else
        {
            pIpmcAddr->dst_ip_addr.ipv4 = (pIpmcAddr->dst_ip_addr.ipv4 & 0xfffffe00) | dip_0_8;
        }
    }
    else
    {
    ...
    dal_longan_ipmc_l3Entry_t *pEntry)
{
    ...
    /* function body */
    RT_ERR_HDL(_dal_longan_l3_hostEntry_get(unit, pEntry->hw_entry_idx, &hostEntry, \
        DAL_LONGAN_L3_API_FLAG_NONE), errHandle, ret);
    RT_ERR_HDL(_dal_longan_ipmc_l3HostEntry2rtkIpmcAddr(unit, pIpmcAddr, &hostEntry, pEntry->groupId), errHandle, ret);
    RT_ERR_HDL(_dal_longan_ipmc_dip_get(unit, pEntry->hw_entry_idx, pIpmcAddr), errHandle, ret);
    ...
    uint32 isRetEntry)
{
    ...
    ret = _dal_longan_l3_hostEntry_get(unit, pEntry->hw_entry_idx, &hostEntry, DAL_LONGAN_L3_API_FLAG_NONE);
    ret += _dal_longan_ipmc_l3HostEntry2rtkIpmcAddr(unit, &ipmcAddr, &hostEntry, pEntry->groupId);
    ret += _dal_longan_ipmc_dip_get(unit, pEntry->hw_entry_idx, &ipmcAddr);
    if (RT_ERR_OK != ret)
    {
...
{
    int32 ret = RT_ERR_OK;
    uint32 tbl, tfIdx, tfL3Oilvalid, tfL3En, valIdx, valL3Oil, valL3En;
    dal_longan_ipmc_l3Entry_t *pEntry;
    dal_longan_l3_hostEntry_t hostEntry;
    ...
    if (L3_ENTRY_IS_VALID(pEntry))
    {
        ...
        if (TRUE == pEntry->ipv6)
        {
            ...
            tfL3En = LONGAN_L3_HOST_ROUTE_IP6MC_L3_ENtf;
        }
        else
        {
            tbl = LONGAN_L3_HOST_ROUTE_IPMCt;
            ...
            tfL3En = LONGAN_L3_HOST_ROUTE_IPMC_L3_ENtf;
        }

        IPMCAST_TABLE_READ_ERR_HDL(unit, tbl, pEntry->hw_entry_idx, hostEntry, "", errHandle, ret);
        IPMCAST_TABLE_FIELD_SET_ERR_HDL(unit, tbl, tfL3Oilvalid, valL3Oil, hostEntry, "", errHandle, ret);
        IPMCAST_TABLE_FIELD_SET_ERR_HDL(unit, tbl, tfIdx, valIdx, hostEntry, "", errHandle, ret);
        IPMCAST_TABLE_FIELD_SET_ERR_HDL(unit, tbl, tfL3En, valL3En, hostEntry, "", errHandle, ret);
        IPMCAST_TABLE_WRITE_ERR_HDL(unit, tbl, pEntry->hw_entry_idx, hostEntry, "", errHandle, ret);
    }
...
/* Function Name:
 *      _dal_longan_ipmc_act_init
 * Description:
 *      Initialize ipmcast module error packet act default.
 * Input:
 *      unit - unit id
 * Output:
 *      None
 ...
 * Note:
 *      1, Must initialize ipmcast module before calling any ipmcast APIs.
 *      2, L3_IPMC_ROUTE_CTRL Chip default
 *          DROP: DMAC_MISMATCH_ACT/ ZERO_SIP_ACT/ BAD_SIP_ACT
 *      3, L3_IP6MC_ROUTE_CTRL chip default
 *          TRAP: HDR_ROUTE_ACT/ HBH_ACT
 *          DROP:  DMAC_MISMATCH_ACT/ ZERO_SIP_ACT/ BAD_SIP_ACT
 */
static int32
_dal_longan_ipmc_defaultAct_init(uint32 unit)
{
    typedef struct regActCtrl_s
    {
        rtk_ipmc_globalCtrlType_t   type;
        int32                      act;
    }regActCtrl_t;
    ...
    regActCtrl_t    regInfo[] = {
        {RTK_IPMC_GCT_IPMC_DMAC_MISMATCH_ACT,    DEFLT_LONGAN_IPMC_MAC_IP_ERR_ACT},
        {RTK_IPMC_GCT_IPMC_BAD_SIP_ACT,          DEFLT_LONGAN_IPMC_MAC_IP_ERR_ACT},
        {RTK_IPMC_GCT_IPMC_ZERO_SIP_ACT,         DEFLT_LONGAN_IPMC_MAC_IP_ERR_ACT},
        {RTK_IPMC_GCT_IP6MC_DMAC_MISMATCH_ACT,   DEFLT_LONGAN_IPMC_MAC_IP_ERR_ACT},
        {RTK_IPMC_GCT_IP6MC_BAD_SIP_ACT,         DEFLT_LONGAN_IPMC_MAC_IP_ERR_ACT},
        {RTK_IPMC_GCT_IP6MC_ZERO_SIP_ACT,        DEFLT_LONGAN_IPMC_MAC_IP_ERR_ACT},

        {RTK_IPMC_GCT_IP6MC_HBH_ACT,             DEFLT_LONGAN_IPMC_EXT_HEAD_ACT},
        {RTK_IPMC_GCT_IP6MC_HDR_ROUTE_ACT,       DEFLT_LONGAN_IPMC_EXT_HEAD_ACT},
    };

    for (i = 0; i < sizeof(regInfo)/sizeof(regActCtrl_t); i++)
    {
        RT_ERR_HDL(dal_longan_ipmc_globalCtrl_set(unit, regInfo[i].type, regInfo[i].act), errHandle, ret);
    }
...
dal_longan_ipmc_init(uint32 unit)
{
    ...
    _pIpmcDb[unit]->l3TblHashKey = DAL_LONGAN_IPMC_L3TBL_HASH_KEY_SIP_GIP;
    if (!IS_L3_ECO_VER(unit))
    {
    ...
    RT_ERR_HDL(_dal_longan_ipmc_defaultAct_init(unit), errHandle, ret);
...
dal_longan_ipmc_addr_add(uint32 unit, rtk_ipmc_addr_t *pIpmcAddr)
{
    ...
    RT_PARAM_CHK(((0 == pIpmcAddr->src_intf.vlan_id) || (pIpmcAddr->src_intf.vlan_id > RTK_VLAN_ID_MAX)), RT_ERR_VLAN_VID);
    ...
    if (NULL != pEntry)
    {
        ...
        if (pIpmcAddr->ipmc_flags & RTK_IPMC_FLAG_REPLACE)
        {
            ...
            if (pEntry->groupId != pIpmcAddr->group)
            {
                ...
                RT_ERR_HDL(_dal_longan_mcast_ipmc_unbind(unit, pEntry->groupId, &entryIdx), errHandle, ret);
                RT_ERR_HDL(_dal_longan_l3_ipmc_mtuEntry_free(unit, mtu.idx, pEntry->groupId), errHandle, ret);
            }

            hostEntry.mtu_max_idx = mtu.idx;
            RT_ERR_HDL(_dal_longan_ipmc_rtkIpmcAddr2l3HostEntry(unit, &hostEntry, pIpmcAddr, reallocMtu), errHandle, ret);
...
dal_longan_ipmc_addr_del(uint32 unit, rtk_ipmc_addr_t *pIpmcAddr)
{
    ...
    RT_ERR_HDL(_dal_longan_mcast_ipmc_unbind(unit, pEntry->groupId, &l3EntryIdx), errHandle, ret);
...
dal_longan_ipmc_addr_getNext(uint32 unit, rtk_ipmc_flag_t flags, int32 *pBase, rtk_ipmc_addr_t *pIpmcAddr)
{
    ...
    for (idx=(*pBase<0)?(0):((*pBase)+1); idx<DAL_LONGAN_L3_TBL_SIZE; idx++)
    {
        if (L3_HWENTRY_IDX_IS_VALID(unit, idx))
        {
            ...
            RT_ERR_HDL(_dal_longan_ipmc_rtkIpmcAddr_get(unit, pIpmcAddr, pEntry), errHandle, ret);
