 ...
 *
 * $Revision: 77499 $
 * $Date: 2017-04-12 15:35:45 +0800 (Wed, 12 Apr 2017) $
 *
...
dal_longan_rate_init(uint32 unit)
{
    ...
    if((ret = dal_longan_rate_cpuEgrBandwidthCtrlRateMode_set(unit, RATE_MODE_PKT)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_RATE), "");
        return ret;
    }
    ...
    burstSize = 5;
    if((ret = dal_longan_rate_portEgrBwCtrlBurstSize_set(unit, HWP_CPU_MACID(unit), burstSize)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_RATE), "");
        return ret;
    }
    ...
    {
        if((ret = dal_longan_rate_portEgrQueueBwCtrlBurstSize_set(unit, HWP_CPU_MACID(unit), queue, burstSize)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_RATE), "");
            return ret;
        }
    ...
    rtk_port_t              port)
{
    ...
    /* write value to CHIP*/
    if ((ret = reg_array_field_write1toClear(unit, LONGAN_IGBW_PORT_LB_RSTr, port, REG_ARRAY_INDEX_NONE, LONGAN_RSTf)) != RT_ERR_OK)
    {
        RATE_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_RATE), "");
        return ret;
...
dal_longan_rate_portEgrBwCtrlRate_set(uint32 unit, rtk_port_t port, uint32 rate)
{
    ...
    /* read value from CHIP*/
    if ((ret = reg_array_field_write(unit, LONGAN_EGBW_PORT_CTRLr, port, REG_ARRAY_INDEX_NONE, LONGAN_RATEf, &rate)) != RT_ERR_OK)
    {
        RATE_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_RATE), "");
        return ret;
    ...
    rtk_enable_t    *pEnable)
{
    ...
    /* read value from CHIP*/
    if ((ret = reg_array_field_read(unit, (HWP_IS_CPU_PORT(unit, port) ? LONGAN_EGBW_CPU_Q_MAX_LB_CTRLr : (HWP_UPLINK_PORT(unit, port) ? LONGAN_EGBW_PORT_Q_MAX_LB_CTRL_SET1r : LONGAN_EGBW_PORT_Q_MAX_LB_CTRL_SET0r)),  (HWP_IS_CPU_PORT(unit, port) ? REG_ARRAY_INDEX_NONE : port), queue, LONGAN_ENf, &value)) != RT_ERR_OK)
    {
        RATE_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_RATE), "");
        return ret;
    ...
    rtk_enable_t    enable)
{
    ...
    /* read value from CHIP*/
    if ((ret = reg_array_field_write(unit, (HWP_IS_CPU_PORT(unit, port) ? LONGAN_EGBW_CPU_Q_MAX_LB_CTRLr : (HWP_UPLINK_PORT(unit, port) ? LONGAN_EGBW_PORT_Q_MAX_LB_CTRL_SET1r : LONGAN_EGBW_PORT_Q_MAX_LB_CTRL_SET0r)),  (HWP_IS_CPU_PORT(unit, port) ? REG_ARRAY_INDEX_NONE : port), queue, LONGAN_ENf, &value)) != RT_ERR_OK)
    {
        RATE_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_RATE), "");
        return ret;
    ...
    uint32      *pRate)
{
    ...
    /* read value from CHIP*/
    if ((ret = reg_array_field_read(unit, (HWP_IS_CPU_PORT(unit, port) ? LONGAN_EGBW_CPU_Q_MAX_LB_CTRLr : (HWP_UPLINK_PORT(unit, port) ? LONGAN_EGBW_PORT_Q_MAX_LB_CTRL_SET1r : LONGAN_EGBW_PORT_Q_MAX_LB_CTRL_SET0r)),  (HWP_IS_CPU_PORT(unit, port) ? REG_ARRAY_INDEX_NONE : port), queue, LONGAN_RATEf, pRate)) != RT_ERR_OK)
    {
        RATE_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_RATE), "");
        return ret;
    ...
    uint32      rate)
{
    ...
    /* write value to CHIP*/
    if ((ret = reg_array_field_write(unit, (HWP_IS_CPU_PORT(unit, port) ? LONGAN_EGBW_CPU_Q_MAX_LB_CTRLr : (HWP_UPLINK_PORT(unit, port) ? LONGAN_EGBW_PORT_Q_MAX_LB_CTRL_SET1r : LONGAN_EGBW_PORT_Q_MAX_LB_CTRL_SET0r)),  (HWP_IS_CPU_PORT(unit, port) ? REG_ARRAY_INDEX_NONE : port), queue, LONGAN_RATEf, &rate)) != RT_ERR_OK)
    {
        RATE_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_RATE), "");
        return ret;
...
dal_longan_rate_portEgrQueueBwCtrlBurstSize_get(uint32 unit, rtk_port_t port, rtk_qid_t queue, uint32 *pBurst_size)
{
    ...
    /* read value from CHIP*/
    if ((ret = reg_array_field_read(unit, (HWP_IS_CPU_PORT(unit, port) ? LONGAN_EGBW_CPU_Q_MAX_LB_CTRLr : (HWP_UPLINK_PORT(unit, port) ? LONGAN_EGBW_PORT_Q_MAX_LB_CTRL_SET1r : LONGAN_EGBW_PORT_Q_MAX_LB_CTRL_SET0r)),  (HWP_IS_CPU_PORT(unit, port) ? REG_ARRAY_INDEX_NONE : port), queue, LONGAN_BURSTf, pBurst_size)) != RT_ERR_OK)
    {
        RATE_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_RATE), "");
        return ret;
...
dal_longan_rate_portEgrQueueBwCtrlBurstSize_set(uint32 unit, rtk_port_t port, rtk_qid_t queue, uint32 burst_size)
{
    ...
    /* write value to CHIP*/
    if ((ret = reg_array_field_write(unit, (HWP_IS_CPU_PORT(unit, port) ? LONGAN_EGBW_CPU_Q_MAX_LB_CTRLr : (HWP_UPLINK_PORT(unit, port) ? LONGAN_EGBW_PORT_Q_MAX_LB_CTRL_SET1r : LONGAN_EGBW_PORT_Q_MAX_LB_CTRL_SET0r)),  (HWP_IS_CPU_PORT(unit, port) ? REG_ARRAY_INDEX_NONE : port), queue, LONGAN_BURSTf, &burst_size)) != RT_ERR_OK)
    {
        RATE_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_RATE), "");
        return ret;
    ...
    rtk_enable_t    *pEnable)
{
    ...
    {
        if ((ret = reg_array_field_read(unit, (HWP_UPLINK_PORT(unit, port) ? LONGAN_EGBW_PORT_Q_ASSURED_LB_CTRL_SET1r : LONGAN_EGBW_PORT_Q_ASSURED_LB_CTRL_SET0r),  port, queue, LONGAN_ENf, &value)) != RT_ERR_OK)
        {
            RATE_SEM_UNLOCK(unit);
            RT_ERR(ret, (MOD_DAL|MOD_RATE), "");
            return ret;
    ...
    {
        if ((ret = reg_array_field_read(unit, (HWP_UPLINK_PORT(unit, port) ? LONGAN_EGBW_PORT_Q_FIX_LB_CTRL_SET1r : LONGAN_EGBW_PORT_Q_FIX_LB_CTRL_SET0r),  port, queue, LONGAN_ENf, &value)) != RT_ERR_OK)
        {
            RATE_SEM_UNLOCK(unit);
            RT_ERR(ret, (MOD_DAL|MOD_RATE), "");
            return ret;
    ...
    rtk_enable_t    enable)
{
    ...
    if(egrPortAssuedRateMode[unit][port].queueRateMode[queue] == ASSURED_MODE_SHARE)
    {
        if ((ret = reg_array_field_write(unit, (HWP_UPLINK_PORT(unit, port) ? LONGAN_EGBW_PORT_Q_ASSURED_LB_CTRL_SET1r : LONGAN_EGBW_PORT_Q_ASSURED_LB_CTRL_SET0r),  port, queue, LONGAN_ENf, &value)) != RT_ERR_OK)
        {
            RATE_SEM_UNLOCK(unit);
            RT_ERR(ret, (MOD_DAL|MOD_RATE), "");
            return ret;
        ...
        value=0;
        if ((ret = reg_array_field_write(unit, (HWP_UPLINK_PORT(unit, port) ? LONGAN_EGBW_PORT_Q_FIX_LB_CTRL_SET1r : LONGAN_EGBW_PORT_Q_FIX_LB_CTRL_SET0r),  port, queue, LONGAN_ENf, &value)) != RT_ERR_OK)
        {
            RATE_SEM_UNLOCK(unit);
            RT_ERR(ret, (MOD_DAL|MOD_RATE), "");
            return ret;
...
dal_longan_rate_portEgrQueueAssuredBwCtrlRate_get(uint32 unit, rtk_port_t port, rtk_qid_t queue, uint32 *pRate)
{
    ...
    /* read value from CHIP*/
    if ((ret = reg_array_field_read(unit, (HWP_UPLINK_PORT(unit, port) ? LONGAN_EGBW_PORT_Q_ASSURED_LB_CTRL_SET1r : LONGAN_EGBW_PORT_Q_ASSURED_LB_CTRL_SET0r),  port, queue, LONGAN_RATEf, pRate)) != RT_ERR_OK)
    {
        RATE_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_RATE), "");
        return ret;
...
dal_longan_rate_portEgrQueueAssuredBwCtrlRate_set(uint32 unit, rtk_port_t port, rtk_qid_t queue, uint32 rate)
{
    ...
    /* read value from CHIP*/
    if ((ret = reg_array_field_write(unit, (HWP_UPLINK_PORT(unit, port) ? LONGAN_EGBW_PORT_Q_ASSURED_LB_CTRL_SET1r : LONGAN_EGBW_PORT_Q_ASSURED_LB_CTRL_SET0r),  port, queue, LONGAN_RATEf, &rate)) != RT_ERR_OK)
    {
        RATE_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_RATE), "");
        return ret;
    }
    if ((ret = reg_array_field_write(unit, (HWP_UPLINK_PORT(unit, port) ? LONGAN_EGBW_PORT_Q_FIX_LB_CTRL_SET1r : LONGAN_EGBW_PORT_Q_FIX_LB_CTRL_SET0r),  port, queue, LONGAN_RATEf, &rate)) != RT_ERR_OK)
    {
        RATE_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_RATE), "");
        return ret;
...
dal_longan_rate_portEgrQueueAssuredBwCtrlBurstSize_get(uint32 unit, rtk_port_t port, rtk_qid_t queue, uint32 *pBurst_size)
{
    ...
    /* read value from CHIP*/
    if ((ret = reg_array_field_read(unit, (HWP_UPLINK_PORT(unit, port) ? LONGAN_EGBW_PORT_Q_ASSURED_FIX_BURST_CTRL_SET1r : LONGAN_EGBW_PORT_Q_ASSURED_FIX_BURST_CTRL_SET0r),  port, queue, LONGAN_BURSTf, pBurst_size)) != RT_ERR_OK)
    {
        RATE_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_RATE), "");
        return ret;
...
dal_longan_rate_portEgrQueueAssuredBwCtrlBurstSize_set(uint32 unit, rtk_port_t port, rtk_qid_t queue, uint32 burst_size)
{
    ...
    if(HWP_IS_CPU_PORT(unit, port))
        ...
        RT_PARAM_CHK((queue >= HAL_MAX_NUM_OF_QUEUE(unit)), RT_ERR_QUEUE_ID);
    RT_PARAM_CHK((burst_size > HAL_BURST_OF_BANDWIDTH_MAX(unit)), RT_ERR_RATE);
    ...
    /* read value from CHIP*/
    if ((ret = reg_array_field_write(unit, (HWP_UPLINK_PORT(unit, port) ? LONGAN_EGBW_PORT_Q_ASSURED_FIX_BURST_CTRL_SET1r : LONGAN_EGBW_PORT_Q_ASSURED_FIX_BURST_CTRL_SET0r),  port, queue, LONGAN_BURSTf, &burst_size)) != RT_ERR_OK)
    {
        RATE_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_RATE), "");
        return ret;
    ...
    rtk_rate_assuredMode_t    mode)
{
    ...
    if(egrPortAssuedRateMode[unit][port].queueRateMode[queue] == ASSURED_MODE_SHARE)
    {
        ...
        valueTmp = 0;
        if ((ret = reg_array_field_write(unit, (HWP_UPLINK_PORT(unit, port) ? LONGAN_EGBW_PORT_Q_ASSURED_LB_CTRL_SET1r : LONGAN_EGBW_PORT_Q_ASSURED_LB_CTRL_SET0r),	port, queue, LONGAN_ENf, &valueTmp)) != RT_ERR_OK)
        {
            RATE_SEM_UNLOCK(unit);
            RT_ERR(ret, (MOD_DAL|MOD_RATE), "");
            return ret;
    ...
    else
    {
        ...
        valueTmp = 0;
        if ((ret = reg_array_field_write(unit, (HWP_UPLINK_PORT(unit, port) ? LONGAN_EGBW_PORT_Q_FIX_LB_CTRL_SET1r : LONGAN_EGBW_PORT_Q_FIX_LB_CTRL_SET0r),  port, queue, LONGAN_ENf, &valueTmp)) != RT_ERR_OK)
        {
            RATE_SEM_UNLOCK(unit);
            RT_ERR(ret, (MOD_DAL|MOD_RATE), "");
            return ret;
    ...
    rtk_rate_storm_group_t  storm_type)
{
    ...
    {
        case STORM_GROUP_UNICAST:
            reg_idx = LONGAN_STORM_PORT_UC_LB_RSTr;
            break;
        case STORM_GROUP_MULTICAST:
            reg_idx = LONGAN_STORM_PORT_MC_LB_RSTr;
            break;
        case STORM_GROUP_BROADCAST:
            reg_idx = LONGAN_STORM_PORT_BC_LB_RSTr;
            break;
        default:
            return RT_ERR_INPUT;
    }
    ...
    /* read value from CHIP*/
    if ((ret = reg_array_field_write1toClear(unit, reg_idx, port, REG_ARRAY_INDEX_NONE, LONGAN_RSTf)) != RT_ERR_OK)
    {
        RATE_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_RATE), "");
        return ret;
    ...
    uint32                 burst_size)
{
    ...
    RT_PARAM_CHK(!HWP_PORT_EXIST(unit, port), RT_ERR_PORT_ID);
    RT_PARAM_CHK((storm_type >= STORM_PROTO_GROUP_END), RT_ERR_INPUT);
    RT_PARAM_CHK((burst_size >HAL_BURST_OF_STORM_PROTO_CONTROL_MAX(unit)), RT_ERR_INPUT);
    ...
    rtk_rate_storm_proto_group_t    storm_type)
{
    ...
    {
        case STORM_PROTO_GROUP_BPDU:
            reg_idx = LONGAN_STORM_PORT_PROTO_BPDU_LB_RSTr;break;
        case STORM_PROTO_GROUP_IGMP:
            reg_idx = LONGAN_STORM_PORT_PROTO_IGMP_LB_RSTr;break;
        case STORM_PROTO_GROUP_ARP:
            reg_idx = LONGAN_STORM_PORT_PROTO_ARP_LB_RSTr;break;
        case STORM_PROTO_GROUP_DHCP:
            reg_idx = LONGAN_STORM_PORT_PROTO_DHCP_LB_RSTr;break;
        default:
            return RT_ERR_INPUT;
    }
