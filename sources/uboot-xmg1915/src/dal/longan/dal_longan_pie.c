 ...
 *
 * $Revision: 80605 $
 * $Date: 2017-07-17 11:05:22 +0800 (Mon, 17 Jul 2017) $
 *
...
rtk_pie_phase_t    pie_blk_type[DAL_LONGAN_MAX_NUM_OF_PIE_BLOCK];
...
rtk_pie_template_t dal_longan_pie_fixedTemplate[] =
{
 ...
 */
static int32
_dal_longan_pie_phy2logTmplteField(uint32 unit, uint32 phy_field_id, rtk_pie_templateFieldType_t *log_field_id);
static int32
    ...
    rtk_pie_template_t *pTemplate)
{
    ...
    {
        osal_memcpy(pTemplate, &dal_longan_pie_fixedTemplate[template_id],
    ...
    pTemplate->field_num = 0;
    for (field_idx = 0; field_idx < DAL_LONGAN_MAX_NUM_OF_TEMPLATE_FIELD; field_idx++)
    {
        PIE_SEM_LOCK(unit);
        if ((ret = reg_array_field_read(unit, LONGAN_PIE_TMPLTE_CTRLr, template_id, field_idx, LONGAN_TMPLTE_FIELDf, &value)) != RT_ERR_OK)
        {
            PIE_SEM_UNLOCK(unit);
            RT_ERR(ret, (MOD_DAL|MOD_PIE), "");
            return ret;
        ...
        if ((ret = _dal_longan_pie_phy2logTmplteField(unit, value, &pTemplate->field[field_idx])) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_PIE), "");
            return ret;
        }
 ...
 */
static int32 _dal_longan_pie_phy2logTmplteField(uint32 unit, uint32 phy_field_id, rtk_pie_templateFieldType_t *log_field_id)
{
    uint32  i;
    ...
    for (i = 0; dal_longan_template_field_list[i].field_type != TMPLTE_FIELD_END; i++)
    {
        if (phy_field_id == dal_longan_template_field_list[i].physical_id)
        {
            /* Get the logical field ID */
            *log_field_id = dal_longan_template_field_list[i].field_type;
            return RT_ERR_OK;
        }
    }
