 ...
 *
 * $Revision: 76832 $
 * $Date: 2017-03-22 20:11:47 +0800 (Wed, 22 Mar 2017) $
 *
 ...
 */
static int32 _dal_longan_qos_init_config(uint32 unit, uint32 queueNum);

/* Function Name:
 *      _dal_longan_qos_init_config
 * Description:
 ...
 */
static int32
_dal_longan_qos_init_config(uint32 unit, uint32 queueNum)
{
   int32 ret;
   uint32 queue;
   ...
   if((ret = dal_longan_qos_schedulingAlgorithm_set(unit, HWP_CPU_MACID(unit), WRR)) != RT_ERR_OK)
   {
    ...
    {
        congAvoidThresh.maxThresh   = 0x69;
        congAvoidThresh.minThresh   = 0x5f;
        congAvoidThresh.probability = 0x7f;
        if ((ret = dal_longan_qos_congAvoidGlobalQueueConfig_set(unit, queue, 0, &congAvoidThresh)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_QOS), "");
            return ret;
        }

        congAvoidThresh.maxThresh   = 0x46;
        congAvoidThresh.minThresh   = 0x3c;
        congAvoidThresh.probability = 0x7f;
        if ((ret = dal_longan_qos_congAvoidGlobalQueueConfig_set(unit, queue, 1, &congAvoidThresh)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_QOS), "");
            return ret;
        }

        congAvoidThresh.maxThresh   = 0x23;
        congAvoidThresh.minThresh   = 0x19;
        congAvoidThresh.probability = 0x7f;
        if ((ret = dal_longan_qos_congAvoidGlobalQueueConfig_set(unit, queue, 2, &congAvoidThresh)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_QOS), "");
            return ret;
        }
    ...
    return RT_ERR_OK;
} /* end of _dal_longan_qos_init_config */
...
dal_longan_qos_init(uint32 unit, uint32 queueNum)
{
    ...
    if ((ret = _dal_longan_qos_init_config(unit, queueNum)) != RT_ERR_OK)
    {
        qos_init[unit] = INIT_NOT_COMPLETED;
        RT_ERR(ret, (MOD_DAL|MOD_QOS), "");
        return ret;
...
dal_longan_qos_port1pDfltPri_get(uint32 unit, rtk_port_t port, rtk_pri_t *pPri)
{
    ...
    /* get value from CHIP*/
    if ((ret = dal_longan_qos_port1pDfltPriExt_get(unit, unit, port, pPri)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_QOS), "");
        return ret;
    }
...
dal_longan_qos_port1pDfltPri_set(uint32 unit, rtk_port_t port, rtk_pri_t pri)
{
    ...
    /* set value to CHIP*/
    if ((ret = dal_longan_qos_port1pDfltPriExt_set(unit, unit, port, pri)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_QOS), "");
        return ret;
    }
 ...
 */
int32
dal_longan_qos_port1pDfltPriExt_get(uint32 unit, uint32 srcUnit, rtk_port_t port, rtk_pri_t *pPri)
{
    int32   ret;
    uint32  idx;
 ...
 */
int32
dal_longan_qos_port1pDfltPriExt_set(uint32 unit, uint32 srcUnit, rtk_port_t port, rtk_pri_t pri)
{
    int32  ret;
    uint32 idx;
    rmk_entry_t rmkEntry;
...
dal_longan_qos_port1pDfltPriSrcSel_get(uint32 unit, rtk_port_t port, rtk_qos_1pDfltPriSrc_t *pSrc)
{
    ...
    /* get value from CHIP*/
    if ((ret = dal_longan_qos_port1pDfltPriSrcSelExt_get(unit, unit, port, pSrc)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_QOS), "");
        return ret;
    }
...
dal_longan_qos_port1pDfltPriSrcSel_set(uint32 unit, rtk_port_t port, rtk_qos_1pDfltPriSrc_t src)
{
    ...
    /* set value to CHIP*/
    if ((ret = dal_longan_qos_port1pDfltPriSrcSelExt_set(unit, unit, port, src)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_QOS), "");
        return ret;
    }
 ...
 */
int32
dal_longan_qos_port1pDfltPriSrcSelExt_get(uint32 unit, uint32 srcUnit, rtk_port_t port, rtk_qos_1pDfltPriSrc_t *pSrc)
{
    int32   ret;
    uint32  value, idx;
    rmk_entry_t rmkEntry;
 ...
 */
int32
dal_longan_qos_port1pDfltPriSrcSelExt_set(uint32 unit, uint32 srcUnit, rtk_port_t port, rtk_qos_1pDfltPriSrc_t src)
{
    int32  ret;
    uint32 value, idx;
    rmk_entry_t rmkEntry;
...
dal_longan_qos_portOuter1pDfltPri_get(uint32 unit, rtk_port_t port, rtk_pri_t *pPri)
{
    ...
    /* get value from CHIP*/
    if ((ret = dal_longan_qos_portOuter1pDfltPriExt_get(unit, unit, port, pPri)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_QOS), "");
        return ret;
    }
...
dal_longan_qos_portOuter1pDfltPri_set(uint32 unit, rtk_port_t port, rtk_pri_t pri)
{
    ...
    /* set value to CHIP*/
    if ((ret = dal_longan_qos_portOuter1pDfltPriExt_set(unit, unit, port, pri)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_QOS), "");
        return ret;
    }
 ...
 */
int32
dal_longan_qos_portOuter1pDfltPriExt_get(uint32 unit, uint32 srcUnit, rtk_port_t port, rtk_pri_t *pPri)
{
    int32   ret;
    uint32  idx;
    rmk_entry_t rmkEntry;
 ...
 */
int32
dal_longan_qos_portOuter1pDfltPriExt_set(uint32 unit, uint32 srcUnit, rtk_port_t port, rtk_pri_t pri)
{
    int32       ret;
    uint32      idx;
    rmk_entry_t rmkEntry;
...
dal_longan_qos_portOuter1pDfltPriSrcSel_get(uint32 unit, rtk_port_t port, rtk_qos_outer1pDfltSrc_t *pType)
{
    ...
    /* get value from CHIP*/
    if ((ret = dal_longan_qos_portOuter1pDfltPriSrcSelExt_get(unit, unit, port, pType)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_QOS), "");
        return ret;
    }
...
dal_longan_qos_portOuter1pDfltPriSrcSel_set(uint32 unit, rtk_port_t port, rtk_qos_outer1pDfltSrc_t type)
{
    ...
    if ((ret = dal_longan_qos_portOuter1pDfltPriSrcSelExt_set(unit, unit, port, type)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_QOS), "");
        return ret;
    }
 ...
 */
int32
dal_longan_qos_portOuter1pDfltPriSrcSelExt_get(uint32 unit, uint32 srcUnit, rtk_port_t port, rtk_qos_outer1pDfltSrc_t *pSrc)
{
    int32   ret;
    uint32  value, idx;
    rmk_entry_t rmkEntry;
 ...
 */
int32
dal_longan_qos_portOuter1pDfltPriSrcSelExt_set(uint32 unit, uint32 srcUnit, rtk_port_t port, rtk_qos_outer1pDfltSrc_t src)
{
    int32       ret;
    uint32      value, idx;
    rmk_entry_t rmkEntry;
