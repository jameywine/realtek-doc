 ...
 *
 * $Revision: 81109 $
 * $Date: 2017-08-08 17:34:45 +0800 (Tue, 08 Aug 2017) $
 *
...
typedef struct dal_mcast_oilEntry_s{
    ...
    uint32      mc_pmsk_bssid_idx;       /* (multicast portmask) idnex */
        ...
        case RTK_MCAST_EGRIF_TYPE_L3:
            {
                ...
                pEntry->mc_pmsk_bssid_idx = pOil->fwdIndex;
            }
            break;

        case RTK_MCAST_EGRIF_TYPE_TUNNEL:
        case RTK_MCAST_EGRIF_TYPE_CAPWAP:
        default:
    ...
    case RTK_MCAST_EGRIF_TYPE_L3:
        {
            ...
            if (TRUE == pOil->bridge_mode)
                pEgrIf->flags = RTK_MCAST_NH_FLAG_BRIDGE;
...
_dal_longan_mcast_oil_sortKey_ret(rtk_mcast_group_t groupId,rtk_mcast_egrif_t *pEgrIf)
{
    ...
    sortKey =   (((pEgrIf->type) & 0xFF) << 24) | \
                      ...
                      (((groupIdx) & 0x7FF) << 1) | \
                      ((pEgrIf->flags & RTK_MCAST_NH_FLAG_BRIDGE)? 0x0 : 0x1);
...
_dal_longan_mcast_oilEntry_set(uint32 unit, uint32 index,dal_longan_mcast_oilEntry_t *pEntry)
{
    ...
    MCAST_TABLE_FIELD_SET_ERR_HDL(unit, LONGAN_L3_EGR_INTF_LISTt, \
        LONGAN_L3_EGR_INTF_LIST_MC_PMSK_IDXtf, pEntry->mc_pmsk_bssid_idx, oilEntry, "", errHandle, ret);
    MCAST_TABLE_FIELD_SET_ERR_HDL(unit, LONGAN_L3_EGR_INTF_LISTt, \
...
_dal_longan_mcast_ipEgrIfL2_add(uint32 unit, dal_longan_mcast_group_t  *pGroup, rtk_mcast_egrif_t *pEgrIf)
{
    ...
    else
    {
        ...
        /* if RTK_MCAST_NH_FLAG_PMSK_REPLACE is set or the portmask is newly created,
         * assign ports from user assigned value
         */
        if ((TRUE == fwdIndexAllocated) || (pEgrIf->flags & RTK_MCAST_NH_FLAG_PMSK_REPLACE))
        {
...
_dal_longan_mcast_ipEgrIfL3_add(uint32 unit, dal_longan_mcast_group_t *pGroup, rtk_mcast_egrif_t *pEgrIf)
{
    ...
    if (FALSE == oilFound)
    {
        ...
        pOil->bridge_mode = (pEgrIf->flags & RTK_MCAST_NH_FLAG_BRIDGE)? TRUE : FALSE;
...
_dal_longan_mcast_ipEgrIfStk_add(uint32 unit, dal_longan_mcast_group_t  *pGroup, rtk_mcast_egrif_t *pEgrIf)
{
    ...
    if (pEgrIf->flags & RTK_MCAST_NH_FLAG_PMSK_REPLACE)
    {
 ...
 */
int32
_dal_longan_mcast_ipmc_unbind(uint32 unit, rtk_mcast_group_t groupId, uint32 *pEntryIdx)
{
    int32 ret = RT_ERR_OK;
