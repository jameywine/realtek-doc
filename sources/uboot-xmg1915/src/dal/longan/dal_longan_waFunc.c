...
static int32 _dal_longan_port_serdes_linkSts_get(uint32 unit, uint32 sdsId, uint32 *pLinkSts)
{
    ...
    if((RTL9301_CHIP_ID_24G == HWP_CHIP_ID(unit)) || (RTL9301_CHIP_ID == HWP_CHIP_ID(unit)))
    {
        if(2 == sdsId)
        {
            if((ret = hal_serdes_reg_get(unit, sdsId, 1, 29 , &reg_data0)) != RT_ERR_OK)
                return ret;

            if((ret = hal_serdes_reg_get(unit, sdsId, 1, 29 , &reg_data1)) != RT_ERR_OK)
                return ret;

            if((ret = hal_serdes_reg_get(unit, sdsId, 1, 29 , &reg_data2)) != RT_ERR_OK)
                return ret;

            if((0x1ff == (reg_data1 & 0x1ff)) && (0x1ff == (reg_data2 & 0x1ff)))
            {
                if((ret = hal_serdes_reg_get(unit, sdsId + 1, 1, 29 , &reg_data0)) != RT_ERR_OK)
                    return ret;

                if((ret = hal_serdes_reg_get(unit, sdsId + 1, 1, 29 , &reg_data1)) != RT_ERR_OK)
                    return ret;

                if((ret = hal_serdes_reg_get(unit, sdsId + 1, 1, 29 , &reg_data2)) != RT_ERR_OK)
                    return ret;
            }
        }
        else if(3 == sdsId)
        {
            if((ret = hal_serdes_reg_get(unit, 10, 1, 29 , &reg_data0)) != RT_ERR_OK)
                return ret;

            if((ret = hal_serdes_reg_get(unit, 10, 1, 29 , &reg_data1)) != RT_ERR_OK)
                return ret;

            if((ret = hal_serdes_reg_get(unit, 10, 1, 29 , &reg_data2)) != RT_ERR_OK)
                return ret;

            if((0x1ff == (reg_data1 & 0x1ff)) && (0x1ff == (reg_data2 & 0x1ff)))
            {
                if((ret = hal_serdes_reg_get(unit, 11, 1, 29 , &reg_data0)) != RT_ERR_OK)
                    return ret;

                if((ret = hal_serdes_reg_get(unit, 11, 1, 29 , &reg_data1)) != RT_ERR_OK)
                    return ret;

                if((ret = hal_serdes_reg_get(unit, 11, 1, 29 , &reg_data2)) != RT_ERR_OK)
                    return ret;
            }
        }
        else
