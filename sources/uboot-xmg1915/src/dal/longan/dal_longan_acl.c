 ...
 *
 * $Revision: 80064 $
 * $Date: 2017-06-27 11:44:49 +0800 (Tue, 27 Jun 2017) $
 *
...
uint32 iacl_only_field_type[] = {USER_FIELD_IPUC_ROUT, USER_FIELD_IPMC_ROUT, USER_FIELD_VACL_DROP_HIT, USER_FIELD_VACL_COPY_HIT,
    USER_FIELD_VACL_REDIRECT_HIT,  USER_FIELD_VACL_ROUTING_HIT, USER_FIELD_ATTACK,
    ...
    USER_FIELD_STP_DROP, USER_FIELD_DPN, USER_FIELD_SRC_FWD_VID, USER_FIELD_END};
...
dal_longan_acl_fieldLocation_t DAL_LONGAN_FIELD_IGR_UNIT[] =
{
    {
        /* template field type */    TMPLTE_FIELD_SLP,
        /* offset address */         0x6,
...
dal_longan_acl_fieldLocation_t DAL_LONGAN_FIELD_DPN[] =
{
    {
        /* template field type */    TMPLTE_FIELD_DLP,
        /* offset address */         0x0,
...
dal_longan_acl_fieldLocation_t DAL_LONGAN_FIELD_EGR_UNIT[] =
{
    {
        /* template field type */    TMPLTE_FIELD_META_DATA,
        /* offset address */         0xc,
...
dal_longan_acl_entryField_t dal_longan_acl_field_list[] =
{
    ...
    {   /* field name    */           USER_FIELD_IGR_UNIT,
        /* field number  */           1,
        /* field pointer */           DAL_LONGAN_FIELD_IGR_UNIT
    },
    ...
    {   /* field name    */           USER_FIELD_DPN,
        /* field number  */           1,
        /* field pointer */           DAL_LONGAN_FIELD_DPN
    },
    {   /* field name    */           USER_FIELD_EGR_UNIT,
        /* field number  */           1,
        /* field pointer */           DAL_LONGAN_FIELD_EGR_UNIT
    },
    ...
    switch (value)
    {
        ...
        case 2:
            ...
            if (0 != (info >> 10))
                ...
                pAction->fwd_data.unit = (info >> 6) & 0xF;
            break;
        case 3:
    ...
    if (ENABLED == pAction->fwd_en)
    {
        switch (pAction->fwd_data.fwd_type)
        {
            ...
            case ACL_ACTION_FWD_COPY_TO_PORTID:
                ...
                {
                    if (pAction->fwd_data.flags & RTK_ACL_FWD_FLAG_LOCAL_UNIT)
                    {
                        fwd_unit = HWP_MY_UNIT_ID();
                    }
                    else
                    {
                        fwd_unit = pAction->fwd_data.unit;
                    }
...
dal_longan_acl_rule_move(uint32 unit, rtk_acl_phase_t phase, rtk_acl_move_t *pData)
{
    ...
    int32           srcEnd, srcLen, srcBlkStart, srcBlkEnd;
    int32           dstEnd, dstBlkStart, dstBlkEnd;
    int32           ret;
    ...
    if (pData->move_from > pData->move_to)
    {
        ...
        srcEnd = pData->move_from + pData->length;

        do
        {
            srcBlkEnd += HAL_MAX_NUM_OF_PIE_BLOCKSIZE(unit);
            if (srcBlkEnd > srcEnd)
                srcLen = srcEnd - srcBlkStart;
            else
                srcLen = srcBlkEnd - srcBlkStart;

            dstBlkStart = pData->move_to + (srcBlkStart - pData->move_from);
            dstBlkEnd = (dstBlkStart / HAL_MAX_NUM_OF_PIE_BLOCKSIZE(unit)) *
                    HAL_MAX_NUM_OF_PIE_BLOCKSIZE(unit);

            dstEnd = dstBlkStart + srcLen;

            do {
                dstBlkEnd += HAL_MAX_NUM_OF_PIE_BLOCKSIZE(unit);

                if (dstBlkEnd > dstEnd)
                    phyMov.length = dstEnd - dstBlkStart;
                else
                    phyMov.length = dstBlkEnd - dstBlkStart;
                ...
                dstBlkStart += phyMov.length;
                srcBlkStart += phyMov.length;
            } while (dstBlkStart < dstEnd);
        } while (srcBlkStart < srcEnd);
    }
    else if (pData->move_from < pData->move_to)
    {
        ...
        srcEnd = pData->move_from;

        do
        {
            if (srcBlkStart < srcEnd)
                srcBlkStart = srcEnd;

            srcLen = srcBlkEnd - srcBlkStart + 1;

            dstBlkEnd = pData->move_to + (srcBlkEnd - pData->move_from);
            dstBlkStart = (dstBlkEnd / HAL_MAX_NUM_OF_PIE_BLOCKSIZE(unit)) *
                    HAL_MAX_NUM_OF_PIE_BLOCKSIZE(unit);

            dstEnd = dstBlkEnd - srcLen + 1;

            do {
                if (dstBlkStart < dstEnd)
                    dstBlkStart = dstEnd;

                phyMov.length = dstBlkEnd - dstBlkStart + 1;

                srcBlkStart = srcBlkEnd - phyMov.length + 1;
                ...
                dstBlkEnd = dstBlkStart;
                srcBlkEnd = srcBlkStart;

                dstBlkStart -= HAL_MAX_NUM_OF_PIE_BLOCKSIZE(unit);
            } while (dstBlkEnd > dstEnd);
            srcBlkStart -= HAL_MAX_NUM_OF_PIE_BLOCKSIZE(unit);
        } while (srcBlkEnd > srcEnd);
    }
