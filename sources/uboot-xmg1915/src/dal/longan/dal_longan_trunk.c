 ...
 *
 * $Revision: 77751 $
 * $Date: 2017-04-14 19:33:10 +0800 (Fri, 14 Apr 2017) $
 *
 * Purpose :
 *
 * Feature :
...
dal_longan_trunk_port_set(uint32 unit, uint32 trk_gid, rtk_portmask_t *pTrunk_member_portmask)
{
    ...
    if (RTK_MAX_NUM_OF_TRUNK_MBR_PORT < num_ports)
        return RT_ERR_PORT_MASK;

    osal_memset(&origin_mbr, 0, sizeof(rtk_portmask_t));
    ret = dal_longan_trunk_localPort_get (unit, trk_gid, &origin_mbr);
    if(RT_ERR_OK != ret)
        return ret;

    ret = dal_longan_trunk_localPort_set (unit, trk_gid, pTrunk_member_portmask);
    if(RT_ERR_OK != ret)
        return ret;
    ...
    HWP_PORT_TRAVS_EXCEPT_CPU(unit, port)
    {
        ...
        {
            unitPort.port = port;
            ret = dal_longan_trunk_srcPortMap_set(unit, unitPort, 0, 0);
            if(RT_ERR_OK != ret)
                return ret;
        }
        ...
        if (RTK_PORTMASK_IS_PORT_SET(*pTrunk_member_portmask, port))
        {
            trk_egr_ports.egr_port[trk_egr_ports.num_ports].unit = unit;
            trk_egr_ports.egr_port[trk_egr_ports.num_ports].port = port;

            ret = dal_longan_trunk_srcPortMap_set(unit, trk_egr_ports.egr_port[trk_egr_ports.num_ports],
                    1, trk_gid);
            if(RT_ERR_OK != ret)
                return ret;
    ...
    {
        trk_egr_ports.egr_port[portIdx].unit = unit;
        trk_egr_ports.egr_port[portIdx].port = INVALID_TRUNK_MEMBER_PORT;
    }

    ret = dal_longan_trunk_egrPort_set (unit, trk_gid, &trk_egr_ports);
    if(RT_ERR_OK != ret)
        return ret;
...
dal_longan_trunk_egrPort_get(uint32 unit, rtk_trk_t trk_gid, rtk_trk_egrPort_t *pTrk_egr_ports)
{
    ...
    if ((ret = table_field_get(unit, LONGAN_LAGt,
                LONGAN_LAG_TRK_UNIT0tf, &val, (uint32 *)&lag_entry)) != RT_ERR_OK)
    {
        TRUNK_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_TRUNK|MOD_DAL), "");
        return ret;
    }
    pTrk_egr_ports->egr_port[0].unit = val;
    ...
    if ((ret = table_field_get(unit, LONGAN_LAGt,
                LONGAN_LAG_TRK_UNIT1tf, &val, (uint32 *)&lag_entry)) != RT_ERR_OK)
    {
        TRUNK_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_TRUNK|MOD_DAL), "");
        return ret;
    }
    pTrk_egr_ports->egr_port[1].unit = val;
    ...
    if ((ret = table_field_get(unit, LONGAN_LAGt,
                LONGAN_LAG_TRK_UNIT2tf, &val, (uint32 *)&lag_entry)) != RT_ERR_OK)
    {
        TRUNK_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_TRUNK|MOD_DAL), "");
        return ret;
    }
    pTrk_egr_ports->egr_port[2].unit = val;
    ...
    if ((ret = table_field_get(unit, LONGAN_LAGt,
                LONGAN_LAG_TRK_UNIT3tf, &val, (uint32 *)&lag_entry)) != RT_ERR_OK)
    {
        TRUNK_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_TRUNK|MOD_DAL), "");
        return ret;
    }
    pTrk_egr_ports->egr_port[3].unit = val;
    ...
    if ((ret = table_field_get(unit, LONGAN_LAGt,
                LONGAN_LAG_TRK_UNIT4tf, &val, (uint32 *)&lag_entry)) != RT_ERR_OK)
    {
        TRUNK_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_TRUNK|MOD_DAL), "");
        return ret;
    }
    pTrk_egr_ports->egr_port[4].unit = val;
    ...
    if ((ret = table_field_get(unit, LONGAN_LAGt,
                LONGAN_LAG_TRK_UNIT5tf, &val, (uint32 *)&lag_entry)) != RT_ERR_OK)
    {
        TRUNK_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_TRUNK|MOD_DAL), "");
        return ret;
    }
    pTrk_egr_ports->egr_port[5].unit = val;
    ...
    if ((ret = table_field_get(unit, LONGAN_LAGt,
                LONGAN_LAG_TRK_UNIT6tf, &val, (uint32 *)&lag_entry)) != RT_ERR_OK)
    {
        TRUNK_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_TRUNK|MOD_DAL), "");
        return ret;
    }
    pTrk_egr_ports->egr_port[6].unit = val;
    ...
    if ((ret = table_field_get(unit, LONGAN_LAGt,
                LONGAN_LAG_TRK_UNIT7tf, &val, (uint32 *)&lag_entry)) != RT_ERR_OK)
    {
        TRUNK_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_TRUNK|MOD_DAL), "");
        return ret;
    }
    pTrk_egr_ports->egr_port[7].unit = val;
...
dal_longan_trunk_egrPort_set(uint32 unit, rtk_trk_t trk_gid, rtk_trk_egrPort_t *pTrk_egr_ports)
{
    ...
    val = pTrk_egr_ports->egr_port[0].unit;
    if ((ret = table_field_set(unit, LONGAN_LAGt,
                LONGAN_LAG_TRK_UNIT0tf, &val, (uint32 *)&lag_entry)) != RT_ERR_OK)
    {
        TRUNK_SEM_UNLOCK(unit);
    ...
    val = pTrk_egr_ports->egr_port[1].unit;
    if ((ret = table_field_set(unit, LONGAN_LAGt,
                LONGAN_LAG_TRK_UNIT1tf, &val, (uint32 *)&lag_entry)) != RT_ERR_OK)
    {
        TRUNK_SEM_UNLOCK(unit);
    ...
    val = pTrk_egr_ports->egr_port[2].unit;
    if ((ret = table_field_set(unit, LONGAN_LAGt,
                LONGAN_LAG_TRK_UNIT2tf, &val, (uint32 *)&lag_entry)) != RT_ERR_OK)
    {
        TRUNK_SEM_UNLOCK(unit);
    ...
    val = pTrk_egr_ports->egr_port[3].unit;
    if ((ret = table_field_set(unit, LONGAN_LAGt,
                LONGAN_LAG_TRK_UNIT3tf, &val, (uint32 *)&lag_entry)) != RT_ERR_OK)
    {
        TRUNK_SEM_UNLOCK(unit);
    ...
    val = pTrk_egr_ports->egr_port[4].unit;
    if ((ret = table_field_set(unit, LONGAN_LAGt,
                LONGAN_LAG_TRK_UNIT4tf, &val, (uint32 *)&lag_entry)) != RT_ERR_OK)
    {
        TRUNK_SEM_UNLOCK(unit);
    ...
    val = pTrk_egr_ports->egr_port[5].unit;
    if ((ret = table_field_set(unit, LONGAN_LAGt,
                LONGAN_LAG_TRK_UNIT5tf, &val, (uint32 *)&lag_entry)) != RT_ERR_OK)
    {
        TRUNK_SEM_UNLOCK(unit);
    ...
    val = pTrk_egr_ports->egr_port[6].unit;
    if ((ret = table_field_set(unit, LONGAN_LAGt,
                LONGAN_LAG_TRK_UNIT6tf, &val, (uint32 *)&lag_entry)) != RT_ERR_OK)
    {
        TRUNK_SEM_UNLOCK(unit);
    ...
    val = pTrk_egr_ports->egr_port[7].unit;
    if ((ret = table_field_set(unit, LONGAN_LAGt,
                LONGAN_LAG_TRK_UNIT7tf, &val, (uint32 *)&lag_entry)) != RT_ERR_OK)
    {
        TRUNK_SEM_UNLOCK(unit);
 ...
 */
int32
dal_longan_trunk_srcPortMap_get(uint32 unit,  rtk_unit_port_t unitPort, uint32 *pIsTrkMbr, rtk_trk_t *pTrk_gid)
{
    int32   ret;
    uint32 tbl_idx;
    uint32 is_trk;
    ...
    RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_TRUNK), "unit=%d,unitPort=%d", unit, unitPort);
    ...
    /*set source port to trunk ID mapping table*/
    tbl_idx = unitPort.unit*64+unitPort.port;
 ...
 */
int32
dal_longan_trunk_srcPortMap_set(uint32 unit, rtk_unit_port_t unitPort, uint32 isTrkMbr, rtk_trk_t trk_gid)
{
    int32   ret;
    uint32 tbl_idx;
    uint32 is_trk;
    ...
    RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_TRUNK), "unit=%d,unitPort=%d,isTrkMbr=%d,trk_gid=%d", unit, unitPort, isTrkMbr, trk_gid);
