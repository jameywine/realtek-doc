...
static int32 _diag_l3_hash_algrithom_9300(uint32 unit, rtk_diag_l3_entry_type_t type, uint32 hash_id, uint32 *sa_ptr, uint32 *da_ptr, uint32 vid, uint32 *bucket_ptr)
{
    ...
    if ((hash_id != 0) && (hash_id != 1)) {
        ...
        if (IS_L3_ECO_VER(unit))
        {
            if (hash_id == 0)
            {
                hash_seed[0] = (da[3] << 18) |          // high 14 bits using dip{13:0}/da[3][13:0]
                               (vlan_id & BITMASK_18B); // low  18 bits using {6'h0, vlan[11:0]}
                hash_seed[1] = (da[2] << 18) |          // high 14 bits using dip{45:32}/da[2][13:0]
                               (da[3] >> 14);           // low  18 bits using dip{31:14}/da[3][31:14]
                hash_seed[2] = (da[1] << 18) |          // high 14 bits using dip{77:64}/da[1][13:0]
                               (da[2] >> 14);           // low  18 bits using dip{63:46}/da[2][31:14]
                hash_seed[3] = (da[0] << 18) |          // high 14 bits using dip{109:96}/da[0][13:0]
                               (da[1] >> 14);           // low  18 bits using dip{95:78}/da[1][31:14]
                hash_seed[4] = (sa[3] << 25) |          // high 14 bits using {sip[6:0]/sa[3][6:0], 7'h0}
                               (da[0] >> 14);           // low  18 bits using dip{127:110}/da[0][31:14]

                hash_seed[5] = (sa[2] << 25) |          // high 7 bits using sip{38:32}/sa[2][6:0]
                               (sa[3] >> 7);            // low 25 bits using sip{31:7}/sa[3][31:7]
                hash_seed[6] = (sa[1] << 25) |          // high 7 bits using sip{70:64}/sa[1][6:0]
                               (sa[2] >> 7);            // low 25 bits using sip{63:39}/sa[2][31:7]
                hash_seed[7] = (sa[0] << 25) |          // high 7 bits using sip{102:96}/sa[0][6:0]
                               (sa[1] >> 7);            // low 25 bits using sip{95:71}/sa[1][31:7]
                hash_seed[8] = (sa[0] >> 7);            // low 25 bits using sip{127:103}/sa[0][31:7]
            }
            else
            {
                hash_seed[0] = (((da[3] >> 9 ) & BITMASK_1B) << 31) |   // the first  1 bits using dip[9]/da[3][9]
                               (((da[3] >> 14) & BITMASK_4B) << 27 ) |  // the second 4 bits using dip[17:14]/da[3][17:14]
                               ((da[3] & BITMASK_9B) << 18) |           // the third  9 bits using dip[8:0]/da[3][8:0]
                               ((vlan_id >> 9) << 4) |                      // the fourth 9 bits using {2'h0, vid[11:9], 4'h0}
                               (vlan_id & BITMASK_9B);                      // the fifth  9 bits using vid[8:0]
                hash_seed[1] = (((da[2] >> 18 ) & BITMASK_1B) << 31) | // the first  1 bits using dip[50]/da[2][18]
                               (((da[2] >> 4) & BITMASK_9B) << 22) |   // the second  9 bits using dip[44:36]/da[2][12:4]
                               ((da[3] >> 27) << 17) |                  // the third  5 bits using dip[31:27]/da[3][31:27]
                               ((da[2] & BITMASK_4B) << 13) |           // the fourth 4 bits using dip[35:32]/da[2][3:0]
                               (((da[3] >> 18) & BITMASK_9B) << 4) |   // the fifth  9 bits using dip[26:18]/da[3][26:18]
                               ((da[3] >> 10 ) & BITMASK_4B);           // the sixth  4 bits using dip[13:10]/da[3][13:10]
                hash_seed[2] = (((da[1] >> 8) & BITMASK_6B) << 26) |    // the first   6 bits using dip[77:72]/da[1][13:8]
                               ((da[1] & BITMASK_4B) << 22) |           // the second   4 bits using dip[67:64]/da[1][3:0]
                               (((da[2] >> 31) & BITMASK_1B) << 21) |   // the third   1 bits using dip[63]/da[2][31]
                               (((da[1] >> 4) & BITMASK_4B) << 17) |    // the fourth  4 bits using dip[71:68]/da[1][7:4]
                               (((da[2] >> 22) & BITMASK_9B) << 8) |    // the fifth   9 bits using dip[62:54]/da[2][30:22]
                               (((da[2] >> 13) & BITMASK_5B) << 3) |    // the sixth   5 bits using dip[49:45]/da[2][17:13]
                               ((da[2] >> 19 ) & BITMASK_3B);           // the seventh 3 bits using dip[53:51]/da[2][21:19]
                hash_seed[3] = (((da[0] >> 12) & BITMASK_2B) << 30) |   // the second  2 bits using dip[109:108]/da[0][13:12]
                               (((da[0] >> 3) & BITMASK_5B) << 25) |    // the second  5 bits using dip[103:99]/da[0][7:3]
                               (((da[0] >> 8) & BITMASK_4B) << 21) |    // the third   4 bits using dip[107:104]/da[0][11:8]
                               ((da[0] & BITMASK_3B) << 18) |           // the fourth   3 bits using dip[98:96]/da[0][2:0]
                               ((da[1] >> 26) << 12) |                  // the fifth   6 bits using dip[95:90]/da[1][31:26]
                               (((da[1] >> 17) & BITMASK_5B) << 7) |    // the sixth   5 bits using dip[85:81]/da[1][21:17]
                               (((da[1] >> 22) & BITMASK_4B) << 3) |    // the seventh 4 bits using dip[89:86]/da[1][25:22]
                               ((da[1] >> 14 ) & BITMASK_3B);           // the eighth  3 bits using dip[80:78]/da[1][16:14]
                hash_seed[4] = ((sa[3] & BITMASK_7B) << 25) |           // the first   14 bits using sip[6:0]/sa[3][6:0], 7'h0}
                               (((da[0] >> 30) & BITMASK_2B) << 16) |   // the second  2 bits using dip[127:126]/da[0][31:30]
                               (((da[0] >> 21) & BITMASK_5B) << 11) |   // the third   5 bits using dip[121:117]/da[0][25:21]
                               (((da[0] >> 26) & BITMASK_4B) << 7) |    // the fourth  4 bits using dip[125:122]/da[0][30:26]
                               ((da[0] >> 14 ) & BITMASK_7B);           // the fifth   7 bits using dip[116:110]/da[0][20:14]
                hash_seed[5] = (((sa[2] >> 4) & BITMASK_3B) << 29) |    // the second  3 bits using sip[38:36]/sa[2][6:4]
                               ((sa[3] >> 27) << 24) |                  // the second  5 bits using sip[31:27]/sa[3][31:27]
                               ((sa[2] & BITMASK_4B) << 20) |           // the third   4 bits using sip[35:32]/sa[2][3:0]
                               (((sa[3] >> 18) & BITMASK_9B) << 11) |   // the fourth  9 bits using sip[26:18]/sa[3][26:18]
                               (((sa[3] >> 9) & BITMASK_5B) << 6) |     // the fitth   5 bits using sip[13:9]/sa[3][13:9]
                               (((sa[3] >> 14) & BITMASK_4B) << 2) |    // the sixth   4 bits using sip[17:14]/sa[3][17:14]
                               ((sa[3] >> 7 ) & BITMASK_2B);            // the fifth   2 bits using sip[7:6]/sa[3][7:6]
                hash_seed[6] = ((sa[1] & BITMASK_3B) << 29) |           // the first   3 bits using sip[66:64]/sa[1][2:0]
                               ((sa[2] >> 31) << 28) |                  // the second  1 bits using sip[63]/sa[2][31]
                               (((sa[1] >> 4) & BITMASK_4B) << 24) |    // the third   4 bits using sip[71:68]/sa[1][7:4]
                               (((sa[2] >> 22) & BITMASK_9B) << 15) |   // the fourth  9 bits using sip[62:54]/sa[2][30:22]
                               (((sa[2] >> 13) & BITMASK_5B) << 10) |   // the fifth   5 bits using sip[49:45]/sa[2][17:13]
                               (((sa[2] >> 18) & BITMASK_4B) << 6) |    // the sixth   4 bits using sip[53:50]/sa[2][21:18]
                               ((sa[2] >> 7 ) & BITMASK_6B);            // the seventh  6 bits using sip[44:39]/sa[2][12:7]
                hash_seed[7] = (((sa[0] >> 8) & BITMASK_4B) << 28) |    // the first   4 bits using sip[107:104]/sa[0][11:8]
                               ((sa[0] & BITMASK_3B) << 25) |           // the second  3 bits using sip[98:96]/sa[0][2:0]
                               ((sa[1] >> 26) << 19) |                  // the third   6 bits using sip[96:90]/sa[1][31:26]
                               (((sa[1] >> 17) & BITMASK_5B) << 14) |   // the fourth  5 bits using sip[85:81]/sa[1][21:17]
                               (((sa[1] >> 22) & BITMASK_4B) << 10) |   // the fifth   4 bits using sip[89:86]/sa[1][25:22]
                               (((sa[1] >> 8) & BITMASK_9B) << 1) |     // the sixth   9 bits using sip[80:72]/sa[1][16:8]
                               ((sa[1] >> 3 ) & BITMASK_1B);            // the seventh  1 bits using sip[67]/sa[1][3]
                hash_seed[8] = ((sa[0] >> 30) << 23) |                  // the first   9 bits using sip{7'h0, [127:126]/sa[0][31:30]}
                               (((sa[0] >> 21) & BITMASK_5B) << 18) |   // the fifth   5 bits using sip[121:117]/sa[0][25:21]
                               (((sa[0] >> 26) & BITMASK_4B) << 14) |   // the third   4 bits using sip[125:122]/sa[0][29:26]
                               (((sa[0] >> 12) & BITMASK_9B) << 5) |    // the fourth  9 bits using sip[116:108]/sa[0][20:12]
                               ((sa[0] >> 3 ) & BITMASK_5B);            // the fifth   5 bits using sip[103:99]/sa[0][7:3]
            }

            *bucket_ptr =  ((hash_seed[8] >> 23) & BITMASK_9B) ^
                           ((hash_seed[8] >> 14) & BITMASK_9B) ^
                           ((hash_seed[8] >> 5) & BITMASK_9B) ^
                           (((hash_seed[8] & BITMASK_5B) << 4) | (hash_seed[7] >> 28)) ^
                           ((hash_seed[7] >> 19) & BITMASK_9B) ^                            // 28 bits of [7], 4 bits remainder
                           ((hash_seed[7] >> 10) & BITMASK_9B) ^
                           ((hash_seed[7] >> 1) & BITMASK_9B) ^
                           (((hash_seed[7] & BITMASK_1B) << 8) | (hash_seed[6] >> 24)) ^
                           ((hash_seed[6] >> 15) & BITMASK_9B) ^                            // 24 bits of [6], 8 bits remainder
                           ((hash_seed[6] >> 6) & BITMASK_9B) ^
                           (((hash_seed[6] & BITMASK_6B) << 3) | (hash_seed[5] >> 29)) ^
                           ((hash_seed[5] >> 20) & BITMASK_9B) ^                            // 29 bits of [5], 3 bits remainder
                           ((hash_seed[5] >> 11) & BITMASK_9B) ^
                           ((hash_seed[5] >> 2) & BITMASK_9B) ^
                           (((hash_seed[5] & BITMASK_2B) << 7) | (hash_seed[4] >> 25)) ^
                           ((hash_seed[4] >> 16) & BITMASK_9B) ^                            // 25 bits of [4], 7 bits remainder
                           ((hash_seed[4] >> 7) & BITMASK_9B) ^
                           (((hash_seed[4] & BITMASK_7B) << 2) | (hash_seed[3] >> 30)) ^
                           ((hash_seed[3] >> 21) & BITMASK_9B) ^                            // 30 bits of [3], 2 bits remainder
                           ((hash_seed[3] >> 12) & BITMASK_9B) ^
                           ((hash_seed[3] >> 3) & BITMASK_9B) ^
                           (((hash_seed[3] & BITMASK_3B) << 6) | (hash_seed[2] >> 26)) ^
                           ((hash_seed[2] >> 17) & BITMASK_9B) ^                            // 26 bits of [2], 8 bits remainder
                           ((hash_seed[2] >> 8) & BITMASK_9B) ^
                           (((hash_seed[2] & BITMASK_8B) << 1) | (hash_seed[1] >> 31)) ^
                           ((hash_seed[1] >> 22) & BITMASK_9B) ^                            // 31 bits of [1], 1 bits remainder
                           ((hash_seed[1] >> 13) & BITMASK_9B) ^
                           ((hash_seed[1] >> 4) & BITMASK_9B) ^
                           (((hash_seed[1] & BITMASK_4B) << 5) | (hash_seed[0] >> 27)) ^
                           ((hash_seed[0] >> 18) & BITMASK_9B) ^                            // 27 bists of [0], 5 bits remainder
                           ((hash_seed[0] >> 9) & BITMASK_9B) ^
                           (hash_seed[0] & BITMASK_9B);
        }
        else /*old hash alg*/
        {
           if (hash_id == 0)
