 ...
 *
 * $Revision: 81303 $
 * $Date: 2017-08-16 09:55:46 +0800 (Wed, 16 Aug 2017) $
 *
...
static void _dal_longan_port_linkChange_isr(uint32 unit, void *isr_param);
#endif /* CONFIG_SDK_DRIVER_NIC && CONFIG_SDK_DRIVER_NIC_USER_MODE */
...
_dal_longan_port_smiPollSel_init(uint32 unit)
{
    ...
    HWP_PORT_TRAVS_EXCEPT_CPU(unit, port)
    {
        ...
        if ((ret = reg_field_write(unit, LONGAN_SMI_GLB_CTRLr, field, &poll_sel)) != RT_ERR_OK)
        {
...
dal_longan_port_init(uint32 unit)
{
    ...
    /* linkscan callback handler */
    {
        ...
        /* register callback */
        if (( ret = drv_intr_link_stat_register(unit, _dal_longan_port_linkChange_isr)) != RT_ERR_OK)
        {
 ...
 */
int32
dal_longan_port_isolationExt_get(uint32 unit, uint32 srcUnit, rtk_port_t srcPort, rtk_portmask_t *pPortmask)
{
    int32 ret = RT_ERR_FAILED;
    uint32 index;
    port_iso_entry_t port_iso_entry;

    RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_PORT), "unit=%d, srcUnit=%d, port=%d", unit, srcUnit, srcPort);
    ...
    /* parameter check */
    RT_PARAM_CHK((NULL == pPortmask), RT_ERR_NULL_POINTER);
    /* check srcUnit is equal to my unit ID. If equal, check port exist or not;
     * if not, we can not know remote unit's hardware profile, so do not check.
     */
    if(unit == srcUnit)
    {
        RT_PARAM_CHK(!HWP_PORT_EXIST(unit, srcPort), RT_ERR_PORT_ID);
    }

    index = srcUnit * 64 + srcPort;
 ...
 */
int32
dal_longan_port_isolationExt_set(uint32 unit, uint32 srcUnit, rtk_port_t srcPort, rtk_portmask_t *pPortmask)
{
    int32 ret = RT_ERR_FAILED;
    uint32 index;
    port_iso_entry_t port_iso_entry;
    ...
    RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_PORT), "unit=%d, srcUnit=%d, port=%d, Portmask=0x%X", unit, srcUnit, srcPort, (*pPortmask).bits[0]);
...
#if defined(CONFIG_SDK_DRIVER_NIC) && defined(CONFIG_SDK_DRIVER_NIC_USER_MODE)
/* Function Name:
 *      _dal_longan_port_linkChange_isr
 * Description:
 *      switch interrupt
 * Input:
 ...
 */
static void
_dal_longan_port_linkChange_isr(uint32 unit, void *isr_param)
{
    ...
    return ;
} /* end of _dal_longan_port_linkChange_isr */
#endif /* CONFIG_SDK_DRIVER_NIC && CONFIG_SDK_DRIVER_NIC_USER_MODE */
...
/* Function Name:
 *      _dal_longan_SDS_SUBMODE_SEL_regField_get
 * Description:
 *      Get serdes mode select register and field of the specified serdes ID
 * Input:
 *      sdsId - serdes ID
 * Output:
 *      pregField - register and field
 * Return:
 *      RT_ERR_OK     - OK
 *      RT_ERR_INPUT - input serdes ID is invalid
 * Note:
 *      None
 */
int32
_dal_longan_SDS_SUBMODE_SEL_regField_get(uint32 sdsId, rtk_regFieldId_t *pregField)
{
   rtk_regFieldId_t sds_mode_sel[] =
    {
    ...
    if (sdsId >= sizeof(sds_mode_sel)/sizeof(rtk_regFieldId_t))
    {
        return RT_ERR_INPUT;
    }

    osal_memcpy(pregField, &sds_mode_sel[sdsId - 2], sizeof(rtk_regFieldId_t));
    return RT_ERR_OK;
}


/* Function Name:
 *      _dal_longan_SDS_MODE_SEL_regField_get
 * Description:
 *      Get serdes mode select register and field of the specified serdes ID
 * Input:
 *      sdsId - serdes ID
 ...
 */
int32
_dal_longan_SDS_MODE_SEL_regField_get(uint32 sdsId, rtk_regFieldId_t *pregField)
{
   rtk_regFieldId_t sds_mode_sel[] =
    {
    ...
    osal_memcpy(pregField, &sds_mode_sel[sdsId], sizeof(rtk_regFieldId_t));
    return RT_ERR_OK;
}

int32 _dal_longan_sds_subMode_get(uint32 unit, uint32 sdsId, rt_serdesMode_t *pmode)
{
    int32   ret;
    uint32  value;
    rtk_regFieldId_t regField;

    /* parameter check */
    RT_PARAM_CHK(sdsId < 2, RT_ERR_INPUT);

    if ((ret = _dal_longan_SDS_SUBMODE_SEL_regField_get(sdsId, &regField)) != RT_ERR_OK)
    {
      return ret;
    }

    if ((ret = reg_field_read(unit, regField.reg, regField.field, &value)) != RT_ERR_OK)
    {
      return ret;
    }
    switch (value)
    {
        case 0x0:
            *pmode = RTK_MII_USXGMII_10GSXGMII;
            break;
        case 0x1:
            *pmode = RTK_MII_USXGMII_10GDXGMII;
            break;
        case 0x2:
        ...
        case 0x3:
            *pmode = RTK_MII_USXGMII_5GSXGMII;
            break;
        case 0x4:
            *pmode = RTK_MII_USXGMII_5GDXGMII;
            break;
        case 0x5:
...
int32 _dal_longan_sds_subMode_set(uint32 unit, uint32 sdsId, rt_serdesMode_t mode)
{
    int32   ret;
    uint32  value;
    rtk_regFieldId_t regField;
    ...
    if ((ret = reg_field_write(unit, regField.reg, regField.field, &value)) != RT_ERR_OK)
    {
      return ret;
    }
...
/* Function Name:
 *      _dal_longan_port_macSerdesMode_get
 * Description:
 *      Get Serdes mode
 * Input:
 *      unit    - unit id
 ...
 * Return:
 *      RT_ERR_OK           - OK
 *      RT_ERR_CHIP_UNDEFINED_VALUE   - chip return undefined value
 * Note:
 *      None
 */
int32
_dal_longan_port_macSerdesMode_get(uint32 unit, uint32 sdsId, rt_serdesMode_t *pmode)
{
    int32   ret;
    uint32  value;
    rtk_regFieldId_t regField;

    if ((ret = _dal_longan_SDS_MODE_SEL_regField_get(sdsId, &regField)) != RT_ERR_OK)
    {
        return ret;
    }
    ...
    switch (value)
    {
      ...
      case 0x17:
        *pmode = RTK_MII_RXAUI_LITE;
        break;
      case 0x1D:
        *pmode = RTK_MII_RXAUISGMII_AUTO;
        break;
      case 0x1E:
        *pmode = RTK_MII_RXAUI1000BX_AUTO;
        break;
      case 0x0:
        *pmode = RTK_MII_RSGMII_PLUS;
        break;
      case 0x2:
        *pmode = RTK_MII_SGMII;
        break;
      ...
      case 0x12:
        *pmode = RTK_MII_HISGMII;
        break;
      case 0x13:
        *pmode = RTK_MII_HISGMII_5G;
        break;
      case 0x14:
        *pmode = RTK_MII_DUAL_HISGMII;
        break;
      case 0x16:
        *pmode = RTK_MII_2500Base_X;
        break;
      case 0xD:
        if (RT_ERR_OK != (ret = _dal_longan_sds_subMode_get(unit, sdsId, pmode)))
        {
            return ret;
        }
        break;
      default:
        return RT_ERR_CHIP_UNDEFINED_VALUE;
    }
...
/* Function Name:
 *      _dal_longan_port_macSerdesMode_set
 * Description:
 *      Set Serdes mode
 * Input:
 *      unit    - unit id
 ...
 */
int32
_dal_longan_port_macSerdesMode_set(uint32 unit, uint32 sdsId, rt_serdesMode_t mode)
{
    int32   ret;
    uint32  value, dis_val;
    rtk_regFieldId_t regField;
    ...
    switch (mode)
    {
      ...
      case RTK_MII_USXGMII_2_5GSXGMII:
        if( (RTL9301_CHIP_ID_24G == HWP_CHIP_ID(unit) || RTL9303_CHIP_ID_8XG == HWP_CHIP_ID(unit)) &&
            (2 == HWP_CHIP_REV(unit)))
        {
            RT_LOG(LOG_INFO, (MOD_DAL|MOD_PORT), "unit=%u, sdsId=%u, not support RTK_MII_USXGMII", unit, sdsId);
            return RT_ERR_CHIP_NOT_SUPPORTED;
        }
        value = 0xD;

        if (RT_ERR_OK != (ret = _dal_longan_sds_subMode_set(unit, sdsId, mode)))
        {
            return ret;
        }
        break;
    ...
    dis_val = 0x1F;
    reg_field_write(unit, regField.reg, regField.field, &dis_val);
    return reg_field_write(unit, regField.reg, regField.field, &value);
}
    ...
    switch(speed)
    {
      ...
      case PORT_SPEED_2_5G:
#if FOR_AQ_DEBUG_PURPOSE_5G_2dot5G
        {
            ...
            _phy_macIntfSerdesMode_get(unit,port,&pserdesMode);
...
{
#if defined(CONFIG_SDK_RTL8295R) /* 95R MAC-POLL-PHY */
    if (((HWP_PHY_MODEL_BY_PORT(unit, port) == RTK_PHYTYPE_RTL8295R) || (HWP_PHY_MODEL_BY_PORT(unit, port) == RTK_PHYTYPE_RTL8295R_C22))&& phy_8295r_swPollEnable_get(unit, port))
    {
        *penable = ENABLED;
    }
    else
...
_dal_longan_port_swMacPollPhy(uint32 unit, rtk_port_t port)
{
    ...
    if ((ret = _phy_swMacPollPhyStatus_get(unit, port, &phyStatus)) != RT_ERR_OK)
    {
        goto EXIT;
    }
...
_dal_longan_port_swChgSerdesMode(uint32 unit, rtk_port_t port)
{
    ...
    if ((ret = _phy_macIntfSerdesMode_get(unit, port, &phySdsMode)) != RT_ERR_OK)
    {
        RT_LOG(LOG_MINOR_ERR, (MOD_DAL|MOD_PORT), "unit=%u, port=%u, _phy_macIntfSerdesMode_get failed", unit, port);
        goto EXIT;
    }

    sdsId = HWP_PORT_SDSID(unit, port);
    if ((ret = _dal_longan_port_macSerdesMode_get(unit, sdsId, &macSdsMode)) != RT_ERR_OK)
    {
    ...
    if (RT_ERR_OK != (ret = _dal_longan_port_macSerdesMode_set(unit, sdsId, phySdsMode)))
    {
