 ...
 *
 * $Revision: 81474 $
 * $Date: 2017-08-18 10:35:49 +0800 (Fri, 18 Aug 2017) $
 *
...
typedef struct dal_longan_l3_drvDb_s
{
    ...
    struct
    {
        ...
        struct
        {
            uint32  mtu_value;
            uint32  ref_count;
            uint8    group_ref[DAL_LONGAN_IPMCAST_L3_ENTRY_MAX];    /*used by ipmc entry*/
    ...
    struct
    {
        ...
        /* [DRV] */
        uint32          ip_mtu_idx; /* [SHADOW] for managing MTU resource */
        rtk_vlan_t      vid;        /* [SHADOW] for creating nexthop entry */

        uint32  mac_idx;       /*only for 9300 managing route mac resource*/
    } intf[DAL_LONGAN_L3_INTF_MAX];
...
#define L3_DB_UPDATE_INTF_VALID(_unit, _intfIdx, _mtuIdx, _macIdx, _vid) \
do {                                                    \
...
do {                                                    \
    ...
    _pL3Db[_unit]->intf[_intfIdx].mac_idx = 0;          \
    _pL3Db[_unit]->intf[_intfIdx].vid = 0;              \
    _pL3Db[_unit]->intf[_intfIdx].valid = FALSE;        \
} while(0)

#define L3_INTF_MTU_IP6_FIX_NEW(_ip6mtu)      ((_ip6mtu > 40)? (_ip6mtu) : 0)
/* Note : the old when ip6mtu<= 40, it also can pass 40 byte */
#define L3_INTF_MTU_IP6_FIX_OLD(_ip6mtu)      ((_ip6mtu > 40)? (_ip6mtu-40) : 0)
...
static inline int32 l3_util_intfMtu_set(uint32 unit, uint32 mtuIdx, uint32 mtuValue)
{
    ...
    if (IS_L3_ECO_VER(unit))
        hwMtuIp6Value = L3_INTF_MTU_IP6_FIX_OLD(mtuValue);
    else
        hwMtuIp6Value = L3_INTF_MTU_IP6_FIX_NEW(mtuValue);

    /* update shadow (currently, IPv6 MTU synced with IPv4 MTU) */
...
dal_longan_l3_init(uint32 unit)
{
    ...
    L3_ROUTE_TBL_SIZE(unit) = DAL_LONGAN_L3_ROUTE_TBL_SIZE;
...
_dal_longan_l3_smac_set(uint32 unit, uint32 index, rtk_mac_t mac)
{
    ...
    RT_ERR_HDL(_dal_longan_l3_intfEgrMac_set(unit, DAL_LONGAN_L3_SMAC_START_IDX + index, mac), errHandle, ret);
    ...
    for (idx=0; idx<DAL_LONGAN_L3_MAC_MAX; idx++)
    {
        if (_pL3Db[unit]->HW.route_mac[idx].ref_count > 0)
        {
            if ((RT_ERR_OK == rt_util_macCmp(_pL3Db[unit]->HW.route_mac[idx].mac.octet, pNewMacEntry->mac.octet)) &&  \
                (_pL3Db[unit]->HW.route_mac[idx].intf_id == _pL3Db[unit]->vid[pNewMacEntry->intf_id].intf_idx))
            {
                _pL3Db[unit]->HW.route_mac[idx].ref_count += 1;
                *pIdx = idx;
...
_dal_longan_l3_ipmc_mtuEntry_alloc(uint32 unit, uint32 mtu, rtk_mcast_group_t group, uint32 *pIdx)
{
    ...
    for (idx=0; idx<DAL_LONGAN_L3_IP_MTU_MAX; idx++)
    {
        if (_pL3Db[unit]->HW.ip4_mtu[idx].ref_count > 0)
        {
            if (_pL3Db[unit]->HW.ip4_mtu[idx].mtu_value == mtu)
            {
                if (FALSE == _pL3Db[unit]->HW.ip4_mtu[idx].group_ref[groupIdx])
                {
                    _pL3Db[unit]->HW.ip4_mtu[idx].ref_count += 1;
                    _pL3Db[unit]->HW.ip4_mtu[idx].group_ref[groupIdx] = TRUE;
                }
                *pIdx = idx;
                goto errOk;
            }
    ...
    if (emptyEntryFound)
    {
        _pL3Db[unit]->HW.ip4_mtu[emptyEntryIdx].mtu_value = mtu;
        _pL3Db[unit]->HW.ip4_mtu[emptyEntryIdx].ref_count = 1;
        _pL3Db[unit]->HW.ip4_mtu[emptyEntryIdx].group_ref[groupIdx] = TRUE;
        _pL3Db[unit]->HW.ip4_mtu_used_count += 1;
...
_dal_longan_l3_ipmc_mtuEntry_free(uint32 unit, uint32 idx, rtk_mcast_group_t group)
{
    ...
    if (_pL3Db[unit]->HW.ip4_mtu[idx].ref_count > 0)
    {
        if (TRUE == _pL3Db[unit]->HW.ip4_mtu[idx].group_ref[groupIdx])
        {
            _pL3Db[unit]->HW.ip4_mtu[idx].ref_count -= 1;
            _pL3Db[unit]->HW.ip4_mtu[idx].group_ref[groupIdx] = FALSE;
            if (_pL3Db[unit]->HW.ip4_mtu[idx].ref_count == 0)
            {
                _pL3Db[unit]->HW.ip4_mtu_used_count -= 1;
            }
...
_dal_longan_l3_ipmc_mtuEntry_realloc(uint32 unit, uint32 newMtu, rtk_mcast_group_t group, uint32 *pIdx)
{
    ...
    /* try to add the new MTU entry */
    if ((_pL3Db[unit]->HW.ip4_mtu[oldIdx].ref_count == 1) &&
         (TRUE == _pL3Db[unit]->HW.ip4_mtu[oldIdx].group_ref[groupIdx]))
    {
        /* just change the MTU value with the original MTU entry */
        _pL3Db[unit]->HW.ip4_mtu[oldIdx].mtu_value = newMtu;
...
static uint32
_dal_longan_l3_hostHash0_ret(uint32 unit, dal_longan_l3_hostHashKey_t *pHashKey, dal_longan_l3_api_flag_t flags)
{
    ...
    if (IS_L3_ECO_VER(unit))
    {
        /* pre-calculate hash index */
        if (ipv6)
        {
            /* destination IPv6 address */
            hashRow[0] = 0x0 | \
                          HASH_BIT_EXTRACT(pHashKey->dip.ipv6.octet[0], 6, 2);      /* DIP [127:126] */
            hashRow[1] = (HASH_BIT_EXTRACT(pHashKey->dip.ipv6.octet[0], 0, 6) << 3) | \
                          HASH_BIT_EXTRACT(pHashKey->dip.ipv6.octet[1], 5, 3);     /* DIP [125:117] */
            hashRow[2] = (HASH_BIT_EXTRACT(pHashKey->dip.ipv6.octet[1], 0, 5) << 4) | \
                          HASH_BIT_EXTRACT(pHashKey->dip.ipv6.octet[2], 4, 4);     /* DIP [116:108] */
            hashRow[3] = (HASH_BIT_EXTRACT(pHashKey->dip.ipv6.octet[2], 0, 4) << 5) | \
                          HASH_BIT_EXTRACT(pHashKey->dip.ipv6.octet[3], 3, 5);     /* DIP [107:99] */
            hashRow[4] = (HASH_BIT_EXTRACT(pHashKey->dip.ipv6.octet[3], 0, 3) << 6) | \
                          HASH_BIT_EXTRACT(pHashKey->dip.ipv6.octet[4], 2, 6);     /* DIP [99:90] */
            hashRow[5] = (HASH_BIT_EXTRACT(pHashKey->dip.ipv6.octet[4], 0, 2) << 7) | \
                          HASH_BIT_EXTRACT(pHashKey->dip.ipv6.octet[5], 1, 7);     /* DIP [89:81] */
            hashRow[6] = (HASH_BIT_EXTRACT(pHashKey->dip.ipv6.octet[5], 0, 1) << 8) | \
                          HASH_BIT_EXTRACT(pHashKey->dip.ipv6.octet[6], 0, 8);     /* DIP [80:72] */
            hashRow[7] = (HASH_BIT_EXTRACT(pHashKey->dip.ipv6.octet[7], 0, 8) << 1) | \
                          HASH_BIT_EXTRACT(pHashKey->dip.ipv6.octet[8], 7, 1);     /* DIP [71:63] */
            hashRow[8] = (HASH_BIT_EXTRACT(pHashKey->dip.ipv6.octet[8], 0, 7) << 2) | \
                          HASH_BIT_EXTRACT(pHashKey->dip.ipv6.octet[9], 6, 2);     /* DIP [62:54] */
            hashRow[9] = (HASH_BIT_EXTRACT(pHashKey->dip.ipv6.octet[9], 0, 6) << 3) | \
                          HASH_BIT_EXTRACT(pHashKey->dip.ipv6.octet[10], 5, 3);    /* DIP [53:45] */
            hashRow[10] = (HASH_BIT_EXTRACT(pHashKey->dip.ipv6.octet[10], 0, 5) << 4) | \
                          HASH_BIT_EXTRACT(pHashKey->dip.ipv6.octet[11], 4, 4);   /* DIP [44:36] */
            hashRow[11] = (HASH_BIT_EXTRACT(pHashKey->dip.ipv6.octet[11], 0, 4) << 5) | \
                          HASH_BIT_EXTRACT(pHashKey->dip.ipv6.octet[12], 3, 5);   /* DIP [35:27] */
            hashRow[12] = (HASH_BIT_EXTRACT(pHashKey->dip.ipv6.octet[12], 0, 3) << 6) | \
                          HASH_BIT_EXTRACT(pHashKey->dip.ipv6.octet[13], 2, 6);   /* DIP [26:18] */
            hashRow[13] = (HASH_BIT_EXTRACT(pHashKey->dip.ipv6.octet[13], 0, 2) << 7) | \
                          HASH_BIT_EXTRACT(pHashKey->dip.ipv6.octet[14], 1, 7);   /* DIP [17:9] */
            hashRow[14] = (HASH_BIT_EXTRACT(pHashKey->dip.ipv6.octet[14], 0, 1) << 8) | \
                          HASH_BIT_EXTRACT(pHashKey->dip.ipv6.octet[15], 0, 8);   /* DIP [8:0] */
        }
        else
        {
            /* destination IPv4 address */
            hashRow[11] = HASH_BIT_EXTRACT(pHashKey->dip.ipv4, 27, 5);
            hashRow[12] = HASH_BIT_EXTRACT(pHashKey->dip.ipv4, 18, 9);
            hashRow[13] = HASH_BIT_EXTRACT(pHashKey->dip.ipv4, 9, 9);
            hashRow[14] = HASH_BIT_EXTRACT(pHashKey->dip.ipv4, 0, 9);
        }
...
static uint32
_dal_longan_l3_hostHash1_ret(uint32 unit, dal_longan_l3_hostHashKey_t *pHashKey, dal_longan_l3_api_flag_t flags)
{
    uint32  ipv6 = (flags & DAL_LONGAN_L3_API_FLAG_IPV6);
    ...
    if (IS_L3_ECO_VER(unit))
    {
        ...
        /* Hash Algorithm 1 */
        hashIdx = \
            (((hashRow[1] & 0x01F) << 4) | ((hashRow[1] & 0x1E0) >> 5)) ^ hashRow[0] ^ \
            (((hashRow[3] & 0x01F) << 4) | ((hashRow[3] & 0x1E0) >> 5)) ^ hashRow[2] ^ \
            (((hashRow[5] & 0x01F) << 4) | ((hashRow[5] & 0x1E0) >> 5)) ^ hashRow[4] ^ \
            (((hashRow[7] & 0x01F) << 4) | ((hashRow[7] & 0x1E0) >> 5)) ^ hashRow[6] ^ \
            (((hashRow[9] & 0x01F) << 4) | ((hashRow[9] & 0x1E0) >> 5)) ^ hashRow[8] ^ \
            (((hashRow[11] & 0x01F) << 4) | ((hashRow[11] & 0x1E0) >> 5)) ^ hashRow[10] ^ \
            (((hashRow[13] & 0x01F) << 4) | ((hashRow[13] & 0x1E0) >> 5)) ^ hashRow[12] ^ \
            hashRow[14];
    }
    else
    {
...
_dal_longan_l3_hostHashIdx_get(uint32 unit, dal_longan_l3_hostHashKey_t *pHashKey, dal_longan_l3_hostHashIdx_t *pHashIdx, dal_longan_l3_api_flag_t flags)
{
    ...
    for (tbl=0; tbl<DAL_LONGAN_L3_HOST_TBL_NUM; tbl++)
    {
        ...
        {
            pHashIdx->idx_of_tbl[tbl] = _dal_longan_l3_hostHash0_ret(unit, pHashKey, flags);
        }
        else if (1 == pHashKey->alg_of_tbl[tbl])
        {
            pHashIdx->idx_of_tbl[tbl] = _dal_longan_l3_hostHash1_ret(unit, pHashKey, flags);
        }
        else
        {
            return RT_ERR_INPUT;
...
_dal_longan_l3_hostEntry_get(uint32 unit, uint32 index, dal_longan_l3_hostEntry_t *pEntry, dal_longan_l3_api_flag_t flags)
{
    ...
    L3_TABLE_READ_ERR_HDL(unit, LONGAN_L3_HOST_ROUTE_IPUCt, index, entry, "", errHandle, ret);
    ...
    *pIndex = -1;
    ...
    for (tbl=0; tbl<DAL_LONGAN_L3_HOST_TBL_NUM; tbl++)
    {
        for (slot=0; slot<DAL_LONGAN_L3_HOST_TBL_WIDTH; slot+=hostAlloc.width)
        {
            ...
            {
                RT_ERR_HDL(_dal_longan_l3_hostEntry_get(unit, hostIdx, &hostEntry, DAL_LONGAN_L3_API_FLAG_NONE), errHandle, ret);
                if ((((FALSE == ipv6) && \
 ...
 */
int32
_dal_longan_l3_routeEntry_get(uint32 unit, uint32 index, dal_longan_l3_routeEntry_t *pEntry)
{
    int32   ret = RT_ERR_OK;
    uint32 ip6[IPV6_ADDR_LEN >> 2];
 ...
 */
int32
_dal_longan_l3_routeEntry_set(uint32 unit, uint32 index, dal_longan_l3_routeEntry_t *pEntry)
{
    int32   ret = RT_ERR_OK;
    prefix_routing_entry_t entry;
    uint32 ip6[IPV6_ADDR_LEN >> 2];
...
_dal_longan_l3_routeEntry_free(uint32 unit, uint32 index)
{
    ...
    {
        /* IPv4 */
        RT_ERR_HDL(_dal_longan_l3_routeEntry_get(unit, index, &routeEntry), errHandle, ret);
...
static int32 _dal_longan_l3_routeEntryIdx_hw_get(uint32 unit, rtk_l3_route_t *pRoute, int32 *pIndex)
{
    ...
    if (pRoute->flags & RTK_L3_FLAG_IPV6)
    {
        ...
        for (routeIdx = minIdx; (routeIdx - minIdx) < length; routeIdx += 3)
        {
            ...
            /* get entry from H/W */
            RT_ERR_HDL(_dal_longan_l3_routeEntry_get(unit, routeIdx, &routeEntry), errHandle, ret);
...
_dal_longan_l3_routeEntry_find(uint32 unit, rtk_l3_route_t *pRoute, int32 *pIndex, dal_longan_l3_routeEntry_t *pRouteEntry)
{
    ...
    if (-1 != *pIndex)
    {
        ...
        /* get entry from H/W */
        RT_ERR_HDL(_dal_longan_l3_routeEntry_get(unit, *pIndex, &routeEntry), errHandle, ret);
        RT_ERR_HDL(l3_util_routeEntry2rtkRoute(&rtkRoute, &routeEntry), errHandle, ret);
...
getEntry:
    RT_ERR_HDL(_dal_longan_l3_routeEntry_get(unit, *pIndex, pRouteEntry), errHandle, ret);
...
dal_longan_l3_info_get(uint32 unit, rtk_l3_info_t *pInfo)
{
    ...
    pInfo->route_ipv4_max = (DAL_LONGAN_L3_ROUTE_TBL_HEIGHT) * (DAL_LONGAN_L3_ROUTE_TBL_WIDTH);
...
dal_longan_l3_intf_create(uint32 unit, rtk_l3_intf_t *pIntf)
{
    ...
    RT_ERR_HDL(_dal_longan_l3_smac_set(unit, macIdx, macEntry.mac), errIntfSet, ret);

    L3_DB_UPDATE_INTF_VALID(unit, intfIdx, mtuIdx, macIdx, pIntf->vid);
...
{
    int32   ret = RT_ERR_OK;
    uint32  intfIdx = DAL_LONGAN_L3_INTF_ID_ENTRY_IDX(intfId);
    dal_longan_l3_macEntry_t macEntry;
    ...
    /* parameter check */
    RT_PARAM_CHK((intfIdx >= HAL_MAX_NUM_OF_INTF(unit)), RT_ERR_OUT_OF_RANGE);
    ...
    if (0 != _pL3Db[unit]->intf[intfIdx].ref_count)
    {
    ...
    if (IS_L3_RTMAC_CARE_VID(unit))
    {
        RT_ERR_HDL(_dal_longan_l3_macEntry_set(unit, intfIdx, &macEntry, DAL_LONGAN_L3_API_FLAG_NONE), errMacEntryFree, ret);
    }
    else
    {
        RT_ERR_HDL(_dal_longan_l3_macEntry_get(unit, _pL3Db[unit]->intf[intfIdx].mac_idx, &macEntry, (DAL_LONGAN_L3_API_FLAG_NONE)), errMacEntryget, ret);
...
dal_longan_l3_intf_destroyAll(uint32 unit)
{
    ...
    for (idx=0; idx<HAL_MAX_NUM_OF_INTF(unit); idx++)
    {
        if (BITMAP_IS_SET(_pL3Db[unit]->HW.intf_used, idx))
        {
            ...
            {
                RT_ERR_HDL(_dal_longan_l3_macEntry_get(unit, intfIdx, &macEntry, (DAL_LONGAN_L3_API_FLAG_NONE)), errMacEntryGet, ret);
                RT_ERR_HDL(_dal_longan_l3_macEntry_free(unit, &macEntry), errMacEntryFree, ret);
            }
            RT_ERR_HDL(_dal_longan_l3_intfEntry_free(unit, intfIdx, (DAL_LONGAN_L3_API_FLAG_NONE)), errIntfFree, ret);
            RT_ERR_HDL(_dal_longan_l3_intf_mtuEntry_free(unit, _pL3Db[unit]->intf[intfIdx].ip_mtu_idx), errMtuFree, ret);
...
dal_longan_l3_intf_get(uint32 unit, rtk_l3_intfKeyType_t type, rtk_l3_intf_t *pIntf)
{
    ...
    else if (RTK_L3_INTFKEYTYPE_VID == type || RTK_L3_INTFKEYTYPE_MAC_AND_VID == type)
    {
        ...
        {
            L3_TABLE_READ_ERR_HDL(unit, LONGAN_L3_ROUTER_MACt, _pL3Db[unit]->intf[intfId].mac_idx, rtMacEntry, "", errMacGet, ret);
            L3_TABLE_FIELD_MAC_GET_ERR_HDL(unit, LONGAN_L3_ROUTER_MACt, \
                LONGAN_L3_ROUTER_MAC_MACtf, smac, rtMacEntry, "", errMacGet, ret);
    ...
    L3_TABLE_READ_ERR_HDL(unit, LONGAN_L3_ROUTER_MACt, intfEntry.egrIntf.smac_idx, rtMacEntry, "", errMacGet, ret);
    L3_TABLE_FIELD_MAC_GET_ERR_HDL(unit, LONGAN_L3_ROUTER_MACt, \
        LONGAN_L3_ROUTER_MAC_MACtf, pIntf->mac_addr, rtMacEntry, "", errMacGet, ret);
...
dal_longan_l3_intf_set(uint32 unit, rtk_l3_intfKeyType_t type, rtk_l3_intf_t *pIntf)
{
    ...
    if (pIntf->mtu != ip_mtu)
    {
        ...
        intfEntry.egrIntf.ip6_mtu_idx = intfEntry.egrIntf.ip_mtu_idx;
    }
    ...
    if (intfChg)
    {
        ...
        if (IS_L3_RTMAC_CARE_VID(unit))
        {
            ...
            RT_ERR_HDL(_dal_longan_l3_macEntry_set(unit, smacIdx, &newMacEntry, DAL_LONGAN_L3_API_FLAG_NONE), errMacRealloc, ret);
        }
        else
        {
            RT_ERR_HDL(_dal_longan_l3_macEntry_realloc(unit, &newMacEntry, &intfEntry.egrIntf.smac_idx), errMacRealloc, ret);
            smacIdx = intfEntry.egrIntf.smac_idx;
        }
    ...
    L3_DB_UPDATE_INTF_VALID(unit, intfId, intfEntry.egrIntf.ip_mtu_idx, smacIdx, pIntf->vid);
...
dal_longan_l3_nextHop_create(uint32 unit, rtk_l3_flag_t flags, rtk_l3_nextHop_t *pNextHop, rtk_l3_pathId_t *pPathId)
{
    ...
    /* parameter check */
    RT_PARAM_CHK(((~RTK_L3_FLAG_REPLACE & flags) != 0), RT_ERR_INPUT);
    ...
    RT_ERR_HDL(_dal_longan_l3_nhDmac_set(unit, nhIdx, pNextHop->mac_addr), errL3NhSet, ret);
...
dal_longan_l3_nextHop_destroy(uint32 unit, rtk_l3_pathId_t pathId)
{
    ...
    if (0 != _pL3Db[unit]->nexthop[nhIdx].ref_count)
    {
        ret = RT_ERR_ENTRY_REFERRED;
        goto errEntryReferred;
    }
...
dal_longan_l3_host_add(uint32 unit, rtk_l3_host_t *pHost)
{
    ...
    if (!(pHost->flags & RTK_L3_FLAG_NULL_INTF))
    {
        if (pHost->flags & RTK_L3_FLAG_WITH_NEXTHOP)
        {
            ...
            {
                RT_ERR_HDL(dal_longan_l3_nextHopPath_find(unit, &pHost->nexthop, &nhIdx), errHandle,ret);
                pHost->path_id = nhIdx;
            }
        }
        else
    ...
    RT_ERR_HDL(_dal_longan_l3_hostEntry_find(unit, pHost, NULL, &hostAlloc, &tmpHostIdx), errHandle,ret);
    if (-1 == tmpHostIdx)
    {
    ...
    else
    {
        ...
        RT_ERR_HDL(_dal_longan_l3_hostEntry_get(unit, tmpHostIdx, &hostEntry, DAL_LONGAN_L3_API_FLAG_NONE), errHandle, ret);
...
dal_longan_l3_host_del(uint32 unit, rtk_l3_host_t *pHost)
{
    ...
    RT_ERR_HDL(_dal_longan_l3_hostEntry_find(unit, pHost, &hostEntry,NULL, &hostIdx), errHandle, ret);
    if (hostIdx >= 0)
    {
...
dal_longan_l3_hostConflict_get(uint32 unit, rtk_l3_key_t *pKey, rtk_l3_host_t *pHostArray, int32 maxHost, int32 *pHostCount)
{
    ...
    for (tbl=0; tbl<DAL_LONGAN_L3_HOST_TBL_NUM; tbl++)
    {
        for (slot=0; slot<DAL_LONGAN_L3_HOST_TBL_WIDTH; slot++)
        {
            ...
            if (_pL3Db[unit]->host[hostIdx].valid)
            {
                RT_ERR_HDL(_dal_longan_l3_hostEntry_get(unit, hostIdx, &hostEntry, flags), errHandle, ret);
...
dal_longan_l3_host_getNext(uint32 unit, rtk_l3_flag_t flags, int32 *pBase, rtk_l3_host_t *pHost)
{
    ...
    for (hostIdx=(*pBase<0)?(0):((*pBase)+1); hostIdx<DAL_LONGAN_L3_HOST_TBL_SIZE; hostIdx++)
    {
        if ((_pL3Db[unit]->host[hostIdx].valid) && \
            (_pL3Db[unit]->host[hostIdx].ipv6 == ipv6))
        {
            RT_ERR_HDL(_dal_longan_l3_hostEntry_get(unit, hostIdx, &hostEntry, ipv6 ? DAL_LONGAN_L3_API_FLAG_IPV6 : DAL_LONGAN_L3_API_FLAG_NONE), errHandle, ret);
...
dal_longan_l3_route_add(uint32 unit, rtk_l3_route_t *pRoute)
{
    ...
    if (!(pRoute->flags & RTK_L3_FLAG_NULL_INTF))
    {
        if (pRoute->flags & RTK_L3_FLAG_WITH_NEXTHOP)
        {
            ...
            {
                RT_ERR_HDL(dal_longan_l3_nextHopPath_find(unit, &pRoute->nexthop, &nhIdx), errHandle,ret);
    ...
    else  /* entry exsit */
    {
        ...
        /* get old nh_idx */
        RT_ERR_HDL(_dal_longan_l3_routeEntry_get(unit, tmpRouteIdx, &routeEntry), errHandle, ret);
    ...
    /* write data into chip */
    RT_ERR_HDL(_dal_longan_l3_routeEntry_set(unit, routeIdx, &routeEntry), errHandle, ret);
...
dal_longan_l3_route_del_byIntfId(uint32 unit, rtk_l3_flag_t flags, rtk_intf_id_t intfId)
{
    ...
    for (routeIdx = baseIdx; L3_ABS(routeIdx, baseIdx) < length; routeIdx += step)
    {
        ...
        RT_ERR_HDL(_dal_longan_l3_routeEntry_get(unit, (uint32)routeIdx, &routeEntry), errHandle, ret);
