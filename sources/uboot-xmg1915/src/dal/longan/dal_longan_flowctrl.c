 ...
 *
 * $Revision: 77638 $
 * $Date: 2017-04-13 16:22:27 +0800 (Thu, 13 Apr 2017) $
 *
...
dal_longan_flowctrl_init(uint32 unit)
{
    ...
    if((ret = dal_longan_flowctrl_portEgrDropRefCongestEnable_set(unit, HWP_CPU_MACID(unit), DISABLED)) != RT_ERR_OK)
    {
    ...
    {
        if((ret = dal_longan_flowctrl_portEgrQueueDropForceEnable_set(unit, HWP_CPU_MACID(unit), queue, ENABLED)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_FLOWCTRL), "");
            return ret;
        }
    ...
    {
        val=0x3;
        if ((ret = reg_array_field_write(unit, LONGAN_SC_P_ENr, port, REG_ARRAY_INDEX_NONE, LONGAN_CNGST_SUST_TMR_LMT_Hf, &val)) != RT_ERR_OK)
        {
        ...
        val=0x3;
        if ((ret = reg_array_field_write(unit, LONGAN_SC_P_ENr, port, REG_ARRAY_INDEX_NONE, LONGAN_CNGST_SUST_TMR_LMTf, &val)) != RT_ERR_OK)
        {
            FLOWCTRL_SEM_UNLOCK(unit);
            RT_ERR(ret, (MOD_DAL|MOD_FLOWCTRL), "");
            return ret;
    ...
    val=0x32;
    if ((ret = reg_field_write(unit, LONGAN_SC_P_CTRLr, LONGAN_DRAIN_OUT_THR_Hf, &val)) != RT_ERR_OK)
    {
        FLOWCTRL_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_FLOWCTRL), "");
        return ret;
    ...
    val=0x32;
    if ((ret = reg_field_write(unit, LONGAN_SC_P_CTRLr, LONGAN_DRAIN_OUT_THRf, &val)) != RT_ERR_OK)
    {
        FLOWCTRL_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_FLOWCTRL), "");
        return ret;
