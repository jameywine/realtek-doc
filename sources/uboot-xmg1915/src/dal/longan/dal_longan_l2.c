...
int32 _dal_longan_l2_getL2Entry(uint32 unit, dal_longan_l2_index_t * pIndex, dal_longan_l2_entry_t *pL2_entry)
{
    ...
    else
    {
        ...
        {
            pL2_entry->unicast.unit_id = (spa>>6) & 0xf;
            pL2_entry->unicast.port = spa & 0x3f;
        }
        else
        {
            pL2_entry->unicast.unit_id = (spa>>9) & 0x1;
...
static int32 _dal_longan_l2_setL2Entry(uint32 unit, dal_longan_l2_index_t * pIndex, dal_longan_l2_entry_t *pL2_entry)
{
    ...
    if(pL2_entry->entry_type == L2_UNICAST)
    {
        ...
        if(pL2_entry->unicast.is_trk==0)
        {
            spa = (pL2_entry->unicast.unit_id<<6) | pL2_entry->unicast.port;
        }
        else
        {
            spa = (pL2_entry->unicast.unit_id<<9) | pL2_entry->unicast.trunk;
        }
...
dal_longan_l2_ucastAddr_flush(uint32 unit, rtk_l2_flushCfg_t *pConfig)
{
    ...
    if(pConfig->flushByPort==ENABLED && pConfig->portOrTrunk==ENABLED)
        RT_PARAM_CHK(((unit==pConfig->unit_id) && !HWP_PORT_EXIST(pConfig->unit_id, pConfig->port)), RT_ERR_PORT_ID);
    ...
    if(pConfig->act == FLUSH_ACT_REPLACE_ALL_UC || pConfig->act == FLUSH_ACT_REPLACE_DYNM_ONLY)
    {
        ...
        RT_PARAM_CHK((ENABLED == pConfig->replacePortOrTrunk && ((unit==pConfig->replacingUnit_id) && !HWP_PORT_EXIST(pConfig->replacingUnit_id, pConfig->replacingPort))), RT_ERR_PORT_ID);
    ...
    value = (pConfig->portOrTrunk==ENABLED) ? ((pConfig->unit_id<<6) | pConfig->port) : ((1<<10)|pConfig->trunk);
    ...
    value = (pConfig->replacePortOrTrunk==ENABLED) ? ((pConfig->replacingUnit_id <<6) | pConfig->replacingPort) : ((1<<10)|pConfig->replacingTrk);
...
dal_longan_l2_agingTime_get(uint32 unit, rtk_l2_ageTimeType_t type, uint32 *pAging_time)
{
    ...
    if(type==L2_AGE_TIME_NORMAL)
        *pAging_time = (uint32)(value*1024*7/10000);
    else
        *pAging_time = (uint32)(value*1024*suspendMax/10000);
...
dal_longan_l2_agingTime_set(uint32 unit, rtk_l2_ageTimeType_t type, uint32 aging_time)
{
    ...
    if(type==L2_AGE_TIME_NORMAL)
    {
        ageUnit = (uint32)(aging_time*10000/1024/7);
    ...
    else
    {
        ...
        ageUnit = (uint32)(value*1024/10000);

        if(aging_time==0 || ageUnit==0)
            value=0;
        else if((uint32)(aging_time/ageUnit)==0)
            value=1;
        else
            value=(uint32)(aging_time/ageUnit);
...
dal_longan_l2_addr_add(uint32 unit, rtk_l2_ucastAddr_t *pL2_addr)
{
    ...
    else
    {
        if(((pL2_addr->flags & RTK_L2_UCAST_FLAG_SA_BLOCK) || (pL2_addr->flags & RTK_L2_UCAST_FLAG_DA_BLOCK) || (pL2_addr->flags & RTK_L2_UCAST_FLAG_NEXTHOP)) && !(pL2_addr->flags & RTK_L2_UCAST_FLAG_STATIC))
            RT_PARAM_CHK((((unit==pL2_addr->unit_id) && !HWP_PORT_EXIST(pL2_addr->unit_id, lPort)) && lPort!=PORT_DONT_CARE_9300), RT_ERR_PORT_ID);
        else
            RT_PARAM_CHK(((unit==pL2_addr->unit_id) && !HWP_PORT_EXIST(pL2_addr->unit_id, lPort)), RT_ERR_PORT_ID);
    }
    ...
    RT_PARAM_CHK((pL2_addr->unit_id >= RTK_MAX_NUM_OF_UNIT), RT_ERR_UNIT_ID);
    ...
    if((ret=reg_field_read(unit, LONGAN_STK_GLB_CTRLr, LONGAN_MY_UNIT_IDf, &myUnitId)) != RT_ERR_OK)
    {
        L2_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_L2), "");
        return ret;
    ...
    else if (ret == RT_ERR_L2_NO_EMPTY_ENTRY && (pL2_addr->add_op_flags & RTK_L2_ADD_OP_FLAG_REPLACE_DYNAMIC) != 0)
    {
        ...
        pL2_addr->macInfo.unit      = l2_entry.unicast.unit_id;
    ...
    if(pL2_addr->flags & RTK_L2_UCAST_FLAG_TRUNK_PORT)
    {
        ...
        l2_entry.unicast.unit_id = (age_val==0 && age_valid) ? 1 : 0;
        l2_entry.unicast.trunk = pL2_addr->trk_gid;
    }
    else
    {
        l2_entry.unicast.unit_id = pL2_addr->unit_id;
    ...
    {
        if(age_valid==0 || (l2_entry.unicast.is_trk==0 && l2_entry.unicast.unit_id==myUnitId))
        {
            l2_entry.unicast.is_trk     = 0;
            l2_entry.unicast.unit_id    = 0;
        ...
        {
            l2_entry.unicast.unit_id = l2_entry.unicast.is_trk ? 1 : l2_entry.unicast.unit_id;
        }
    }
...
dal_longan_l2_addr_get(uint32 unit, rtk_l2_ucastAddr_t *pL2_addr)
{
    ...
    if(l2_entry.unicast.is_trk)
    {
        ...
        pL2_addr->unit_id = l2_entry.unicast.unit_id;   /*when trunk, unit_id indicate if entry is notified*/
        pL2_addr->trk_gid = l2_entry.unicast.trunk;
    }
    else
    {
        pL2_addr->unit_id = l2_entry.unicast.unit_id;
...
_dal_longan_l2_nexthop_add(uint32 unit, dal_longan_l2_ucastNhAddr_t *pNexthop_addr)
{
    ...
    else if (ret == RT_ERR_L2_NO_EMPTY_ENTRY && (pNexthop_addr->add_op_flags & RTK_L2_ADD_OP_FLAG_REPLACE_DYNAMIC) != 0)
    {
        ...
        pNexthop_addr->macInfo.unit      = l2_entry.unicast.unit_id;
...
dal_longan_l2_mcastAddr_addByIndex(uint32 unit, rtk_l2_mcastAddr_t *pMcast_addr)
{
    ...
    else if (ret == RT_ERR_L2_NO_EMPTY_ENTRY && (pMcast_addr->add_op_flags & RTK_L2_ADD_OP_FLAG_REPLACE_DYNAMIC) != 0)
    {
        ...
        pMcast_addr->macInfo.unit      = l2_entry.unicast.unit_id;
...
dal_longan_l2_cpuMacAddr_add(uint32 unit, rtk_vlan_t vid, rtk_mac_t *pMac)
{
    ...
    osal_memcpy(&l2_entry.unicast.mac, pMac, sizeof(rtk_mac_t));
    l2_entry.unicast.is_trk = 0;
    l2_entry.unicast.unit_id = HWP_MY_UNIT_ID();
...
dal_longan_l2_addrEntry_get(uint32 unit, uint32 index, rtk_l2_entry_t *pL2_entry)
{
    ...
    if(pL2_entry->entry_type == FLOW_TYPE_UNICAST)
    {
        ...
        {
            pL2_entry->unicast.unit_id = l2_entry.unicast.unit_id;
    ...
    for (blk0_index.hashdepth= 0; blk0_index.hashdepth < HAL_L2_HASHDEPTH(unit) && cf_num < cfAddrList_size; blk0_index.hashdepth++)
    {
        ...
        if(l2_entry.is_entry_valid==1)
        {
            ...
            if(l2_entry.entry_type == L2_UNICAST)
            {
                ...
                {
                     (pCfAddrList + cf_num)->unicast.unit_id = l2_entry.unicast.unit_id;
    ...
    rtk_l2_entry_t  *pEntry)
{
    ...
    if (l2_entry.entry_type==L2_UNICAST)
    {
        ...
        if (l2_entry.unicast.is_trk)
        {
            ...
            pEntry->unicast.unit_id = l2_entry.unicast.unit_id;
