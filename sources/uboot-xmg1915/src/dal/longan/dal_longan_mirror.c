 ...
 *
 * $Revision: 76797 $
 * $Date: 2017-03-22 14:57:36 +0800 (Wed, 22 Mar 2017) $
 *
    ...
    rtk_mirror_entry_t  *pMirrorEntry)
{
    ...
    {
        pMirrorEntry->mirroring_unit = ((rtk_port_t)value >> 6) & 0xF;
    ...
    rtk_mirror_entry_t  *pMirrorEntry)
{
    ...
    {
        RT_PARAM_CHK(pMirrorEntry->mirroring_unit >= RTK_MAX_NUM_OF_UNIT, RT_ERR_UNIT_ID);
        ...
        if ((ret = dal_longan_stack_unit_get(unit, &myUnit)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_MIRROR), "");
            return ret;
        }

        if(myUnit == pMirrorEntry->mirroring_unit)
        {
    ...
    else
    {
        ...
        /* MTP port means UNIT+PORT */
        value = ((uint32)pMirrorEntry->mirroring_unit<<6) + (uint32)pMirrorEntry->mirroring_port;
        if ((ret = reg_array_field_write(unit, LONGAN_MIR_CTRLr, REG_ARRAY_INDEX_NONE, mirror_id,
                    LONGAN_MTP_PORTf, &value)) != RT_ERR_OK)
        {
            MIRROR_SEM_UNLOCK(unit);
