 ...
 *
 * $Revision: 79974 $
 * $Date: 2017-06-23 10:45:00 +0800 (Fri, 23 Jun 2017) $
 *
...
int32 dal_linkMon_devInit(uint32 unit)
{
    ...
    if (RT_ERR_OK != ret){
        RT_INIT_ERR(ret, (MOD_INIT|MOD_DAL), "RT_MAPPER(unit)->port_linkChange_register Failed!!");
        return ret;
    }
    RT_LOG(LOG_EVENT, (MOD_INIT|MOD_DAL), "RT_MAPPER(unit)->port_linkChange_register Completed!!");
...
int32 dal_linkMon_register(rtk_port_linkMon_callback_t linkMon_callback)
{
    ...
    {   /* Do this only when first CB register */
        ...
        HWP_UNIT_TRAVS_LOCAL(unit){
            ...
            {
                ioal_mem32_read(unit, RTL9300_ISR_GLB_ADDR, &value);
                value |= RTL9300_ISR_GLB_ISR_GLB_LINK_CHG_MASK;
                ioal_mem32_write(unit, RTL9300_ISR_GLB_ADDR, value);
...
static int32 _dal_linkMon_linkChange(int32 unit, rtk_portmask_t *pChanged_portmask)
{
    ...
    RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_PORT), "unit=%d, pChanged_portmask=%x",
