 ...
 *
 * $Revision: 81342 $
 * $Date: 2017-08-16 17:24:22 +0800 (Wed, 16 Aug 2017) $
 *
...
dal_phy_init(uint32 unit)
{
    ...
    HWP_PORT_TRAVS_EXCEPT_CPU(unit, port)
    {
        ...
        else if (HWP_10GE_PORT(unit,port))
        {
            ...
            /* auto-nego ability */
            if (HWP_SERDES_PORT(unit,port) || HWP_FIBER_PORT(unit,port))
            {
                phy_ability.Half_1000 = RTK_DEFAULT_PORT_1000HALF_CAPABLE;
                phy_ability.Full_1000 = RTK_DEFAULT_PORT_1000FULL_CAPABLE;
            }
    ...
    rtk_enable_t *pEnable)
{
    ...
    /* parameter check */
    RT_PARAM_CHK((!HWP_PORT_EXIST(unit, port) || HWP_IS_CPU_PORT(unit, port)), RT_ERR_PORT_ID);
    RT_PARAM_CHK((!(HWP_GE_PORT(unit, port)&&HWP_COMBO_PORT(unit, port))), RT_ERR_PORT_ID);
    RT_PARAM_CHK((NULL == pEnable), RT_ERR_NULL_POINTER);
...
_dal_phy_serdesLinkDown_handler(uint32 unit)
{
    ...
    HWP_PHY_TRAVS(unit,scanPhyIdx)
    {
        ...
        if((RTK_PHYTYPE_RTL8218D != phyModel) && (RTK_PHYTYPE_RTL8218D_NMP != phyModel))
        {
            continue;
        }
...
dal_phy_portEyeMonitor_start(uint32 unit, rtk_port_t port, uint32 frameNum)
{
    ...
    RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_PHY), "unit=%d,port=%d,frameNum=%d", unit, port, frameNum);
    ...
    if ((ret = phy_portEyeMonitor_start(unit, port, frameNum)) != RT_ERR_OK)
    {
        PHY_SEM_UNLOCK(unit);
        RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_PHY), "Error Code: 0x%X", ret);
        return ret;
