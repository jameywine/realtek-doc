 ...
 *
 * $Revision: 74509 $
 * $Date: 2016-12-21 17:20:13 +0800 (Wed, 21 Dec 2016) $
 *
 ...
 *      (2) The stacking portmask indicates if stacking ports from port ID 52 to 55
...
rtk_trunk_srcPortMap_get(uint32 unit,  rtk_unit_port_t unitPort, uint32 *pIsTrkMbr, rtk_trk_t *pTrk_gid)
{
    ...
    return RT_MAPPER(unit)->trunk_srcPortMap_get(unit, unitPort, pIsTrkMbr, pTrk_gid);
...
rtk_trunk_srcPortMap_set(uint32 unit, rtk_unit_port_t unitPort, uint32 isTrkMbr, rtk_trk_t trk_gid)
{
    ...
    return RT_MAPPER(unit)->trunk_srcPortMap_set(unit, unitPort, isTrkMbr, trk_gid);
