 ...
 *
 * $Revision: 76477 $
 * $Date: 2017-03-14 11:41:24 +0800 (Tue, 14 Mar 2017) $
 *
...
rtk_time_refTime_get(uint32 unit, rtk_time_timeStamp_t *pTimeStamp)
{
    ...
    return RT_MAPPER(unit)->time_refTime_get(unit, pTimeStamp);
...
rtk_time_refTime_set(uint32 unit, rtk_time_timeStamp_t timeStamp)
{
    ...
    return RT_MAPPER(unit)->time_refTime_set(unit, timeStamp);
...
rtk_time_refTimeAdjust_set(uint32 unit, uint32 sign, rtk_time_timeStamp_t timeStamp)
{
    ...
    return RT_MAPPER(unit)->time_refTimeAdjust_set(unit, sign, timeStamp);
...
rtk_time_refTimeEnable_get(uint32 unit, rtk_enable_t *pEnable)
{
    ...
    return RT_MAPPER(unit)->time_refTimeEnable_get(unit, pEnable);
} /* end of rtk_time_refTimeEnable_get */
...
rtk_time_refTimeEnable_set(uint32 unit, rtk_enable_t enable)
{
    ...
    return RT_MAPPER(unit)->time_refTimeEnable_set(unit, enable);
} /* end of rtk_time_refTimeEnable_set */

/* Function Name:
 *      rtk_time_refTimeFreq_get
 * Description:
 *      Get the frequency of PTP reference time of the specified device.
 * Input:
 *      unit    - unit id
 * Output:
...
rtk_time_refTimeFreq_get(uint32 unit, uint32 *pFreq)
{
    ...
    return RT_MAPPER(unit)->time_refTimeFreq_get(unit, pFreq);
}   /* end of rtk_time_refTimeFreq_get */


/* Function Name:
 *      rtk_time_refTimeFreq_set
 * Description:
 *      Set the frequency of PTP reference time of the specified device.
 * Input:
 *      unit   - unit id
...
rtk_time_refTimeFreq_set(uint32 unit, uint32 freq)
{
    ...
    return RT_MAPPER(unit)->time_refTimeFreq_set(unit, freq);
...
rtk_time_correctionFieldTransparentValue_get(uint32 unit, int64 oriCf, rtk_time_timeStamp_t rxTimeStamp, int64 *pTransCf)
{
    ...
    {
        *pTransCf = ((c_sec & 0x7FFFF)<<44);
        *pTransCf = (*pTransCf | (c_subns & 0xFFFFFFFC)>>1);
    }
