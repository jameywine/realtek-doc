...
rtk_switch_deviceCapability_print(uint32 unit)
{
    ...
    osal_printf("HAL_CAPWAP_HASHDEPTH                                %u\n", HAL_CAPWAP_HASHDEPTH(unit));
...
rtk_switch_deviceInfo_get(uint32 unit, rtk_switch_devInfo_t *pDevInfo)
{
    ...
    /* STACK */
    pDevInfo->stack.portNum = HWP_STACK_PORT_COUNT(unit);
    osal_memset(&pDevInfo->stack.portmask, 0, sizeof(rtk_portmask_t));
    for (i=0; i<RTK_MAX_NUM_OF_PORTS; i++) {
        if (HWP_PORT_EXIST(unit, i) && HWP_STACK_PORT(unit, i))
 ...
 *      8380, 8390, 9300, 9310
 * Note:
 *      (1) Checksum fail type is as following \ Chip :     8390 9300 9310
 *          - LAYER2_CHKSUM_FAIL                              O    O    O
 *      (2) Forwarding action is as following \ Chip :      8390 9300 9310
...
/* Function Name:
 *      rtk_switch_tagPauseAction_get
 * Description:
 *      Get forwarding action of the pause frame with tag.
 * Input:
 *      unit   - unit id
 * Output:
 *      action - forwarding action of the pause frame with tag.
 * Return:
 *      RT_ERR_OK
 *      RT_ERR_FAILED
 *      RT_ERR_NOT_INIT         - The module is not initial
...
rtk_switch_tagPauseAction_get(uint32 unit, rtk_action_t *action)
{
    ...
    return RT_MAPPER(unit)->switch_tagPauseAction_get(unit, action);
}   /* end of rtk_switch_tagPauseAction_get */

/* Function Name:
 *      rtk_switch_tagPauseAction_set
 * Description:
 *      Set forwarding action of the pause frame with tag.
 * Input:
 *      unit   - unit id
 *      action - forwarding action of the pause frame with tag.
 * Output:
...
rtk_switch_tagPauseAction_set(uint32 unit, rtk_action_t action)
{
    ...
    return RT_MAPPER(unit)->switch_tagPauseAction_set(unit, action);
}   /* end of rtk_switch_tagPauseAction_set */

/* Function Name:
 *      rtk_switch_halfConsecutiveRetryEnable_get
 * Description:
 ...
 */
int32
rtk_switch_halfConsecutiveRetryEnable_get(uint32 unit, rtk_enable_t *pEnable)
{
    /* function body */
    RT_PARAM_CHK((unit > RTK_MAX_UNIT_ID) || NULL == RT_MGMT(unit), RT_ERR_UNIT_ID);

    return RT_MAPPER(unit)->switch_halfConsecutiveRetryEnable_get(unit, pEnable);
}   /* end of rtk_switch_halfConsecutiveRetryEnable_get */


/* Function Name:
 *      rtk_switch_halfConsecutiveRetryEnable_set
 * Description:
 ...
 */
int32
rtk_switch_halfConsecutiveRetryEnable_set(uint32 unit, rtk_enable_t enable)
{
    /* function body */
    RT_PARAM_CHK((unit > RTK_MAX_UNIT_ID) || NULL == RT_MGMT(unit), RT_ERR_UNIT_ID);

    return RT_MAPPER(unit)->switch_halfConsecutiveRetryEnable_set(unit, enable);
}   /* end of rtk_switch_halfConsecutiveRetryEnable_set */
