 ...
 *
 * $Revision: 75080 $
 * $Date: 2017-01-06 11:04:17 +0800 (Fri, 06 Jan 2017) $
 *
...
/* Function Name:
 *      rtk_rate_portIgrQueueFixedBwCtrlEnable_get
 * Description:
 *      Get enable status of ingress fixed bandwidth control on specified queue.
 * Input:
 *      unit    - unit id
 *      port    - port id
 *      queue   - queue id
 * Output:
 *      pEnable - Pointer to enable status of ingress queue fixed bandwidth control
 * Return:
 *      RT_ERR_OK
 *      RT_ERR_FAILED
 *      RT_ERR_UNIT_ID      - invalid unit id
    ...
    rtk_enable_t    *pEnable)
{
    ...
    return RT_MAPPER(unit)->rate_portIgrQueueFixedBwCtrlEnable_get(unit, port, queue, pEnable);

} /* end of rtk_rate_portIgrQueueFixedBwCtrlEnable_get */

/* Function Name:
 *      rtk_rate_portIgrQueueFixedBwCtrlEnable_set
 * Description:
 *      Set enable status of ingress fixed bandwidth control on specified queue.
 * Input:
 *      unit   - unit id
 *      port   - port id
 *      queue  - queue id
 *      enable - enable status of ingress queue fixed bandwidth control
 * Output:
 *      None
 * Return:
 *      RT_ERR_OK
    ...
    rtk_enable_t    enable)
{
    ...
    return RT_MAPPER(unit)->rate_portIgrQueueFixedBwCtrlEnable_set(unit, port, queue, enable);

} /* end of rtk_rate_portIgrQueueFixedBwCtrlEnable_set */
 ...
 *      (3) The unit of granularity is 16Kbps or 1pps in 8390/9310 CPU port depend on mode set by 'rtk_rate_cpuEgrBandwidthCtrlRateMode_set'.
 * Changes:
 *      [SDK_3.0.0]
 *          New added function.
