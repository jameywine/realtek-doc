 ...
 * Input:
 *      unit  - unit id
 *      entry - VNI entry (intf_id and vni as match key)
 * Output:
 *      None
 * Return:
 *      RT_ERR_OK
...
rtk_vxlan_vni_add(uint32 unit, rtk_vxlan_vniEntry_t entry)
{
    ...
    return RT_MAPPER(unit)->vxlan_vni_add(unit, entry);
...
rtk_vxlan_vni_del(uint32 unit, rtk_vxlan_vniEntry_t entry)
{
    ...
    return RT_MAPPER(unit)->vxlan_vni_del(unit, entry);
...
rtk_vxlan_vni_set(uint32 unit, rtk_vxlan_vniEntry_t entry)
{
    ...
    return RT_MAPPER(unit)->vxlan_vni_set(unit, entry);
