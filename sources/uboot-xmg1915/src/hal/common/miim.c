 ...
 *
 * $Revision: 81210 $
 * $Date: 2017-08-14 15:33:42 +0800 (Mon, 14 Aug 2017) $
 *
...
#define MIIM_NULL_PHY_DRV(pHalCtrl, port)            ((pHalCtrl->pPhy_ctrl[port] == NULL) || (pHalCtrl->pPhy_ctrl[port]->pPhydrv == NULL))
...
do {\
    if (osal_sem_mutex_take(hal_ctrl[unit].miim_sem, OSAL_SEM_WAIT_FOREVER) != RT_ERR_OK)\
    {\
...
do {\
    if (osal_sem_mutex_give(hal_ctrl[unit].miim_sem) != RT_ERR_OK)\
    {\
...
{
    return hal_miim_pollingEnable_get(unit, port, pEnabled);
}

/* Function Name:
 *      _phy_sts_pollingEnable_set
 * Description:
...
{
    return hal_miim_pollingEnable_set(unit, port, enabled);
}

/* Function Name:
 *      _phy_sts_globalPollingEnable_get
 * Description:
 *      Get the mac polling PHY status.
 * Input:
 *      unit     - unit id
 * Output:
 *      pEnabled - pointer buffer of mac polling PHY status
...
_phy_sts_globalPollingEnable_get(
    ...
    rtk_enable_t    *pEnabled)
{
    return hal_miim_globalPollingEnable_get(unit, pEnabled);
}

/* Function Name:
 *      _phy_sts_globalPollingEnable_set
 * Description:
 *      Set the mac polling PHY status.
 * Input:
 *      unit    - unit id
 *      enabled - mac polling PHY status
 * Output:
...
_phy_sts_globalPollingEnable_set(
    ...
    rtk_enable_t    enabled)
{
    return hal_miim_globalPollingEnable_set(unit, enabled);
}
...
phy_media_set(uint32 unit, rtk_port_t port, rtk_port_media_t media)
{
    ...
    if ((ret_polling = _phy_sts_pollingEnable_get(unit, port, &pollSts)) != RT_ERR_OK)
    {
        MIIM_SEM_UNLOCK(unit);
    ...
    /* disable MAC polling */
    if ((ret_polling = _phy_sts_pollingEnable_set(unit, port, DISABLED)) != RT_ERR_OK)
    {
        MIIM_SEM_UNLOCK(unit);
        RT_ERR(ret_polling, (MOD_HAL|MOD_PORT), "port %u disable MAC polling fail (0x%x)", port, ret_polling);
        return ret_polling;
    ...
    if ((ret_polling = _phy_sts_pollingEnable_set(unit, port, pollSts)) != RT_ERR_OK)
    {
        MIIM_SEM_UNLOCK(unit);
...
int32 phy_serdes_rst(uint32 unit, rtk_port_t port)
{
    ...
    switch (pHalCtrl->pPhy_ctrl[port]->pPhydrv->phydrv_index)
    {
        ...
        hal_miim_write(unit, port, 0, 30, 8);
        /*serdes 0 reset */
        hal_miim_write(unit, port, 0x400, 0x10, 0x9700);
        hal_miim_write(unit, port, 0x400, 0x10, 0x9703);

        hal_miim_read(unit, port, 0x260, 18, &reg_data);
        /*for qsgmii serdes 1 reset*/
        if(0xd0 == (reg_data & 0xF0))
        {
            hal_miim_write(unit, port, 0x500, 0x10, 0x9400);
            hal_miim_write(unit, port, 0x500, 0x10, 0x9403);
        }
        hal_miim_write(unit, port, 0, 30, 0);
        break;
      #endif
...
phy_construct_enable_set(uint32 phyType, uint32 unit, rtk_port_t port, rtk_enable_t enable)
{
    ...
    {
        pPhyDrv = phy_identify_driver_find_by_phyType((phy_type_t)phyType);
    }
...
phy_chipRevId_get(uint32 unit, rtk_port_t port, uint16 *pChip_rev_id)
{
    ...
    _phy_sts_pollingEnable_get(unit, port, &pollSts);
    _phy_sts_pollingEnable_set(unit, port, DISABLED);

    ret = pHalCtrl->pPhy_ctrl[port]->pPhydrv->fPhydrv_chipRevId_get(unit, port, pChip_rev_id);

    _phy_sts_pollingEnable_set(unit, port, pollSts);
 ...
 */
int32
phy_portEyeMonitor_start(uint32 unit, rtk_port_t port, uint32 frameNum)
{
    hal_control_t   *pHalCtrl;
    int32           ret;
    ...
    ret = pHalCtrl->pPhy_ctrl[port]->pPhydrv->fPhydrv_portEyeMonitor_start(unit, port, frameNum);
 ...
 */
int32
_phy_macIntfSerdesMode_get(uint32 unit, rtk_port_t port, rt_serdesMode_t *pserdesMode)
{
    hal_control_t   *pHalCtrl;
    int32           ret;
    ...
    if (HWP_PHY_MODEL_BY_PORT(unit, port) == RTK_PHYTYPE_CUST1)
    {
        ret = phy_cust1_macIntfSerdesMode_get(unit, port, pserdesMode);
    }
    else
#endif
    {
