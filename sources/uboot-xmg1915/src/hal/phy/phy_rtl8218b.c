 ...
 *
 * $Revision: 79280 $
 * $Date: 2017-05-31 10:57:12 +0800 (Wed, 31 May 2017) $
 *
...
void phy_8218Bdrv_ge_mapperInit(rt_phydrv_t *pPhydrv)
{
    ...
    pPhydrv->fPhydrv_gigaLiteEnable_get = phy_8218b_gigaLiteEnable_get;
    pPhydrv->fPhydrv_gigaLiteEnable_set = phy_8218b_gigaLiteEnable_set;
...
phy_8214fc_init(uint32 unit, rtk_port_t port)
{
    ...
    if ((ret = phy_8214fc_media_get(unit, port, &restore_media)) != RT_ERR_OK)
        return ret;
    ...
    if (restore_media != PORT_MEDIA_COPPER)
    {
        if ((ret = phy_8214fc_media_set(unit, port, restore_media)) != RT_ERR_OK)
            return ret;
    }
...
phy_8218b_speed_get(uint32 unit, rtk_port_t port, uint32 *pSpeed)
{
    ...
    {
        media = PORT_MEDIA_FIBER;
        if ((ret = phy_8218b_media_set(unit, port, media)) != RT_ERR_OK)
            return ret;
    }
...
phy_8218fb_speed_set(uint32 unit, rtk_port_t port, uint32 speed)
{
    ...
    /* Link-down */
    if ((ret = phy_8218fb_media_get(unit, port, &restore_media)) != RT_ERR_OK)
        return ret;

    if (restore_media == PORT_MEDIA_FIBER_AUTO)
    {
        media = PORT_MEDIA_FIBER;
        if ((ret = phy_8218fb_media_set(unit, port, media)) != RT_ERR_OK)
            return ret;
    }
    else if (restore_media == PORT_MEDIA_COPPER_AUTO)
    {
    ...
    if (restore_media == PORT_MEDIA_FIBER_AUTO || restore_media == PORT_MEDIA_COPPER_AUTO)
    {
        if ((ret = phy_8218fb_media_set(unit, port, restore_media)) != RT_ERR_OK)
            return ret;
    }
...
phy_8214fc_speed_set(uint32 unit, rtk_port_t port, uint32 speed)
{
    ...
    {
        media = PORT_MEDIA_FIBER;
        if ((ret = phy_8214fc_media_set(unit, port, media)) != RT_ERR_OK)
            return ret;
    }
    else if (restore_media == PORT_MEDIA_COPPER_AUTO)
    {
...
phy_8214fc_loopback_get(uint32 unit, rtk_port_t port, rtk_enable_t *pEnable)
{
    ...
    if ((ret = _phy_8214fc_intMedia_get(unit, port, &media, &link_status)) != RT_ERR_OK)
        return ret;
