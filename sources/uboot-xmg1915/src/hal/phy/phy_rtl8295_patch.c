 ...
 *
 * $Revision: 81011 $
 * $Date: 2017-08-06 17:25:16 +0800 (Sun, 06 Aug 2017) $
 *
...
{
    phy_8295r_patch_t       s1TxAttri8431[RTK_MAX_NUM_OF_PORTS];
    phy_8295r_patch_t       s1TxAttriDacLong[RTK_MAX_NUM_OF_PORTS];
...
#define PATCH_MSLEEP(_t)                                                                            \
            do {                                                                                    \
                /* PATCH_PRINT("SLEEP %u ms", _t);  */                                                   \
                osal_time_usleep((_t) * 1000);                                                      \
            } while(0)
...
void
_phy_8295_patch_1g_config(uint32 unit, uint32 basePort, uint32 mdxMacId)
{
...
void
_phy_8295_patch_10g_config(uint32 unit, uint32 port, uint32 mdxMacId)
{
    uint32      sdsId;
...
_phy_8295_rxCali_a_start_1d1d1(phy_8295_rxCaliSds_t *ps, phy_8295_rxCaliInfo_t *pinfo)
{
    ...
    PHY_SDS_FIELD_WR(ps->unit, ps->mdxMacId, ps->sdsId, 188, 19, 5, 0, 0x10);
    ...
    PHY_SDS_WR(ps->unit, ps->mdxMacId, ps->sdsId, 189, 19, 0x7C0F);
...
_phy_8295_rxCali_a_start_1d3d1(phy_8295_rxCaliSds_t *ps, phy_8295_rxCaliInfo_t *pinfo)
{
    ...
    uint32      leq_temp, sum10, avg10, int10;
    uint32      LEQ_OFFSET;
    uint32      flag_dac_long = pinfo->cfg.cable_offset;
...
phy_8214qf_patch_set(uint32 unit, uint32 basePort)
{
    ...
    _phy_8295_patch_1g_config(unit, basePort, mdxMacId);
}
...
phy_8295_patch_rxCali(uint32 unit, uint32 basePort, uint32 sdsId, phy_8295_rxCaliParam_t *param)
{
    ...
    RT_8295_LOG(LOG_INFO, (MOD_HAL|MOD_PORT), "unit=%u,port=%u,sds=%u cabOffset %u", unit, basePort, sdsId, param->cable_offset);
    ...
    else
    {
        ...
        do {
            ...
            rxCaliInfo.cfg.cable_offset = param->cable_offset;
            ...
            {
                RT_8295_LOG(LOG_INFO, (MOD_HAL|MOD_PORT), "unit=%u,port=%u flag_recal=%d", unit, basePort, rxCaliInfo.status.flag_recal);
            }
            retry--;
        } while (retry > 0);
        ...
        if (rxCaliInfo.status.flag_recal != 0)
        {
            int32   ret;
            ret = RT_ERR_FAILED;
            RT_ERR(ret, (MOD_HAL), "unit=%u,port=%u,sds=%u flag_recal=%d failed.", unit, basePort, sdsId, rxCaliInfo.status.flag_recal);
        }
    }
}
...
phy_8295r_patch_set(uint32 unit, uint32 basePort)
{
    ...
    _phy_8295_patch_10g_config(unit, basePort, mdxMacId);
}
...
phy_8295r_patch_txAttri_set(uint32 unit, rtk_port_t port, rtk_port_10gMedia_t media)
{
    ...
    uint32      mdxMacId;
    int32       ret;
    phy_8295_patch_type_t   type;
    phy_8295r_patch_t       *p;

    switch (media)
    {
      case PORT_10GMEDIA_FIBER_10G: //STD compliance
      case PORT_10GMEDIA_DAC_50CM:
      case PORT_10GMEDIA_DAC_100CM:
        type = PHY_8295_PATCH_TYPE_TXATTR_S18431;
        break;
      case PORT_10GMEDIA_DAC_300CM:
      case PORT_10GMEDIA_DAC_500CM://DAC_LONG
        type = PHY_8295_PATCH_TYPE_TXATTR_S1DACLONG;
        break;
    ...
    switch (type)
    {
      case PHY_8295_PATCH_TYPE_TXATTR_S18431:
        p = &gPhy8295rPI[unit].s1TxAttri8431[port];
        break;
      case PHY_8295_PATCH_TYPE_TXATTR_S1DACLONG:
        p = &gPhy8295rPI[unit].s1TxAttriDacLong[port];
        break;
      default:
        return RT_ERR_OK;
    }
    ...
    RT_8295_LOG(LOG_INFO, (MOD_HAL|MOD_PORT), "unit=%u,port=%u,patchType=%d", unit, port, type);
    ...
    for (i = 0; i < p->size; i++)
    {
        RT_8295_LOG(LOG_INFO, (MOD_HAL|MOD_PORT), "unit=%u,port=%u,S1TXATTR:page=%x,reg=%x,val=%x", unit, port, (uint32)p->prv[i].page, (uint32)p->prv[i].reg, (uint32)p->prv[i].val);
        //SDS_PATCH(unit, mdxMacId, 1, (uint32)p->prv[i].page, (uint32)p->prv[i].reg, (uint32)p->prv[i].val);
        PHY_REG_WR(unit, mdxMacId, (uint32)p->prv[i].page, (uint32)p->prv[i].reg, (uint32)p->prv[i].val);
    }
...
/* Function Name:
 *      phy_8295r_patchParam_init
 * Description:
 *      initial patch parameter of the port.
 *      The reg is dynamically configured when phy_8295r_patch_txAttri_set is called
 * Input:
 *      unit   - unit id
 *      port   - port id
 *      prv    - page/reg/value
...
phy_8295r_patchParam_init(uint32 unit, rtk_port_t port, phy_8295_patch_type_t type, void *prv, int32 size)
{
    ...
    if (type == PHY_8295_PATCH_TYPE_TXATTR_S18431)
    {
        ppi->s1TxAttri8431[port].prv = prv;
        ppi->s1TxAttri8431[port].size = size;
        return RT_ERR_OK;
    }
    else if (type == PHY_8295_PATCH_TYPE_TXATTR_S1DACLONG)
    {
        ppi->s1TxAttriDacLong[port].prv = prv;
        ppi->s1TxAttriDacLong[port].size = size;
        return RT_ERR_OK;
    }
...
phy_8295r_rxCaliConf_set(uint32 unit, phy_8295_rxCaliConf_t *conf)
{
    ...
    osal_memcpy(&gPhy8295rPI[unit].rxCaliConf, conf, sizeof(phy_8295_rxCaliConf_t));
...
phy_8295r_rxCaliConf_get(uint32 unit, phy_8295_rxCaliConf_t *conf)
{
    ...
    osal_memcpy(conf, &gPhy8295rPI[unit].rxCaliConf, sizeof(phy_8295_rxCaliConf_t));
    return RT_ERR_OK;
}
#endif
