 ...
 *
 * $Revision: 81480 $
 * $Date: 2017-08-18 11:45:43 +0800 (Fri, 18 Aug 2017) $
 *
...
#define RTL8295_MSLEEP(_ms)                                                                         \
            do {                                                                                    \
                osal_time_usleep((_ms) * 1000);                                                     \
            } while(0)
...
typedef struct phy_8295r_info_s
{
    ...
    rtk_portmask_t          fiberLos;       /* LOS. 1:LOS, 0:has signal. */
    rtk_portmask_t          waMonRxCalied;  /* rxCali is done in waMon. This flag shall be clear when link change down */
...
#define PORT_FIBER_LOS(unit, port)          (RTK_PORTMASK_IS_PORT_SET(g8295rInfo[unit]->fiberLos, port)?1:0)
...
phy_8295_mdxMacId_get(uint32 unit, rtk_port_t port, uint32 *pMdxMacId)
{
    ...
    if (HWP_CHIP_ID(unit) == RTL8396M_CHIP_ID)
    {
        ...
        if (HWP_10GE_PORT(unit, port) && (HWP_PORT_SMI(unit, port) == 1))
        {
            if (port == 24)
            {
                *pMdxMacId = 25;
...
_phy_8295_portMacSmiProto_get(uint32 unit, rtk_port_t port, drv_smi_mdxProtoSel_t *pProto)
{
    ...
    if (hal_miim_smiMdxProto_get(unit, smi_id, &proto) == RT_ERR_OK)
        *pProto = proto;
    else
        *pProto = DRV_SMI_MDX_PROTO_C22;
    return RT_ERR_OK;
...
_phy_8295_rxCaliParam_get(uint32 unit, rtk_port_t port, uint32 sdsId, phy_8295_rxCaliParam_t *rxCaliParam)
{
    ...
    if (HWP_PHY_MODEL_BY_PORT(unit, port) == RTK_PHYTYPE_RTL8295R)
    {
        if (sdsId == 1)
        {
            ...
            {
                ret = RT_ERR_NOT_INIT;
                RT_ERR(ret, (MOD_HAL), "unit=%u,port=%u,sdsId=%u,g8295rInfo not init.", unit, port, sdsId);
                return;
            }
            ...
            switch (PORT_10G_MEDIA(unit, port))
            {
              ...
              case PORT_10GMEDIA_DAC_500CM: //DAC_LONG
                ...
                {
                    RT_ERR(ret, (MOD_HAL), "unit=%u,port=%u,sdsId=%u,rxCaliConf_get fail", unit, port, sdsId);
                    return;
                }
                rxCaliParam->cable_offset = rxCaliConf.s1rxCaliDacLongCableOffset;
                break;
              default:
                break;
            }
...
phy_8295r_resolutionReg_get(uint32 unit, uint32 port, uint32 *pResl)
{
    ...
    *pResl = 0;
    ...
    if ((ret = hal_miim_smiMdxProto_get(unit, HWP_PORT_SMI(unit, port), &proto)) != RT_ERR_OK)
    {
        /* default using C22 */
...
#if defined(CONFIG_SDK_RTL8295R)
/* Function Name:
 *      _phy_8295r_txAttri_set
 * Description:
 *      Configure TX attribute
 * Input:
 *      unit    - unit id
 *      port    - port id
 *      media   - 10G media
 ...
 */
int32
_phy_8295r_txAttri_set(uint32 unit, rtk_port_t port, rtk_port_10gMedia_t media)
{
  return phy_8295r_patch_txAttri_set(unit, port, media);
}
...
_phy_8295r_10gMedia_set(uint32 unit, rtk_port_t port, rtk_port_10gMedia_t media)
{
    ...
    switch (media)
    {
      ...
      case PORT_10GMEDIA_FIBER_1G:
        ...
        break;
      case PORT_10GMEDIA_FIBER_100M:
        sdsMode = PHY_8295_SDS_MODE_FIB100M;
        break;
      case PORT_10GMEDIA_NONE:
    ...
    RTK_PORTMASK_PORT_CLEAR(g8295rInfo[unit]->waMonRxCalied, port);
    ...
    if (g8295rS0SdsAutoChgEn == FALSE)
    {
        ...
        {
          case PORT_10GMEDIA_FIBER_100M:
            sdsModeS0 = PHY_8295_SDS_MODE_FIB100M;
            break;
          case PORT_10GMEDIA_FIBER_1G:
...
phy_8295r_10gMedia_set(uint32 unit, rtk_port_t port, rtk_port_10gMedia_t media)
{
    ...
    _phy_8295r_txAttri_set(unit, port, media);

    if (media == PORT_10GMEDIA_FIBER_100M)
        speed = PORT_SPEED_1000M;
...
phy_8295r_autoNegoAbility_set(uint32 unit, rtk_port_t port, rtk_port_phy_ability_t *pAbility)
{
    ...
    if (port_flag & PHY_8295_PORT_1G)
    {
        ret = _phy_8295r_autoNegoAbilityStaFiberStdReg_set(unit, port, pAbility);
    }
 ...
 */
int32
phy_8295r_speed_get(uint32 unit, rtk_port_t port, uint32 *pSpeed)
{
 ...
 */
int32
phy_8214qf_speed_get(uint32 unit, rtk_port_t port, uint32 *pSpeed)
{
    int32           ret;
 ...
 */
int32
phy_8295r_speed_set(uint32 unit, rtk_port_t port, uint32 speed)
{
    ...
    {
      case PORT_SPEED_10G:
        phy_8295r_10gMedia_set(unit, port, PORT_10GMEDIA_FIBER_10G);
        break;
      case PORT_SPEED_1000M:
        phy_8295r_10gMedia_set(unit, port, PORT_10GMEDIA_FIBER_1G);
        break;
      case PORT_SPEED_100M:
        phy_8295r_10gMedia_set(unit, port, PORT_10GMEDIA_FIBER_100M);
        break;
      default:
        return RT_ERR_INPUT;
    }/* switch */
 ...
 */
int32
phy_8214qf_speed_set(uint32 unit, rtk_port_t port, uint32 speed)
{
    int32           ret;
    uint32          speedSel;
    uint32          data;
...
phy_8214qf_enable_set(uint32 unit, rtk_port_t port, rtk_enable_t enable)
{
    ...
    if (enable == ENABLED)
    {
        if (g8295rInfo[unit])
        {
            sdsMode = PORT_SDS_MODE_CFG(unit, port);
            RT_8295_LOG(LOG_INFO, (MOD_HAL|MOD_PORT), "unit=%u,port=%u,sdsMode=0x%x", unit, port, sdsMode);
            _phy_8295_portSdsMode_set(unit, port, sdsMode);
...
phy_8295_8214_fiberOAMLoopBackEnable_set(uint32 unit, rtk_port_t port, rtk_enable_t enable)
{
    ...
    if (enable)
    {
        data = REG32_FIELD_SET(data, 0, 0x12, (0x1 << 0x12));       /* ANEN */
        data = REG32_FIELD_SET(data, 1, 0x8, (0x1 << 0x8));         /* FULLDUP */
        data = REG32_FIELD_SET(data, 1, 0x5, (0x1 << 0x5));         /* FRCTX */
    }
    else
    {
        /* back to default */
        data = REG32_FIELD_SET(data, 0, 0x12, (0x1 << 0x12));
        data = REG32_FIELD_SET(data, 1, 0x8, (0x1 << 0x8));
...
phy_8295r_fiberOAMLoopBackEnable_set(uint32 unit, rtk_port_t port, rtk_enable_t enable)
{
    ...
    if ((ret = phy_8295_8214_fiberOAMLoopBackEnable_set(unit, port, enable)) != RT_ERR_OK)
    {
        return ret;
    }
    ...
    if (enable == ENABLED)
    {
        if (g8295rMacAutoDetectSdsMode == FALSE)
        {
            rtk_port_10gMedia_t     macMedia, expeMacMedia;
            uint32   sdsModeS0 = PHY_8295_SDS_MODE_10GR;

            if (_switch_port_10gMedia_get(unit, port, &macMedia) == RT_ERR_OK)
            {
                _phy_8295_serdesModeReg_set(unit, port, mdxMacId, 0, sdsModeS0);
                if (sdsModeS0 == PHY_8295_SDS_MODE_FIB100M)
                    expeMacMedia = PORT_10GMEDIA_FIBER_100M;
                else if (sdsModeS0 == PHY_8295_SDS_MODE_FIB1G)
                    expeMacMedia = PORT_10GMEDIA_FIBER_1G;
                else
                    expeMacMedia = PORT_10GMEDIA_FIBER_10G;
                if (macMedia != expeMacMedia)
                {
                    RT_8295_LOG(LOG_INFO, (MOD_HAL|MOD_PORT), "unit=%u,port=%u,mac media %d -> %d", unit, port, macMedia, expeMacMedia);
                    _switch_port_10gMedia_set(unit, port, expeMacMedia);
                }
            }
        }
    }
...
phy_8295r_loopback_get(uint32 unit, rtk_port_t port, rtk_enable_t *pEnable)
{
    ...
    switch (sdsMode)
    {
      ...
      case PHY_8295_SDS_MODE_FIB100M:
        ...
        break;
      default:
        RT_8295_LOG(LOG_INFO, (MOD_HAL|MOD_PORT), "unit=%u,port=%u,sdsMode=%u not support", unit, port, sdsMode);
        return RT_ERR_PORT_NOT_SUPPORTED;
    }
    ...
    if (is_10g == TRUE)
    {
        ...
        *pEnable = (REG32_FIELD_GET(data, 2, 1<<2) == 1) ? ENABLED : DISABLED;
    }
    else
    {
        /* 1G */
        PHY_8295_SDSREG_READ(unit, mdxMacId, sdsId, RTL8295_SDS0_SDS_REG00_PAGE, RTL8295_SDS0_SDS_REG00_REG, &data);
        *pEnable = (REG32_FIELD_GET(data, 4, 0x10) == 1) ? ENABLED : DISABLED;
    }
...
phy_8295r_loopback_set(uint32 unit, rtk_port_t port, rtk_enable_t enable)
{
    ...
    if (is_10g == TRUE)
    {
        ...
        PHY_8295_SDSREG_READ(unit, mdxMacId, sdsId, RTL8295_SDS0_TGR_PRO_0_REG01_PAGE, RTL8295_SDS0_TGR_PRO_0_REG01_REG, &data);
        if (enable == ENABLED)
            data = REG32_FIELD_SET(data, 1, 2, 1<<2);
        else
            data = REG32_FIELD_SET(data, 0, 2, 1<<2);
    ...
    else
    {
        ...
        PHY_8295_SDSREG_READ(unit, mdxMacId, sdsId, RTL8295_SDS0_SDS_REG00_PAGE, RTL8295_SDS0_SDS_REG00_REG, &data);
        if (enable == ENABLED)
            data = REG32_FIELD_SET(data, 1, 4, 0x10);
        else
            data = REG32_FIELD_SET(data, 0, 4, 0x10);
...
phy_8295r_fiberRxEnable_set(uint32 unit, rtk_port_t port, rtk_enable_t enable)
{
    int32       ret;
    uint32      mdxMacId, sdsId;
    uint32      data10g, data1p25g, sdsCtrlData, sdsCtrlDataTmp;
...
#if defined(CONFIG_SDK_RTL8295R)
void
_phy_8295r_rxCaliS1(uint32 unit, rtk_port_t port)
{
    uint32      resl;
    phy_8295_rxCaliParam_t  rxCaliParam;

    _phy_8295_rxCaliParam_get(unit, port, 1, &rxCaliParam);
    phy_8295_patch_rxCali(unit, port, 1, &rxCaliParam);
...
_phy_8295r_rxCaliS0(uint32 unit, rtk_port_t port)
{
    ...
    try_cnt = PHY_8295_LINK_RETRY_CNT;
    do {
        phy_8295_patch_rxCali(unit, port, 0, &rxCaliParam);
...
#if defined(CONFIG_SDK_RTL8295R)
int32
_phy_8295r_linkChangeUp_process(uint32 unit, rtk_port_t port)
{
    uint32          macMedia;
    int32           ret;
    ...
    switch (PORT_10G_MEDIA(unit, port))
    {
      ...
      case PORT_10GMEDIA_DAC_50CM:
      case PORT_10GMEDIA_DAC_100CM:
        macMedia = PORT_10GMEDIA_FIBER_10G;
        if (RTK_PORTMASK_IS_PORT_SET(g8295rInfo[unit]->s1RxCaliEn, port))
        {
            ...
            _phy_8295r_rxCaliS1(unit, port);
        }
        break;
      case PORT_10GMEDIA_DAC_300CM:
      case PORT_10GMEDIA_DAC_500CM: //DAC_LONG
        RT_8295_LOG(LOG_INFO, (MOD_HAL|MOD_PORT), "unit=%u,port=%u DAC-long (cali:%u)", unit, port, RTK_PORTMASK_IS_PORT_SET(g8295rInfo[unit]->waMonRxCalied, port));
        macMedia = PORT_10GMEDIA_FIBER_10G;
        if (RTK_PORTMASK_IS_PORT_CLEAR(g8295rInfo[unit]->waMonRxCalied, port))
        {
        ...
        break;
      case PORT_10GMEDIA_FIBER_1G:
        macMedia = PORT_10GMEDIA_FIBER_1G;
        break;
      case PORT_10GMEDIA_FIBER_100M:
        macMedia = PORT_10GMEDIA_FIBER_100M;
        break;
      default:
        return RT_ERR_OK;
    }

    RT_8295_LOG(LOG_INFO, (MOD_HAL|MOD_PORT), "unit=%u,port=%u,macMedia=%u  g8295rMacAutoDetectSdsMode=%u", unit, port, macMedia, g8295rMacAutoDetectSdsMode);
    if (g8295rMacAutoDetectSdsMode == FALSE)
    {
        uint32          preMacMedia = PORT_10GMEDIA_NONE;

        _switch_port_10gMedia_get(unit, port, &preMacMedia);
        if (preMacMedia != macMedia)
        {
            _switch_port_10gMedia_set(unit, port, macMedia);
            if (macMedia == PORT_10GMEDIA_FIBER_1G)
            {
    ...
    RT_8295_LOG(LOG_INFO, (MOD_HAL|MOD_PORT), "unit=%u,port=%u,S0 caliEn=%u,calied=%u", unit, port,
                RTK_PORTMASK_IS_PORT_SET(g8295rInfo[unit]->s0RxCaliEn, port),
                RTK_PORTMASK_IS_PORT_SET(g8295rInfo[unit]->s0RxCalied, port));
...
phy_8295r_linkChange_process(uint32 unit, rtk_port_t port, rtk_port_linkStatus_t linkSts)
{
    ...
    {
        /* link change up process */
        _phy_8295r_linkChangeUp_process(unit, port);
    }
    else
    {
        /* link change up process */
...
_phy_8295r_es_10g_rxIdle(uint32 unit, rtk_port_t port, uint32 mdxMacId, uint32 sdsId, uint32 *pRxIdle)
{
    ...
    *pRxIdle = (data >> 15) & 0x1;
    ...
    //for debug
    //{
    //    uint32      tmp = (data & (0x1 << 15)) ? 1 : 0;
    //    if (tmp == 0)
    //    {
    //        osal_printf("RX_IDLE(%x): %u / %u\n", data, *pRxIdle, tmp);
    //    }
    //}
...
#if defined(CONFIG_SDK_RTL8295R)
/* Function Name:
 *      phy_8295r_fiberLosStatus_notify
 * Description:
 *      Notify fiber LOS status
 * Input:
 *      unit   - unit id
 *      port   - port id
 *      los    - 1: Lost-of-signal; 0: signal detected
 * Output:
 *      None
 * Return:
 *      RT_ERR_OK
 ...
 */
int32
phy_8295r_fiberLosStatus_notify(uint32 unit, rtk_port_t port, int32 los)
{
    if (!g8295rInfo[unit])
    {
        return RT_ERR_NOT_INIT;
    ...
    if (los)    /* Lost of signal */
    {
        RTK_PORTMASK_PORT_SET(g8295rInfo[unit]->fiberLos, port);
    }
    else        /* signal detected */
    {
        RTK_PORTMASK_PORT_CLEAR(g8295rInfo[unit]->fiberLos, port);
    }
    ...
    switch (PORT_10G_MEDIA(unit, port))
    {
      ...
      case PORT_10GMEDIA_DAC_500CM: //DAC_LONG
        ...
        /* link check */
        {
            ...
            if (data & RTL8295_SDS_CTRL_SDS_10G_LINK_SDS_S1TG_LINK_MASK)
            {
                RT_8295_LOG(LOG_INFO, (MOD_HAL|MOD_PORT), "unit=%u,port=%d Link up after cali", unit, port);
                /* mark the port as rx-cali is done.
                 * In this condition, the port will not need to process rx-cali again in link-up process.
                 */
                RTK_PORTMASK_PORT_SET(g8295rInfo[unit]->waMonRxCalied, port);
            }
        }
        phy_8295r_rxCali_process_sts_set(unit, port, PHY_8295_RXCALI_PROCESS_STS_IDLE);
        break;
...
_phy_8295r_mdcProto_sel(uint32 unit, rtk_port_t port)
{
    ...
    if (proto == DRV_SMI_MDX_PROTO_C22)
    {
        /* change MAC to use C45 */
        if (hal_miim_smiMdxProto_set(unit, smi_id, DRV_SMI_MDX_PROTO_C45) != RT_ERR_OK)
        {
            g8295rStatusSwPoll = TRUE;
            return RT_ERR_OK;
        }
    ...
    /* the 95R supports C45, then try to enable MAC polling PHY on MAC */
    osal_memset(&pollConf, 0, sizeof(drv_miim_pollConf_t));
    pollConf.eth_type = HWP_XGE;
    pollConf.pollExtPhy = ENABLED;
    /* enable MAC to poll resolution reg */
    if (hal_miim_pollingConfig_set(unit, port, &pollConf) != RT_ERR_OK)
    {
        RT_8295_LOG(LOG_INFO, (MOD_HAL|MOD_PORT), "unit=%u,port=%u configure MAC resolution poll fail", unit, port);
        g8295rStatusSwPoll = TRUE;
        return RT_ERR_OK;
    }
...
phy_8295r_init(uint32 unit, rtk_port_t port)
{
    ...
    if (HWP_PHY_BASE_MACID(unit, port) == port)
    {
        ...
        if (((sdsId = HWP_PORT_SDSID(unit, port)) != HWP_NONE))
        {
            ...
            {
                g8295rMacAutoDetectSdsMode  = FALSE;
                g8295rLinkPassThroughEn     = TRUE; /*For 0770B; FALSE for 6488*/
                g8295rS0SdsAutoChgEn        = FALSE;
            }
        }
...
{
    /* Porting to setup the ports that needs for rxCaliS01. */
    return FALSE;
}
...
_phy_8214qf_rxCali(uint32 unit, rtk_port_t basePort)
{
        ...
        && (_phy_8214qf_rxCali_needed(unit, basePort, sdsId)))
    {
        ...
        phy_8295_patch_rxCali(unit, basePort, sdsId, &rxCaliParam);
    }
...
phy_8295_diag_set(uint32 unit, rtk_port_t port, rtk_port_t mdxMacId, uint32 sds, uint8 *name)
{
    ...
    {
        RT_8295_LOG(LOG_INFO, (MOD_HAL|MOD_PORT), "unit=%u,baseport=%u,sdsId=%u", unit, mdxMacId, sds);
...
#endif
        {
            ...
            phy_8295_patch_rxCali(unit, port, sds, &rxCaliParam);
        }
