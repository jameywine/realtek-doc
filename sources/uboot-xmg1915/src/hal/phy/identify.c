 ...
 *
 * $Revision: 81346 $
 * $Date: 2017-08-16 18:42:45 +0800 (Wed, 16 Aug 2017) $
 *
...
static int32 _phy_identify_8295_95R(uint32 unit, rtk_port_t port, uint32 model_id, uint32 rev_id);
...
static int32 _phy_identify_8295_14QF(uint32 unit, rtk_port_t port, uint32 model_id, uint32 rev_id);
#endif /* CONFIG_SDK_RTL8214QF */
#if (defined(CONFIG_SDK_RTL8295R) || defined(CONFIG_SDK_RTL8214QF))
static int32 _phy_identify_8295(uint32 unit, rtk_port_t port, uint32 model_id, uint32 rev_id, phy_type_t phyType);
#endif
...
#endif
#if defined(CONFIG_SDK_RTL8218D)
    {_phy_identify_8218d, PHY_MODEL_ID_RTL8218D,  PHY_REV_NO_D, RTK_PHYTYPE_RTL8218D, NULL, phy_8218Ddrv_ge_mapperInit,   0, &phy_8218D_info},
...
#endif
#if defined(CONFIG_SDK_RTL8295R)
    {_phy_identify_8295R,     PHY_MODEL_ID_RTL8295R,  PHY_REV_NO_A, RTK_PHYTYPE_RTL8295R,   NULL, phy_8295Rdrv_mapperInit,  0, &phy_8295R_info},
    {_phy_identify_8295_95R,  0,                      PHY_REV_NO_A, RTK_PHYTYPE_RTL8295R_C22,   NULL, phy_8295Rdrv_mapperInit,  0, &phy_8295R_info},
#endif
#if defined(CONFIG_SDK_RTL8214QF)
    {_phy_identify_8214QF,    PHY_MODEL_ID_RTL8214QF, PHY_REV_NO_A, RTK_PHYTYPE_RTL8214QF,      NULL, phy_8214QFdrv_mapperInit, 0, &phy_8214QF_info},
    {_phy_identify_8295_14QF, 0,                      PHY_REV_NO_A, RTK_PHYTYPE_RTL8214QF_NC5,  NULL, phy_8214QFdrv_mapperInit, 0, &phy_8214QF_info},
#endif
...
{
#if defined(CONFIG_SDK_RTL9300)
    {_phy_identify_9300_serdes, (uint32)NULL, (uint32)NULL, RTK_PHYTYPE_SERDES, NULL, phy_9300_serdes_mapperInit, 0, &phy_9300sds_info},
#endif /* CONFIG_SDK_RTL9300 */
#if defined(CONFIG_SDK_RTL9310)
    {_phy_identify_9310_serdes, (uint32)NULL, (uint32)NULL, RTK_PHYTYPE_SERDES, NULL, phy_9310_serdes_mapperInit, 0, &phy_9310sds_info},
...
#endif
    {_phy_identify_8380_serdes_ge, (uint32)NULL, (uint32)NULL, RTK_PHYTYPE_SERDES, NULL, phy_8380_serdes_ge_mapperInit, 0, &phy_8380sds_info},
#endif
#if defined(CONFIG_SDK_RTL8390)
    {_phy_identify_8390_serdes_10ge, (uint32)NULL, (uint32)NULL, RTK_PHYTYPE_SERDES, NULL, phy_8390_serdes_ge_mapperInit, 0, &phy_8390sds10G_info},
    {_phy_identify_8390_serdes_ge, (uint32)NULL, (uint32)NULL, RTK_PHYTYPE_SERDES, NULL, phy_8390_serdes_ge_mapperInit, 0, &phy_8390sds_info},
#endif
};

/* PHY control linking list indexes by RTK_PHYTYPE_xxx */
uint32                          phyctrlLnkList_init = FALSE;
static rt_phyctrl_linkNode_t    *phyctrlLnkList[RTK_PHYTYPE_END] = {NULL};
 ...
 *      _phy_identify_8295
 * Description:
 *      Identify the port is 8295 PHY or not?
 * Input:
 *      unit     - unit id
 *      port     - port id
 *      model_id - model id
 ...
 *      RT_ERR_OK
 *      RT_ERR_FAILED
 *      RT_ERR_PHY_NOT_MATCH
 * Note:
 *      None
 */
static int32
_phy_identify_8295(uint32 unit, rtk_port_t port, uint32 model_id, uint32 rev_id, phy_type_t phyType)
{
    hal_control_t   *pHalCtrl = NULL;
    uint32          mdxMacId, identify_port;
    uint32          real_model_id, real_rev_id;
    int32           ret, retSmi;
    ...
    {
        if ((phyType == RTK_PHYTYPE_RTL8214QF_NC5) || (phyType == RTK_PHYTYPE_RTL8214QF))
        {
            mdxMacId = port - (port % 4);
        }
        else
        {
            mdxMacId = port;
    ...
    retSmi = hal_miim_smiMdxProto_get(unit, HWP_PORT_SMI(unit, port), &mdxProto);
    if ((retSmi == RT_ERR_OK) && (mdxProto != DRV_SMI_MDX_PROTO_C22))
    {
        hal_miim_smiMdxProto_set(unit, HWP_PORT_SMI(unit, port), DRV_SMI_MDX_PROTO_C22);
    }

    identify_port = ((phyType == RTK_PHYTYPE_RTL8295R_C22) || (phyType == RTK_PHYTYPE_RTL8295R)) ? mdxMacId : port;
    if ((ret = phy_identify_phyid_get(unit, identify_port, &real_model_id, &real_rev_id)) != RT_ERR_OK)
    {
        RT_LOG(LOG_TRACE, MOD_HAL, "phy_identify_phyid_get(unit %d, port %d) failed - real_model_id = 0x%x, real_rev_id = 0x%x!!", unit, port, real_model_id, real_rev_id);
        goto EXIT;
    }
    ...
    if ((real_model_id == 0) && (real_rev_id >= rev_id))
    {
        uint32          data, mode;

        //RTL8295_BOND_STR_REG1_ADDR
        if (MACDRV(pHalCtrl)->fMdrv_miim_read(unit, mdxMacId, RTL8295_GLB_CTRL_BOND_STR_REG1_PAGE, RTL8295_GLB_CTRL_BOND_STR_REG1_REG, &data) != RT_ERR_OK)
        {
            RT_LOG(LOG_TRACE, MOD_HAL, "unit %u port %u red reg 0x4 failed %x", unit, port, ret);
            goto EXIT;
        }

        ret = RT_ERR_PHY_NOT_MATCH;
        mode = data & 0xF;  /* RTL8295_BOND_STR_REG1_BOND_MODE_SEL_OFFSET */
        if ((mode == 0x4) && ((phyType == RTK_PHYTYPE_RTL8295R_C22) || (phyType == RTK_PHYTYPE_RTL8295R)))    //PHY_8295_MODE_B0
        {
            ret = RT_ERR_OK;
        }
        else if (((mode == 0x1) || (mode == 0xB) || (mode == 0xD))  //PHY_8295_MODE_A1, PHY_8295_MODE_C3, PHY_8295_MODE_C5
                && ((phyType == RTK_PHYTYPE_RTL8214QF_NC5) || (phyType == RTK_PHYTYPE_RTL8214QF)))
        {
            ret = RT_ERR_OK;
        }
    }
  ...
  EXIT:
    if ((retSmi == RT_ERR_OK) && (mdxProto != DRV_SMI_MDX_PROTO_C22))
        hal_miim_smiMdxProto_set(unit, HWP_PORT_SMI(unit, port), mdxProto);
 ...
 */
static int32
_phy_identify_8295_95R(uint32 unit, rtk_port_t port, uint32 model_id, uint32 rev_id)
{
    return _phy_identify_8295(unit, port, model_id, rev_id, RTK_PHYTYPE_RTL8295R_C22);
}
#endif
 ...
 */
static int32
_phy_identify_8295_14QF(uint32 unit, rtk_port_t port, uint32 model_id, uint32 rev_id)
{
    return _phy_identify_8295(unit, port, model_id, rev_id, RTK_PHYTYPE_RTL8214QF_NC5);
}
#endif
