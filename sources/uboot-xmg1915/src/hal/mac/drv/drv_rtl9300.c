 ...
 *
 * $Revision: 81198 $
 * $Date: 2017-08-14 11:42:14 +0800 (Mon, 14 Aug 2017) $
 *
...
/* Function Name:
 *      _rtl9300_miim_pollingConfig_set
 * Description:
 *      configure MAC polling PHY
 * Input:
 *      unit  - unid ID
 *      macId - mac(port) id
 *      pPollConf    - polling config
 * Output:
 *      None
 * Return:
 *      RT_ERR_OK     - OK
 ...
 */
int32
_rtl9300_miim_pollingConfig_set(uint32 unit, uint32 macId, drv_miim_pollConf_t *pPollConf)
{
    int32       ret;
    uint32      field,value;
    ...
    if (pPollConf->pollExtPhy == ENABLED)
    {
        switch(pPollConf->eth_type)
        {
            case HWP_FE:
                value = 2;
                break;
 ...
 */
int32
rtl9300_regDefault_init(uint32 unit)
{
 ...
 */
int32
rtl9300_miim_smiMdxProto_set(uint32 unit, uint32 smi_id, drv_smi_mdxProtoSel_t proto)
{
    int32       ret;
    uint32      field, val;
 ...
 */
int32
rtl9300_miim_smiMdxProto_get(uint32 unit, uint32 smi_id, drv_smi_mdxProtoSel_t *pProto)
{
    int32       ret;
    uint32      field, val = 0;
...
/* Function Name:
 *      _rtl9300_8295r_mdxC22_init
 * Description:
 *      Initialize for SMI to use Clause 22 for 8295R
 * Input:
 *      unit - unit id
 * Output:
 *      None
 ...
 */
void
_rtl9300_8295r_mdxC22_init(int32 unit)
{
    uint32      mac_id, value, smi_id, field;
    uint32      smiProtoSetMsk = 0;
    int32       ret;
    ...
    HWP_10GE_PORT_TRAVS(unit, mac_id)
    {
        ...
        if (field != 0)
        {
            value = 0;//0:10G/1G Fiber; 1:10G/2.5G PHY

            RT_LOG(LOG_DEBUG, (MOD_HAL), "unit %u macId %u set phy type %u", unit, mac_id, value);
            if ((ret = reg_field_write(unit, LONGAN_SMI_MAC_TYPE_CTRLr, field, &value)) != RT_ERR_OK)
            {
                RT_LOG(LOG_DEBUG, (MOD_HAL), "unit %u macId %u set phy type %u fail %d", unit, mac_id, value, ret);
                return;
            }
        }

        /* configure C22 for PHY probe */
        smi_id = HWP_PORT_SMI(unit, mac_id);
        if ((smi_id != HWP_NONE) && ((smiProtoSetMsk & (0x1 << smi_id)) == 0))
        {
            smiProtoSetMsk |= (0x1 << smi_id);
            rtl9300_miim_smiMdxProto_set(unit, smi_id, DRV_SMI_MDX_PROTO_C22);
        }/* end if */
...
rtl9300_init(uint32 unit)
{
    ...
    if ((ret = rtl9300_regDefault_init(unit)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_INIT), "ref default init failed");
    }
...
#if defined(CONFIG_SDK_RTL8295R)
    _rtl9300_8295r_mdxC22_init(unit);
#endif/* CONFIG_SDK_RTL8295R */
...
/* Function Name:
 *      rtl9300_miim_pollingConfig_set
 * Description:
 *      configure MAC polling PHY parameters
 * Input:
 *      unit    - unit ID
 *      port    - port ID
 *      pPollConf - polling config
 ...
 */
int32
rtl9300_miim_pollingConfig_set(uint32 unit, uint32 port, drv_miim_pollConf_t *pPollConf)
{
    return _rtl9300_miim_pollingConfig_set(unit, port, pPollConf);
}
...
rt_macdrv_t rtl9300_macdrv =
{
    ...
    .fMdrv_miim_globalPollingEnable_get     = NULL,
    .fMdrv_miim_globalPollingEnable_set     = NULL,
    ...
    .fMdrv_miim_pollingConfig_set           = rtl9300_miim_pollingConfig_set,
    .fMdrv_miim_smiMdxProto_get             = rtl9300_miim_smiMdxProto_get,
    .fMdrv_miim_smiMdxProto_set             = rtl9300_miim_smiMdxProto_set,
