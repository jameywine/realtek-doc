 ...
 *
 * $Revision: 80905 $
 * $Date: 2017-07-28 16:21:56 +0800 (Fri, 28 Jul 2017) $
 *
...
typedef struct rt_portinfo_s
{
    ...
    rt_portType_info_t fe;      /* port ethernet type: Fast Ethernet */
    rt_portType_info_t ge;      /* port ethernet type: Giga Ethernet */
    rt_portType_info_t xge;     /* port ethernet type: 10 Giga Ethernet */
    rt_portType_info_t serdes;  /* port medium type: Serdes */
    rt_portType_info_t copper;  /* port medium type: Copper */
    rt_portType_info_t fiber;   /* port medium type: Fiber */
    rt_portType_info_t combo;   /* port medium type: Combo */
    rt_portType_info_t ether;   /* port attribute type: Ethernet ports (Network port) */
    rt_portType_info_t stack;   /* port attribute type: Stack ports */
    rt_portType_info_t all;     /* port attribute type: All exist ports (including ether, cpu, and stack) */
} rt_portinfo_t;
...
typedef struct rt_register_capacity_s
{
    ...
    uint32 burst_rate_of_acl_meter_dlb_min;
    uint32 burst_rate_of_acl_meter_dlb_max;
    ...
    /* CAPWAP */
    uint32  max_num_of_capwap_hashdepth;
...
/* Function Name:
 *      hal_stackPortStartEnd_get
 * Description:
 *      Get start and end stacking port
 * Input:
 *      chip_id - chip id
 *      type - start or end stacking port type.
 * Output:
 *      None
 * Return:
 *      start or end stacking port
 *      RTK_MAX_PORT_ID_PER_UNIT - when didn't match any chip id and type equal to STACK_PORT_INDEX_TYPE_START.
 *      0 - when didn't match any chip id and type equal to STACK_PORT_INDEX_TYPE_START.
 * Note:
 *      None
 */

extern uint32
hal_stackPortStartEnd_get(uint32 chip_id, stack_port_indexType_t type);
