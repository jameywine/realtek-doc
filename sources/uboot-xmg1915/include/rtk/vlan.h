 ...
 *
 * $Revision: 81326 $
 * $Date: 2017-08-16 15:44:01 +0800 (Wed, 16 Aug 2017) $
 *
...
typedef struct rtk_vlan_macVlanEntry_s
{
    ...
    rtk_enable_t                capwap_ts;/* roaming packet to CAPWAP tunnel */
    rtk_mac_t                   bssid;
#endif
...
typedef struct rtk_vlan_igrVlanCnvtEntry_s
{
    ...
    struct {
        ...
        uint8               outer_vid_ignore;       /* set to 1 to ignore the search key 'ovid' */
        ...
        uint32              port_type;              /* just follow LAG/TRUNK module  */
        rtk_port_t          port;                   /* search key, ingress port */
    } match_key;

    /* action */
    struct {
        rtk_vlan_tagStatusAct_t   inner_tag_sts;
        rtk_vlan_tagStatusAct_t   outer_tag_sts;
        rtk_vlan_tagVidAct_t      inner_vid_action;
        rtk_vlan_t  inner_vid_value;                /* value for assign or shift used */
        rtk_vlan_tagVidAct_t     outer_vid_action;
        rtk_vlan_t  outer_vid_value;
        rtk_vlan_tagPriorityAct_t inner_priority_action;
        rtk_pri_t   inner_priority;
        rtk_vlan_tagPriorityAct_t outer_priority_action;
        rtk_pri_t   outer_priority;
        rtk_vlan_tagTpidAct_t inner_tpid_action;
        uint32      inner_tpid_idx;
        rtk_vlan_tagTpidAct_t outer_tpid_action;
        uint32      outer_tpid_idx;
    } action;
...
rtk_vlan_l2LookupMode_get(
    ...
    rtk_vlan_l2LookupMode_t *pLookupMode);
...
rtk_vlan_l2LookupMode_set(
    ...
    rtk_vlan_t               vid,
    rtk_vlan_l2mactype_t     type,
    rtk_vlan_l2LookupMode_t  lookupMode);
 ...
 *      (2) For 8380/8390,The inner/outer TPID is used when transmiiting a inner/outer-tagged pacekt only if it is received as
 *          an inner/outer-untag pacekt.
