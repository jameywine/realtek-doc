 ...
 *
 * $Revision: 81320 $
 * $Date: 2017-08-16 15:21:35 +0800 (Wed, 16 Aug 2017) $
 *
...
EXPORT_SYMBOL(myParsedInfo);
EXPORT_SYMBOL(unitMapStruct);
EXPORT_SYMBOL(hwp_myLocalChipUnitID);
EXPORT_SYMBOL(myLocalChipUnitID_mask);
...
EXPORT_SYMBOL(hwp_localChipFamily_check);
EXPORT_SYMBOL(hwp_profile_find);
EXPORT_SYMBOL(hwp_identifier_get);
EXPORT_SYMBOL(hwp_chipID_get);
EXPORT_SYMBOL(_hwp_stackingPort_set);
...
EXPORT_SYMBOL(hwp_unitID_change);
