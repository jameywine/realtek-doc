...
rtk_l2_addrEntry_get(uint32 unit, uint32 index, rtk_l2_entry_t *pL2_entry)
{
    ...
    osal_memset(&config, 0, sizeof(rtdrv_l2Cfg_t));
    config.l2_search.unit = unit;
    config.l2_search.index = index;
    GETSOCKOPT(RTDRV_L2_ADDR_ENTRY_GET, &config, rtdrv_l2Cfg_t, 1);
    osal_memcpy(pL2_entry, &config.l2_search.l2_entry, sizeof(rtk_l2_entry_t));
    ...
    uint32          *pCf_retCnt)
{
    ...
    osal_memset(&config, 0, sizeof(rtdrv_l2Cfg_t));
    config.l2_search.unit = unit;
    osal_memcpy(&config.l2_search.l2_entry, pL2Addr, sizeof(rtk_l2_entry_t));
    config.l2_search.cfAddrList_size = cfAddrList_size;
    GETSOCKOPT(RTDRV_L2_CONFLICT_ADDR_GET, &config, rtdrv_l2Cfg_t, 1);
    osal_memcpy(pCfAddrList, config.l2_search.cfAddrList, (sizeof(rtk_l2_entry_t) * cfAddrList_size));
    (*pCf_retCnt) = config.l2_search.cf_retCnt;
    ...
    rtk_l2_entry_t  *pEntry)
{
    ...
    osal_memset(&config, 0, sizeof(rtdrv_l2Cfg_t));
    config.l2_search.unit = unit;
    config.l2_search.scan_idx = *pScan_idx;
    config.l2_search.type = type;
    GETSOCKOPT(RTDRV_L2_HW_NEXT_VALID_ADDR_GET, &config, rtdrv_l2Cfg_t, 1);
    osal_memcpy(pEntry, &config.l2_search.l2_entry, sizeof(rtk_l2_entry_t));
    *pScan_idx = config.l2_search.scan_idx;
...
rtk_l2_portCtrl_set(uint32 unit, rtk_port_t port, rtk_l2_portCtrlType_t type, int32 arg)
{
    ...
    GETSOCKOPT(RTDRV_L2_PORT_CTRL_TYPE_SET, &config, rtdrv_l2Cfg_t, 1);
...
int32
rtk_l2_mcastAddrEcidPmsk_add(uint32 unit, rtk_l2_mcastAddr_t *pMcast_addr,
    rtk_bpe_pmskEntry_t *pEntry)
{
    rtdrv_l2EcidCfg_t cfg;

    /* parameter check */
    RT_PARAM_CHK((NULL == pEntry), RT_ERR_NULL_POINTER);

    /* function body */
    osal_memset(&cfg, 0, sizeof(rtdrv_l2EcidCfg_t));
    osal_memcpy(&cfg.unit, &unit, sizeof(uint32));
    osal_memcpy(&cfg.mcastAddr, pMcast_addr, sizeof(rtk_l2_mcastAddr_t));
    osal_memcpy(&cfg.entry, pEntry, sizeof(rtk_bpe_pmskEntry_t));
    SETSOCKOPT(RTDRV_L2_MCASTADDRECIDPMSK_ADD, &cfg, rtdrv_l2EcidCfg_t, 1);
    osal_memcpy(pEntry, &cfg.entry, sizeof(rtk_bpe_pmskEntry_t));
    osal_memcpy(pMcast_addr, &cfg.mcastAddr, sizeof(rtk_l2_mcastAddr_t));

    return RT_ERR_OK;
}   /* end of rtk_l2_mcastAddrEcidPmsk_add */

int32
rtk_l2_mcastAddrEcidPmsk_del(uint32 unit, rtk_l2_mcastAddr_t mcastAddr,
    rtk_bpe_pmskEntry_t *pEntry)
{
    rtdrv_l2EcidCfg_t cfg;
    ...
    osal_memset(&cfg, 0, sizeof(rtdrv_l2EcidCfg_t));
    osal_memcpy(&cfg.unit, &unit, sizeof(uint32));
    osal_memcpy(&cfg.mcastAddr, &mcastAddr, sizeof(rtk_l2_mcastAddr_t));
    osal_memcpy(&cfg.entry, pEntry, sizeof(rtk_bpe_pmskEntry_t));
    SETSOCKOPT(RTDRV_L2_MCASTADDRECIDPMSK_DEL, &cfg, rtdrv_l2EcidCfg_t, 1);

    return RT_ERR_OK;
}   /* end of rtk_l2_mcastAddrEcidPmsk_del */

int32
rtk_l2_mcastAddrEcidPmsk_get(uint32 unit, rtk_l2_mcastAddr_t *pMcastAddr,
    rtk_bpe_pmskEntry_t *pEntry)
{
    rtdrv_l2EcidCfg_t cfg;
    ...
    osal_memset(&cfg, 0, sizeof(rtdrv_l2EcidCfg_t));
    osal_memcpy(&cfg.unit, &unit, sizeof(uint32));
    osal_memcpy(&cfg.mcastAddr, pMcastAddr, sizeof(rtk_l2_mcastAddr_t));
    osal_memcpy(&cfg.entry, pEntry, sizeof(rtk_bpe_pmskEntry_t));
    GETSOCKOPT(RTDRV_L2_MCASTADDRECIDPMSK_GET, &cfg, rtdrv_l2EcidCfg_t, 1);
    osal_memcpy(pEntry, &cfg.entry, sizeof(rtk_bpe_pmskEntry_t));
    osal_memcpy(pMcastAddr, &cfg.mcastAddr, sizeof(rtk_l2_mcastAddr_t));

    return RT_ERR_OK;
}   /* end of rtk_l2_mcastAddrEcidPmsk_get */

int32
rtk_l2_mcastAddrEcidPmskNextValid_get(uint32 unit,
    rtk_l2_mcastAddr_t *pMcastAddr, rtk_bpe_pmskEntry_t *pEntry)
{
    rtdrv_l2EcidCfg_t cfg;
    ...
    osal_memcpy(&cfg.mcastAddr, pMcastAddr, sizeof(rtk_l2_mcastAddr_t));
    osal_memcpy(&cfg.entry, pEntry, sizeof(rtk_bpe_pmskEntry_t));
    GETSOCKOPT(RTDRV_L2_MCASTADDRECIDPMSKNEXTVALID_GET, &cfg, rtdrv_l2EcidCfg_t, 1);
    osal_memcpy(pEntry, &cfg.entry, sizeof(rtk_bpe_pmskEntry_t));
    osal_memcpy(pMcastAddr, &cfg.mcastAddr, sizeof(rtk_l2_mcastAddr_t));

    return RT_ERR_OK;
}   /* end of rtk_l2_mcastAddrEcidPmskNextValid_get */
