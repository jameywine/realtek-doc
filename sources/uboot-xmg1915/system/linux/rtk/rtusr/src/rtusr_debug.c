...
int32
hal_rtl8295_reg_read(uint32 unit, uint32 port, uint32 reg_addr, uint32 *pData)
{
...
int32
hal_rtl8295_reg_write(uint32 unit, rtk_port_t port, uint32 reg_addr, uint32 data)
{
    rtdrv_scRegInfo_t       scRegInfo;
...
int32
hal_rtl8295_sds_read(uint32 unit, rtk_port_t port, uint32 sds_idx, uint32 page, uint32 reg, uint32 *pData)
{
...
int32
hal_rtl8295_sds_write(uint32 unit, rtk_port_t port, uint32 sds_idx, uint32 page, uint32 reg, uint32 data)
{
    rtdrv_scSdsInfo_t       scSdsInfo;
...
{
    rtdrv_8295r_rxCaliConf_t    cfg;

    osal_memset(&cfg, 0, sizeof(rtdrv_8295r_rxCaliConf_t));
    cfg.unit = unit;
    cfg.s1rxCaliDacLongCableOffset = conf->s1rxCaliDacLongCableOffset;
    SETSOCKOPT(RTDRV_DIAG_SC_8295R_RXCALICONF_SET, &cfg, rtdrv_8295r_rxCaliConf_t, 1);
    return RT_ERR_OK;
}
...
{
    rtdrv_8295r_rxCaliConf_t    info;

    osal_memset(&info, 0, sizeof(rtdrv_8295r_rxCaliConf_t));
    info.unit = unit;

    GETSOCKOPT(RTDRV_DIAG_SC_8295R_RXCALICONF_GET, &info, rtdrv_8295r_rxCaliConf_t, 1);

    conf->s1rxCaliDacLongCableOffset = info.s1rxCaliDacLongCableOffset;
