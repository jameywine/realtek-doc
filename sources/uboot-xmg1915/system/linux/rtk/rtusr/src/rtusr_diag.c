...
int32 drv_nic_pktHdrMBuf_dump(uint32 unit, uint32 mode, uint32 start, uint32 end, uint32 flags)
{
    rtdrv_nicCfg_t nic_cfg;
    ...
    return RT_ERR_OK;
} /* end of drv_nic_pktHdrMBuf_dump */
...
int32 drv_nic_pkt_send(uint32 unit, uint32 port, uint32 num)
{
    rtdrv_nicCfg_t nic_cfg;
    ...
    nic_cfg.port = port;
    nic_cfg.num  = num;
    SETSOCKOPT(RTDRV_NIC_PKT_SEND, &nic_cfg, rtdrv_nicCfg_t, 1);
...
int32 drv_spiPin_init(uint32 unit, spi_init_info_t *init_info)
{
...
int32 drv_spi_write(uint32 unit, uint32 mAddrs, uint32 *buff)
{
...
int32 drv_spi_read(uint32 unit, uint32 mAddrs, uint32 *buff)
{
    rtdrv_spiCfg_t spi_cfg;
