...
rtk_of_l3CamFlowEntry_move(uint32 unit, rtk_of_flowMove_t *pClrIdx)
{
    ...
    osal_memset(&cfg, 0, sizeof(rtdrv_openflowCfg_t));
    osal_memcpy(&cfg.unit, &unit, sizeof(uint32));
    osal_memcpy(&cfg.moveIdx, pClrIdx, sizeof(rtk_of_flowMove_t));
...
rtk_of_actionBucket_get(uint32 unit, uint32 entry_idx, rtk_of_actionBucket_t *pEntry)
{
    ...
    osal_memcpy(&cfg.unit, &unit, sizeof(uint32));
    osal_memcpy(&cfg.entry_idx, &entry_idx, sizeof(uint32));
    GETSOCKOPT(RTDRV_OF_ACTIONBUCKET_GET, &cfg, rtdrv_openflowCfg_t, 1);
...
rtk_of_actionBucket_set(uint32 unit, uint32 entry_idx, rtk_of_actionBucket_t *pEntry)
{
    ...
    SETSOCKOPT(RTDRV_OF_ACTIONBUCKET_SET, &cfg, rtdrv_openflowCfg_t, 1);
