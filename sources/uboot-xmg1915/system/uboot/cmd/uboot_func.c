 ...
 *      rtk_comboport_auto
 * Description:
 *      Configure Combo-port as auto detect mode
 * Input:
...
void rtk_comboport_auto(void)
{
    ...
    {
    _rtk_comboport_media_set(unit, PORT_MEDIA_FIBER_AUTO);
}
}
#endif
            ...
            case RTK_PHYTYPE_RTL8212B:
                {
                    ...
                    phy_reg_set(unit, rtl8214fb_phy0_macid + 1,
                            HAL_MIIM_PAGE_ID_MAX(unit), 31, 8);
                    phy_reg_get(unit, rtl8214fb_phy0_macid + 1,
                            HAL_MIIM_PAGE_ID_MAX(unit), 16, &forceReg);
                    phy_reg_set(unit, rtl8214fb_phy0_macid + 1,
                            HAL_MIIM_PAGE_ID_MAX(unit), 31, 8);
              #if defined(CONFIG_SDK_RTL8380)
                    if(HWP_8380_30_FAMILY(unit))
                    ...
                    for (idx = 0; idx < 4; ++idx)
                    {
                        phy_reg_get(unit, rtl8214fb_phy0_macid + idx,
                                HAL_MIIM_PAGE_ID_MAX(unit), 0, &phyData);
 ...
 */
int rtk_extGpio_device_init(uint deviceAddress)
{
    int32   ret;
    uint32  unit = HWP_MY_UNIT_ID();
 ...
 *      None
 */
int rtk_extGpio_pin_set(uint pinNum, uint pinStatus)
{
    drv_generalCtrlGpio_pinConf_t gpioConfig;
    uint32  unit = HWP_MY_UNIT_ID();
 ...
 *      None
 */
int rtk_extGpio_pin_get(uint pinNum, uint *pinStatus)
{
...
void rtk_swledtest_on(int port, int index)
{
    ...
    unsigned int val_led_mode_sel;
    unsigned int val_led_p_en_ctrl;
    /*Backup led value*/
    if(HWP_8380_30_FAMILY(unit))
    {
...
int rtk_phyreg_get(char * const argv[])
{
    ...
    osal_printf("Get Port %02d page %02d reg %02d", portid, page, reg);
...
int rtk_phyreg_set(char * const argv[])
{
    ...
    osal_printf("Set Port %02d page %02d reg %02d val: 0x%04X", portid, page, reg, val);
...
int rtk_phyreg_portmask_set(char * const argv[])
{
    ...
    osal_printf("Set PortMask %08x page %02d reg %02d val: 0x%04X \n", (unsigned int)port_mask, page, reg, val);
...
void rtk_sysEsd_set(int state)
{
    ...
    HWP_PORT_TRAVS(unit, macId)
    {
        ...
        phy_reg_set(unit, baseMacId, HAL_MIIM_PAGE_ID_MAX(unit), 27, 0x0004);
        phy_reg_get(unit, baseMacId, HAL_MIIM_PAGE_ID_MAX(unit), 28, &romId);
        if (romId > 2)
        {
...
{
    unsigned int unit = simple_strtoul(argv[2], NULL, 10);
    unsigned int mode = simple_strtoul(argv[3], NULL, 10);
    unsigned int port = simple_strtoul(argv[4], NULL, 10);
    unsigned int chn = simple_strtoul(argv[5], NULL, 10);
    unsigned int ret = 0;
 ...
 */
void rtk_i2c_sw_init(unsigned int i2c_dev, unsigned int clk_pin_dev, unsigned int clk_pin, unsigned int data_pin_dev, unsigned int data_pin, char *typeStr, unsigned int chipid, unsigned int delay, unsigned int rtl8231_addr)
{
    uint32           unit = HWP_MY_UNIT_ID();
 ...
 */
void rtk_i2c_hw_init(unsigned int i2c_dev, unsigned int data_pin, char *typeStr, unsigned int chipid, unsigned int freq, unsigned int intf_id)
{
    uint32           unit = HWP_MY_UNIT_ID();
    uint32           i2c_type;
    i2c_devConf_t    i2cDevice;
...
void
rtk_10gMedia_fiber100m_set(unsigned int port)
{
    _rtk_10gMedia_set(port, PORT_10GMEDIA_FIBER_100M);
}
void
