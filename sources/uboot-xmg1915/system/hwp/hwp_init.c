 ...
 *
 * $Revision: 81247 $
 * $Date: 2017-08-15 10:43:09 +0800 (Tue, 15 Aug 2017) $
 *
...
hwp_parsedInfo_t            *myParsedInfo=NULL;
...
uint32                      hwp_myLocalChipUnitID[RTK_MAX_NUM_OF_UNIT_LOCAL];
uint32                      myLocalChipUnitID_mask = 0x0;
 ...
 */
static int32
_hwp_chipID_mapping(hwp_swDescp_t *pSw, hwp_unitInfo_t *unitInfo)
{
    switch(pSw->chip_id)
    {
        ...
        case RTL8353MES_CHIP_ID:
            unitInfo->hwp_chip_family_id = RTL8350_FAMILY_ID;
            break;
        ...
        case RTL8396MES_CHIP_ID:
            unitInfo->hwp_chip_family_id = RTL8390_FAMILY_ID;
            break;
        case RTL8330M_CHIP_ID:
        case RTL8332M_CHIP_ID:
            unitInfo->hwp_chip_family_id = RTL8330_FAMILY_ID;
            break;
        case RTL8380M_CHIP_ID:
        case RTL8381M_CHIP_ID:
        case RTL8382M_CHIP_ID:
            unitInfo->hwp_chip_family_id = RTL8380_FAMILY_ID;
            break;
        case RTL9310_CHIP_ID:
        case RTL9311_CHIP_ID:
        case RTL9313_CHIP_ID:
            unitInfo->hwp_chip_family_id = RTL9310_FAMILY_ID;
            break;
        ...
        case RTL9302D_CHIP_ID:
            unitInfo->hwp_chip_family_id = RTL9300_FAMILY_ID;
            break;
        default:
...
/* Function Name:
 *      _hwp_localUnitId_get
 * Description:
 *      Give a hwp_unitInfo_t, and find out which local unit it belongs to
 * Input:
 *      *unitInfo   - the pointer of hwp_unitInfo_t
 * Output:
 *      *id         - the local unit ID of this unitInfo
 * Return:
 *      RT_ERR_OK       - if success
 *      RT_ERR_FAILED   - if failed
 * Note:
 ...
 */
int32
_hwp_localUnitId_get(uint32 *id, hwp_unitInfo_t *unitInfo)
{
    int i;
    hwp_parsedInfo_t *parsedInfo = (hwp_parsedInfo_t *)unitInfo->parsed_info;

    for(i=0;i<RTK_MAX_NUM_OF_UNIT_LOCAL;i++)
    {
        if(&parsedInfo->unitInfo[i] == unitInfo)
        {
            *id = i;
            return RT_ERR_OK;
        }
 ...
 *      None
 * Output:
 *      hwp_unitInfo_t     - Pointer of unitInfo struct.
 * Return:
 *      None
 * Note:
 *      None
 */
static void
_hwp_portCountAll_default(hwp_unitInfo_t *unitInfo)
{
    _hwp_portCount_default(&unitInfo->hwp_portCntEther);
    _hwp_portCount_default(&unitInfo->hwp_portCntUplink);
    _hwp_portCount_default(&unitInfo->hwp_portCntStack);
    _hwp_portCount_default(&unitInfo->hwp_portCntCascade);
    _hwp_portCount_default(&unitInfo->hwp_portCntFe);
    _hwp_portCount_default(&unitInfo->hwp_portCntGe);
    _hwp_portCount_default(&unitInfo->hwp_portCntXge);
    _hwp_portCount_default(&unitInfo->hwp_portCntSXge);
    _hwp_portCount_default(&unitInfo->hwp_portCntCopper);
    _hwp_portCount_default(&unitInfo->hwp_portCntFiber);
    _hwp_portCount_default(&unitInfo->hwp_portCntCombo);
    _hwp_portCount_default(&unitInfo->hwp_portCntSerdes);
}
 ...
 */
static void
_hwp_portMaskAll_default(hwp_unitInfo_t *unitInfo)
{
    int     i;

    RTK_PORTMASK_RESET(unitInfo->hwp_allPortMask);

    for (i = 0; i < HWP_ATTRI_ID_END; i++)
    {
        RTK_PORTMASK_RESET(unitInfo->hwp_attriPortMask[i]);
    }

    for (i = 0; i < HWP_ETHTYPE_ID_END; i++)
    {
        RTK_PORTMASK_RESET(unitInfo->hwp_ethTypePortMask[i]);
    }

    for (i = 0; i < HWP_MEDIUM_ID_END; i++)
    {
        RTK_PORTMASK_RESET(unitInfo->hwp_mediumPortMask[i]);
    }
}
...
_hwp_parsedInfo_default(hwp_parsedInfo_t *parsedInfo)
{
    ...
    parsedInfo->hwp_maxUnitId = 0;
    parsedInfo->hwp_minUnitId = RTK_MAX_NUM_OF_UNIT;
...
_hwp_chip_parse(hwp_parsedInfo_t *parsedInfo)
{
    ...
    for(i = 0;i < p->sw_count; i++)
    {
        ...
        if (p->swDescp[i]->unit_id >= RTK_MAX_NUM_OF_UNIT)
        {
            RT_ERR(RT_ERR_FAILED, MOD_INIT, "Wrong hardware profile [%s]: wrong unit ID!",p->identifier.name);
            return RT_ERR_FAILED;
        }
        ...
        /* init hwp_maxUnitId & hwp_minUnitId */
        if (p->swDescp[i]->unit_id > parsedInfo->hwp_maxUnitId)
            parsedInfo->hwp_maxUnitId = p->swDescp[i]->unit_id;
        if (p->swDescp[i]->unit_id < parsedInfo->hwp_minUnitId)
            parsedInfo->hwp_minUnitId = p->swDescp[i]->unit_id;

        /* init chip family ID */
        _hwp_chipID_mapping(p->swDescp[i], &parsedInfo->unitInfo[i]);

    }

    /* init hwp_embeddedCpuUnitId */
    parsedInfo->hwp_embeddedCpuUnitId = p->swDescp[p->soc.swDescp_index]->unit_id;
...
_hwp_portCount_init(hwp_unitInfo_t *unitInfo, hwp_portDescp_t *portDescp)
{
    ...
    if (portDescp->attr & HWP_STACK)
        _hwp_portCount_add(&unitInfo->hwp_portCntStack, macId);
...
_hwp_portMask_init(hwp_unitInfo_t *unitInfo, hwp_portDescp_t *portDescp)
{
    ...
    if(portDescp->attr & HWP_STACK)
        RTK_PORTMASK_PORT_SET(unitInfo->hwp_attriPortMask[HWP_STACK_ID], macId);
    if(portDescp->attr & HWP_CASCADE)
...
_hwp_led_parse(hwp_parsedInfo_t *parsedInfo)
{
    ...
    for(i = 0; i < (p->sw_count); i++)
    {
        ...
        while(swDescp->port.descp[j].mac_id != HWP_END)
        {
            ...
            {
                hwp_portDescp_t *pd_min = &swDescp->port.descp[parsedInfo->unitInfo[i].hwp_minMacId];
                ...
                if((j <= 23) && (pd_min->led_c != pd->led_c) && (pd->led_c != HWP_NONE))
                {
                    osal_printf("Port0~Port23 led mode must be the same!\n");
                }

                if((j > 23) && (pd_min24_27->led_c != pd->led_c) && (pd->led_c != HWP_NONE))
                {
...
_hwp_cascade_parse(hwp_parsedInfo_t *parsedInfo)
{
    ...
    if ( (hwp->sw_count ==2) && (hwp->soc.slaveInterruptPin != HWP_NONE) )
    {
        ...
        for (i = 0; i < (hwp->sw_count); i++)
        {
            ...
            {
                parsedInfo->hwp_cascadeSlaveUnitID = hwp->swDescp[i]->unit_id;
                break;
            }
        }
    }
...
hwp_unit_show(uint32 unit)
{
    ...
    {
        osal_printf("UNITMAP[%d]:%x,parsed_info=%x,swDescp=%x \n", i,(uint32)UNITMAP(i),
            (uint32)UNITMAP(i)->parsed_info,(uint32)UNITMAP(i)->swDescp);
    }

    for(i=0;i<RTK_MAX_NUM_OF_UNIT_LOCAL;i++)
    {
        osal_printf("hwp_myLocalChipUnitID[%d]=%d \n", i,hwp_myLocalChipUnitID[i]);
    }
...
{
    int i,j,k;
    hwp_parsedInfo_t *info = (hwp_parsedInfo_t *)UNITMAP(unit)->parsed_info;
    ...
    osal_printf("info->hwp_maxUnitId=%d         \n",info->hwp_maxUnitId);
    osal_printf("info->hwp_minUnitId=%d         \n",info->hwp_minUnitId);
    ...
    for (i=0; i<info->hwp_localUnitCnt; i++)
    {
        ...
        for(j=0;j<RTK_MAX_PORT_PER_UNIT;j++)
        {
            osal_printf("p[%d]=%x,", j, (uint32)info->unitInfo[i].hwp_macID2PortDescp[j]);
        ...
        _hwp_debug_portCnt_show(&info->unitInfo[i].hwp_portCntStack     ,"Stack  ");
        ...
        for(j=0;j<RTK_MAX_SDS_PER_UNIT;j++)
            osal_printf("sds[%d]=%x,",j,(uint32)info->unitInfo[i].hwp_sdsID2SerdesDescp[j]);
        ...
        for(j=0;j<RTK_MAX_PORT_PER_UNIT;j++)
        {
            if (info->unitInfo[i].hwp_macID2SCDescp[j] == &scDescpEmpty)
                continue;
            osal_printf(" mac[%d]=%x,", j, (uint32)info->unitInfo[i].hwp_macID2SCDescp[j]);
        }
...
hwp_info_show(uint32 target_unit)
{
    ...
    hwp_parsedInfo_t *parsedInfo = UNITMAP(target_unit)->parsed_info;
    ...
    RT_INIT_MSG("      Chip count: %d (minUnitId:%d, maxUnitId:%d)\n", hardwareProfile->sw_count,parsedInfo->hwp_minUnitId,parsedInfo->hwp_maxUnitId);
    ...
    HWP_UNIT_TRAVS_LOCAL(unit)
    {
        ...
        RT_INIT_MSG("          nic support: %d\n", UNITMAP(unit)->swDescp->nic_supported);
        ...
        RT_INIT_MSG("          CpuMacId: %d  (attr)\n", UNITMAP(unit)->hwp_portCpuMacId);
        _hwp_infoPortCntPortMask_show("Ether",      "A", &(UNITMAP(unit)->hwp_portCntEther),    &(UNITMAP(unit)->hwp_attriPortMask[HWP_ETH_ID]));
        _hwp_infoPortCntPortMask_show("Uplink",     "A", &(UNITMAP(unit)->hwp_portCntUplink),   &(UNITMAP(unit)->hwp_attriPortMask[HWP_UPLINK_ID]));
        _hwp_infoPortCntPortMask_show("Stack",      "A", &(UNITMAP(unit)->hwp_portCntStack),    &(UNITMAP(unit)->hwp_attriPortMask[HWP_STACK_ID]));
        _hwp_infoPortCntPortMask_show("Cascade",    "A", &(UNITMAP(unit)->hwp_portCntCascade),  &(UNITMAP(unit)->hwp_attriPortMask[HWP_CASCADE_ID]));
        _hwp_infoPortCntPortMask_show("FE",         "E", &(UNITMAP(unit)->hwp_portCntFe),       &(UNITMAP(unit)->hwp_ethTypePortMask[HWP_FE_ID]));
        _hwp_infoPortCntPortMask_show("GE",         "E", &(UNITMAP(unit)->hwp_portCntGe),       &(UNITMAP(unit)->hwp_ethTypePortMask[HWP_GE_ID]));
        _hwp_infoPortCntPortMask_show("2_5GE",      "E", &(UNITMAP(unit)->hwp_portCnt2_5ge),    &(UNITMAP(unit)->hwp_ethTypePortMask[HWP_2_5GE_ID]));
        _hwp_infoPortCntPortMask_show("XGE",        "E", &(UNITMAP(unit)->hwp_portCntXge),      &(UNITMAP(unit)->hwp_ethTypePortMask[HWP_XGE_ID]));
        _hwp_infoPortCntPortMask_show("SXGE",       "E", &(UNITMAP(unit)->hwp_portCntSXge),     &(UNITMAP(unit)->hwp_ethTypePortMask[HWP_SXGE_ID]));
        _hwp_infoPortCntPortMask_show("Copper",     "M", &(UNITMAP(unit)->hwp_portCntCopper),   &(UNITMAP(unit)->hwp_mediumPortMask[HWP_COPPER_ID]));
        _hwp_infoPortCntPortMask_show("Fiber",      "M", &(UNITMAP(unit)->hwp_portCntFiber),    &(UNITMAP(unit)->hwp_mediumPortMask[HWP_FIBER_ID]));
        _hwp_infoPortCntPortMask_show("Combo",      "M", &(UNITMAP(unit)->hwp_portCntCombo),    &(UNITMAP(unit)->hwp_mediumPortMask[HWP_COMBO_ID]));
        _hwp_infoPortCntPortMask_show("Serdes",     "M", &(UNITMAP(unit)->hwp_portCntSerdes),   &(UNITMAP(unit)->hwp_mediumPortMask[HWP_SERDES_ID]));

        for (i = 0; i < UNITMAP(unit)->swDescp->port.count; i++)
        {
            RT_INIT_MSG("          port:%2d, phyIdx%u\n", UNITMAP(unit)->swDescp->port.descp[i].mac_id, UNITMAP(unit)->swDescp->port.descp[i].phy_idx);
        }
        RT_INIT_MSG("\n");


        RT_INIT_MSG("          PHY Count:%u\n", (uint32)UNITMAP(unit)->swDescp->phy.count);
        for (i = 0; i < UNITMAP(unit)->swDescp->phy.count; i++)
        {
            RT_INIT_MSG("            type:%02u(%-12s) port:%u baseMac:%u\n", UNITMAP(unit)->swDescp->phy.descp[i].chip,
                                        _phyType_name((phy_type_t)UNITMAP(unit)->swDescp->phy.descp[i].chip),
                                        UNITMAP(unit)->swDescp->phy.descp[i].phy_max,
                                        UNITMAP(unit)->swDescp->phy.descp[i].mac_id);
        }

        j=0;
        while(UNITMAP(unit)->swDescp->phy.descp[j].chip != HWP_END)
        {
            RT_INIT_MSG("          PHY[%d]: chipID %2d, baseMacId %2d, phy_max %2d \n",j,
            UNITMAP(unit)->swDescp->phy.descp[j].chip,
            UNITMAP(unit)->swDescp->phy.descp[j].mac_id,
            UNITMAP(unit)->swDescp->phy.descp[j].phy_max);
            j++;
        }
        ...
        /* dump SC info */
        {
            ...
            for (i = 0; i < UNITMAP(unit)->swDescp->port.count; i++)
            {
                if ((UNITMAP(unit)->swDescp->port.descp[i].attr & HWP_SC) && (sc_idx = UNITMAP(unit)->swDescp->port.descp[i].sc_idx) < RTK_MAX_SC_PER_UNIT)
                {
                    if (sc_idx != prev_sc_idx)
                    {
                        RT_INIT_MSG("          SC[%d]: chipID %2d, smi %2d, addr %2d \n", sc_idx,
                            UNITMAP(unit)->swDescp->sc.descp[sc_idx].chip, UNITMAP(unit)->swDescp->sc.descp[sc_idx].smi, UNITMAP(unit)->swDescp->sc.descp[sc_idx].phy_addr);
                        prev_sc_idx = sc_idx;
                    }
                }
            }/* end for */
...
hwp_init(void)
{
    ...
    RT_INIT_REENTRY_CHK(hwp_init_status);
    ...
    for(i=0; i<RTK_MAX_NUM_OF_UNIT; i++)
    {
        UNITMAP(i) = &parsedInfoEmpty.unitInfo[HWP_LOCALUNIT_EMPTY];
        unitMapStruct[i].flag = 0;
    }
    for(i=0; i<RTK_MAX_NUM_OF_UNIT_LOCAL; i++)
    {
        hwp_myLocalChipUnitID[i] = HWP_NONE;
    }
    ...
    /* map unit */
    for(i=0; i<hwp_myHwProfile->sw_count; i++)
    {
        uint32 unit_id = hwp_myHwProfile->swDescp[i]->unit_id;
        UNITMAP(unit_id) = &myParsedInfo->unitInfo[i];
        hwp_myLocalChipUnitID[i] = unit_id;
        unitMapStruct[unit_id].flag |= HWP_UNITMAPFLAG_LOCALCHIP;
    }

    if (HWP_ID_TYPE_STRING == myParsedInfo->hwp->identifier.type)
        RT_INIT_MSG(" (%s)\n",myParsedInfo->hwp->identifier.name);
    else if(HWP_ID_TYPE_INTEGER == myParsedInfo->hwp->identifier.type)
        RT_INIT_MSG(" (%d)\n",myParsedInfo->hwp->identifier.id);
    else
        RT_INIT_MSG(" ()\n");
