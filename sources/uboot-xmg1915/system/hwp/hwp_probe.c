 ...
 *
 * $Revision: 81095 $
 * $Date: 2017-08-08 14:14:42 +0800 (Tue, 08 Aug 2017) $
 *
...
hwp_identifier_get(hwp_identifier_t *identifier)
{
    ...
    else
    {
        ...
        char    *pboard_id = board_id;
  #ifdef CONFIG_SDK_KERNEL_LINUX_KERNEL_MODE
        int     index;
        char    val;
        const   char base = 10;
  #endif
       if (RT_ERR_OK != (ret = rt_util_flashEnv_get("boardid",  pboard_id, HWP_PROFILE_NAME_LEN_MAX)))
            return RT_ERR_FAILED;
        ...
        for(index = 0; index < HWP_PROFILE_NAME_LEN_MAX; index++)
        {
            ...
            {
                identifier->id = identifier->id * base + val;
            }
            else
                return RT_ERR_FAILED;
        }
            ret = RT_ERR_OK;
    #else
        if(osal_sscanf(pboard_id, "%d", &identifier->id))
            ret = RT_ERR_OK;
    #endif
  #else
        identifier->id = (int)simple_strtoul((const char*)pboard_id, NULL, 10);
  #endif
    }
...
int32
hwp_defaultProfilePort_update(hwp_hwProfile_t *defaultHwp, uint8 mac_id, uint8 attr, uint8 medi, uint8 eth, uint8 phy_idx)
{
...
hwp_probe(void)
{
    ...
    if( hwp_identifier_get(&myIdentifier) == RT_ERR_OK)
    {
