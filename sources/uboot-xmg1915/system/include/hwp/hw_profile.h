...
typedef enum rt_port_attri_e
{
    ...
    HWP_STACK           = (0x1 << HWP_STACK_ID),      /* Stack port */
...
{
    uint32                  unit_id;                /* each chip has a unique unit ID, must be from 0 ~ (RTK_MAX_NUM_OF_UNIT-1) */
...
typedef struct hwp_unitInfo_s
{
    ...
    hwp_portCount_t         hwp_portCntStack;
...
typedef struct hwp_parsedInfo_s
{
    ...
    uint32                      hwp_maxUnitId;          /* Max unit ID on this board */
    uint32                      hwp_minUnitId;          /* Min unit ID on this board */
...
extern hwp_parsedInfo_t         *myParsedInfo;
extern hwp_unitMap_t            unitMapStruct[];
extern uint32                   hwp_myLocalChipUnitID[];
extern uint32                   myLocalChipUnitID_mask;
...
#define UNITMAP(unit)                           (unitMapStruct[unit].unitInfo)
...
#define HWP_MY_CHIP_ID()                        (HWP_CHIP_ID(myParsedInfo->hwp_embeddedCpuUnitId))
#define HWP_MY_CHIP_FAMILY_ID()                 (HWP_CHIP_FAMILY_ID(myParsedInfo->hwp_embeddedCpuUnitId)) /* e.g. RTL8350_FAMILY_ID, RTL8390_FAMILY_ID, ... */
#define HWP_MY_UNIT_ID()                        (myParsedInfo->hwp_embeddedCpuUnitId)
#define HWP_IS_CPU_UNIT(unit)                   (myParsedInfo->hwp_embeddedCpuUnitId==unit) /* check if the CPU of this unit is the main CPU of the board*/
#define HWP_CASCADE_SLAVE_UNIT_ID()             (myParsedInfo->hwp_cascadeSlaveUnitID)
#define HWP_CASCADE_MODE()                      (myParsedInfo->hwp_cascade_mode)
...
#define LOCAL_UNIT(n)                           (hwp_localChipUnitID_get(n)) /* To get local chip Unit ID. n is a 0-based ID: 0, 1, 2, ... for 1st, 2nd, 3rd,... chip on local board */
...
/* Traverse switch chips on local board */
#define HWP_UNIT_TRAVS_LOCAL(unit)              for (unit = myParsedInfo->hwp_minUnitId; unit <= myParsedInfo->hwp_maxUnitId; unit++)    \
                                                    if (HWP_UNIT_VALID_LOCAL(unit))
/* Traverse all units in whole stacking system, include the ones on local board */
#define HWP_UNIT_TRAVS_STACK(unit)              for (unit = 0; unit < RTK_MAX_NUM_OF_UNIT; unit++)    \
                                                    if (HWP_UNIT_VALID_STACK(unit))
/* traverse all units of a hardware profile */
#define HWP_UNIT_TRAVS_BY_HWP(unit, i, pHwp)    for (i = 0; (i < pHwp->sw_count) && (unit = pHwp->swDescp[i]->unit_id, TRUE); i++)
...
#define HWP_PORT_IN_RANGE(unit,mac_id)              (((unit) <= RTK_MAX_UNIT_ID) && ((mac_id) <= RTK_MAX_PORT_ID_PER_UNIT))
...
#define HWP_STACK_PORT(unit, mac_id)            (HWP_PORT_ATTRI(unit, mac_id) & HWP_STACK)
#define HWP_STACK_PORT_COUNT(unit)              (UNITMAP(unit)->hwp_portCntStack.count)
#define HWP_STACK_PORT_TRAVS(unit, mac_id)      for (mac_id = UNITMAP(unit)->hwp_portCntStack.min; mac_id <= UNITMAP(unit)->hwp_portCntStack.max; mac_id++) \
                                                    if(HWP_STACK_PORT(unit, mac_id))
