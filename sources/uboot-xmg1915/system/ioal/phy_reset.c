 ...
 *
 * $Revision: 77072 $
 * $Date: 2017-04-01 13:21:45 +0800 (Sat, 01 Apr 2017) $
 *
 ...
 */
static int32
rt_bsp_phy_reset_support(uint32 unit, drv_generalCtrlGpio_devId_t *devId, uint32 *pinId)
{
    char    *name;
    ...
    if (hwp_myHwProfile->sw_count == 1)
    {
        ...
        {
            *devId = GEN_GPIO_DEV_ID0_INTERNAL;
            *pinId = 23;
            return RT_ERR_OK;
        }
...
static void
rt_bsp_phyReset_init(drv_generalCtrlGpio_devId_t devId, uint32 pinId)
{
    ...
    ret = drv_generalCtrlGPIO_dev_init(HWP_MY_UNIT_ID(), devId, NULL);
    if (ret != RT_ERR_OK)
    {
    ...
    drv_generalCtrlGPIO_devEnable_set(HWP_MY_UNIT_ID(), devId, ENABLED);
    ...
    drv_generalCtrlGPIO_pin_init(HWP_MY_UNIT_ID(), devId, pinId, &gpioConfig);
...
rt_bsp_phy_reset(uint32 unit)
{
    ...
    if ((ret = rt_bsp_phy_reset_support(unit, &devId, &pinId)) != RT_ERR_OK)
        return ret;

    rt_bsp_phyReset_init(devId, pinId);

    drv_generalCtrlGPIO_dataBit_set(cpu_unit, devId, pinId, 0);
    osal_time_mdelay(10 * 2);    /* Min 10mS (from Spec) * 2 = 20mS (double for safety) */
    drv_generalCtrlGPIO_dataBit_set(cpu_unit, devId, pinId, 1);
