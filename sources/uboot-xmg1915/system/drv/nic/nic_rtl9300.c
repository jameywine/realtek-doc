 ...
 *
 * $Revision: 76296 $
 * $Date: 2017-03-08 13:48:49 +0800 (Wed, 08 Mar 2017) $
 *
...
int32 r9300_holRingCnt_get(uint32 unit, uint32 ring, uint32 *pVal)
{
    ...
    *pVal = (tmpVal >> RTL9300_DMA_IF_RX_RING_CNTR_CNTR_OFFSET(ring)) & RTL9300_DMA_IF_RX_RING_CNTR_CNTR_MASK(ring);
...
int32 r9300_cpuTagFromRaw_cnvt(uint32 unit, uint8 *pCpuTag, drv_nic_pkt_t *pPacket)
{
    ...
    if (pCpuTagInfo)
    {
        ...
        else
        {
            ...
            {
                pPacket->rx_tag.ext_unit        = pHdrtag->un.rx.PORT_DATA >> 6;
...
int32 r9300_cpuTagToRaw_cnvt(uint32 unit, drv_nic_pkt_t *pPacket, uint8 *pCpuTag)
{
    ...
    if (pPacket->as_txtag)
    {
        ...
        pHdrtag->un.tx.SW_UNIT       = pPacket->tx_tag.unit;
...
int32 r9300_rawTag_dump(uint32 unit, uint8 *pCpuTag)
{
    ...
    osal_printf(" SW_UNIT : 0x%0x \n", pHdrtag->un.tx.SW_UNIT);
