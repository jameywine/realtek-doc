...
_nic_isr_rxRoutine(uint32 unit, uint32 ringId)
{
    ...
    do
    {
        ...
        if (pktHdr->more && jumboFlag == FALSE)     /* Jumbo head */
        {
            ...
            if (RT_ERR_FAILED == _nic_init_conf.pkt_alloc(unit, 9000, 0, &pPacket))
            {
                jumboFlag = FALSE;
        ...
        if (jumboFlag == 0 || (jumboFlag && pktHdr->more == 0))
        {
            ...
            if (0 != _nic_rx_intr_cb_cnt)
            {
                for (i = 0; i < NIC_RX_CB_PRIORITY_NUMBER; i++)
                {
                    ...
                    {
                        nic_rx_handle = _nic_rx_cb_tbl[i].rx_callback(unit, pktHdr->packet, _nic_rx_cb_tbl[i].pCookie);
                    }
        ...
        if (jumboFlag == 0)
        {
            if (NULL == pktHdr->packet)
            {
                ...
                {
                    RT_LOG(LOG_WARNING, (MOD_NIC|MOD_DAL), "Out of memory ! (alloc a new packet data buffer failed)");
                    reclaim_mbuf = FALSE;
                }
            }
            else
...
drv_nic_isr_handler(void *isr_param)
{
    ...
    _nic_intrMask_set(unit, NIC_RX_DONE, 0xff);
    _nic_intrMask_set(unit, NIC_RX_RUNOUT, 0xff);
...
_nic_init(uint32 unit, drv_nic_initCfg_t *pInitCfg)
{
    ...
    _nic_intrSts_set(unit, NIC_RX_DONE, 0xff);
    _nic_intrSts_set(unit, NIC_RX_RUNOUT, 0xff);
...
drv_nic_rx_start(uint32 unit)
{
    ...
    if (ret != RT_ERR_OK)
    {
        RT_LOG(LOG_DEBUG, MOD_NIC, "Error: thread create fail!");
        goto EXIT;
    }
...
drv_nic_rx_stop(uint32 unit)
{
    ...
    if (ret != RT_ERR_OK)
    {
        RT_LOG(LOG_DEBUG, MOD_NIC, "Error: thread destroy fail!");
    }
