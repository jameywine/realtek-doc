 ...
 *
 * $Revision: 81477 $
 * $Date: 2017-08-18 11:17:49 +0800 (Fri, 18 Aug 2017) $
 *
...
r9300_l2ntfy_pktCpuQueBwCtrlEnable_set(uint32 unit, uint32 queId, rtk_enable_t enable)
{
    ...
    ioal_mem32_field_write(unit, RTL9300_EGBW_CPU_Q_MAX_LB_CTRL_ADDR(queId) + 0x4,  20,
                            (0x1 << 20), enable);
...
r9300_l2ntfy_pktCpuQueRate_set(uint32 unit, uint32 queId, uint32 rate)
{
    ...
    ioal_mem32_field_write(unit, RTL9300_EGBW_CPU_Q_MAX_LB_CTRL_ADDR(queId) + 0x4,  0,
                            (0xFFFFF << 0), rate);
