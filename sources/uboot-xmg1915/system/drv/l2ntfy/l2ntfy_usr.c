...
extern uint32                           l2ntfy_ring_base[RTK_MAX_NUM_OF_UNIT];
extern uint32                           l2ntfy_buf_base[RTK_MAX_NUM_OF_UNIT];
...
#define  L2NTFY_RING_ADDR_KRN2USR(unit, krn_addr)    (((uint32)krn_addr - (uint32)KRNVIRT(l2ntfy_ring_phys_base)) + l2ntfy_ring_base[unit])
#define  L2NTFY_RING_ADDR_USR2KRN(unit, usr_addr)    (((uint32)usr_addr - l2ntfy_ring_base[unit]) + (uint32)KRNVIRT(l2ntfy_ring_phys_base))
#define  L2NTFY_BUF_ADDR_KRN2USR(unit, krn_addr)    (((uint32)krn_addr - (uint32)KRNVIRT(l2ntfy_buf_phys_base)) + l2ntfy_buf_base[unit])
#define  L2NTFY_BUF_ADDR_USR2KRN(unit, usr_addr)    (((uint32)usr_addr - l2ntfy_buf_base[unit]) + (uint32)KRNVIRT(l2ntfy_buf_phys_base))
...
_l2ntfy_isr_init(uint32 unit)
{
    ...
    //l2ntfy_ring_vir_addr = (uint32 *)UNCACHE(l2ntfy_ring_vir_addr);
    ...
    //l2ntfy_buf_vir_addr = (uint8 *)UNCACHE(l2ntfy_buf_vir_addr);
...
_l2ntfy_isr_unInit(uint32 unit)
{
    ...
    {
        value = L2NTFY_RING_ADDR_USR2KRN(unit, eventRing[index]) & L2NTFY_RING_OWNER_MASK;
...
drv_l2ntfy_init(uint32 unit)
{
    ...
    if(TRUE == HWP_CASCADE_MODE())
    {
        ...
        /* Compatible */
        if( (RTL9301_CHIP_ID_24G == HWP_CHIP_ID(unit)) && ( (1==HWP_CHIP_REV(unit)) || (2==HWP_CHIP_REV(unit)) ) )
        {
...
drv_l2ntfy_pkt_handler(uint32 unit, drv_nic_pkt_t *pPacket, void *pCookie)
{
        ...
        &&(*(pPacket->data + proLoc + 2) == 0x05))
    {
        ...
        drv_l2ntfy_magicNum_get(pktUnit, &mgcNum);
        if(mgcNum == *(pPacket->data + proLoc + 3))
        {
            ...
            {
                //drv_l2ntfy_maxEvent_get(pktUnit, &maxEvtNum);
                //pktLength = pPacket->length;
                //blockMemSize = sizeof(ntfy_collectArrayList_t) + pktLength * sizeof(uint8);
                l2NtfyDataLen = pPacket->length - (proLoc + 3);
                num = (l2NtfyDataLen - (l2NtfyDataLen % entrySize))/entrySize;
                //?????
...
drv_l2ntfy_pktDrop_handler(uint32 unit, rtk_l2ntfy_runOutType_t dropType)
{
    ...
    pktDropCnt[unit]++;
    if (runout_callback)
        runout_callback(unit, L2_NOTIFY_PKT_MISS);
    ...
    if(TRUE == HWP_CASCADE_MODE())
    {
        ...
        l2DBG("FUN:%s---LINE:%d,master=%d, slave=%d,runOutType=%d,new mgcNum=%d\n", __FUNCTION__ , __LINE__,masterUnitId, slaveUnitId, dropType,mgcNum);
...
drv_l2ntfy_dst_set(uint32 unit, rtk_l2ntfy_dst_t dst)
{
    ...
    /* Should action when change mode, not action when slave unit in cascade */
    if(org_dst != dst)
    {
...
drv_l2ntfy_magicNum_set(uint32 unit, uint32 magicNum)
{
    ...
    return L2NTFY_CTRL(unit).l2ntfy_magicNum_set(unit, magicNum);
}
