 ...
 *
 * $Revision: 81265 $
 * $Date: 2017-08-15 14:16:30 +0800 (Tue, 15 Aug 2017) $
 *
...
static rtk_l2ntfy_dst_t                 l2ntfy_dst = L2NTFY_DST_PKT_TO_LOCAL;
...
static atomic_t                         mem_usage = ATOMIC_INIT(0);
static atomic_t                         mem_lock = ATOMIC_INIT(0);
static uint32                           lockCnt, releaseCnt;
static atomic_t                         pkt_missIsrLock = ATOMIC_INIT(0);
...
void _l2ntfy_bhWQ(void *data);
void _l2ntfy_runoutHandle_bhWQ(void *data);
#else
void _l2ntfy_bhWQ(struct work_struct *data);
void _l2ntfy_runoutHandle_bhWQ(struct work_struct *data);
#endif
...
_l2ntfy_isr_unInit(uint32 unit)
{
    ...
    for(index = 0; index < ringSize; index ++)
    {
        value = eventRing[index] & L2NTFY_RING_OWNER_MASK;
        while( L2NTFY_RING_OWNER_CPU == value )
        {
            osal_time_mdelay(5);
            value = eventRing[index] & L2NTFY_RING_OWNER_MASK;
        }
...
drv_l2ntfy_init(uint32 unit)
{
    ...
    if(TRUE == HWP_CASCADE_MODE())
    {
        ...
        /* Compatible */
        if((HWP_9300_FAMILY_ID(unit)) && ( (CHIP_REV_ID_A == HWP_CHIP_REV(unit)) || (CHIP_REV_ID_B == HWP_CHIP_REV(unit))))
        {
...
#endif
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,27))
    OSAL_INIT_WORK(&workq, _l2ntfy_bhWQ);
    OSAL_INIT_LIST_HEAD(&ntfyListHead.list);
    OSAL_INIT_WORK(&runoutWorkq, _l2ntfy_runoutHandle_bhWQ);
    OSAL_INIT_LIST_HEAD(&runOutListHead.list);
#endif
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,27))
void
_l2ntfy_bhWQ(void *data)
#else
void
_l2ntfy_bhWQ(struct work_struct *data)
#endif
{
    ...
    osal_list_for_each_entry_safe(pEntry, n, &ntfyListHead.list, list)
    {
        ...
        {
            num = (pEntry->blockSize - (pEntry->blockSize % entrySize))/entrySize;
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,27))
void
_l2ntfy_runoutHandle_bhWQ(void *data)
#else
void
_l2ntfy_runoutHandle_bhWQ(struct work_struct *data)
#endif
{
    ...
    osal_list_for_each_entry_safe(pEntry, n, &runOutListHead.list, list)
    {
        ...
        if(TRUE == HWP_CASCADE_MODE())
        {
            drv_l2ntfy_magicNum_get(masterUnitId, &mgcNum);
            mgcNum = (mgcNum + 1)% 0xFF;
            l2DBG("FUN:%s---LINE:%d,master=%d, slave=%d,runOutType=%d,new mgcNum=%d\n", __FUNCTION__ , __LINE__,masterUnitId, slaveUnitId, pEntry->runOutType,mgcNum);
            drv_l2ntfy_magicNum_set(masterUnitId, mgcNum);
            drv_l2ntfy_magicNum_set(slaveUnitId, mgcNum);
        }
        else
        {
            drv_l2ntfy_magicNum_get(pEntry->unit, &mgcNum);
            mgcNum = (mgcNum + 1)% 0xFF;
            l2DBG("FUN:%s---LINE:%d,unit=%d,runOutType=%d,new mgcNum=%d\n", __FUNCTION__ , __LINE__,pEntry->unit,pEntry->runOutType,mgcNum);
            drv_l2ntfy_magicNum_set(pEntry->unit, mgcNum);
        }
        spin_lock_irqsave(&irqLock, flags);
        osal_list_del(&pEntry->list);
        spin_unlock_irqrestore(&irqLock, flags);
        if(L2_NOTIFY_PKT_MISS == pEntry->runOutType)
        {
            atomic_set(&pkt_missIsrLock, 0);
        }
        osal_free(pEntry);
...
drv_l2ntfy_pkt_handler(uint32 unit, drv_nic_pkt_t *pPacket, void *pCookie)
{
    ...
    l2DBG("rxCpuTag: unit %d  source_port %d pysical_port %d,length %u, proLocStart=%d",
        pPacket->rx_tag.unit, pPacket->rx_tag.source_port, pPacket->rx_tag.physical_rx_port, (uint32)pPacket->length, proLoc);
#endif
...
#if defined(CONFIG_SDK_RTL9300)
        pktUnit = pPacket->rx_tag.unit;
#endif
        curSeqId = sqcId[pktUnit];
        sqcId[pktUnit] = (sqcId[pktUnit] + 1) % sqcIdMax;
        ...
        if(mgcNum == *(pPacket->data + proLoc + 3))
        {
            ...
            if(curSeqId == pktSeqId)
            {
                ...
                else
                {
                    ...
                    pCllctAryList->blockSize = pktLength - (proLoc + 3);
                    pCllctAryList->pNBufTmp = osal_alloc(pktLength * sizeof(uint8));
                    ...
                    osal_memcpy(pCllctAryList->pNBufTmp, pPacket->data + proLoc + 6, pktLength * sizeof(uint8));
            ...
            else
            {
                ...
                /* Use this packet sequence ID as new sequence ID */
                sqcId[pktUnit] = pktSeqId + 1;
            }
...
Err:
    ...
    {
#if defined(CONFIG_SDK_RTL9300)
        l2DBG("free rxCpuTag: unit %d  source_port %d pysical_port %d", pPacket->rx_tag.unit, pPacket->rx_tag.source_port, pPacket->rx_tag.physical_rx_port);
#endif
...
drv_l2ntfy_pktDrop_handler(uint32 unit, rtk_l2ntfy_runOutType_t dropType)
{
    ...
    if(atomic_read(&pkt_missIsrLock))
    {
        return RT_ERR_OK;
    }
    atomic_set(&pkt_missIsrLock, 1);
...
drv_l2ntfy_dst_set(uint32 unit, rtk_l2ntfy_dst_t dst)
{
    ...
    return L2NTFY_CTRL(unit).l2ntfy_dst_set(unit, dst);
}
