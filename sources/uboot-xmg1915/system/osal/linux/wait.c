 ...
 *
 * $Revision: 76506 $
 * $Date: 2017-03-14 17:14:14 +0800 (Tue, 14 Mar 2017) $
 *
...
typedef struct osal_linux_wait_event_s {
    ...
    osal_atomic_t               osal_linux_wait_event_intr;
...
osal_wait_module_create(uint32 *event_id)
{
    ...
    if(event_index_cur == NULL)
    {
        ...
        osal_atomic_set(&p->osal_linux_wait_event_intr, 0);
...
osal_wait_event(uint32 event)
{
    ...
    {
        osal_atomic_inc(&p->osal_linux_wait_event_intr);
    }

    do
    {
        ret = wait_event_interruptible(p->osal_linux_wait_event_queue, osal_atomic_read(&p->osal_linux_wait_event_intr) <= 0);
        if(ret != 0)
        {
...
osal_wake_up(uint32 event)
{
    ...
    if (osal_atomic_dec_return(&p->osal_linux_wait_event_intr) >= 0)
    {
