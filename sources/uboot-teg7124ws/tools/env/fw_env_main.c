...
#define	CMD_SYS_PRINTENV	"fw_printsys"
#define CMD_SYS_SETENV	    "fw_setsys"
...
main(int argc, char *argv[])
{
	...
	} else if (strcmp(cmdname, CMD_SYS_PRINTENV) == 0) {
		i4commandflg = 1;
		if (fw_printenv (argc, argv) != 0)
			return EXIT_FAILURE;
	...
	} else if (strcmp(cmdname, CMD_SYS_SETENV) == 0) {
		i4commandflg = 1;
		if (!script_file) {
			if (fw_setenv(argc, argv) != 0)
				return EXIT_FAILURE;
	...
	fprintf (stderr,
		...
		"' or as `" CMD_SETENV "' or as `" CMD_SYS_PRINTENV "' or as `" CMD_SYS_SETENV "' but not as `%s'\n",
		cmdname);
	return EXIT_FAILURE;
}
