...
/* Frank increase from 8MByte to 30MByte*/
#define CONFIG_SYS_BOOTM_LEN	0x2400000	/* use 30MByte as default max gunzip size */
#endif
...
/*SENAO Bruce add for support rescue image.*/
#define PARTITION_RESCUE_ADDR (CONFIG_SYS_FLASH_BASE + PARTITION_ADDR(FLASH_INDEX_RESCUE_KERNEL))
#endif /* end of CONFIG_SENAO_RESCUE_IMAGE */

#ifndef CONFIG_BOOT_SPI_NAND
/* SENAO Frank Modify for Bootm Support NAND Flash when PARTITION_SIZE(FLASH_INDEX_KERNEL) is zero
NAND Flash partition reference sdk/system/linux/linux-3.18.24.x/drivers/mtd/nand/rtk_spi_nand/luna_mtd_nand.c
*/
#ifdef CONFIG_SPI_NAND_FLASH
#define PARTITION_NAND_ADDR_0   0x4000000
#define PARTITION_NAND_SIZE_0   0x1E00000 

#ifdef CONFIG_DUAL_IMAGE
#define PARTITION_NAND_ADDR_1   0x5E00000
#define PARTITION_NAND_SIZE_1   0x1E00000 
#endif
#include <nand.h>
...
#ifdef CONFIG_SPI_NAND_FLASH
static int _do_boota_nand_img_check(uint32_t addr)
{
    image_header_t ih;
	...
	len = sizeof(ih);
    nand_read_skip_bad(nand, addr, &len, (u_char *)&ih);

    if (!image_check_hcrc(&ih))
    {
        printf("## Invalid NAND image header\n");
        return 1;
    }
    ...
    if (imgSize <= sizeof(ih) || imgSize > PARTITION_NAND_SIZE_0)
    {
        printf("## Invalid NAND image size\n");
        return 1;
    }

    /* Copy flash data to memory and boot from memorty to increse speed */
	len = imgSize;
    nand_read_skip_bad(nand, addr, &len, (u_char *)RUNTIME_SRC_ADDR);

    if (!image_check_dcrc((image_header_t *)RUNTIME_SRC_ADDR))
    {
        printf("## Invalid NAND image data on addr 0x%x\n", addr);
        return 1;
    }
...
#ifdef CONFIG_SPI_NAND_FLASH
static int _do_bootm_nand_addr(uint32_t addr)
{
    cmd_tbl_t *run = NULL;
    char *argu[5];
    image_header_t ih;
...
#ifdef CONFIG_SENAO_RESCUE_IMAGE
    if (imgSize <= sizeof(ih) || imgSize > PARTITION_SIZE(FLASH_INDEX_RESCUE_KERNEL))
    {
        printf("## Invalid image size\n");
        return 1;
    }
#else /* else of CONFIG_SENAO_RESCUE_IMAGE */
...
static int dual_image_sel (int index)
{
    ...
    if (-1 == index)
    {
...
#ifdef CONFIG_SPI_NAND_FLASH
            if (PARTITION_SIZE(FLASH_INDEX_KERNEL) > 0 ) {
            rc = _do_bootm_addr((const void *) PARTITION_ADDR_1);
            } else {
                rc = _do_bootm_nand_addr(PARTITION_NAND_ADDR_1);            
            }
#else
            rc = _do_bootm_addr((const void *) PARTITION_ADDR_1);
#endif
...
#endif /* end of CONFIG_SENAO_RESCUE_IMAGE */      	
        {
            ...
            rc = _do_bootm_addr((const void *) PARTITION_ADDR_0);
                ...
                rc = _do_bootm_nand_addr(PARTITION_NAND_ADDR_0);            
            }
#else
            rc = _do_bootm_addr((const void *) PARTITION_ADDR_0);
#endif
        ...
        {
            if (PARTITION_SIZE(FLASH_INDEX_RESCUE_KERNEL) > 0)
            {
                printf ("## Booting [RESCUE] image from partition ... %d\n", IMG_RESCUE);
    
                if (PARTITION_SIZE(FLASH_INDEX_RESCUE_KERNEL) > 0 ) {
                    rc = _do_bootm_addr((const void *) PARTITION_RESCUE_ADDR);
                } else {
                    printf ("## Rescue Partition %d image invalid\n", IMG_RESCUE);
                }            
    
                selPartition = 2;
            }else
                printf ("## No any partitions can be boot ...\n");
        }
#endif /* end of CONFIG_SENAO_RESCUE_IMAGE */        
    }
    else if (IMG_PART0 == index)
    {
        ...
        if (PARTITION_SIZE(FLASH_INDEX_KERNEL) > 0 ) {
            ...
            if (_do_boota_nand_img_check(PARTITION_NAND_ADDR_0))
            {
                printf ("## NAND Partition %d image invalid\n", IMG_PART0);
                return 1;
            }        
        }
#else 
        ...
        rc = _do_bootm_addr((const void *) PARTITION_ADDR_0);
            ...
            /* SENAO Frank
               _do_boota_nand_img_check have copy flash to RUNTIME_SRC_ADDR_STR.
           */
            run = find_cmd("bootm");
            if (NULL == run) {
                printf("bootm command is not supported!\n");
    ...
    else if (IMG_PART1 == index)
    {
        ...
        if (PARTITION_SIZE(FLASH_INDEX_KERNEL) > 0 ) {
            ...
            if (_do_boota_nand_img_check(PARTITION_NAND_ADDR_1))
            {
                printf ("## NAND Partition %d image invalid\n", IMG_PART1);
                return 1;
            }        
...
#ifdef CONFIG_SENAO_RESCUE_IMAGE    	
        if (PARTITION_SIZE(FLASH_INDEX_RESCUE_KERNEL) > 0 ) {
            printf ("## Please enter correct index (0 or 1 (normal), 2 (Rescue))\n");
            return 1;
        }
#else /* else of CONFIG_SENAO_RESCUE_IMAGE */
    ...
    if (rc)
    {
        if (selPartition == IMG_PART0)
        {
            ...
            if (PARTITION_SIZE(FLASH_INDEX_KERNEL) > 0 ) {
                ...
                /*spi_nand_erase(PARTITION_NAND_ADDR_0, 0x1000);*/            
            }
#else
            eraseFlash(PARTITION_ADDR_0, 0x1000);
#endif
        ...
        else
        {
            ...
            if (PARTITION_SIZE(FLASH_INDEX_KERNEL) > 0 ) {
                ...
                /*spi_nand_erase(PARTITION_NAND_ADDR_1, 0x1000);*/            
            }
#else
            eraseFlash(PARTITION_ADDR_1, 0x1000);
#endif
...
#ifdef CONFIG_DUAL_IMAGE
#ifdef CONFIG_SENAO_RESET_CHG_PARTITION
void change_partition(void)
{
    int selPartition = 0;
    char *partStr = NULL;
    int partition = 0;
    ...
    if ( partition == IMG_PART1 )
    {
        selPartition = IMG_PART0;
    }
    else
    {
        selPartition = IMG_PART1;
    }

    /*SENAO Frank Auto save */
    sprintf(valStr, "%d", selPartition);
    setsys(SYSINFO_VAR_DUALACTPART, valStr);
    savesys();
    ...
    else if (2 == argc)
    {
        ...
        else if (argv[1][0] == '2')
        {
            if (PARTITION_SIZE(FLASH_INDEX_RESCUE_KERNEL) > 0 )
            {
                ...
                printf ("\r\n### No any Rescue image can be boot ...###\n");
        }
#endif /* end of CONFIG_SENAO_RESCUE_IMAGE */	    
        else
        {
