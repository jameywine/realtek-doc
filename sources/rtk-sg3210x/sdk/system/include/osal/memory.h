 ...
 *      osal_alloc_coherent
 * Description:
 *      Allocate uncached memory based on user require size.
 * Input:
 *      dev         - pointer to struct device (only required in Linux)
 *      size        - size of allocate memory
 * Output:
 *      dma_handle  - pointer to the bus addrsss for DMA
 * Return:
 *      NULL        - failed
 *      others      - pointer of the allocated memory area.
 * Note:
 *      Linux Kernel Mode -
 *               Implemented by using dma_alloc_coherent with GFP_ATOMIC flag.
 */
void *
osal_alloc_coherent(void *dev, uint32 size, uintptr *dma_handle, uint32 flag);

/* Function Name:
 *      osal_free_coherent
 * Description:
 *      Free the uncached memory buffer
 * Input:
 *      size        - size of allocate memory
 *      vaddr       - address of buffer that want to free
 *      dma_handle  - pointer to the bus addrsss for DMA
 * Output:
 *      None
 * Return:
 ...
 * Note:
 *      Linux Kernel Mode -
 *               Implemented by using dma_free_coherent.
 *               Should NOT called when IRQ disabled.
 */
void
osal_free_coherent(void *dev, uint32 size, void *vaddr, uintptr dma_handle);
 ...
 *      osal_memory_cleanup
 * Description:
 *      Free all memory that ever alllocated
 * Input:
 *      None
 * Output:
 *      None
 ...
 */
extern void
osal_memory_cleanup(void);

/* Function Name:
 *      osal_memory_baseAddr_p2v
 * Description:
 *      mapping physical base address to virtual base address
 * Input:
 ...
 */
extern uintptr
osal_memory_baseAddr_p2v(uint32 baseAddr);
