 ...
 * All rights reserved.
 *
 * Purpose : Related implementation of the RTL9311B board
 *
 * Feature : RTL9311B board database
 *
 */


static hwp_swDescp_t rtl9311b_3x8218e_4xge_swDescp = {
    ...
    .port.descp = {
        ...
        { .mac_id = 24, .attr = HWP_ETH,   .eth = HWP_XGE,  .medi = HWP_SERDES,  .sds = HWP_1SDS(3),       .phy_idx = HWP_NONE, .smi = HWP_NONE, .phy_addr = HWP_NONE, .led_c = 0,        .led_f = 3,       .led_layout = DOUBLE_SET, .phy_mdi_pin_swap = 0,},
        { .mac_id = 25, .attr = HWP_ETH,   .eth = HWP_XGE,  .medi = HWP_SERDES,  .sds = HWP_1SDS(4),       .phy_idx = HWP_NONE, .smi = HWP_NONE, .phy_addr = HWP_NONE, .led_c = 0,        .led_f = 3,       .led_layout = DOUBLE_SET, .phy_mdi_pin_swap = 0,},
        { .mac_id = 26, .attr = HWP_ETH,   .eth = HWP_XGE,  .medi = HWP_SERDES,  .sds = HWP_1SDS(5),       .phy_idx = HWP_NONE, .smi = HWP_NONE, .phy_addr = HWP_NONE, .led_c = 0,        .led_f = 3,       .led_layout = DOUBLE_SET, .phy_mdi_pin_swap = 0,},
        { .mac_id = 27, .attr = HWP_ETH,   .eth = HWP_XGE,  .medi = HWP_SERDES,  .sds = HWP_1SDS(6),       .phy_idx = HWP_NONE, .smi = HWP_NONE, .phy_addr = HWP_NONE, .led_c = 0,        .led_f = 3,       .led_layout = DOUBLE_SET, .phy_mdi_pin_swap = 0,},
        ...
        { .mac_id = RTK_OOB_IDX_TO_PORT(0), .attr = HWP_OOB, .eth = HWP_GE, .medi = HWP_COPPER,  .sds = HWP_1SDS(RTK_OOB_IDX_TO_SDS(0)), .phy_idx = 3, .smi = RTK_OOB_IDX_TO_SMI(0), .phy_addr =  0, .led_c = 4, .led_f = HWP_NONE,.led_layout = SINGLE_SET, .phy_mdi_pin_swap = 0, },
        { .mac_id = HWP_END },
    },  /* port.descp */
    ...
    .led.descp = {
        ...
        .led_definition_set[0].led[2] = 0x0A01,
        .led_definition_set[0].led[3] = 0x8000,
        .led_definition_set[0].led[4] = HWP_LED_END,
        .led_definition_set[3].led[0] = 0x0A01,
        .led_definition_set[3].led[1] = 0x0A20,
        .led_definition_set[3].led[2] = 0x0,
        .led_definition_set[3].led[3] = 0x8000,
        .led_definition_set[3].led[4] = HWP_LED_END,
        /* OOB LED */
        .led_definition_set[4].led[0] = (RTK_PHY_CTRL_LED_MODE_1G|RTK_PHY_CTRL_LED_MODE_100M|RTK_PHY_CTRL_LED_MODE_10M), /* as intr */
        .led_definition_set[4].led[1] = (RTK_PHY_CTRL_LED_MODE_1G),
        .led_definition_set[4].led[2] = (RTK_PHY_CTRL_LED_MODE_100M),
        .led_definition_set[4].led[3] = HWP_LED_END,      /* None */
    },/* led.descp */
 ...
 * hardware profile
 */
static hwp_hwProfile_t rtl9311b_3x8218e_4xge = {
    .identifier.name        = "RTL9311B_3x8218E_4XGE",
    .identifier.id          = HWP_RTL9311B_3x8218E_4XGE,
    ...
    .sw_count               = 1,
    .swDescp = {
        [0]                 = &rtl9311b_3x8218e_4xge_swDescp,
    }
};
