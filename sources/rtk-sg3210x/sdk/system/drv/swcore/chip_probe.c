...
#endif
#if (defined(CONFIG_SDK_RTL9330) || defined(CONFIG_RTL9330_SERIES))
...
#define RTL9311B_FAMILY_ID_MASK  0xfffff800
 ...
 * Data Declaration
 */
init_state_t chip_probe_init = INIT_NOT_COMPLETED;
rtk_chip_probe_chipId_t   rtk_chip_probe_info[RTK_MAX_NUM_OF_UNIT];

#if !defined(CONFIG_SDK_EXTERNAL_CPU)
#if defined(__BOOTLOADER__) || defined(__KERNEL__) || defined(__ATF__)
...
int drv_swcore_early_init = TRUE; /* TRUE: for early init procedure usage, FLASE: else */
...
#endif
#if (defined(CONFIG_SDK_RTL9311B) || defined(CONFIG_RTL9311B_SERIES))
static int32 _drv_swcore_cid9311b_get(uint32 unit, uint32 *pChip_id, uint32 *pChip_rev_id);
#endif
#if (defined(CONFIG_SDK_RTL9330) || defined(CONFIG_RTL9330_SERIES))
static int32 _drv_swcore_cid9330_get(uint32 unit, uint32 *pChip_id, uint32 *pChip_rev_id);
#endif
...
#endif
#if (defined(CONFIG_SDK_RTL9311B) || defined(CONFIG_RTL9311B_SERIES))
        _drv_swcore_cid9311b_get,
#endif
#if (defined(CONFIG_SDK_RTL9330) || defined(CONFIG_RTL9330_SERIES))
        _drv_swcore_cid9330_get,
#endif
};
    ...
    switch (model_info)
    {
        ...
        case RTL9313_CHIP_ID & 0xffff0000:
            *pChip_id = RTL9313_CHIP_ID;
            break;
        case RTL8392B_CHIP_ID & 0xfffffff0:
            *pChip_id = RTL8392B_CHIP_ID;
            break;
        default:
            return RT_ERR_FAILED;
    }
...
#if (defined(CONFIG_SDK_RTL9311B) || defined(CONFIG_RTL9311B_SERIES))
static int32
_drv_swcore_cid9311b_get(uint32 unit, uint32 *pChip_id, uint32 *pChip_rev_id)
{
    uint32 model_info = 0;

#if defined(__MODEL_USER__)
    *pChip_id = model_info = RTL9311BM_CHIP_ID;
    *pChip_rev_id = CHIP_REV_ID_A;
#else
    drv_swcore_reg32_read(unit, RTL9311B_MODEL_NAME_INFO_ADDR, &model_info);

    *pChip_rev_id = (model_info & RTL9311B_MODEL_NAME_INFO_RTL_VID_MASK) >> RTL9311B_MODEL_NAME_INFO_RTL_VID_OFFSET;

    if( *pChip_rev_id > RTL9311B_LASTEST_REV)
        *pChip_rev_id = RTL9311B_LASTEST_REV;

    model_info &= 0xFFFFFFC0;
    switch (model_info)
    {
        case RTL9311BR_CHIP_ID & 0xFFFFFFC0:
            *pChip_id = RTL9311BR_CHIP_ID;
            break;
        case RTL9311BM_CHIP_ID & 0xFFFFFFC0:
            *pChip_id = RTL9311BM_CHIP_ID;
            break;
        default:
            osal_printf("%s %x\n", __func__, model_info);
            return RT_ERR_FAILED;
    }
    if (*pChip_rev_id == CHIP_REV_ID_A && *pChip_id == RTL9311BR_CHIP_ID)
    {
        *pChip_id = RTL9311BM_CHIP_ID;
    }
...
#endif
    return RT_ERR_OK;
} /* end of _bsp_drv_swcore_cid9311b_get */
#endif /*9311b series*/

#if (defined(CONFIG_SDK_RTL9330) || defined(CONFIG_RTL9330_SERIES))
static int32
_drv_swcore_cid9330_get(uint32 unit, uint32 *pChip_id, uint32 *pChip_rev_id)
{
    uint32 model_info = 0;

#if defined(__MODEL_USER__)
    *pChip_id = model_info = RTL9330_CHIP_ID;
    *pChip_rev_id = CHIP_REV_ID_A;
#else
    drv_swcore_reg32_read(unit, RTL9330_MODEL_NAME_INFO_ADDR, &model_info);
    ...
    *pChip_rev_id = model_info & 0xF;
    ...
    switch (model_info)
    {
        case RTL9330_CHIP_ID & 0xffff0000:
            *pChip_id = RTL9330_CHIP_ID;
            break;
        default:
            osal_printf("%s %x unit %d\n", __func__, model_info, unit);
            return RT_ERR_FAILED;
    }
#endif

    return RT_ERR_OK;
} /* end of _bsp_drv_swcore_cid9330_get */
#endif /*9330 series*/
...
drv_swcore_chipFamilyId_get(uint32 chip_id)
{
    ...
    if(RTL8392B_CHIP_ID == chip_id)
    {
        return RTL9310_FAMILY_ID;
    }
    if((RTL9311BM_CHIP_ID & RTL9311B_FAMILY_ID_MASK) == (chip_id & RTL9311B_FAMILY_ID_MASK))
    {
        return RTL9311B_FAMILY_ID;
    }
...
drv_swcore_chip_probe(uint32 unit)
{
    ...
    if (INIT_NOT_COMPLETED == chip_probe_init)
    {
        osal_memset(rtk_chip_probe_info, 0x0, sizeof(rtk_chip_probe_info));
        chip_probe_init = INIT_COMPLETED;
    }

    ret = drv_swcore_cid_get(unit, &chip_id, &chip_rev_id);
    if (RT_ERR_OK != ret)
    {
        osal_printf("drv_swcore_chip_probe unit:%d fail", unit);
        return ret;
    }
    rtk_chip_probe_info[unit].chip_id = chip_id;
    rtk_chip_probe_info[unit].chip_rev = chip_rev_id;
    rtk_chip_probe_info[unit].chip_family = drv_swcore_chipFamilyId_get(chip_id);
