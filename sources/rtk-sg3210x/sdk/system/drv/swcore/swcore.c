...
drv_swcore_jtag_intf_set(uint32 unit, drv_swcore_jtag_intf_t intf_sel)
{
    ...
    switch(RTK_CHIP_FAMILY_ID(unit))
    {
   #if defined(CONFIG_SDK_RTL8390)
...
#endif
#if defined(CONFIG_SDK_RTL8390)
    if(RTK_8390_FAMILY_ID(unit))
    {
        *pFamily_id = RTL8390_FAMILY_ID;
    }
    else
    if(RTK_8350_FAMILY_ID(unit))
    {
...
#endif
#if defined(CONFIG_SDK_RTL8380)
    if(RTK_8380_FAMILY_ID(unit))
    {
        *pFamily_id = RTL8380_FAMILY_ID;
    }
    else
    if(RTK_8330_FAMILY_ID(unit))
    {
...
int32
drv_swcore_CPU_freq_get_9311b(uint32 unit, uint32 * pCPU_freq)
{
    return RT_ERR_OK;
}
...
_drv_swcore_uart0RegBase_get(uint32 unit, uint32 *pReg_base_offset, uint32 *pReg_num)
{
    ...
    if(RTK_9311B_FAMILY_ID(unit))
    {
        //TODO:fix compile error, delete RTL9311B_UART0_PBR_THR_DLL_ADDR
        *pReg_base_offset = 0;
        *pReg_num     = 7;
    }
    else
...
_drv_swcore_gpioRegBase_get(uint32 unit, uint32 *pReg_base_offset, uint32 *pReg_num)
{
    ...
    if(RTK_9311B_FAMILY_ID(unit))
    {
        *pReg_base_offset = RTL9311B_PABCD_CNR_ADDR;
        *pReg_num     = 6;
    }
    else
#endif
