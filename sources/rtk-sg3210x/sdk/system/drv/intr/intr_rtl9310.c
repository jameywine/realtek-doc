...
int32
rtl9310_intr_swcoreIsrSts_get(uint32 unit, drv_intr_isrBitmap_t *pisr_sts)
{
    uint32      reg_data = 0;
    int32       i, size;
    ...
    for (i = 0; i < size; i++)
    {
        ...
        {
            INTR_ISR_BITMAP_SET(*pisr_sts, rtl9310_isr_glb_reg_id_map[i].isr_id);
        }
    }
...
rtl9310_intr_swcoreSts_get(uint32 unit, drv_intr_isr_t isrId, drv_intr_data_t *pFunc_sts)
{
    ...
    {
        ioal_mem32_read(unit, RTL9310_ISR_PORT_LINK_STS_CHG_ADDR(0), &reg_data);
        RTK_PORTMASK_WORD_SET(pFunc_sts->u.portmask, 0, reg_data);
        ioal_mem32_write(unit, RTL9310_ISR_PORT_LINK_STS_CHG_ADDR(0), reg_data);

        ioal_mem32_read(unit, RTL9310_ISR_PORT_LINK_STS_CHG_ADDR(32), &reg_data);
        RTK_PORTMASK_WORD_SET(pFunc_sts->u.portmask, 1, reg_data);
        ioal_mem32_write(unit, RTL9310_ISR_PORT_LINK_STS_CHG_ADDR(32), reg_data);
        return RT_ERR_OK;
    }
    else if (isrId == INTR_ISR_EXT_GPIO)
    {
...
int32
rtl9310_intr_swcore_exit(uint32 unit)
{
    ...
    {
        rtl9310_intr_swcoreImrEnable_set(unit, isrId, DISABLED);
    }

    /* read clear status */
    rtl9310_intr_swcoreIsrSts_get(unit, &pisr_sts);
