...
int32
rtl9300_intr_swcoreIsrSts_get(uint32 unit, drv_intr_isrBitmap_t *pisr_sts)
{
    uint32      reg_data = 0;
    int32       i, size;
    ...
    for (i = 0; i < size; i++)
    {
        ...
        {
            INTR_ISR_BITMAP_SET(*pisr_sts, rtl9300_isr_glb_reg_id_map[i].isr_id);
        }
    }
...
int32
rtl9300_intr_swcore_exit(uint32 unit)
{
    ...
    {
        rtl9300_intr_swcoreImrEnable_set(unit, isrId, DISABLED);
    }

    /* read clear status */
    rtl9300_intr_swcoreIsrSts_get(unit, &pisr_sts);
