...
int32
rtl8380_intr_swcoreIsrSts_get(uint32 unit, drv_intr_isrBitmap_t *pisr_sts)
{
    uint32      reg_data = 0;
    int32       i, size;
    ...
    for (i = 0; i < size; i++)
    {
        ...
        {
            INTR_ISR_BITMAP_SET(*pisr_sts, rtl8380_isr_glb_reg_id_map[i].isr_id);
        }
    }
    return RT_ERR_OK;
}
...
int32
rtl8380_intr_swcore_exit(uint32 unit)
{
    ...
    {
        rtl8380_intr_swcoreImrEnable_set(unit, isrId, DISABLED);
    }

    /* read clear status */
    rtl8380_intr_swcoreIsrSts_get(unit, &pisr_sts);
