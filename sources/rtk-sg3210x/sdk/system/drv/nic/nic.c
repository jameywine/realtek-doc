...
static uintptr                 tx_desc_vaddr;
static uintptr                 tx_desc_phy_addr;
static uintptr                 rx_desc_vaddr;
static uintptr                 rx_desc_phy_addr;
...
nic_pkthdr_t        *pOri_RxPktHdr; //original RxPktHdr that don't shift address for alignment
nic_pkthdr_t        *pOri_TxPktHdr;
nic_pkthdr_t        *pRxPktHdr;
nic_pkthdr_t        *pTxPktHdr;
static uintptr      *dma_rx_base;
static uintptr      *dma_tx_base;
static uint32       aligned_pktHdr_size;
    ...
    do
    {
        ...
        if (pktHdr->more && jumboFlag[ringId] == FALSE)     /* Jumbo head */
        {
            ...
            if (RT_ERR_OK == _nic_init_conf.pkt_alloc(unit, gJumboSize, 0, &pPacket[ringId]))
            {
                ...
                osal_memcpy(pPacket[ringId]->data, (uint8 *)UNCACHE(pktHdr->packet->data),
                            pktHdr->buf_len);
            }
            else
            {
        ...
        else if (jumboFlag[ringId] == TRUE)     /* Jumbo other */
        {
            if (pPacket[ringId])
            {
                osal_memcpy(pPacket[ringId]->data + pPacket[ringId]->length,
                    (uint8 *)UNCACHE(pktHdr->packet->data), pktHdr->buf_len);
                pPacket[ringId]->length += pktHdr->buf_len;
        ...
        if (jumboFlag[ringId] == 0)
        {
            ...
            if (RT_ERR_OK == _nic_init_conf.pkt_alloc(unit, _nic_init_conf.pkt_size, 0, &pPacket[ringId]))
            {
                ...
                if (RTK_8380_30_FAMILY(unit))
                    pktHdr->buf_addr = (uint8 *)((uint32)(pPacket[ringId]->data) & 0x0fffffff);
                else
            ...
            {
                /* Reopen rx runout interrupt because there is a packet
                   allocation failure, should allocate again later. */
                _nic_intrMask_get(unit, NIC_RX_RUNOUT, &temp);
                temp |= (1 << ringId);
                _nic_intrMask_set(unit, NIC_RX_RUNOUT, temp);
            }
        }
        else
        {
        ...
        if ((ret = osal_cache_memory_flush((uint32)pktHdr, aligned_pktHdr_size)) != RT_ERR_OK)
        {
            _nic_init_conf.pkt_free(unit, pPacket[ringId]);
            NIC_RX_UNLOCK(unit, &spl_flags);
            return ret;
...
static int32
_nic_isr_mbRoutine(uint32 unit, uint32 ringId)
{
    uintptr temp;
    int32 ret = RT_ERR_FAILED;
    ...
    /* Update software current pointer */
    _nic_ringCurAddr_get(unit, NIC_DIR_RX, ringId, &temp);
    pNic_rxCDPIdx[ringId] = (uint32 *)temp;
    desc = pNic_rxCDPIdx[ringId];
    do
    {
        nic_pkthdr_t *pPktHdr;

        if ((*desc & NIC_RING_SWOWNBIT) != 0)
            goto next;

        /* Prepare to be reclaim */
        pPktHdr = (nic_pkthdr_t *)(*desc & NIC_ADDR_MASK);
        if (RTK_8380_30_FAMILY(unit))
        ...
        if (NULL == pPktHdr->packet)
        {
            ...
            if (RTK_8380_30_FAMILY(unit))
                pPktHdr->buf_addr = (uint8 *)((uint32)(pPacket->data) & 0x0fffffff);
            else
                pPktHdr->buf_addr = pPacket->data;
        }
        else
            pPacket = pPktHdr->packet;

        pPktHdr->buf_size = _nic_init_conf.pkt_size;
        pPktHdr->buf_len = 0;
        if ((ret = osal_cache_memory_flush((uint32)pPktHdr, aligned_pktHdr_size)) != RT_ERR_OK)
        {
...
next:
        desc++;
        if (desc == (pNic_rxFDPBase[ringId] + rxRingIdSize[ringId]))
            desc = pNic_rxFDPBase[ringId];
    } while (desc != pNic_rxRDPIdx[ringId]);

    /* Disable runout interrupt here because the runout state has been lifted. */
    _nic_intrMask_get(unit, NIC_RX_RUNOUT, &temp);
    temp &= ~(1 << ringId);
    _nic_intrMask_set(unit, NIC_RX_RUNOUT, temp);
    _nic_intrSts_set(unit, NIC_RX_RUNOUT, (1 << ringId));
...
static void
_nic_ring_buff_release(uint32 unit)
{
    uint32  i;
    ...
    {
        nic_pkthdr_t *pktHdr = (nic_pkthdr_t*)((uint8*)pRxPktHdr + i * aligned_pktHdr_size);
        if (pktHdr->packet != NULL)
        {
            _nic_init_conf.pkt_free(unit, pktHdr->packet);
            pktHdr->packet = NULL;
        }
    ...
    if (pRxPktHdr != NULL)
    {
        osal_free((void*)pOri_RxPktHdr);
        pRxPktHdr = NULL;
    }
    if (pTxPktHdr != NULL)
    {
        osal_free((void*)pOri_TxPktHdr);
        pTxPktHdr = NULL;
    }
    ...
    for (i = 0; i < rxRingNum; i++)
    {
        ...
        {
            osal_free_coherent(_nic_init_conf.dev, sizeof(uint32) * rxRingIdSize[i], pNic_rxFDPBase[i], dma_rx_base[i]);
            pNic_rxFDPBase[i] = NULL;
        }
    }
    ...
    for (i = 0; i < txRingNum; i++)
    {
        ...
        {
            osal_free_coherent(_nic_init_conf.dev, sizeof(uint32) * txRingIdSize[i], pNic_txFDPBase[i], dma_tx_base[i]);
            pNic_txFDPBase[i] = NULL;
        }
    }
    if (pNic_rxFDPBase != NULL)
    {
        osal_free((void*)pNic_rxFDPBase);
        pNic_rxFDPBase = NULL;
    }
    if (pNic_txFDPBase != NULL)
    {
        osal_free((void*)pNic_txFDPBase);
        pNic_txFDPBase = NULL;
    }
}
...
drv_nic_isr_handler(void *isr_param)
{
    ...
    if (cpu_iisr)
    {
        for (i = rxRingNum - 1; i >= 0; i--)
        {
            if (cpu_iisr & (0x1 << i))
            {
                _nic_intrSts_set(unit, NIC_RX_RUNOUT, 0x1 << i);
                _nic_isr_mbRoutine(unit, i);
            }
        }
    }
...
_nic_init(uint32 unit, drv_nic_initCfg_t *pInitCfg)
{
    ...
    _nic_init_conf.dev       = pInitCfg->dev;
    ...
    ptr = osal_alloc(sizeof(uint32*) * rxRingNum * 4);       /* 3: pNic_rxFDPBase, pNic_rxCDPIdx, pNic_rxRDPIdx and */
    if (NULL == ptr)
    {
        RT_LOG(LOG_DEBUG, MOD_NIC, "Error: Out of memory!");
        return RT_ERR_FAILED;
    }
    osal_memset(ptr, 0, sizeof(uint32*) * rxRingNum * 4);
    pNic_rxFDPBase = ptr;
    ptr += rxRingNum;
    pNic_rxCDPIdx = ptr;
    ptr += rxRingNum;
    pNic_rxRDPIdx = ptr;
    ptr += rxRingNum;
    dma_rx_base = (uintptr*)ptr;

    ptr = osal_alloc(sizeof(uint32*) * txRingNum * 4);
    if (NULL == ptr)
    {
        RT_LOG(LOG_DEBUG, MOD_NIC, "Error: Out of memory!");
        goto err_alloc;
    }
    osal_memset(ptr, 0, sizeof(uint32*) * txRingNum * 4);
    pNic_txFDPBase = ptr;
    ptr += txRingNum;
    pNic_txCDPIdx = ptr;
    ptr += txRingNum;
    pNic_txRDPIdx = ptr;
    ptr += txRingNum;
    dma_tx_base = (uintptr*)ptr;


    for (i = 0; i < rxRingNum; i++)
    {
        dma_addr_t dma_handle = 0;
        pNic_rxFDPBase[i] = osal_alloc_coherent(pInitCfg->dev, sizeof(uint32) * rxRingIdSize[i], (uintptr*)&dma_handle, 0);
        dma_rx_base[i] = (uintptr)dma_handle;
    ...
    {
        dma_addr_t dma_handle = 0;
        pNic_txFDPBase[i] = osal_alloc_coherent(pInitCfg->dev, sizeof(uint32) * txRingIdSize[i], (uintptr*)&dma_handle, 0);
        dma_tx_base[i] = (uintptr)dma_handle;
    ...
    //extend sizeof(nic_pkthdr_t) to CACHE_LINE_SIZE byte aligned
    aligned_pktHdr_size = (sizeof(nic_pkthdr_t) + CACHE_LINE_SIZE - 1) & ~(CACHE_LINE_SIZE - 1);
    pRxPktHdr = (nic_pkthdr_t*) osal_alloc(aligned_pktHdr_size * totalRxRingNum + CACHE_LINE_SIZE * 2);
    pTxPktHdr = (nic_pkthdr_t*) osal_alloc(aligned_pktHdr_size * totalTxRingNum + CACHE_LINE_SIZE * 2);
    ...
    osal_memset(pRxPktHdr, 0, aligned_pktHdr_size * totalRxRingNum + CACHE_LINE_SIZE * 2);
    osal_memset(pTxPktHdr, 0, aligned_pktHdr_size * totalTxRingNum + CACHE_LINE_SIZE * 2);
    RT_ERR_HDL(osal_cache_memory_flush((uint32)pRxPktHdr, aligned_pktHdr_size * totalRxRingNum + CACHE_LINE_SIZE * 2), err_alloc, ret);
    RT_ERR_HDL(osal_cache_memory_flush((uint32)pTxPktHdr, aligned_pktHdr_size * totalTxRingNum + CACHE_LINE_SIZE * 2), err_alloc, ret);
    pOri_RxPktHdr = pRxPktHdr;
    pOri_TxPktHdr = pTxPktHdr;
    pRxPktHdr = (nic_pkthdr_t *)(((uintptr)pRxPktHdr + CACHE_LINE_SIZE - 1) & ~(CACHE_LINE_SIZE - 1));
    pTxPktHdr = (nic_pkthdr_t *)(((uintptr)pTxPktHdr + CACHE_LINE_SIZE - 1) & ~(CACHE_LINE_SIZE - 1));
    ...
    for (j = 0; j < rxRingNum; j++)
    {
        for (k = 0; k < rxRingIdSize[j]; k++)
        {
            ...
            pPktHdr = (nic_pkthdr_t*)((uint8*)pRxPktHdr + i * aligned_pktHdr_size);
            pPktHdr->buf_size   = 0;
    ...
    for (j = 0; j < txRingNum; j++)
    {
        for (k = 0; k < txRingIdSize[j]; k++)
        {
            ...
            pPktHdr = (nic_pkthdr_t*)((uint8*)pTxPktHdr + i * aligned_pktHdr_size);
            pPktHdr->buf_size   = 0;
            pPktHdr->pkt_offset = 0;
            pPktHdr->buf_len    = 0;
    ...
    for (i = 0; i < rxRingNum; i++)
    {
        RT_ERR_HDL(_nic_isr_mbRoutine(unit, i), err_alloc, ret);
    }
...
err_alloc:
    for (i = 0; i < rxRingNum; i++)
    {
        ...
        {
            osal_free_coherent(pInitCfg->dev, sizeof(uint32) * rxRingIdSize[i], pNic_rxFDPBase[i], dma_rx_base[i]);
        }
    }
    for (i = 0; i < txRingNum; i++)
    {
        ...
        {
            osal_free_coherent(pInitCfg->dev, sizeof(uint32) * txRingIdSize[i], pNic_txFDPBase[i], dma_tx_base[i]);
        }
    }
    ...
    if (NULL != pOri_RxPktHdr)
        osal_free(pOri_RxPktHdr);
    if (NULL != pOri_TxPktHdr)
        osal_free(pOri_TxPktHdr);
...
drv_nic_exit(uint32 unit)
{
    ...
    _nic_intrMask_set(unit, NIC_TX_DONE, 0);
    _nic_intrMask_set(unit, NIC_TX_ALLDONE, 0);
    _nic_cpuL2FloodMask_remove(unit);
...
#ifndef __BOOTLOADER__
    osal_spl_spin_lock_destroy(spl_rx_lock);
    osal_spl_spin_lock_destroy(spl_tx_lock);
#endif
    ...
    if ((ret = osal_isr_unregister(RTK_DEV_NIC)) != RT_ERR_OK)
    {
        return ret;
    }
...
drv_nic_ringbuf_dump(uint32 unit, nic_dbg_opt_t *pOpt)
{
    ...
    if (pOpt->dir == NIC_DIR_RX)
    {
        ...
        else
        {
            ...
            for (j = 0; j < rxRingIdSize[pOpt->ringId]; j++)
                osal_printf("  rxFDPBase %u :(p)%#08x   (v)%#08x\n", pOpt->ringId, (uint32)(pNic_rxFDPBase[pOpt->ringId] + j), *(pNic_rxFDPBase[pOpt->ringId] + j));
            osal_printf("--------------------------------------------------------------------------------\n");
        }
        osal_printf("\n");
    }
    ...
    if (pOpt->dir == NIC_DIR_TX)
    {
        ...
        else
        {
            ...
            for (j = 0; j < txRingIdSize[pOpt->ringId]; j++)
                osal_printf("  txFDPBase %u :(p)%#08x   (v)%#08x\n", pOpt->ringId, (uint32)(pNic_txFDPBase[pOpt->ringId] + j), *(pNic_txFDPBase[pOpt->ringId] + j));
            osal_printf("--------------------------------------------------------------------------------\n");
        }
        osal_printf("\n");
    }
    ...
    osal_printf("CHIP_FAMILY_ID=0x%x]\n", RTK_CHIP_FAMILY_ID(unit));

    if (pOpt->dir == NIC_DIR_TX)
    {
        if (RTK_8380_30_FAMILY(unit) || RTK_8390_50_FAMILY(unit))
        {
...
drv_nic_desc_init(uint32 unit, uintptr vir_base, uintptr phy_base)
{
    ...
    tx_desc_vaddr = vir_base;
    rx_desc_vaddr = tx_desc_vaddr + NUM_TX_DESC * sizeof(nic_desc_t) * 4;
    tx_desc_phy_addr = phy_base;
    rx_desc_phy_addr = tx_desc_phy_addr + NUM_TX_DESC * sizeof(nic_desc_t) * 4;

#if defined(CONFIG_SDK_KERNEL_LINUX_USER_MODE)
        pkt_buf_vir_addres[unit]  = rx_desc_vaddr + NUM_RX_DESC * sizeof(nic_desc_t) * 12;
        pkt_phy_addr[unit]        = rx_desc_phy_addr + NUM_RX_DESC * sizeof(nic_desc_t) * 12;
#endif
