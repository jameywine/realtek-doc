...
_nic_isr_rxRoutine(uint32 unit, uint32 ringId)
{
    ...
    do
    {
        ...
        if (NULL == pktHdr->packet)
        {
            ...
            {
                RT_LOG(LOG_DEBUG, (MOD_NIC|MOD_DAL), "%s:%d:  Out of memory ! (alloc a new packet data buffer failed)", __FUNCTION__, __LINE__);
                reclaim_mbuf = FALSE;
...
_nic_isr_mbRoutine(uint32 unit, uint32 ringId)
{
    ...
    uint32 *desc;
     ...
     RT_LOG(LOG_DEBUG, MOD_NIC, "_nic_isr_mbRoutine(%d)", ringId);
    ...
    do
    {
        ...
        /* Prepare to be reclaim */
        pPktHdr = (nic_pkthdr_t *)((_desc_addr_krn2usr(unit, *desc) & NIC_ADDR_MASK));
        if (RTK_8380_30_FAMILY(unit))
...
err_pkt_alloc:
    /* Enable runout interrupt here because the runout state has not been
       lifted. */
    _nic_intrMask_set(unit, NIC_RX_RUNOUT, NIC_RX_RING_ALL);
    return RT_ERR_OK;
}
...
drv_nic_isr_handler(void *isr_param)
{
    ...
    if (cpu_iisr)
    {
        ...
        {
            /* Workaround for runout irq status reverse issue. */
            for (i = 0; i < rxRingNum; i++)
            {
                if (cpu_iisr & (0x1 << i))
                    _nic_isr_mbRoutine(unit, (rxRingNum - i - 1));
            }
        }
        else
        {
 ...
 *      8380, 8390, 9300, 9310
 * Note:
 *      Not support in user mode NIC driver.
 * Changes:
 *      [SDK_4.0.0]
 *          New added function.
 */
