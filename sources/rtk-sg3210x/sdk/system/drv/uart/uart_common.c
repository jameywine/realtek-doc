 ...
 * Data Declaration
 */
uart_reg_definition_t uart_reg[DRV_REG_END] =
{
    ...
    [DRV_REG_9311B] =
    {
        .uart1_rx_tx_div_lsb_addr.reg_addr = RTL9311B_UART1_DLL_RBR_THR_ADDR,
        .uart1_intr_en_div_msb_addr.reg_addr = RTL9311B_UART1_DLM_IER_ADDR,
        .uart1_line_sts_addr.reg_addr = RTL9311B_UART1_LSR_ADDR,
        .uart1_line_ctrl_addr.reg_addr = RTL9311B_UART1_LCR_ADDR,
        .uart1_intr_id_fifo_ctrl_addr.reg_addr = RTL9311B_UART1_IIR_FCR_ADDR,
        .uart1_modem_ctrl_addr.reg_addr = RTL9311B_UART1_MCR_ADDR,
    },
#endif
...
uart_init(uint32 unit, drv_uart_rxcallback_t rxcallback, drv_uart_txcallback_t txcallback)
{
    ...
    while ((serial_in(unit, UART_REG(unit).uart1_line_sts_addr.reg_addr) & UART_LSR_DR) != 0)
    {
        ...
        (void)Data;/*Ignore compile error.*/
    }

#if defined(UART1_USING_INTERRUPT) && !defined(CONFIG_SDK_EXTERNAL_CPU)
...
#endif
    {
        (void)val; /* prevent compile warning */
        return RT_ERR_OK;
    }
    return RT_ERR_OK;
}
...
uart_baudrate_set(uint32 unit, drv_uart_baudrate_t baudrate)
{
    ...
    if(RTK_9310_FAMILY_ID(unit) || RTK_8380_FAMILY_ID(unit) || RTK_8390_FAMILY_ID(unit))
    {
 ...
 */
int32
hsuart_baudrate_set(uint32 unit, drv_uart_baudrate_t baudrate)
{
    uint32 divisor;
    uint32 sysClk = 100 * 1000 * 1000;
    uint32 baudrate_val = 0;
    ...
    divisor = ((sysClk + ((16 * baudrate_val) / 2)) / (16 * baudrate_val));
