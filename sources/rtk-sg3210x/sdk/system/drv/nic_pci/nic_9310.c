...
extern void             *ioaddr;
static uintptr                 tally_vaddr;
static uintptr                 tally_phy_addr;
...
#if defined(CONFIG_SDK_KERNEL_LINUX_USER_MODE)
#define DMA_ADDR_USR2PHY(usr_addr, unit)    ((usr_addr - pkt_buf_vir_addres[unit]) + pkt_phy_addr[unit])
#endif
...
static int
rtl8168_alloc_rx_pktBuf(uint32 unit, drv_nic_pkt_t **ppPacket,
                     struct RxDesc *desc,
                     int rx_buf_sz,
                     u8 in_intr)
{
...
#if defined(CONFIG_SDK_KERNEL_LINUX_USER_MODE)
        mapping = DMA_ADDR_USR2PHY((uintptr)pPacket->data, unit);
#else
...
static u32
rtl8168_rx_fill(uint32 unit, u32 start, u32 end, u8 in_intr)
{
        u32 cur;
        ...
        for (cur = start; end - cur > 0; cur++) {
                ...
                ret = rtl8168_alloc_rx_pktBuf(unit, tp->Rx_skbuff + i,
                                           tp->RxDescArray + i,
                                           tp->rx_buf_sz,
                                           in_intr);
...
rtl8168_init_ring(uint32 unit)
{
        ...
        if (rtl8168_rx_fill(unit, 0, NUM_RX_DESC, 0) != NUM_RX_DESC)
                goto err_out;
    ...
    for (cnt = 0; cnt < NUM_RX_DESC; cnt++)
    {
        ...
        if (jumboFlag == 0 || (jumboFlag && (status & LastFrag) && jumboBuffAlloc))
        {
            ...
            else
            {
                ...
                ret = nic_rx_queueInfo_get(unit, schd_queId, &pRx_queue);
                if ((RT_ERR_OK == ret) && (pRx_queue->drop_thresh > 0) &&
                    (pRx_queue->count < pRx_queue->drop_thresh))
                {
...
#else
            mapping = DMA_ADDR_USR2PHY((uintptr)tp->Rx_skbuff[tp->cur_rx]->data, unit);
#endif
...
drv_nic_init(uint32 unit, drv_nic_initCfg_t *pInitCfg)
{
        ...
        RT_ERR_HDL(rtl8168_open(unit), err_alloc, rc);
