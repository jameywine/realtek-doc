 ...
 * $Date$
 *
 * Purpose : 9310/9300 SPI master to 9330 SPI slave
 *
 * Feature : The file have include the following module and sub-modules
 *           1) spi read and write from 9310/9300 SPI master to 9330 SPI slave
 *
 */
...
#include <private/drv/spi/spi_dev7.h>
...
#define SPI_DEV7_READY_NOT_READ_YET 999
int32
_rtl9300_spi_clk_set(uint32 unit,     uint32 spi_clk)
{
    ...
    switch (val)
    {
        ...
        case 0xD:
            ...
            osal_printf("System clock 150Mhz\n");
            break;
        default:
            system_clk = 400;
            osal_printf("System clock 400Mhz\n");
            break;
    }
    ...
    for (i = 0; i < (sizeof(rtl9300_spi_config)/sizeof(spi_para_t)); i++)
    {
        if((spi_clk == rtl9300_spi_config[i].spi_clk) && (system_clk == rtl9300_spi_config[i].system_clk))
        {
            ...
            osal_printf("unit %d clk %d\n", unit, spiCurr.clk_regVal);
...
int32
_spi_dev7Ready_wait(uint32 unit)
{
    uint32 value=SPI_DEV7_READY_NOT_READ_YET, retry=0;
    WAIT_COMPLETE_VAR();
    ...
    if((SPI_EXTRA_MDELAY)!=0)
        osal_time_udelay(SPI_EXTRA_MDELAY * 50 / 6);
...
void _rtl9300_spi_master_init(uint32 unit)
{
    ...
    hwp_identifier_get(&identifier);

    osal_printf("boardmoel %s\n", identifier.name);
    ...
    if(osal_strcmp(identifier.name, "RTL9300_RTL9310_FPGA") == 0)
    {
        osal_printf("ADDR WIDTH = 2\n");
        ioal_mem32_field_write(unit,RTL9300_SPI_CTRL1_ADDR,
            RTL9300_SPI_CTRL1_ADDR_WIDTH_OFFSET,RTL9300_SPI_CTRL1_ADDR_WIDTH_MASK, 0x2);
    }
    else
    {
        osal_printf("ADDR WIDTH = 3\n");
        ioal_mem32_field_write(unit,RTL9300_SPI_CTRL1_ADDR,
            RTL9300_SPI_CTRL1_ADDR_WIDTH_OFFSET,RTL9300_SPI_CTRL1_ADDR_WIDTH_MASK, 0x3);
    }
...
void _rtl9310_spi_master_init(uint32 unit)
{
    ...
    osal_printf("9310 ADDR WIDTH = 3\n");
     ioal_mem32_field_write(unit,RTL9310_SPI_CTRL0_ADDR,
         RTL9310_SPI_CTRL0_ADDR_WIDTH_OFFSET,RTL9310_SPI_CTRL0_ADDR_WIDTH_MASK, 0x3);
...
int32 spi_dev7Reg_read(uint32 unit, uint32 mAddrs, uint32 *buff)
{
    uint32 value, address_tmp;
    uint32 org_val, times = 0;
...
TRY_MULTI:
    ...
    if (RTK_9310_FAMILY_ID(master_unit))
    {
        ...
        /* check if ready */
        _spi_dev7Ready_wait(master_unit);
    ...
    if(unit == 1 && mAddrs == 4 && times <= 5)
    {
        if(*buff != 0x93300000)
        {
            SPI_MASTER_SEM_UNLOCK;

            osal_printf("FAILED %s unit %d addr %x val %x\n", __FUNCTION__, unit, mAddrs, *buff);
    ...
    //osal_printf("%s unit %d addr %x val %x\n", __FUNCTION__, unit, mAddrs, *buff);
    //osal_time_mdelay(200);

    return RT_ERR_OK;
}

int32 spi_dev7Reg_write(uint32 unit, uint32 mAddrs, uint32 *buff)
{
    uint32 master_unit = HWP_MY_UNIT_ID();
    uint32 org_val;
...
int32 spi_dev7_init(uint32 unit)
{
    uint32 val;
    uint32 clk, div;
    uint32 i, error;
    ...
    else
    {
        ...
        for(clk = 5; clk <= 7; clk ++) //try SPI clock base from 12.5M to 1Mhz
        {

            ioal_mem32_field_write(master_unit,RTL9300_SPI_CTRL1_ADDR,
                RTL9300_SPI_CTRL1_SPI_CLK_DIV_OFFSET, RTL9300_SPI_CTRL1_SPI_CLK_DIV_MASK, clk);

            error = 1;
            for(div = 0x1; div <= 0xF; div ++)
            {
                uint32 buff;
                if(clk == 6 && div > 1) // for clk= 5, only try div=1 (1.25M is almost the same as 1M)
                    continue;

                val = REG32(0xBB00E220);
                osal_printf("0xBB00E220: %x\n", REG32(0xBB00E220));
                val = (val & ~(0xF << 5)) | (div << 5);
                REG32(0xBB00E220) = val;
                osal_printf("CLK %d DIV %x 0xBB00E220: %x\n", clk, div, REG32(0xBB00E220));

                error = 0;
                for(i = 0; i < 10; i ++)
                {
                    spi_dev7Reg_read(1, 4, &buff);
                    if((buff & 0xFFFF0000) != 0x93300000)
                    {
                        error = 1;
                        break;
                    }
                ...
                if(error == 0)
                {
                    osal_printf("Succeed! CLK %d DIV %x: %x\n", clk, div, buff);
                    break;
                }
            }
