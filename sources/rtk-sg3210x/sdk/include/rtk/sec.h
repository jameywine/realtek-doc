...
typedef enum rtk_sec_ipMacBindFlag_e
{
    ...
    RTK_SEC_IPMACBIND_FLAG_BIND_SMAC        = 0x00000008,
...
typedef enum rtk_sec_attackType_e
{
    ...
    SYNFIN_DENY,
    SYNRST_DENY,
    TCP_FRAG_OFF_MIN_CHECK,     /* Applicable to 8380, 8390, 9300, 9310, 9330 */
    ARP_INVALID,                            /* Applicable to 8380, 8390, 9300, 9310, 9330 */
    GRATUITOUS_ARP,                     /* Applicable to 8380, 8390 */
    IPV4_INVALID_LEN,                   /* Applicable to 9300 */
    IPV4_INVALID_HDR,                   /* Applicable to 9310, 9330 */
    ATTACK_TYPE_END
} rtk_sec_attackType_t;

#define RTK_SEC_ATTACK_TYPE_NAME \
    { \
        "DMAC EQ SMAC", \
        "LAND", \
        "UDP BLAT", \
        "TCP BLAT", \
        "POD", \
        "IPv6 min fragment size", \
        "ICMP fragment packet", \
        "ICMPv4 ping packet over max length", \
        "ICMPv6 ping packet over max length", \
        "SMURF", \
        "TCP header min length", \
        "SYN=1 and source port < 1024", \
        "NULL SCAN", \
        "XMAS", \
        "SYN=1 and FIN=1", \
        "SYN=1 and RST=1", \
        "Fragment packet with too small TCP header", \
        "Invalid ARP", \
        "Gratuitous ARP", \
        "IPv4 packet with invalid length", \
        "Invalid IPv4 header", \
        "End" \
    }

typedef struct rtk_sec_attackHitSts_s
{
    rtk_bitmap_t    hit_sts[BITMAP_ARRAY_CNT(ATTACK_TYPE_END)];
} rtk_sec_attackHitSts_t;
 ...
 *          - rtk_sec_attackType_t \ Chip:  8390    8380    9300    9310, 9330
 *          - GRATUITOUS_ARP                O       O       X       X
 *          - ARP_INVALID                   O       O       O       O
 *      (2) Supported action is as following:
 *          - rtk_action_t \ Chip:          8390    8380    9300    9310, 9330
 ...
 * Description:
 *      Get hit indication for all attack types.
 * Input:
 *      unit            - unit id
 * Output:
 ...
 */
extern int32
rtk_sec_attackPreventHit_get(uint32 unit, rtk_sec_attackHitSts_t *pHit);

/* Function Name:
 *      rtk_sec_attackPreventCnt_get
 * Description:
 *      Get counter for each kind of attack.
 * Input:
 *      unit            - unit id
 *      attack_type     - type of attack
 * Output:
 *      pCnt            - pointer to counter for attack
 * Return:
 *      RT_ERR_OK
 *      RT_ERR_FAILED
 *      RT_ERR_NOT_INIT         - The module is not initial
 ...
 */
extern int32
rtk_sec_attackPreventCnt_get(uint32 unit,
    rtk_sec_attackType_t attack_type, uint32 *pCnt);

/* Function Name:
 *      rtk_sec_attackPreventCnt_clear
 * Description:
 *      Clear counter for each kind of attack.
 * Input:
 *      unit            - unit id
 *      attack_type     - type of attack
 * Output:
 ...
 */
extern int32
rtk_sec_attackPreventCnt_clear(uint32 unit,
    rtk_sec_attackType_t attack_type);
