 ...
 *
 * $Revision: 104897 $
 * $Date: 2020-03-10 14:50:12 +0800 (Tue, 10 Mar 2020) $
 *
...
typedef enum rtk_l3_globalCtrlType_e
{
    ...
    RTK_L3_GCT_IPUC_URPF_GLB_EN,                        /* rtk_enable_t */
    RTK_L3_GCT_IP6UC_URPF_GLB_EN,                       /* rtk_enable_t */
    RTK_L3_GCT_PBR_DMAC_CHK_BYPASS,                     /* rtk_enable_t */
...
typedef enum rtk_l3_dst_if_type_e
{
    RTK_L3_DST_IF_TYPE_DPORT = 0,    /* { 4-bit dev, 6-bit port } */
    RTK_L3_DST_IF_TYPE_TRUNK,        /* LAG/TRUNK id */
    RTK_L3_DST_IF_TYPE_MC,           /* MC_IDX (for multiple egress ports use) */
    RTK_L3_DST_IF_TYPE_L2_TNL,       /* L2_TNL_IDX (for L2/VXLAN tunnel, only 0~255+16-1 are available) */
    RTK_L3_DST_IF_TYPE_END
} rtk_l3_dst_if_type_t;
...
typedef struct rtk_l3_nextHop_s
{
    ...
    rtk_l3_dst_if_type_t dst_if_type;
    union {
        struct{
            uint32 dev;
            uint32 port;
 ...
 *      9300, 9310, 9311B
 * Note:
 *      This API is used to perform software aging (should be called once per user-defined interval).
 *      It will scan all entries and either delete aged entry (which hit status is clear) or
 *      clear the hit status if its hit status is set (as being referred after last check).
 *      During the scanning period, the callback function will be called per aged entry if available.
 *      (1) Applicable flags:
 *          RTK_L3_FLAG_IPV6    - to indicate the entry format is IPv6 rather than IPv4
 *          RTK_L3_FLAG_HIT     - if set, only clear the hit bits of the traversed entries when
 *                                their hit bits are set (will NOT remove aged entries from H/W).
 * Changes:
 *      [SDK_3.0.0]
 *          New added function.
 */
...
/* Function Name:
 *      rtk_l3_routeRsvMcRpfId_get
 * Description:
 *      Get routing reserve multicast entry with rpf id info
 * Input:
 *      unit         - unit id
 * Output:
 *      pId          - pointer to rpf id
 * Return:
 *      RT_ERR_OK
 *      RT_ERR_FAILED
 *      RT_ERR_UNIT_ID      - invalid unit id
 ...
 */
extern int32
rtk_l3_routeRsvMcRpfId_get(uint32 unit, rtk_vlan_t *pId);

/* Function Name:
 *      rtk_l3_routeRsvMcRpfId_set
 * Description:
 *      Set routing reserve multicast entry with rpf id info
 * Input:
 *      unit         - unit id
 *      id           - rpf id
 * Output:
 ...
 */
extern int32
rtk_l3_routeRsvMcRpfId_set(uint32 unit, rtk_vlan_t id);
