...
typedef enum rtk_switch_rstCtrl_e {
    RTK_SWITCH_RSTCTRL_RSTIO_WD_RST_EN = 0,       /* Enable to send reset low signal to RST I/O pin when watchdog triggers reset */
    RTK_SWITCH_RSTCTRL_RSTIO_CHIP_RST_EN,         /* Enable to send reset low signal to RST I/O pin when software chip reset is triggered */
    RTK_SWITCH_RSTCTRL_END,
} rtk_switch_rstCtrl_t;

typedef enum rtk_switch_rstType_e
{
    RTK_SWITCH_RSTTYPE_SW_CHIP,
    RTK_SWITCH_RSTTYPE_END
} rtk_switch_rstType_t;
...
typedef enum rtk_switch_maxPktLen_linkSpeed_e
{
    ...
    MAXPKTLEN_LINK_SPEED_FE_RX = MAXPKTLEN_LINK_SPEED_FE,
    MAXPKTLEN_LINK_SPEED_1G_2P5G_5G_10G_RX = MAXPKTLEN_LINK_SPEED_GE,
    MAXPKTLEN_LINK_SPEED_25G_50G_100G_RX,
    MAXPKTLEN_LINK_SPEED_FE_TX,
    MAXPKTLEN_LINK_SPEED_1G_2P5G_5G_10G_TX,
    MAXPKTLEN_LINK_SPEED_25G_50G_100G_TX,
...
/* thermal parameter for different temperature range */
typedef enum rtk_switch_thermalPara_e
{
    THERMAL_PARA_NORMAL_TEMP = 0,
    THERMAL_PARA_HIGH_TEMP,
    THERMAL_PARA_LOW_TEMP,
    THERMAL_PARA_END,
} rtk_switch_thermalPara_t;
...
/* Function Name:
 *      rtk_switch_rstCtrl_get
 * Description:
 *      Get switch reset-control specific setting
 * Input:
 *      unit - unit id
 *      ctrl_type - reset control type
 * Output:
 ...
 *      RT_ERR_NOT_INIT     - The module is not initial
 *      RT_ERR_NULL_POINTER - input parameter may be null pointer
 *      RT_ERR_NOT_SUPPORTED     - functions not supported
 * Applicable:
 *      9311B
 * Note:
 *      None
 ...
 */
extern int32
rtk_switch_rstCtrl_get(uint32 unit, rtk_switch_rstCtrl_t ctrl_type, uint32 *pValue);

/* Function Name:
 *      rtk_switch_rstCtrl_set
 * Description:
 *      Set switch reset-control specific setting
 * Input:
 *      unit - unit id
 *      ctrl_type - reset control type
 *      value    - setting value
 ...
 */
extern int32
rtk_switch_rstCtrl_set(uint32 unit, rtk_switch_rstCtrl_t ctrl_type, uint32 value);

/* Function Name:
 *      rtk_switch_reset
 * Description:
 *      Do switch reset function
 * Input:
 *      unit - unit id
 *      type - reset type
 * Output:
 ...
 */
extern int32
rtk_switch_reset(uint32 unit, rtk_switch_rstType_t type);

/* Function Name:
 *      rtk_switch_thermalTemp_get
 * Description:
 *      Get device temperature from thermal meter of the specific unit.
 * Input:
 *      unit - unit id
 *      thermal_id - thermal meter id
 *      para - thermal parameter
 * Output:
 *      pTemp - temperature of the specific thermal,
 *              the value in degrees Celsius is *pTemp / 1024.
 * Return:
 *      RT_ERR_OK
 *      RT_ERR_FAILED
 *      RT_ERR_NOT_INIT     - The module is not initial
 ...
 */
extern int32
rtk_switch_thermalTemp_get(uint32 unit, uint32 thermal_id, rtk_switch_thermalPara_t para, int32 *pTemp);
