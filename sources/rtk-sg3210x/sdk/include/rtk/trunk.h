...
#if defined(CONFIG_SDK_RTL9300) || defined(CONFIG_SDK_RTL9310) || defined(CONFIG_SDK_RTL9311B) || defined(CONFIG_SDK_RTL9311B)
...
/*
 * Trunk Flexible hash keys of Distribution algorithm. definition for 9330 and later on chips
 */
#define TRUNK_DIST_ALGO_MSK_SP_BIT                 (0x1 << 0)      /* Hash key include source port */
#define TRUNK_DIST_ALGO_MSK_SMAC_BIT                (0x1 << 1)      /* Hash key include source MAC address */
#define TRUNK_DIST_ALGO_MSK_DMAC_BIT                (0x1 << 2)      /* Hash key include destination MAC address */
#define TRUNK_DIST_ALGO_MSK_VLAN_BIT                (0x1 << 3)      /* Hash key include VLAN index */
#define TRUNK_DIST_ALGO_MSK_SIP_BIT                 (0x1 << 4)      /* Hash key include source IP */
#define TRUNK_DIST_ALGO_MSK_DIP_BIT                 (0x1 << 5)      /* Hash key include dest IP  */
#define TRUNK_DIST_ALGO_MSK_SRC_L4PORT_BIT          (0x1 << 6)      /* Hash key include layer-4 source port  */
#define TRUNK_DIST_ALGO_MSK_DST_L4PORT_BIT          (0x1 << 7)      /* Hash key include layer-4 dest port */
#define TRUNK_DIST_ALGO_MSK_PROTO_BIT               (0x1 << 8)      /* Hash key include protocol ID */
#define TRUNK_DIST_ALGO_MSK_FLOW_LABEL_BIT          (0x1 << 9)      /* Hash key include IPv6 flow label */
#define TRUNK_DIST_ALGO_MSK_ETHER_TYPE_BIT          (0x1 << 10)     /* Hash key include ether type */
#define TRUNK_DIST_ALGO_MSK_GRE_KEY_BIT             (0x1 << 11)     /* Hash key include GRE key */
#define TRUNK_DIST_ALGO_MSK_MPLS_LABEL_BIT          (0x1 << 12)     /* Hash key include MPLS label */
#define TRUNK_DIST_ALGO_MSK_TEID_BIT                (0x1 << 13)     /* Hash key include TEID */
#define TRUNK_DIST_ALGO_MSK_ECID_BIT                (0x1 << 14)     /* Hash key include ECID */
#define TRUNK_DIST_ALGO_MSK_VNI_BIT                 (0x1 << 15)     /* Hash key include VNI */
#define TRUNK_DIST_ALGO_MSK_TNL_AUX_DATA_BIT        (0x1 << 16)     /* Hash key include tunnel aux data */

/*
 * Distribution algorithm select
 */
#define TRUNK_DIST_ALGO_SEL_SA_VLAN_ETH_GIP                0   /* Source MAC, VLAN index, Ether Type, global ingress port */
#define TRUNK_DIST_ALGO_SEL_DA_VLAN_ETH_GIP                1   /* Destination MAC, VLAN index, Ether Type, global ingress port */
#define TRUNK_DIST_ALGO_SEL_SA_DA_VLAN_ETH_GIP             2   /* Source and destination MAC, VLAN index, Ether Type, global ingress port */
#define TRUNK_DIST_ALGO_SEL_SIP_L4SPORT                    3   /* Source IP, Layer-4 source port */
#define TRUNK_DIST_ALGO_SEL_DIP_L4DPORT                    4   /* Destination IP, Layer-4 destination port */
#define TRUNK_DIST_ALGO_SEL_SIP_DIP_L4SPORT_L4DPORT        5   /* Source and destination IP, Layer-4 source and destination port */
#define TRUNK_DIST_ALGO_SEL_FLEXIBLE_HASH                  6   /* Flexible Hash */
...
typedef struct  rtk_trunk_distAlgoShift_s
{
    ...
    uint32 flex_tnl_aux_data_shift;
    uint32 teid_shift;
    uint32 vni_shift;
    uint32 ecid_shift;
    uint32 mpls_label_shift;
    uint32 gre_key_shift;
    uint32 eth_shift;
#endif
...
/* Function Name:
 *      rtk_trunk_distributionAlgorithmSelect_get
 * Description:
 *      Get the trunk distribution algorithm selection
 * Input:
 *      unit       - unit id
 *      trk_gid    - trunk group id
 * Output:
 *      pAlgo_sel  - distribution algorithm selection
 * Return:
 *      RT_ERR_OK
 *      RT_ERR_FAILED
 *      RT_ERR_UNIT_ID       - invalid unit id
 ...
 */
extern int32
rtk_trunk_distributionAlgorithmSelect_get(uint32 unit, rtk_trk_t trk_gid, uint32 *pAlgo_sel);

/* Function Name:
 *      rtk_trunk_distributionAlgorithmSelect_set
 * Description:
 *      Set the trunk distribution algorithm selection
 * Input:
 *      unit       - unit id
 *      trk_gid    - trunk group id
 *      algo_sel  - distribution algorithm selection
 * Output:
 *      None
 * Return:
 *      RT_ERR_OK
 ...
 */
extern int32
rtk_trunk_distributionAlgorithmSelect_set(uint32 unit, rtk_trk_t trk_gid, uint32 algo_sel);
