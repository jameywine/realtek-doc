...
/* Direction of Mirror sampling */
typedef enum rtk_mirror_sampleDirection_e
{
    MIRROR_SAMPLE_DIRECTION_RX,
    MIRROR_SAMPLE_DIRECTION_TX,
    MIRROR_SAMPLE_DIRECTION_END
} rtk_mirror_sampleDirection_t;
...
/* Mirror Encap Type */
typedef enum rtk_mirror_encap_type_e
{
    MIRROR_ENCAP_TYPE_SPAN = 0,
    MIRROR_ENCAP_TYPE_RSPAN,
    MIRROR_ENCAP_TYPE_ERSPAN,
    MIRROR_ENCAP_TYPE_END
} rtk_mirror_encap_type_t;
...
typedef enum rtk_mirror_erspan_type_e
{
    MIRROR_ERSPAN_TYPE_I = 0,
    MIRROR_ERSPAN_TYPE_II,
    MIRROR_ERSPAN_TYPE_III,
    MIRROR_ERSPAN_TYPE_END
} rtk_mirror_erspan_type_t;
...
typedef enum rtk_mirror_tag_status_e
{
    MIRROR_TAG_STATUS_UNTAG,
    MIRROR_TAG_STATUS_ITAG,
    MIRROR_TAG_STATUS_OTAG,
    MIRROR_TAG_STATUS_DOUBLE_TAG,
    MIRROR_TAG_STATUS_END
} rtk_mirror_tag_status_t;

typedef enum rtk_mirror_ref_tag_status_e
{
    MIRROR_REF_TAG_STATUS_NONE,
    MIRROR_REF_TAG_STATUS_RX,
    MIRROR_REF_TAG_STATUS_TX,
    MIRROR_REF_TAG_STATUS_BOTH,
    MIRROR_REF_TAG_STATUS_END
} rtk_mirror_ref_tag_status_t;
...
typedef struct rtk_mirror_entry_s
{
    ...
    rtk_mirror_encap_type_t     mir_encapType;
#endif

#if defined(CONFIG_SDK_RTL9310) || defined(CONFIG_SDK_RTL9311B) || defined(CONFIG_SDK_RTL9300) || defined(CONFIG_SDK_RTL9330)
    ...
    /* Mirror Option to control the mirror traffic */
    rtk_mirror_op_t             oper_of_igr_and_egr_ports;
    rtk_enable_t                mirror_orginalPkt;
    rtk_mirror_egrMode_t        mirroring_port_egrMode;
#if defined(CONFIG_SDK_RTL8380) || defined(CONFIG_SDK_RTL9300) || defined(CONFIG_SDK_RTL9310) || defined(CONFIG_SDK_RTL9311B) || defined(CONFIG_SDK_RTL9330)
    rtk_enable_t                self_flter;
    rtk_enable_t                duplicate_fltr;
    rtk_mirror_vlan_mode_t      mir_mode;
#endif
#if defined(CONFIG_SDK_RTL9330)
    rtk_enable_t                mir_decapErspanHdr;
    rtk_mirror_tag_status_t     mir_tagStatus;
    rtk_mirror_ref_tag_status_t mir_refTagStatus;
#endif
...
/* Encapsulation field */
typedef struct rtk_mirror_encap_s
{
    /* MAC header info */
    rtk_mac_t                   da;
    rtk_mac_t                   sa;
    ...
    rtk_mirror_erspan_type_t    erspanType;
    union
    {
        /* ERSPAN Type I - only IP + GRE hdr (38B: 14B(MAC) + 20B(IP) + 4B(GRE)) */

        /* ERSPAN Type II */
        struct erspanTypeII_s
        {
            /* GRE header info */
            uint32              seqNum;
 ...
 *      8380, 8390, 9300, 9310, 9311B, 9330
 * Note:
 *      (1) The valid range of mirror_id is 0~3 in 8380, 8390, 9300, 9310, 9330.
 * Changes:
 *      [SDK_3.0.0]
...
/* Module Name    : Mirror */
/* Sub-module Name: Encap (ERSPAN) */

/* Function Name:
 *      rtk_mirror_encap_get
 * Description:
 *      Get content of encapulation info on specified mirror group.
 * Input:
 *      unit        - unit id
 *      mirror_id   - mirror id
 * Output:
 *      pEncap      - pointer to content of encap info
 * Return:
 *      RT_ERR_OK
 *      RT_ERR_FAILED
 *      RT_ERR_UNIT_ID      - invalid unit id
 ...
 */
extern int32
rtk_mirror_encap_get(uint32 unit, uint32 mirror_id, rtk_mirror_encap_t *pEncap);

/* Function Name:
 *      rtk_mirror_rspanTag_set
 * Description:
 *      Set content of encapulation info on specified mirroring group.
 * Input:
 *      unit        - unit id
 *      mirror_id   - mirror id
 *      pEncap      - content of encap info
 * Output:
 *      None
 * Return:
 *      RT_ERR_OK
 ...
 */
extern int32
rtk_mirror_encap_set(uint32 unit, uint32 mirror_id, rtk_mirror_encap_t *pEncap);
 ...
 *      8380, 8390
 * Note:
 *      9300, 9310, 9330 donot support this featrue no longer.
 ...
 *      unit      - unit id
 *      mirror_id - mirror id
 *      direction - direction of sampling
 * Output:
 *      pRate     - pointer to sampling rate
 * Return:
 *      RT_ERR_OK
 ...
 */
extern int32
rtk_mirror_sampleRate_get(uint32 unit, uint32 mirror_id, rtk_mirror_sampleDirection_t direction, uint32 *pRate);
 ...
 */
extern int32
rtk_mirror_sampleRate_set(uint32 unit, uint32 mirror_id, rtk_mirror_sampleDirection_t direction, uint32 rate);
