...
#define RTK_PIE_BLOCKMASK_BLOCK_SET(blockmask, block) \
do {\
    if ((block) <= RTK_MAX_NUM_OF_PIE_BLOCKS) {BITMAP_SET((blockmask).bits, (block));}\
} while (0)

#define RTK_PIE_BLOCKMASK_BLOCK_CLEAR(blockmask, block) \
do {\
    if ((block) <= RTK_MAX_NUM_OF_PIE_BLOCKS) {BITMAP_CLEAR((blockmask).bits, (block));}\
} while (0)

#define RTK_PIE_BLOCKMASK_IS_BLOCK_SET(blockmask, block) \
    (((block) < RTK_MAX_NUM_OF_PIE_BLOCKS) ? BITMAP_IS_SET((blockmask).bits, (block)): 0)

#define RTK_PIE_BLOCKMASK_SCAN(blockmask, block)       \
    for (block = 0; block <= RTK_MAX_NUM_OF_PIE_BLOCKS; block++) \
        if(RTK_PIE_BLOCKMASK_IS_BLOCK_SET(blockmask, block))

#define RTK_PIE_BLOCKMASK_GET_LAST_BLOCK(blockmask) \
    rt_bitop_findLastBitInAaray((blockmask).bits, RTK_TOTAL_NUM_OF_WORD_FOR_1BIT_PIE_BLOCK)
...
{
#if defined(CONFIG_SDK_RTL8390) || defined(CONFIG_SDK_RTL9300) || defined(CONFIG_SDK_RTL9310) || defined(CONFIG_SDK_RTL9311B)
...
#endif  /* defined(CONFIG_SDK_RTL8390) || defined(CONFIG_SDK_RTL9300) || defined(CONFIG_SDK_RTL9310) || defined(CONFIG_SDK_RTL9311B) */
...
/* Function Name:
 *      rtk_pie_blockGroupId_set
 * Description:
 *      Set the group id of multiple blocks. This is commonly used to change
 *      group id for multiple configured blocks.
 * Input:
 *      unit       - unit id
 ...
 *          the entry with lowest index.
 *      (2) Group blocks which belong to different phase is forbidden.
 *      (3) Per block logic id should be set individually
 * Changes:
 *      [SDK_4.0.3]
 *          New added function.
 */
extern int32
rtk_pie_blockGroupId_set(uint32 unit, rtk_pie_blockmask_t block_mask, uint32 group_id);
