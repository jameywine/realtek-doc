...
#define RTK_MAX_SIZE_OF_ACL_ENTRY           60  /* maximum entry size among chips*/
...
#if (defined(CONFIG_SDK_RTL9300) || defined(CONFIG_SDK_RTL9310) || defined(CONFIG_SDK_RTL9311B))
...
#endif  /* (defined(CONFIG_SDK_RTL9300) || defined(CONFIG_SDK_RTL9310) || defined(CONFIG_SDK_RTL9311B)) */
...
#endif
#if defined(CONFIG_SDK_RTL9310) || defined(CONFIG_SDK_RTL9311B) || defined(CONFIG_SDK_RTL9330)
...
typedef enum rtk_acl_statBlockMode_e
{
    STAT_BLOCK_MODE_STAT = 0,
    STAT_BLOCK_MODE_IPFIX,
    STAT_BLOCK_MODE_IPFIX_LATENCY,
    STAT_BLOCK_MODE_END
} rtk_acl_statBlockMode_t;
...
typedef enum rtk_acl_internalPriAct_e
{
    ACL_ACTION_INTERNAL_PRI_ASSIGN_NEW_PRI,
    ACL_ACTION_INTERNAL_PRI_COPY_FROM_IP,
    ACL_ACTION_INTERNAL_PRI_COPY_FROM_INNER,
    ACL_ACTION_INTERNAL_PRI_COPY_FROM_OUTER,
    ACL_ACTION_INTERNAL_PRI_END
} rtk_acl_internalPriAct_t;
...
typedef struct rtk_acl_actionPri_s
{
    ...
    rtk_acl_internalPriAct_t  internal_pri_act;
#endif
    rtk_pri_t   pri;
} rtk_acl_actionPri_t;
...
typedef struct rtk_acl_actionMetaData_s
{
    ...
    uint32  mask;
#endif  /* defined(CONFIG_SDK_RTL9310) || defined(CONFIG_SDK_RTL9311B) */
...
typedef enum rtk_acl_actionColorDrop_e
{
...
typedef enum rtk_acl_actionColorCopy_e
{
    ACL_ACTION_COLOR_COPY_PERMIT,
    ACL_ACTION_COLOR_COPY_COPY,
    ACL_ACTION_COLOR_COPY_END
} rtk_acl_actionColorCopy_t;

typedef enum rtk_acl_actionCopyType_e
{
    ACL_ACTION_COPY_TO_LOCAL,
    ACL_ACTION_COPY_TO_MASTER,
    ACL_ACTION_COPY_END
} rtk_acl_actionCopyType_t;
...
#if defined(CONFIG_SDK_RTL9311B)
    rtk_enable_t                green_copy_en;
    rtk_acl_actionColorCopy_t   green_copy_data;
    rtk_enable_t                yellow_copy_en;
    rtk_acl_actionColorCopy_t   yellow_copy_data;
    rtk_acl_actionCopyType_t    color_copy_type;
#endif
} rtk_acl_vAct_t;
...
typedef struct rtk_acl_statistics_s
{
    union
    {
        struct
        ...
        {
            uint64 byte_cnt;        
            rtk_time_timeStamp_t latency;
            rtk_time_timeStamp_t time;
        }ipfix_latency;
    }info;
} rtk_acl_statistics_t;
...
/* Function Name:
 *      rtk_acl_mirSflowBypassEacl_set
 * Description:
 *      Set configuration that if mirror and sflow packet can bypass EACL
 * Input:
 *      unit        - unit id
 *      bypassEbl    - bypass enable status
 * Output:
 *      None
 * Return:
 *      RT_ERR_OK
 ...
 */
extern int32
rtk_acl_mirSflowBypassEacl_set(uint32 unit, rtk_enable_t bypassEbl);

/* Function Name:
 *      rtk_acl_mirSflowBypassEacl_get
 * Description:
 *      Get configuration that if mirror and sflow packet can bypass EACL
 * Input:
 *      unit        - unit id
 * Output:
 *      pBypassEbl  - pointer to bypass status
 * Return:
 *      RT_ERR_OK
 *      RT_ERR_FAILED
 *      RT_ERR_UNIT_ID     - invalid unit id
 ...
 */
extern int32
rtk_acl_mirSflowBypassEacl_get(uint32 unit, rtk_enable_t *pBypassEbl);

/* Function Name:
 *      rtk_acl_statMode_get
 * Description:
 *      Get statistic counter mode of the IACL or EACL block.
 * Input:
 *      unit        - unit id
 *      phase       - PIE lookup phase (IACL or EACL)
 *      block_idx   - block index
 * Output:
 *      pMode - statictic counter mode
 * Return:
 *      RT_ERR_OK
 *      RT_ERR_FAILED
 *      RT_ERR_UNIT_ID      - invalid unit id
 *      RT_ERR_NOT_INIT     - The module is not initial
 *      RT_ERR_ACL_PHASE    - the ACL phase is wrong
 *      RT_ERR_NULL_POINTER - input parameter may be null pointer
 * Applicable:
 *      9330
 * Note:
 ...
 */
extern int32
rtk_acl_statMode_get(uint32 unit, rtk_acl_phase_t phase,
    uint32 block_idx, rtk_acl_statBlockMode_t *pMode);

/* Function Name:
 *      rtk_acl_statMode_set
 * Description:
 *      Set statistic counter mode of the IACL or EACL block.
 * Input:
 *      unit        - unit id
 *      phase       - PIE lookup phase (IACL or EACL)
 *      block_idx   - block index
 *      mode        - statictic counter mode 
 * Output:
 *      None
 * Return:
 *      RT_ERR_OK
 ...
 *      RT_ERR_NOT_INIT     - The module is not initial
 *      RT_ERR_ACL_PHASE    - the ACL phase is wrong
 *      RT_ERR_INPUT - wrong mode input
 *      RT_ERR_NOT_SUPPORTED -    mode is not supported
 * Applicable:
 *      9330
 * Note:
 *      None
 ...
 */
extern int32
rtk_acl_statMode_set(uint32 unit, rtk_acl_phase_t phase,
    uint32 block_idx, rtk_acl_statBlockMode_t mode);


/* Function Name:
 *      rtk_acl_ipfixThr_get
 * Description:
 *      Get IPFIX counter wrap threshold of byte or packet counter.
 * Input:
 *      unit        - unit id
 *      mode        - packet or byte mode
 * Output:
 *      pThr        - counter exceed threshold
 * Return:
 *      RT_ERR_OK
 *      RT_ERR_FAILED
 *      RT_ERR_UNIT_ID      - invalid unit id
 *      RT_ERR_NOT_INIT     - The module is not initial
 *      RT_ERR_INPUT   - the mode configuration is wrong
 *      RT_ERR_NULL_POINTER - input parameter may be null pointer
 * Applicable:
 *      9330
 * Note:
 ...
 */
extern int32
rtk_acl_ipfixThr_get(uint32 unit, rtk_acl_statMode_t mode,
    uint64 *pThr);

/* Function Name:
 *      rtk_acl_ipfixThr_set
 * Description:
 *      Set IPFIX counter wrap threshold of byte or packet counter.
 * Input:
 *      unit        - unit id
 *      mode        - packet or byte mode
 *      thr         - counter exceed threshold
 * Output:
 *      None
 * Return:
 *      RT_ERR_OK
 ...
 */
extern int32
rtk_acl_ipfixThr_set(uint32 unit, rtk_acl_statMode_t mode,
    uint64 thr);

/* Function Name:
 *      rtk_acl_ipfixWrap_get
 * Description:
 *      Get IPFIX counter wrap status of byte or packet counter.
 * Input:
 *      unit        - unit id
 *      phase       - ACL phase (IACL or EACL)
 *      mode        - packet or byte mode
 *      entry_idx   - ipfix counter entry index
 * Output:
 *      pWrap       - counter wrap threshold
 * Return:
 *      RT_ERR_OK
 *      RT_ERR_FAILED
 *      RT_ERR_UNIT_ID      - invalid unit id
 *      RT_ERR_NOT_INIT     - The module is not initial
 *      RT_ERR_ACL_PHASE    - the ACL phase is wrong 
 *      RT_ERR_INPUT   - the mode configuration is wrong or entry_idx is odd
 *      RT_ERR_NULL_POINTER - input parameter may be null pointer
 * Applicable:
 *      9330
 * Note:
 *      entry_idx should be even value since an ipfix entry takes space of two ACL counter
 * Changes:
 *      [SDK_4.0.0]
 *          New added function. 
 */
extern int32
rtk_acl_ipfixWrap_get(uint32 unit, rtk_acl_phase_t phase, 
    rtk_acl_statMode_t mode, uint32 entry_idx, uint32 *pWrap);


/* Function Name:
 *      rtk_acl_ipfixWrap_reset
 * Description:
 *      Reset IPFIX counter wrap status of byte or packet counter.
 * Input:
 *      unit        - unit id
 *      phase       - ACL phase (IACL or EACL)
 *      mode        - packet or byte mode
 ...
 */
extern int32
rtk_acl_ipfixWrap_reset(uint32 unit, rtk_acl_phase_t phase, 
    rtk_acl_statMode_t mode, uint32 entry_idx);

/* Function Name:
 *      rtk_acl_statistics_get
 * Description:
 *      Get counter or ipfix statistics data of the entry id.
 * Input:
 *      unit        - unit id
 ...
 */
extern int32
rtk_acl_statistics_get(uint32 unit, rtk_acl_phase_t phase,
    rtk_pie_id_t entryIdx, rtk_acl_statistics_t *pStat);

/* Function Name:
 *      rtk_acl_statistics_clear
 * Description:
 *      Clear statistic counter of the log id.
 * Input:
 *      unit        - unit id
 ...
 */
extern int32
rtk_acl_statistics_clear(uint32 unit, rtk_acl_phase_t phase,
    rtk_pie_id_t entryIdx);
