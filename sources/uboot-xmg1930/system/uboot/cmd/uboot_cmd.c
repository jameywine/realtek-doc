...
/*set internal GPIO pin status*/
    ...
    {
        NetSendPacket(pkt_content, sizeof(pkt_content));
        eth_rx();
        return 0;
    }
            ...
            for(speedMode = 0; speedMode < 3; speedMode ++)
            {
                ...
                HWP_PORT_TRAVS(unit, port)
                {
                    ...
                    if (HWP_SERDES_PORT(unit, port))
                    {
                        ...
                        switch(speedMode)
                        {
                            case 0: rtk_port_10gMedia_set(unit, port, PORT_10GMEDIA_FIBER_10G); break;
                            case 1: rtk_port_10gMedia_set(unit, port, PORT_10GMEDIA_FIBER_1G); break;
                            case 2:
