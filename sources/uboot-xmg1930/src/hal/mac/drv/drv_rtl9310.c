 ...
 *
 * $Revision: 94096 $
 * $Date: 2018-12-06 19:40:17 +0800 (Thu, 06 Dec 2018) $
 *
...
_rtl9310_miim_page_set(uint32 unit, uint32 park_page)
{
    ...
    _rtl9310_miim_phyRegAccessCtrlRegFields_set(unit,
...
int32 rtl9310_smiAddr_init(uint32 unit)
{
    ...
    uint32      cust_phy[DRV_RTL9310_SMI_MAX] = { FALSE };
    ...
    for (smi_id = 0; smi_id < DRV_RTL9310_SMI_MAX; smi_id++)
    {
        ...
        if (TRUE == cust_phy[smi_id])
        {
            val = 0x1;
            ret = reg_field_write(unit, MANGO_SMI_GLB_CTRL0r, smiPrvte0_field[smi_id], &val);
            if (ret != RT_ERR_OK)
            {
