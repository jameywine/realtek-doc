 ...
 *
 * $Revision: 93896 $
 * $Date: 2018-11-27 16:54:05 +0800 (Tue, 27 Nov 2018) $
 *
...
rtk_pie_template_t dal_mango_pie_fixedTemplate[] =
{
 ...
 */
static int32
_dal_mango_pie_phy2logTmplteField(uint32 unit, uint32 phy_field_id,
    rtk_pie_templateFieldType_t *log_field_id)
{
    uint32  i;
    ...
    {
        if (phy_field_id == dal_mango_template_common_field_list[i].physical_id)
        {
            *log_field_id = dal_mango_template_common_field_list[i].field_type;
            return RT_ERR_OK;
        }
    }
    ...
    {
        if (phy_field_id == dal_mango_template_vi_field_list[i].physical_id)
        {
            *log_field_id = dal_mango_template_vi_field_list[i].field_type;
            return RT_ERR_OK;
        }
    }
    ...
    {
        if (phy_field_id == dal_mango_template_i_field_list[i].physical_id)
        {
            *log_field_id = dal_mango_template_i_field_list[i].field_type;
            return RT_ERR_OK;
        }
    }
    ...
    {
        if (phy_field_id == dal_mango_template_e_field_list[i].physical_id)
        {
            *log_field_id = dal_mango_template_e_field_list[i].field_type;
            return RT_ERR_OK;
        }
    }

    return RT_ERR_PIE_FIELD_TYPE;
}   /* end of _dal_mango_pie_phy2logTmplteField */
    ...
    uint32 *template1_id)
{
    ...
    if ((ret = reg_array_field_read(unit, MANGO_PIE_BLK_TMPLTE_CTRLr,
...
dal_mango_pie_phase_get(uint32 unit, uint32 block_idx, rtk_pie_phase_t *pPhase)
{
    ...
    if ((ret = reg_array_field_read(unit, MANGO_PIE_BLK_PHASE_CTRLr,
    ...
    {
        case 0:
            *pPhase = PIE_PHASE_VACL;
            break;
        case 1:
            *pPhase = PIE_PHASE_IACL;
            break;
        case 2:
            *pPhase = PIE_PHASE_EACL;
            break;
        case 3:
            *pPhase = PIE_PHASE_IGR_FLOW_TABLE_0;
            break;
        case 4:
            *pPhase = PIE_PHASE_IGR_FLOW_TABLE_3;
            break;
        case 5:
            *pPhase = PIE_PHASE_EGR_FLOW_TABLE_0;
            break;
        case 6:
            *pPhase = PIE_PHASE_DIS;
            break;
        default:
            RT_LOG(LOG_DEBUG, (MOD_PIE|MOD_DAL), "val=%d", val);
    ...
    rtk_pie_template_t *pTemplate)
{
    ...
    {
        osal_memcpy(pTemplate, &dal_mango_pie_fixedTemplate[template_id],
                sizeof(rtk_pie_template_t));

        return RT_ERR_OK;
    }

    /* User defined template then get value from chip */
    for (fieldIdx = 0; fieldIdx < HAL_MAX_NUM_OF_PIE_TEMPLATE_FIELD(unit); ++fieldIdx)
    {
        PIE_SEM_LOCK(unit);
        if ((ret = reg_array_field_read(unit, MANGO_PIE_TMPLTE_CTRLr, template_id,
                fieldIdx, MANGO_TMPLTE_FIELDf, &val)) != RT_ERR_OK)
        {
            PIE_SEM_UNLOCK(unit);
            RT_ERR(ret, (MOD_DAL|MOD_PIE), "template id %u, field idx %u", template_id, fieldIdx);
            return ret;
        }
        PIE_SEM_UNLOCK(unit);

        if ((ret = _dal_mango_pie_phy2logTmplteField(unit, val,
                &pTemplate->field[fieldIdx])) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_PIE), "");
            return ret;
        }
...
/* Function Name:
 *      _dal_mango_pie_meterTokenRefill_set
 * Description:
 *      Set token refill T/B val of meter block.
 * Input:
 *      unit         - unit id
 *      blockIdx     - block id
 ...
 */
int32
_dal_mango_pie_meterTokenRefill_set(uint32  unit, uint32 blockIdx,
    uint32 tickPeriod, uint32 tokenLen)
{
    int32 ret;

    RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_PIE), "unit=%d, blockIdx=%d tickPeriod=%d tokenLen=%d",
            unit, blockIdx, tickPeriod, tokenLen);
    ...
    /* program val to CHIP*/
    if ((ret = reg_array_field_write(unit, MANGO_METER_BYTE_TB_CTRLr,
            REG_ARRAY_INDEX_NONE, blockIdx, MANGO_TICK_PERIODf,
    ...
    if ((ret = reg_array_field_write(unit, MANGO_METER_BYTE_TB_CTRLr,
            REG_ARRAY_INDEX_NONE, blockIdx, MANGO_TKNf,
            &tokenLen)) != RT_ERR_OK)
    {
        PIE_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_PIE), "");
    ...
    return RT_ERR_OK;
}   /* end of _dal_mango_pie_meterTokenRefill_set */
...
_dal_mango_pie_init_config(uint32 unit)
{
    ...
    {
        if ((ret = dal_mango_pie_phase_set(unit, i, PIE_PHASE_DIS)) != RT_ERR_OK)
        {
