...
dal_mango_stack_port_set (uint32 unit, rtk_portmask_t *pStkPorts)
{
    ...
    {
        if ((ret = dal_mango_stack_devPortMap_get (unit, targetUnit, &mapPorts[targetUnit])) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_STACK), "");
            return ret;
        }
        ...
        if ((ret = dal_mango_stack_nonUcastBlockPort_get (unit, targetUnit, &blockPorts[targetUnit])) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_STACK), "");
            return ret;
        }
    ...
    {
        if ((ret = dal_mango_trunk_stkTrkPort_get(unit, trkId, &stkTrkPorts[trkId])) != RT_ERR_OK)
        {
        ...
        if (idx2stackPortId[unit][idx] != NON_DEF_PORT_IDX)
        {
            if(RTK_PORTMASK_IS_PORT_CLEAR(*pStkPorts, idx2stackPortId[unit][idx]))
            {
                ...
                if ((ret = reg_array_field_write(unit,
                          ...
                          idx,
                          MANGO_STK_PORT_IDf,
                          &idx2stackPortId[unit][idx])) != RT_ERR_OK)
                {
                    STACK_SEM_UNLOCK(unit);
                    RT_ERR(ret, (MOD_DAL|MOD_STACK), "");
                    return ret;
    ...
    for(targetUnit = 0; targetUnit <= RTK_MAX_UNIT_ID; targetUnit ++)
    {
        if ((ret = dal_mango_stack_devPortMap_set (unit, targetUnit, &mapPorts[targetUnit])) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_STACK), "");
            return ret;
        }

        if ((ret = dal_mango_stack_nonUcastBlockPort_set (unit, targetUnit, &blockPorts[targetUnit])) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_STACK), "");
            return ret;
        }
    ...
    for(trkId = 0; trkId < 8; trkId ++)
    {
        if ((ret = dal_mango_trunk_stkTrkPort_set(unit, trkId, &stkTrkPorts[trkId])) != RT_ERR_OK)
        {
            if(RT_ERR_NOT_INIT == ret)
                continue;
            RT_ERR(ret, (MOD_DAL|MOD_STACK), "");
...
dal_mango_stack_loopGuard_get(uint32 unit, rtk_enable_t *pEnable)
{
    ...
    *pEnable = val;
    ...
    return RT_ERR_OK;
}   /* end of dal_mango_stack_dropLoopPacket_get */
...
dal_mango_stack_loopGuard_set(uint32 unit, rtk_enable_t enable)
{
    ...
    return RT_ERR_OK;
}   /* end of dal_mango_stack_dropLoopPacket_set */
...
_dal_mango_stack_init_config(uint32 unit)
{
    ...
    stkPorts.bits[1] = RTK_DEFAULT_STACK_PORTMASK_1;
    if ((ret = dal_mango_stack_port_set (unit, &stkPorts)) != RT_ERR_OK)
    {
    ...
    stkPorts.bits[0] = RTK_DEFAULT_STACK_NONUCAST_BLOCK_PORTMASK_0;
    stkPorts.bits[1] = RTK_DEFAULT_STACK_NONUCAST_BLOCK_PORTMASK_1;
    for(idx = 0; idx < RTK_MAX_NUM_OF_UNIT; idx ++)
    {
        if ((ret = dal_mango_stack_nonUcastBlockPort_set (unit, idx, &stkPorts)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_TRUNK|MOD_DAL), "");
            return ret;
        }
