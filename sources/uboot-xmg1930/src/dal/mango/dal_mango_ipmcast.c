 ...
 *
 * $Revision: 94059 $
 * $Date: 2018-12-05 13:35:28 +0800 (Wed, 05 Dec 2018) $
 *
    ...
    rtk_ipmc_addr_t *pIpmcAddr)
{
    ...
    /* L3 available */
    if (pEntry->oil_idx_valid || (pEntry->stack_fwd_pmsk & 0xFFFF))
    {
        pEntry->l3_en = 1;
    }
...
_dal_mango_ipmc_stkPmsk_update(uint32 unit, uint32 l3EntryIdx, uint32 pmskVal)
{
    ...
    osal_printf("l3EntryIdx = %x, pmskVal = %x\n", l3EntryIdx, pmskVal);
...
{
    int32 ret = RT_ERR_OK;
    uint32 tbl, tfL3Valid, tfListValid, tfListIdx, valL3Valid, valListValid, valListIdx;
    dal_mango_ipmc_l3Entry_t *pEntry;
    l3_host_route_entry_t hostEntry;
    l3_prefix_route_entry_t routeEntry;
    ...
    if (L3_ENTRY_IS_VALID(pEntry))
    {
        ...
        if (FALSE == pEntry->cam)
        {
            ...
            {
                tbl = MANGO_L3_HOST_ROUTE_IP6MC_0t;
                tfL3Valid = MANGO_L3_HOST_ROUTE_IP6MC_0_L3_ENtf;
            ...
            {
                tbl = MANGO_L3_HOST_ROUTE_IPMC_0t;
                tfL3Valid = MANGO_L3_HOST_ROUTE_IPMC_0_L3_ENtf;
                tfListValid = MANGO_L3_HOST_ROUTE_IPMC_0_OIL_IDX_VALIDtf;
                tfListIdx = MANGO_L3_HOST_ROUTE_IPMC_0_OIL_IDXtf;
            }

            IPMCAST_TABLE_READ_ERR_HDL(unit, tbl, pEntry->hw_entry_idx, hostEntry, "", errHandle, ret);
            IPMCAST_TABLE_FIELD_SET_ERR_HDL(unit, tbl, tfL3Valid, valL3Valid, hostEntry, "", errHandle, ret);
            IPMCAST_TABLE_FIELD_SET_ERR_HDL(unit, tbl, tfListValid, valListValid, hostEntry, "", errHandle, ret);
            IPMCAST_TABLE_FIELD_SET_ERR_HDL(unit, tbl, tfListIdx, valListIdx, hostEntry, "", errHandle, ret);
            IPMCAST_TABLE_WRITE_ERR_HDL(unit, tbl, pEntry->hw_entry_idx, hostEntry, "", errHandle, ret);
        }
        else
        {
            ...
            {
                tbl = MANGO_L3_PREFIX_ROUTE_IP6MC_0t;
                tfL3Valid = MANGO_L3_PREFIX_ROUTE_IP6MC_0_L3_ENtf;
            ...
            {
                tbl = MANGO_L3_PREFIX_ROUTE_IPMC_0t;
                tfL3Valid = MANGO_L3_PREFIX_ROUTE_IPMC_0_L3_ENtf;
                tfListValid = MANGO_L3_PREFIX_ROUTE_IPMC_0_OIL_IDX_VALIDtf;
                tfListIdx = MANGO_L3_PREFIX_ROUTE_IPMC_0_OIL_IDXtf;
            }

            IPMCAST_TABLE_READ_ERR_HDL(unit, tbl, pEntry->hw_entry_idx, routeEntry, "", errHandle, ret);
            IPMCAST_TABLE_FIELD_SET_ERR_HDL(unit, tbl, tfL3Valid, valL3Valid, routeEntry, "", errHandle, ret);
            IPMCAST_TABLE_FIELD_SET_ERR_HDL(unit, tbl, tfListValid, valListValid, routeEntry, "", errHandle, ret);
            IPMCAST_TABLE_FIELD_SET_ERR_HDL(unit, tbl, tfListIdx, valListIdx, routeEntry, "", errHandle, ret);
            IPMCAST_TABLE_WRITE_ERR_HDL(unit, tbl, pEntry->hw_entry_idx, routeEntry, "", errHandle, ret);
        }
    ...
    if (NULL != pEntry)
    {
        ...
        if (pIpmcAddr->ipmc_flags & RTK_IPMC_FLAG_REPLACE)
        {
            if (pIpmcAddr->group != pEntry->groupId)
            {
                ...
                RT_ERR_HDL(_dal_mango_mcast_ipmc_unbind(unit, pEntry->groupId, &entryIdx), errHandle, ret);
            }
...
dal_mango_ipmc_addr_del(uint32 unit, rtk_ipmc_addr_t *pIpmcAddr)
{
    ...
    RT_ERR_HDL(_dal_mango_mcast_ipmc_unbind(unit, pEntry->groupId, &l3EntryIdx), errHandle, ret);
