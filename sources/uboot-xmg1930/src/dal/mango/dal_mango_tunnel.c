 ...
 *
 * $Revision: 92931 $
 * $Date: 2018-10-19 19:22:51 +0800 (Fri, 19 Oct 2018) $
 *
...
static int32 _dal_mango_tunnel_tunnelIntfEntry_find(uint32 unit, rtk_intf_id_t intfId, uint32 *pIndex)
{
    int32 index;
...
_dal_mango_tunnel_encapEntry_set(uint32 unit, uint32 index, dal_mango_tunnel_encapEntry_t *pEntry)
{
    ...
    TUNNEL_TABLE_FIELD_SET_ERR_HDL(unit, MANGO_TUNNEL_START_ENCAPt, \
        MANGO_TUNNEL_START_ENCAP_NH_INFO_VALIDtf, pEntry->dbl_tag_sts, encapEntry, "", errHandle, ret);
    TUNNEL_TABLE_FIELD_SET_ERR_HDL(unit, MANGO_TUNNEL_START_ENCAPt, \
        MANGO_TUNNEL_START_ENCAP_NH_DMAC_IDXtf, pEntry->itag_vid, encapEntry, "", errHandle, ret);
    TUNNEL_TABLE_FIELD_SET_ERR_HDL(unit, MANGO_TUNNEL_START_ENCAPt, \
...
_dal_mango_tunnel_intf2tunnelIdx(uint32 unit, uint32 *pIndex, rtk_intf_id_t intfId)
{
    ...
    if (DAL_MANGO_L3_INTF_ID_IS_TUNNEL(intfId))
    {
        RT_ERR_HDL(_dal_mango_tunnel_tunnelIntfEntry_find(unit, intfId, pIndex), errIntfFind, ret);
    }
    else if (DAL_MANGO_L3_INTF_ID_IS_L2_TUNNEL(intfId))
    {
...
dal_mango_tunnel_intf_set(uint32 unit, rtk_tunnel_intf_t *pIntf)
{
    ...
    if (pIntf->encap.path_id != _pTunnelDb[unit]->tunnel_intf[tunnelIntfIdx].path_id)
    {
