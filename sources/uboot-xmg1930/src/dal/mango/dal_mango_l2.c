 ...
 *
 * $Revision: 94495 $
 * $Date: 2018-12-27 17:16:43 +0800 (Thu, 27 Dec 2018) $
 *
    ...
    if (!(pL2_addr->flags & RTK_L2_UCAST_FLAG_L2_TUNNEL))
    {
        ...
        else
        {
            if (((pL2_addr->flags & RTK_L2_UCAST_FLAG_SA_BLOCK) || (pL2_addr->flags & RTK_L2_UCAST_FLAG_DA_BLOCK) || (pL2_addr->flags & RTK_L2_UCAST_FLAG_NEXTHOP)) && !(pL2_addr->flags & RTK_L2_UCAST_FLAG_STATIC))
                RT_PARAM_CHK((!HWP_PORT_EXIST(unit, lPort) && lPort != PORT_DONT_CARE_9310), RT_ERR_PORT_ID);
            else
    ...
    rtk_l2_ucastAddr_t  *pL2_addr)
{
    ...
    pL2_addr->l2_idx = TBL_IDX_XLATE_L(*pScan_idx);
    return ret;
}
...
_dal_mango_l2_nexthop_add(uint32 unit, dal_mango_l2_ucastNhAddr_t *pNexthop_addr)
{
    ...
    ret = _dal_mango_l2_setL2Entry(unit, &index_entry, &l2_entry);
    pNexthop_addr->l2_idx = L2_ENTRY_UNIQUE_IDX(unit, index_entry);
...
_dal_mango_l2_nexthop_del(uint32 unit, dal_mango_l2_ucastNhAddr_t *pNexthop_addr)
{
    ...
    if (l2_entry.unicast.nh)
    {
        index = L2_ENTRY_UNIQUE_IDX(unit, index_entry);
...
_dal_mango_l2_nexthop_del_byIdx(uint32 unit, uint32 index)
{
    ...
    RT_PARAM_CHK((index >= hashTable_size[unit] + cam_size[unit]), RT_ERR_INPUT);
    ...
    for (blk0_index.hashdepth = 0; blk0_index.hashdepth < HAL_L2_HASHDEPTH(unit) && cf_num < cfAddrList_size; blk0_index.hashdepth++)
    {
        ...
        if (l2_entry.is_entry_valid == 1)
        {
            ...
            else if (l2_entry.entry_type == L2_MULTICAST)
            {
                ...
                /* group id */
                (pCfAddrList + cf_num)->l2mcast.groupId = l2Db->pEntry[((pCfAddrList + cf_num)->l2mcast.l2_idx)].groupId;
            }
            else
            {
                L2_SEM_UNLOCK(unit);
    ...
    rtk_l2_entry_t  *pEntry)
{
    ...
    if ((*pScan_idx >= (int32)(TBL_SIZE_XLATE_P(hashTable_size[unit]) + cam_size[unit])) || ((*pScan_idx >= TBL_SIZE_XLATE_P((int32)hashTable_size[unit])) && (l2CamEbl == DISABLED)))
        return RT_ERR_INPUT;
    ...
    if (*pScan_idx < TBL_SIZE_XLATE_P(hashTable_size[unit]))
    {
        value = 1;
        if ((ret = reg_field_set(unit, MANGO_TBL_ACCESS_CTRL_0r, MANGO_EXECf, &value, &data)) != RT_ERR_OK)
        {
            L2_SEM_UNLOCK(unit);
            RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L2), "err ret = %d", ret);
            return ret;
        ...
        value = *pScan_idx;
        if ((ret = reg_field_set(unit, MANGO_TBL_ACCESS_CTRL_0r, MANGO_ADDRf, &value, &data)) != RT_ERR_OK)
        {
            L2_SEM_UNLOCK(unit);
            RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L2), "err ret = %d", ret);
            return ret;
        }
        if ((ret = reg_write(unit, MANGO_TBL_ACCESS_CTRL_0r, &data)) != RT_ERR_OK)
        {
            L2_SEM_UNLOCK(unit);
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        ...
        while(1)
        {
            if ((ret = reg_field_read(unit, MANGO_TBL_ACCESS_CTRL_0r, MANGO_EXECf, &value)) != RT_ERR_OK)
            {
                L2_SEM_UNLOCK(unit);
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
        ...
        if ((ret = reg_field_read(unit, MANGO_TBL_ACCESS_L2_METHOD_CTRLr, MANGO_HITf, &is_found)) != RT_ERR_OK)
        {
            L2_SEM_UNLOCK(unit);
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
    ...
    if (l2CamEbl == ENABLED && is_found == 0)
    {
        ...
        value = 1;
        if ((ret = reg_field_set(unit, MANGO_TBL_ACCESS_CTRL_0r, MANGO_TBLf, &value, &data)) != RT_ERR_OK)
        {
            L2_SEM_UNLOCK(unit);
            RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L2), "err ret = %d", ret);
            return ret;
        }
        index = (*pScan_idx < TBL_SIZE_XLATE_P((int32)hashTable_size[unit])) ? 0 : *pScan_idx - TBL_SIZE_XLATE_P((int32)hashTable_size[unit]);
        if ((ret = reg_field_set(unit, MANGO_TBL_ACCESS_CTRL_0r, MANGO_ADDRf, &index, &data)) != RT_ERR_OK)
        {
            L2_SEM_UNLOCK(unit);
            RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L2), "err ret = %d", ret);
            return ret;
    ...
    if ((ret = reg_field_read(unit, MANGO_TBL_ACCESS_L2_METHOD_CTRLr, MANGO_HIT_ADDRf, &index)) != RT_ERR_OK)
    {
        L2_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_L2), "");
        return ret;
    ...
    if (index_type == L2_IN_HASH)
        ...
        *pScan_idx  = TBL_SIZE_XLATE_P(hashTable_size[unit]) + index;
    ...
    if (type < L2_NEXT_VALID_TYPE_MC)
    {
        ...
        pEntry->unicast.l2_idx  = TBL_IDX_XLATE_L(*pScan_idx);
    }
    else
    {
        ...
        pEntry->l2mcast.l2_idx      = TBL_IDX_XLATE_L(*pScan_idx);
        pEntry->l2mcast.groupId     = l2Db->pEntry[(pEntry->l2mcast.l2_idx)].groupId;
...
dal_mango_l2_entryCnt_get(uint32 unit, rtk_l2_entryType_t type, uint32 *pCnt)
{
    ...
    for(index=0; index<hashTable_size[unit]+(l2CamEbl==ENABLED ? cam_size[unit] : 0); index++)
    {
        ...
        if((ret=dal_mango_l2_addrEntry_get(unit, index, &l2_entry))!=RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }
