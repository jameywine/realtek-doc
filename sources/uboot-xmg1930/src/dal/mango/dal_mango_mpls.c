 ...
 *
 * $Revision: 92777 $
 * $Date: 2018-10-11 11:28:59 +0800 (Thu, 11 Oct 2018) $
 *
 * Purpose :
 *
 * Feature :
...
static uint32       _mango_mpls_nh2path[MANGO_MPLS_NHENTRY_MAX];
...
dal_mango_mpls_init(uint32 unit)
{
    ...
    osal_memset(&_mango_mpls_nh2path, 0, (sizeof(uint32) * MANGO_MPLS_NHENTRY_MAX));
...
dal_mango_mpls_trapTarget_get(uint32 unit, rtk_trapTarget_t *pTarget)
{
    ...
    ret = reg_field_read(unit, MANGO_MPLS_GLB_CTRLr, MANGO_CPU_SELf, &val);
    if (ret != RT_ERR_OK)
    {
...
dal_mango_mpls_trapTarget_set(uint32 unit, rtk_trapTarget_t target)
{
    ...
    ret = reg_field_write(unit, MANGO_MPLS_GLB_CTRLr, MANGO_CPU_SELf, &val);
    if (ret != RT_ERR_OK)
    {
        MPLS_SEM_UNLOCK(unit);
    ...
    rtk_action_t *pAction)
{
    ...
    ret = reg_field_read(unit, MANGO_MPLS_GLB_CTRLr, field, &val);
    if (ret != RT_ERR_OK)
    {
        MPLS_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "field read fail");
    ...
    rtk_action_t action)
{
    ...
    ret = reg_field_write(unit, MANGO_MPLS_GLB_CTRLr, field, &val);
    if (ret != RT_ERR_OK)
    {
        MPLS_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "field write fail");
    ...
    rtk_mpls_nextHop_t *pNh)
{
    ...
    ret = dal_mango_l3_intf_get(unit, RTK_L3_INTFKEYTYPE_INTF_ID, &l3Intf);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "Get L3 interface info fail");
        return ret;
    }
    ...
    ret = dal_mango_vlan_l2LookupMode_get(unit, l3Intf.vid, VLAN_L2_MAC_TYPE_UC, &mode);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "Get L2 lookup mode fail");
        return ret;
    }
    ...
    {
        ret = dal_mango_vlan_svlMode_get(unit, &svlMode);
        if (ret != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_MPLS), "Get SVL mode fail");
            return ret;
        }
    ...
    ret = _dal_mango_l2_nexthop_add(unit, &l2Entry);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "Can't find next-hop MAC in L2");
        return ret;
    }

    l3nh.dmac_idx = l2Entry.l2_idx;
    l3nh.l3_egr_intf_idx = pNh->intfIdx;
    ret = _dal_mango_l3_nhEntry_set(unit, nhId, &l3nh,
            DAL_MANGO_L3_API_FLAG_MOD_MPLS);

    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "Set L3 NH %u fail", nhId);
        return ret;
    }
    ...
    val = 1;
    ret = table_field_set(unit, MANGO_MPLS_NHt,
            MANGO_MPLS_NH_VALIDtf, &val, (uint32 *)&nh);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table set %u field fail", nhId);
        return ret;
    }

    ret = table_field_set(unit, MANGO_MPLS_NHt,
            MANGO_MPLS_NH_MPLS_ENCAP_IDXtf, &pNh->encapId, (uint32 *)&nh);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table set %u field fail", nhId);
        return ret;
    ...
    ret = table_write(unit, MANGO_MPLS_NHt, nhId, (uint32 *)&nh);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table write %u field fail", nhId);
        return ret;
    }
    ...
    rtk_l3_pathId_t *pPathId)
{
    ...
    /* function body */
    /* create L3 NH */
    ret = _dal_mango_l3_nhEntry_alloc(unit, &idx, DAL_MANGO_L3_API_FLAG_MOD_MPLS);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "Create L3 NH fail");
        return ret;
    }

    ret = _dal_mango_l3_nhEntryPathId_get(unit, idx, &_mango_mpls_nh2path[idx]);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "Get L3 NH %u pathId", idx);
        _dal_mango_l3_nhEntry_free(unit, idx, DAL_MANGO_L3_API_FLAG_MOD_MPLS);
        return ret;
    }

    MPLS_SEM_LOCK(unit);

    ret = _dal_mango_mpls_nextHop_config(unit, idx, pNh);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "Set NH %u", idx);
        _dal_mango_l3_nhEntry_free(unit, idx, DAL_MANGO_L3_API_FLAG_MOD_MPLS);
        MPLS_SEM_UNLOCK(unit);
        return ret;
    }
    ...
    *pPathId = _mango_mpls_nh2path[idx];

    return RT_ERR_OK;
}   /* end of dal_mango_mpls_nextHop_create */

/* Function Name:
 *      _dal_mango_mpls_nhId_get
 * Description:
 *      Get MPLS Next-Hop index by path ID
 * Input:
 *      unit    - unit id
 *      pathId  - pointer to MPLS path ID
 * Output:
 *      nhId    - pointer to MPLS NH index
 * Return:
 *      RT_ERR_OK
 *      RT_ERR_FAILED
 *      RT_ERR_UNIT_ID      - invalid unit id
 ...
 */
int32
_dal_mango_mpls_nhId_get(uint32 unit, rtk_l3_pathId_t pathId, uint32 *nhId)
{
    uint32  i;

    for (i = 0; i < MANGO_MPLS_NHENTRY_MAX; ++i)
    {
        if (_mango_mpls_nh2path[i] == pathId)
            break;
    }

    if (MANGO_MPLS_NHENTRY_MAX == i)
        return RT_ERR_FAILED;

    *nhId = i;

    return RT_ERR_OK;
}   /* end of _dal_mango_mpls_nhId_get */

/* Function Name:
 *      _dal_mango_mpls_nhValid_check
 * Description:
 *      Check MPLS next hop entry valid or not
 * Input:
 *      unit    - unit id
 *      nhId    - pointer to MPLS NH index
 * Output:
 ...
 */
int32
_dal_mango_mpls_nhValid_check(uint32 unit, uint32 nhId)
{
    mpls_nextHop_entry_t    nh;
    uint32                  val;
    int32                   ret;

    RT_PARAM_CHK((HAL_MAX_NUM_OF_NEXTHOP(unit) <= nhId), RT_ERR_INPUT);

    ret = table_read(unit, MANGO_MPLS_NHt, nhId, (uint32 *)&nh);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table read %u fail", nhId);
        return ret;
    }

    ret = table_field_get(unit, MANGO_MPLS_NHt,
            MANGO_MPLS_NH_VALIDtf, &val, (uint32 *)&nh);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table get %u field fail", nhId);
        return ret;
    }

    if (0 == val)
    {
        RT_ERR(RT_ERR_FAILED, (MOD_DAL|MOD_MPLS), "entry %u is invlaid", nhId);
        return RT_ERR_FAILED;
    }

    return ret;
}   /* end of _dal_mango_mpls_nhValid_check */
...
dal_mango_mpls_nextHop_destroy(uint32 unit, rtk_l3_pathId_t pathId)
{
    ...
    ret = _dal_mango_mpls_nhId_get(unit, pathId, &nhId);
    if (ret != RT_ERR_OK)
    {
    ...
    ret = _dal_mango_mpls_nhValid_check(unit, nhId);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "");
        MPLS_SEM_UNLOCK(unit);
    ...
    ret = _dal_mango_l3_nhEntry_free(unit, nhId, DAL_MANGO_L3_API_FLAG_MOD_MPLS);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "L3 entry %u fail", nhId);
        MPLS_SEM_UNLOCK(unit);
        return ret;
    }
    ...
    rtk_mpls_nextHop_t *pNh)
{
    ...
    ret = _dal_mango_l3_nhEntry_get(unit, nhId, &l3nh,
            DAL_MANGO_L3_API_FLAG_MOD_MPLS);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "Get L3 nh %u fail", nhId);
        return ret;
    }

    pNh->intfIdx = l3nh.l3_egr_intf_idx;

    ret = dal_mango_l2_addrEntry_get(unit, l3nh.dmac_idx, &l2Entry);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "Get L2 entry %u fail", l3nh.dmac_idx);
        return ret;
    }

    osal_memcpy(&pNh->mac_addr, &l2Entry.unicast.mac, sizeof(rtk_mac_t));
    ...
    rtk_mpls_encap_t *pEncap)
{
    ...
    ret = table_field_set(unit, MANGO_MPLS_ENCAPt,
            MANGO_MPLS_ENCAP_LABEL_ACTtf, &val, (uint32 *)&entry);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table set %u field fail", encapId);
        return ret;
    }

    ret = table_field_set(unit, MANGO_MPLS_ENCAPt,
            MANGO_MPLS_ENCAP_LABELtf, &pEncap->label, (uint32 *)&entry);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table set %u field fail", encapId);
        return ret;
    ...
    if (pEncap->flags & RTK_MPLS_FLAG_TTL_ASSIGN)
    {
        val = 1;
        ret = table_field_set(unit, MANGO_MPLS_ENCAPt,
                MANGO_MPLS_ENCAP_TTL_ASSIGNtf, &val, (uint32 *)&entry);
        if (ret != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table set %u field fail", encapId);
            return ret;
        ...
        val = pEncap->ttl;
        ret = table_field_set(unit, MANGO_MPLS_ENCAPt,
                MANGO_MPLS_ENCAP_TTLtf, &val, (uint32 *)&entry);
        if (ret != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table set %u field fail", encapId);
            return ret;
    ...
    ret = table_field_set(unit, MANGO_MPLS_ENCAPt,
            MANGO_MPLS_ENCAP_TC_ASSIGNtf, &val, (uint32 *)&entry);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table set %u field fail", encapId);
        return ret;
    ...
    if (RTK_MPLS_TC_ASSIGN == pEncap->tcAct)
    {
        val = pEncap->tc;
        ret = table_field_set(unit, MANGO_MPLS_ENCAPt,
                MANGO_MPLS_ENCAP_TCtf, &val, (uint32 *)&entry);
        if (ret != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table set %u field fail", encapId);
            return ret;
    ...
    if (pEncap->flags & RTK_MPLS_FLAG_NEXTLABEL)
    {
        val = 1;
        ret = table_field_set(unit, MANGO_MPLS_ENCAPt,
                MANGO_MPLS_ENCAP_NEXT_ENtf, &val, (uint32 *)&entry);
        if (ret != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table set %u field fail", encapId);
            return ret;
        ...
        ret = table_field_set(unit, MANGO_MPLS_ENCAPt,
                MANGO_MPLS_ENCAP_NEXT_IDXtf, &pEncap->nextEntryId, (uint32 *)&entry);
        if (ret != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table set %u field fail", encapId);
            return ret;
    ...
    ret = table_write(unit, MANGO_MPLS_ENCAPt, encapId, (uint32 *)&entry);
    if (ret != RT_ERR_OK)
    {
        MPLS_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table write %u field fail", encapId);
        return ret;
    }
    ...
    rtk_mpls_entryId_t *pEncapId)
{
    ...
    {
        maxEntryNum /= DAL_MANGO_MAX_ENTRY_RTL9311E_DIV;
    }
    ...
    ret = _dal_mango_mpls_encap_config(unit, idx, pEncap);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "create entry %u fail", idx);
        return ret;
    }
    ...
    rtk_mpls_encap_t *pEncap)
{
    ...
    ret = table_read(unit, MANGO_MPLS_ENCAPt, encapId, (uint32 *)&entry);
    if (ret != RT_ERR_OK)
    {
        MPLS_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table read %u fail", encapId);
        return ret;
    }

    MPLS_SEM_UNLOCK(unit);

    ret = table_field_get(unit, MANGO_MPLS_ENCAPt,
            MANGO_MPLS_ENCAP_LABEL_ACTtf, &val, (uint32 *)&entry);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table get %u field fail", encapId);
        return ret;
    }
    ...
    rtk_mpls_encap_t *pEncap)
{
    ...
    /* function body */
    ret = _dal_mango_mpls_encap_config(unit, encapId, pEncap);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "set entry %u fail", encapId);
        return ret;
    }
    ...
    rtk_mpls_entryId_t *pEncapId)
{
    ...
    for (idx = 0; idx < maxEntryNum; ++idx)
    {
        ...
        ret = table_read(unit, MANGO_MPLS_ENCAPt, idx, (uint32 *)&entry);
        if (ret != RT_ERR_OK)
        {
            MPLS_SEM_UNLOCK(unit);
            RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table read %u fail", idx);
            return ret;
        }
        ...
        ret = table_field_get(unit, MANGO_MPLS_ENCAPt,
                MANGO_MPLS_ENCAP_LABELtf, &val, (uint32 *)&entry);
        if (ret != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table get %u field fail", idx);
            return ret;
        }
    ...
    if (maxEntryNum == idx)
    {
        RT_ERR(RT_ERR_ENTRY_NOTFOUND, (MOD_DAL|MOD_MPLS), "label %u action %u",
                pEncap->label, pEncap->labelAct);
...
dal_mango_mpls_hashAlgo_get(uint32 unit, uint8 *pHashAlgo)
{
    ...
    ret = reg_field_read(unit, MANGO_MPLS_GLB_CTRLr, MANGO_HASH_SELf, &val);
    if (ret != RT_ERR_OK)
    {
        MPLS_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "field read fail");
...
dal_mango_mpls_hashAlgo_set(uint32 unit, uint8 hashAlgo)
{
    ...
    ret = reg_field_write(unit, MANGO_MPLS_GLB_CTRLr, MANGO_HASH_SELf, &val);
    if (ret != RT_ERR_OK)
    {
        MPLS_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "field read fail");
    ...
    rtk_mpls_decapEntry_t *pDecap)
{
    ...
    val = 1;
    ret = table_field_set(unit, table, MANGO_MPLS_DECAP_VALIDtf,
            &val, (uint32 *)&entry);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table set %u field fail", decapId);
        return ret;
    }

    /* label */
    ret = table_field_set(unit, table, MANGO_MPLS_DECAP_LABELtf,
            &pDecap->label, (uint32 *)&entry);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table set %u field fail", decapId);
        return ret;
    ...
    ret = table_field_set(unit, table, MANGO_MPLS_DECAP_DECAP_ACTtf,
            &val, (uint32 *)&entry);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table set %u field fail", decapId);
    ...
    ret = table_field_set(unit, table, MANGO_MPLS_DECAP_ECMP_ENtf,
            &val, (uint32 *)&entry);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table set %u field fail", decapId);
    ...
    ret = table_field_set(unit, table, MANGO_MPLS_DECAP_TTL_INHERITtf,
            &val, (uint32 *)&entry);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table set %u field fail", decapId);
    ...
    ret = table_field_set(unit, table, MANGO_MPLS_DECAP_DSCP_INHERITtf,
            &val, (uint32 *)&entry);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table set %u field fail", decapId);
    ...
    ret = table_field_set(unit, table, MANGO_MPLS_DECAP_PRI_SEL_TBL_IDXtf,
            &val, (uint32 *)&entry);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table set %u field fail", decapId);
    ...
    {
        val = 1;
        ret = table_field_set(unit, table, MANGO_MPLS_DECAP_INT_PRI_ENtf,
                &val, (uint32 *)&entry);
        if (ret != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table set %u field fail", decapId);
        ...
        ret = table_field_set(unit, table, MANGO_MPLS_DECAP_INT_PRItf,
                &pDecap->intPri, (uint32 *)&entry);
        if (ret != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table set %u field fail", decapId);
            return ret;
    ...
    ret = table_field_set(unit, table, MANGO_MPLS_DECAP_INTF_IDtf,
            &pDecap->intfId, (uint32 *)&entry);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table set %u field fail", decapId);
        return ret;
    ...
    ret = table_write(unit, table, entryId, (uint32 *)&entry);
    if (ret != RT_ERR_OK)
    {
        MPLS_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table write %u field fail", decapId);
        return ret;
    }
    ...
    uint32 *pHashId)
{
    ...
    /* function body */
    ret = dal_mango_mpls_hashAlgo_get(unit, &hashAlgo);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "Get hash algorithm fail");
        return ret;
    }
    ...
    rtk_mpls_entryId_t *pDecapId)
{
    ...
    ret = _dal_mango_mpls_labelToHashId_get(unit, pDecap->label, &hashId);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "Get hash index fail");
        return ret;
    }
    ...
    for (idx = 0; idx < HAL_MPLS_HASHDEPTH(unit); ++idx)
    {
        ...
        ret = table_read(unit, MANGO_MPLS_DECAPt, entryId, (uint32 *)&entry);
        if (ret != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table read %u fail", entryId);
            return ret;
        }

        MPLS_SEM_UNLOCK(unit);

        ret = table_field_get(unit, MANGO_MPLS_DECAPt,
                MANGO_MPLS_DECAP_VALIDtf, &val, (uint32 *)&entry);
        if (ret != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table read %u valid field fail", entryId);
            return ret;
        }
    ...
    if (HAL_MPLS_HASHDEPTH(unit) == idx)
    {
        ...
        {
            ret = table_field_get(unit, MANGO_MPLS_DECAP_CAMt,
                    MANGO_MPLS_DECAP_VALIDtf, &val, (uint32 *)&entry);
            if (ret != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table read %u valid field fail", entryId);
    ...
    ret = _dal_mango_mpls_decap_config(unit, entryId, pDecap);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "create entry %u fail", entryId);
        return ret;
    }
    ...
    rtk_mpls_decapEntry_t *pDecap)
{
    ...
    ret = table_read(unit, table, entryId, (uint32 *)&entry);
    if (ret != RT_ERR_OK)
    {
        MPLS_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table read %u fail", decapId);
        return ret;
    }

    MPLS_SEM_UNLOCK(unit);

    ret = table_field_get(unit, table, MANGO_MPLS_DECAP_VALIDtf,
            &val, (uint32 *)&entry);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table get %u field fail", decapId);
        return ret;
    }
    ...
    /* label */
    ret = table_field_get(unit, table, MANGO_MPLS_DECAP_LABELtf,
            &pDecap->label, (uint32 *)&entry);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table get %u field fail", decapId);
    ...
    /* label action */
    ret = table_field_get(unit, table, MANGO_MPLS_DECAP_DECAP_ACTtf,
            &val, (uint32 *)&entry);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table get %u field fail", decapId);
    ...
    switch (val)
    {
        ...
        default:
            RT_ERR(RT_ERR_FWD_ACTION, (MOD_DAL|MOD_MPLS),
                    "label action %u is not support", val);
            return RT_ERR_FWD_ACTION;
    }

    /* ECMP */
    ret = table_field_get(unit, table, MANGO_MPLS_DECAP_ECMP_ENtf,
            &val, (uint32 *)&entry);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table get %u field fail", decapId);
    ...
    /* TTL inherit */
    ret = table_field_get(unit, table, MANGO_MPLS_DECAP_TTL_INHERITtf,
            &val, (uint32 *)&entry);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table get %u field fail", decapId);
    ...
    /* TC inherit */
    ret = table_field_get(unit, table, MANGO_MPLS_DECAP_DSCP_INHERITtf,
            &val, (uint32 *)&entry);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table get %u field fail", decapId);
    ...
    /* priority select table index */
    ret = table_field_get(unit, table, MANGO_MPLS_DECAP_PRI_SEL_TBL_IDXtf,
            &val, (uint32 *)&entry);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table get %u field fail", decapId);
    ...
    /* internal priority */
    ret = table_field_get(unit, table, MANGO_MPLS_DECAP_INT_PRI_ENtf,
            &val, (uint32 *)&entry);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table get %u field fail", decapId);
    ...
    if (1 == val)
    {
        ...
        ret = table_field_get(unit, table, MANGO_MPLS_DECAP_INT_PRItf,
                &pDecap->intPri, (uint32 *)&entry);
        if (ret != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table get %u field fail", decapId);
    ...
    /* interface, next hop, ECMP index */
    ret = table_field_get(unit, table, MANGO_MPLS_DECAP_INTF_IDtf,
            &pDecap->intfId, (uint32 *)&entry);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table get %u field fail", decapId);
    ...
    rtk_mpls_decapEntry_t *pDecap)
{
    ...
    /* function body */
    ret = _dal_mango_mpls_decap_config(unit, decapId, pDecap);
    if (ret != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "create entry %u fail", decapId);
        return ret;
    }
    ...
    rtk_mpls_entryId_t *pDecapId)
{
    ...
    for (idx = 0; idx < HAL_MPLS_HASHDEPTH(unit); ++idx)
    {
        ...
        if (ret != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table read %u valid field fail", idx);
            return ret;
        }
        ...
        ret = table_field_get(unit, MANGO_MPLS_DECAPt,
                MANGO_MPLS_DECAP_LABELtf, &val, (uint32 *)&entry);
        if (ret != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table read %u valid field fail", idx);
            return ret;
    ...
    for (idx = 0; idx < HAL_MAX_NUM_OF_MPLS_DECAP_CAM(unit); ++idx)
    {
        ...
        ret = table_read(unit, MANGO_MPLS_DECAP_CAMt, idx, (uint32 *)&entry);
        if (ret != RT_ERR_OK)
        {
            MPLS_SEM_UNLOCK(unit);
            RT_ERR(ret, (MOD_DAL|MOD_MPLS), "table read %u fail", idx);
...
dal_mango_mpls_egrTcMap_get(uint32 unit, rtk_mpls_egrTcMapSrc_t *pSrc, uint8 *pTc)
{
    ...
    /* function body */
    MPLS_SEM_LOCK(unit);
    if ((ret = reg_array_field_read(unit, MANGO_MPLS_DPINTPRI2TC_CTRLr,
            pSrc->dp, pSrc->pri, MANGO_TCf, &val)) != RT_ERR_OK)
    {
        MPLS_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "");
        return ret;
...
dal_mango_mpls_egrTcMap_set(uint32 unit, rtk_mpls_egrTcMapSrc_t *pSrc, uint8 tc)
{
    ...
    val = tc;
    MPLS_SEM_LOCK(unit);
    if ((ret = reg_array_field_write(unit, MANGO_MPLS_DPINTPRI2TC_CTRLr,
            pSrc->dp, pSrc->pri, MANGO_TCf, &val)) != RT_ERR_OK)
    {
        MPLS_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_MPLS), "");
