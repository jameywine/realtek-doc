...
dal_mango_trunk_port_set(uint32 unit, uint32 trk_gid, rtk_portmask_t *pTrunk_member_portmask)
{
    ...
    ret = dal_mango_trunk_localPort_get (unit, trk_gid, &origin_mbr);
    if(RT_ERR_OK != ret)
        return ret;
    ret = dal_mango_trunk_localPort_set (unit, trk_gid, pTrunk_member_portmask);
    if(RT_ERR_OK != ret)
        return ret;
    ...
    HWP_PORT_TRAVS_EXCEPT_CPU(unit, port)
    {
        ...
        {
            devPort.port = port;
            ret = dal_mango_trunk_srcPortMap_set(unit, devPort, 0, 0);
            if(RT_ERR_OK != ret)
                return ret;
        }
        ...
        if (RTK_PORTMASK_IS_PORT_SET(*pTrunk_member_portmask, port))
        {
            ...
            ret = dal_mango_trunk_srcPortMap_set(unit, trk_egr_ports.egr_port[trk_egr_ports.num_ports],
                1, trk_gid);
            if(RT_ERR_OK != ret)
                return ret;
    ...
    ret = dal_mango_trunk_egrPort_set (unit, trk_gid, &trk_egr_ports);
    if(RT_ERR_OK != ret)
        return ret;
...
dal_mango_trunk_localPort_get (uint32 unit, rtk_trk_t trk_gid, rtk_portmask_t *pTrk_local_ports)
{
    ...
    /* parameter check */
    if(TRUNK_MODE_STACKING == trunkMode)
        RT_PARAM_CHK_EHDL(trk_gid >= HAL_MAX_NUM_OF_TRUNK_IN_STACKING_MODE(unit), RT_ERR_LA_TRUNK_ID, TRUNK_SEM_UNLOCK(unit););
    else
        RT_PARAM_CHK_EHDL(trk_gid >= HAL_MAX_NUM_OF_TRUNK(unit), RT_ERR_LA_TRUNK_ID, TRUNK_SEM_UNLOCK(unit););

    RT_PARAM_CHK_EHDL((NULL == pTrk_local_ports), RT_ERR_NULL_POINTER, TRUNK_SEM_UNLOCK(unit););
...
dal_mango_trunk_localPort_set (uint32 unit, rtk_trk_t trk_gid, rtk_portmask_t *pTrk_local_ports)
{
    ...
    if (RTK_PORTMASK_GET_PORT_COUNT(*pTrk_local_ports) != 0)
    {
        ...
        {
            RT_PARAM_CHK_EHDL((RTK_PORTMASK_IS_PORT_SET(*pTrk_local_ports, port) && !HWP_PORT_EXIST(unit, port)), RT_ERR_LA_PORTMASK, TRUNK_SEM_UNLOCK(unit););
        }
    }
    ...
    for (grp_num = 0; grp_num < HAL_MAX_NUM_OF_TRUNK_IN_STACKING_MODE(unit); grp_num++)
    {
        if (grp_num != trk_gid)
        {
            RT_PARAM_CHK_EHDL((pTrk_local_ports->bits[0] & pTrunkMemberSet[unit][grp_num].bits[0])
                        , RT_ERR_LA_MEMBER_OVERLAP, TRUNK_SEM_UNLOCK(unit););
            RT_PARAM_CHK_EHDL((pTrk_local_ports->bits[1] & pTrunkMemberSet[unit][grp_num].bits[1])
                        , RT_ERR_LA_MEMBER_OVERLAP, TRUNK_SEM_UNLOCK(unit););
        }
    }
