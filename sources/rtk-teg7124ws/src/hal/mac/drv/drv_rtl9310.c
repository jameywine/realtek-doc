...
#if defined (CONFIG_SENAO_RESET_CHG_PARTITION) || defined (CONFIG_SENAO_CUSTOMER_UBOOT)
extern int rtk_extGpio_pin_get(unsigned int unit, /*uint*/unsigned int pinNum, /*uint*/unsigned int *pinStatus);
extern int rtk_extGpio_pin_set(unsigned int unit, /*uint*/unsigned int pinNum, /*uint*/unsigned int pinStatus);
extern int rtk_extGpio_device_init(unsigned int unit, /*uint*/unsigned int deviceAddress);
extern void change_partition(void);
#endif
#endif
#endif
...
rtl9310_init(uint32 unit)
{
    ...
    /*Detect reset button is pressed, and change partition*/
#ifdef CONFIG_DUAL_IMAGE
#if defined(__BOOTLOADER__)
#if defined (CONFIG_SENAO_RESET_CHG_PARTITION) || defined (CONFIG_SENAO_CUSTOMER_UBOOT)
    // printf("HWP_IDENTIFIER_ID: %d\n", HWP_IDENTIFIER_ID());
    switch(HWP_IDENTIFIER_ID())
    {
        ...
        case HWP_RTL9311_6x8218D_6XGE_DEMO:
            ext_address = 3;
            break;
        case HWP_RTL9311_6x8218E_6XGE_DEMO:
            ext_address = 3;
            break;
        default:
            ext_address = 3;
    ...
    if(0 != rtk_extGpio_device_init(0, ext_address))
    {
        printf("The generalCtrl GPIO device init failed.\n");
        return RT_ERR_OK;
    }
#endif /* defined (CONFIG_SENAO_RESET_CHG_PARTITION) || defined (CONFIG_SENAO_CUSTOMER_UBOOT) */
#ifdef CONFIG_SENAO_RESET_CHG_PARTITION
    if(0 != rtk_extGpio_pin_get(0, 6,&pinStatus))
    {
        printf("The ext pinNum (%d) is invalid.\n", 6);
        return RT_ERR_OK;
    }
    if(pinStatus == 0)
    {
        printf("Button is pressed, change partition.\n\r");
        change_partition();

        /*Blink Fault LED*/
        if(0 != rtk_extGpio_pin_set(0, 2, 0))
        {
            printf("The ext pinNum (%d) is invalid.\n", 2);
        }
        osal_time_mdelay(600);
        if(0 != rtk_extGpio_pin_set(0, 2, 1))
        {
            printf("The ext pinNum (%d) is invalid.\n", 2);
        }
        osal_time_mdelay(600);
...
#endif /* CONFIG_SENAO_RESET_CHG_PARTITION */
