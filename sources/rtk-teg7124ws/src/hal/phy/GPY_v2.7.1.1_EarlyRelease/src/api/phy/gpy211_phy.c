		      ...
		      struct gpy211_led_cfg *cfg)
{
	...
	/* Max LEDs supported on P34X is 3 */
...
int gpy2xx_fw_frw_init(struct gpy211_device *phy, u32 timeout)
{
	...
	/* Check for P34X master slice's PHYADDR to use for FFU */
	if ((phy->id.family == ID_P34X || phy->id.family == ID_P34X_LGS) &&
	    (phy->phy_addr % 4 != 0)) {
		LOG_CRIT("One should MUST use master slice's PHYADDR on P34X device\n");
		return -EINVAL;
	}
