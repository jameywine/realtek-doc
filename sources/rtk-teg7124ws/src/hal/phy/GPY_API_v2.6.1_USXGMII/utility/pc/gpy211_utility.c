...
static struct {
	...
	{
		"gpy2xx_usxgmii_loopback_set",
		"Configures the USXGMII serial and parrallel loop back",
		"gpy2xx_usxgmii_loopback_set [tx_to_rx_serial_loopback_en=?] [rx_to_tx_parallel_loopback_en=?]\n"
		"Tx-to-Rx Serial Loopback in the PHY\n"
		"Valid 0 - disabled / 1 - enabled\n"
		"	tx_to_rx_serial_loopback_en\n"
		"Rx-to-Tx parallel Loopback in DWC_xpcs\n"
		"Valid 0 - disabled / 1 - enabled\n"
		"	rx_to_tx_parallel_loopback_en\n",
	},
	...
	{
		"gpy2xx_usxgmii_tx_bert_config",
		"Configures TX bert",
		"gpy2xx_usxgmii_tx_bert_config  [tx_bert_mode=?] [nval_sel=?]\n"
	...
	{
		"gpy2xx_usxgmii_rx_bert_config",
		"Configures RX bert",
		"gpy2xx_usxgmii_rx_bert_config  [rx_bert_mode=?]\n"
	...
	{
		"gpy2xx_usxgmii_get_vr_reset_sts",
		"Get VR reset complete status",
		NULL
	},

	{
		"gpy2xx_usxgmii_anegconfig_set",
		"Configures ANEG for Port 0 to 3 (based on phy address)",
		"gpy2xx_usxgmii_anegconfig_set  [usxgmii_aneg_disable=?] [usxgmii_force_linkspeed_enable=?] [usxgmii_force_linkspeed=?]  [usxgmii_force_duplex=?] [usxgmii_restart_aneg=?]\n"
		"ANEG configuration\n"
...
//#endif
		...
		if (scanParamArg(argc, argv, "trace_len", sizeof(reach_cfg.usxgmii_trace_len), &reach_cfg.usxgmii_trace_len) == 0)
			reach_cfg.usxgmii_trace_len = -1;

		cnt += scanParamArg(argc, argv, "tx_eq_main", sizeof(reach_cfg.usxgmii_tx_eq_main), &reach_cfg.usxgmii_tx_eq_main);
		cnt += scanParamArg(argc, argv, "tx_eq_pre", sizeof(reach_cfg.usxgmii_tx_eq_pre), &reach_cfg.usxgmii_tx_eq_pre);
		cnt += scanParamArg(argc, argv, "tx_eq_post", sizeof(reach_cfg.usxgmii_tx_eq_post), &reach_cfg.usxgmii_tx_eq_post);
		cnt += scanParamArg(argc, argv, "tx_vboost_en", sizeof(reach_cfg.usxgmii_tx_vboost_en_0), &reach_cfg.usxgmii_tx_vboost_en_0);
		cnt += scanParamArg(argc, argv, "tx_vboost_lvl", sizeof(reach_cfg.usxgmii_tx_vboost_lvl), &reach_cfg.usxgmii_tx_vboost_lvl);
		cnt += scanParamArg(argc, argv, "tx_iboost_lvl", sizeof(reach_cfg.usxgmii_tx_iboost_lvl_0), &reach_cfg.usxgmii_tx_iboost_lvl_0);
		cnt += scanParamArg(argc, argv, "rx_eq_att_lvl", sizeof(reach_cfg.usxgmii_rx_eq_att_lvl_0), &reach_cfg.usxgmii_rx_eq_att_lvl_0);
		cnt += scanParamArg(argc, argv, "rx_eq_vga1_gain", sizeof(reach_cfg.usxgmii_rx_eq_vga1_gain_0), &reach_cfg.usxgmii_rx_eq_vga1_gain_0);
		cnt += scanParamArg(argc, argv, "rx_eq_vga2_gain", sizeof(reach_cfg.usxgmii_rx_eq_vga2_gain_0), &reach_cfg.usxgmii_rx_eq_vga2_gain_0);
		cnt += scanParamArg(argc, argv, "rx_eq_ctle_boost", sizeof(reach_cfg.usxgmii_rx_eq_ctle_boost_0), &reach_cfg.usxgmii_rx_eq_ctle_boost_0);
		cnt += scanParamArg(argc, argv, "rx_eq_ctle_pole", sizeof(reach_cfg.usxgmii_rx_eq_ctle_pole_0), &reach_cfg.usxgmii_rx_eq_ctle_pole_0);
		cnt += scanParamArg(argc, argv, "rx_eq_dfe_tap1", sizeof(reach_cfg.usxgmii_rx_eq_dfe_tap1_0), &reach_cfg.usxgmii_rx_eq_dfe_tap1_0);
		cnt += scanParamArg(argc, argv, "rx_afe_adapt_en", sizeof(reach_cfg.usxgmii_rx_afe_adapt_en), &reach_cfg.usxgmii_rx_afe_adapt_en);
		cnt += scanParamArg(argc, argv, "rx_dfe_adapt_en", sizeof(reach_cfg.usxgmii_rx_dfe_adapt_en), &reach_cfg.usxgmii_rx_dfe_adapt_en);
		...
		printf("\t%40s:\t%s\n", "trace_len", (reach_cfg.usxgmii_trace_len) == 0 ? "SHORT" : ((reach_cfg.usxgmii_trace_len) == 1 ? "MEDIUM" : ((reach_cfg.usxgmii_trace_len) == 2 ? "LONG" : "CUSTOM")));
		printf("\t%40s:\t0x%x\n", "tx_eq_main", reach_cfg.usxgmii_tx_eq_main);
		printf("\t%40s:\t0x%x\n", "tx_eq_pre", reach_cfg.usxgmii_tx_eq_pre);
		printf("\t%40s:\t0x%x\n", "tx_eq_post", reach_cfg.usxgmii_tx_eq_post);
		printf("\t%40s:\t%s\n", "tx_vboost_en", (reach_cfg.usxgmii_tx_vboost_en_0 == 0) ? "Disabled" : "Enabled");
		printf("\t%40s:\t0x%x\n", "tx_vboost_lvl", reach_cfg.usxgmii_tx_vboost_lvl);
		printf("\t%40s:\t0x%x\n", "tx_iboost_lvl", reach_cfg.usxgmii_tx_iboost_lvl_0);
		printf("\t%40s:\t0x%x\n", "rx_eq_att_lvl", reach_cfg.usxgmii_rx_eq_att_lvl_0);
		printf("\t%40s:\t0x%x\n", "rx_eq_vga1_gain", reach_cfg.usxgmii_rx_eq_vga1_gain_0);
		printf("\t%40s:\t0x%x\n", "rx_eq_vga2_gain", reach_cfg.usxgmii_rx_eq_vga2_gain_0);
		printf("\t%40s:\t0x%x\n", "rx_eq_ctle_boost", reach_cfg.usxgmii_rx_eq_ctle_boost_0);
		printf("\t%40s:\t0x%x\n", "rx_eq_ctle_pole", reach_cfg.usxgmii_rx_eq_ctle_pole_0);
		printf("\t%40s:\t0x%x\n", "rx_eq_dfe_tap1", reach_cfg.usxgmii_rx_eq_dfe_tap1_0);
		printf("\t%40s:\t%s\n", "rx_afe_adapt_en", (reach_cfg.usxgmii_rx_afe_adapt_en == 0) ? "Disabled" : "Enabled");
		printf("\t%40s:\t%s\n", "rx_dfe_adapt_en", (reach_cfg.usxgmii_rx_dfe_adapt_en == 0) ? "Disabled" : "Enabled");
	}  else if (xstrncasecmp(argv[0], "gpy2xx_usxgmii_anegstatus_get", strlen("gpy2xx_usxgmii_anegstatus_get")) == 0) {
		struct gpy211_usxgmii_aneg_linkstatus status;
		memset(&status, 0, sizeof(struct gpy211_usxgmii_aneg_linkstatus));

		ret = gpy2xx_usxgmii_anegstatus_get(&phy, &status);

		if (ret < 0) {
			printf("\nERROR:Egr usxgmii_anegstatus_get failed.\n");
			return -1;
		}

		printf("\t%40s:\t0x%x\n", "phy_addr", status.phy_addr);
		printf("\t%40s:\t%s\n", "usxgmii_tx_config", (status.usxgmii_tx_config[status.phy_addr & 0x3] == 0) ? "usxgmii_mac" : "usxgmii_phy");
		printf("\t%40s:\t%s\n", "usxgmii_aneg_enabled", (status.usxgmii_aneg_enabled[status.phy_addr & 0x3] == 0) ? "Disabled" : "Enabled");

		if (status.usxgmii_tx_config[status.phy_addr & 0x3] == 0)
			printf("\t%40s:\t%s\n", "usxgmii_force_mode", (status.usxgmii_force_mode[status.phy_addr & 0x3] == 0) ? "Disabled" : "Enabled");

		printf("\t%40s:\t%s\n", "usxgmii_aneg_linksts_status", (status.usxgmii_aneg_linksts_status[status.phy_addr & 0x3] == 0) ? "Link Down" : "Link Up");
		printf("\t%40s:\t%s\n", "usxgmii_aneg_complete_status", (status.usxgmii_aneg_complete_status[status.phy_addr & 0x3] == 0) ? "Not Complete" : "Complete");
		printf("\t%40s:\t%d\n", "usxgmii_aneg_linkspeed", status.usxgmii_aneg_linkspeed[status.phy_addr & 0x3]);
		printf("\t%40s:\t%s\n", "usxgmii_aneg_duplex", (status.usxgmii_aneg_duplex[status.phy_addr & 0x3] == 0) ? "Half Duplex" : "Full Duplex");

	}  else if (xstrncasecmp(argv[0], "gpy2xx_usxgmii_anegconfig_set", strlen("gpy2xx_usxgmii_anegconfig_set")) == 0) {
		int cnt = 0;
		struct gpy211_usxgmii_aneg_linkstatus status;
		struct gpy211_usxgmii_aneg_config config;

		memset(&status, 0, sizeof(struct gpy211_usxgmii_aneg_linkstatus));
		memset(&config, 0, sizeof(struct gpy211_usxgmii_aneg_config));

		ret = gpy2xx_usxgmii_anegstatus_get(&phy, &status);

		if (ret < 0) {
			printf("\nERROR:Egr usxgmii_anegstatus_gett failed.\n");
			return -1;
		}

		cnt += scanParamArg(argc, argv, "usxgmii_aneg_disable", sizeof(config.usxgmii_aneg_disable[status.phy_addr & 0x3]), &config.usxgmii_aneg_disable[status.phy_addr & 0x3]);
		cnt += scanParamArg(argc, argv, "usxgmii_restart_aneg", sizeof(config.usxgmii_restart_aneg[status.phy_addr & 0x3]), &config.usxgmii_restart_aneg[status.phy_addr & 0x3]);
		cnt += scanParamArg(argc, argv, "usxgmii_force_linkspeed_enable", sizeof(config.usxgmii_force_linkspeed_enable[status.phy_addr & 0x3]), &config.usxgmii_force_linkspeed_enable[status.phy_addr & 0x3]);
		cnt += scanParamArg(argc, argv, "usxgmii_force_linkspeed", sizeof(config.usxgmii_force_linkspeed[status.phy_addr & 0x3]), &config.usxgmii_force_linkspeed[status.phy_addr & 0x3]);
		cnt += scanParamArg(argc, argv, "usxgmii_force_duplex", sizeof(config.usxgmii_force_duplex[status.phy_addr & 0x3]), &config.usxgmii_force_duplex[status.phy_addr & 0x3]);

		ret = gpy2xx_usxgmii_anegconfig_set(&phy, &config);

		if (ret < 0) {
			printf("\nERROR:Egr gpy2xx_usxgmii_anegconfig_set failed.\n");
			return -1;
		}

		printf("\t%40s:\t0x%x\n", "Usxgmii Aneg configuration applied for phy_addr", status.phy_addr);

	} else if (xstrncasecmp(argv[0], "gpy2xx_usxgmii_pcs_pma_linkstatus_get", strlen("gpy2xx_usxgmii_pcs_pma_linkstatus_get")) == 0) {
		struct gpy211_usxgmii_pcs_pma_linkstatus linkstatus;
		memset(&linkstatus, 0, sizeof(struct gpy211_usxgmii_pcs_pma_linkstatus));

		ret = gpy2xx_usxgmii_pcs_pma_linkstatus_get(&phy, &linkstatus);

		if (ret < 0) {
			printf("\nERROR:Egr gpy2xx_usxgmii_pcs_pma_linkstatus_get failed.\n");
			return -1;
		}

		printf("\t%40s:\t%s\n", "usxgmii_pma_latched_linksts", (linkstatus.usxgmii_pma_latched_linksts == 0) ? "Link Down" : "Link Up");
		printf("\t%40s:\t%s\n", "usxgmii_pma_linksts", (linkstatus.usxgmii_pma_linksts == 0) ? "Link Down" : "Link Up");
		printf("\t%40s:\t%s\n", "usxgmii_pcs_latched_linksts", (linkstatus.usxgmii_pcs_latched_linksts == 0) ? "Link Down" : "Link Up");
		printf("\t%40s:\t%s\n", "usxgmii_pcs_linksts", (linkstatus.usxgmii_pcs_linksts == 0) ? "Link Down" : "Link Up");

	}  else if (xstrncasecmp(argv[0], "gpy2xx_usxgmii_mii_linkstatus_get", strlen("gpy2xx_usxgmii_mii_linkstatus_get")) == 0) {
		struct gpy211_usxgmii_mii_linkstatus linkstatus;
		memset(&linkstatus, 0, sizeof(struct gpy211_usxgmii_mii_linkstatus));

		ret = gpy2xx_usxgmii_mii_linkstatus_get(&phy, &linkstatus);

		if (ret < 0) {
			printf("\nERROR:Egr gpy2xx_usxgmii_mii_linkstatus_get failed.\n");
			return -1;
		}

		printf("\t%40s:\t0x%x\n", "phy_addr", linkstatus.phy_addr);
		printf("\t%40s:\t%s\n", "usxgmii_mii_latched_linksts_status", (linkstatus.usxgmii_mii_latched_linksts_status[linkstatus.phy_addr & 0x3] == 0) ? "Link Down" : "Link Up");
		printf("\t%40s:\t%s\n", "usxgmii_mii_linksts_status", (linkstatus.usxgmii_mii_linksts_status[linkstatus.phy_addr & 0x3] == 0) ? "Link Down" : "Link Up");
		...
		memset(&alignmentmarker, 0, sizeof(struct gpy211_usxgmii_alignmentmarker));
		cnt += scanParamArg(argc, argv, "usxgmii_am_count", sizeof(alignmentmarker.usxgmii_am_count), &alignmentmarker.usxgmii_am_count);
		ret = gpy2xx_usxgmii_alignmentmarker_set(&phy, &alignmentmarker);
		...
		struct gpy211_usxgmii_alignmentmarker alignmentmarker;
		memset(&alignmentmarker, 0, sizeof(struct gpy211_usxgmii_alignmentmarker));
		ret = gpy2xx_usxgmii_alignmentmarker_get(&phy, &alignmentmarker);
		...
		printf("\t%40s:\t0x%x\n", "usxgmii_am_count", alignmentmarker.usxgmii_am_count);
	...
	}  else if (xstrncasecmp(argv[0], "gpy2xx_usxgmii_get_vr_reset_sts", strlen("gpy2xx_usxgmii_get_vr_reset_sts")) == 0) {
		struct gpy211_usxgmii_vr_reset_status vr_reset_status;
		memset(&vr_reset_status, 0, sizeof(struct gpy211_usxgmii_vr_reset_status));

		ret = gpy2xx_usxgmii_vr_reset_sts_get(&phy, &vr_reset_status);

		if (ret < 0) {
			printf("\nERROR:Egr gpy2xx_usxgmii_get_vr_reset_sts failed.\n");
			return -1;
		}

		printf("\t%40s:\t%s\n", "VR reset ", (vr_reset_status.usxgmii_vr_reset_successful == 0) ? "not complete/unsucessfull" : "complete/sucessfull");

	} else if (xstrncasecmp(argv[0], "gpy2xx_usxgmii_tx_bert_config", strlen("gpy2xx_usxgmii_tx_bert_config")) == 0) {
		int cnt = 0;
		struct gpy211_usxgmii_tx_bert_config bert_tx_config;
		memset(&bert_tx_config, 0, sizeof(struct gpy211_usxgmii_tx_bert_config));

		cnt += scanParamArg(argc, argv, "tx_bert_mode", sizeof(bert_tx_config.tx_bert_mode), &bert_tx_config.tx_bert_mode);
		cnt += scanParamArg(argc, argv, "nval_sel", sizeof(bert_tx_config.nval_sel), &bert_tx_config.nval_sel);

		ret = gpy2xx_usxgmii_tx_bert_config(&phy, &bert_tx_config);

		if (ret < 0) {
			printf("\nERROR:Egr gpy2xx_usxgmii_tx_bert_config failed.\n");
			return -1;
		}
	} else if (xstrncasecmp(argv[0], "gpy2xx_usxgmii_get_tx_bert_config", strlen("gpy2xx_usxgmii_get_tx_bert_config")) == 0) {
		struct gpy211_usxgmii_tx_bert_config bert_tx_config;
		memset(&bert_tx_config, 0, sizeof(struct gpy211_usxgmii_tx_bert_config));

		ret = gpy2xx_usxgmii_tx_bert_config_get(&phy, &bert_tx_config);

		if (ret < 0) {
			printf("\nERROR:Egr gpy2xx_usxgmii_get_tx_bert_config failed.\n");
			return -1;
		}
	...
	} else if (xstrncasecmp(argv[0], "gpy2xx_usxgmii_rx_bert_config", strlen("gpy2xx_usxgmii_rx_bert_config")) == 0) {
		int cnt = 0;
		struct gpy211_usxgmii_rx_bert_config bert_rx_config;
		memset(&bert_rx_config, 0, sizeof(struct gpy211_usxgmii_rx_bert_config));

		cnt += scanParamArg(argc, argv, "rx_bert_mode", sizeof(bert_rx_config.rx_bert_mode), &bert_rx_config.rx_bert_mode);

		ret = gpy2xx_usxgmii_rx_bert_config(&phy, &bert_rx_config);

		if (ret < 0) {
			printf("\nERROR:Egr gpy211_usxgmii_rx_bert_config failed.\n");
			return -1;
		}
	} else if (xstrncasecmp(argv[0], "gpy2xx_usxgmii_get_rx_bert_config", strlen("gpy2xx_usxgmii_get_rx_bert_config")) == 0) {
		struct gpy211_usxgmii_rx_bert_config bert_rx_config;
		memset(&bert_rx_config, 0, sizeof(struct gpy211_usxgmii_rx_bert_config));

		ret = gpy2xx_usxgmii_rx_bert_config_get(&phy, &bert_rx_config);

		if (ret < 0) {
			printf("\nERROR:Egr gpy2xx_usxgmii_get_rx_bert_config failed.\n");
			return -1;
		}
	...
	} else if (xstrncasecmp(argv[0], "gpy2xx_usxgmii_rx_bert_statistics_get", strlen("gpy2xx_usxgmii_rx_bert_statistics_get")) == 0) {
		...
		ret = gpy2xx_usxgmii_rx_bert_statistics_get(&phy, &rx_bert_statistics);

		if (ret < 0) {
			printf("\nERROR:Egr gpy2xx_usxgmii_rx_bert_statistics_get failed.\n");
			return -1;
		}
	...
	} else if (xstrncasecmp(argv[0], "gpy2xx_usxgmii_loopback_set", strlen("gpy2xx_usxgmii_loopback_set")) == 0) {
		int cnt = 0;
		struct gpy211_usxgmii_loopback_config loopback;
		memset(&loopback, 0, sizeof(struct gpy211_usxgmii_loopback_config));

		cnt += scanParamArg(argc, argv, "tx_to_rx_serial_loopback_en", sizeof(loopback.tx_to_rx_serial_loopback_en), &loopback.tx_to_rx_serial_loopback_en);
		cnt += scanParamArg(argc, argv, "rx_to_tx_parallel_loopback_en", sizeof(loopback.rx_to_tx_parallel_loopback_en), &loopback.rx_to_tx_parallel_loopback_en);

		ret = gpy2xx_usxgmii_loopback_set(&phy, &loopback);

		if (ret < 0) {
			printf("\nERROR:Egr gpy2xx_usxgmii_loopback_set failed.\n");
			return -1;
		}
		...
		ret = gpy2xx_usxgmii_loopback_get(&phy, &loopback);
		...
		printf("\t%40s:\t%s\n", "tx_to_rx_serial_loopback_en", (loopback.tx_to_rx_serial_loopback_en == 0) ? "Disabled" : "Enabled");
		printf("\t%40s:\t%s\n", "rx_to_tx_parallel_loopback_en", (loopback.rx_to_tx_parallel_loopback_en == 0) ? "Disabled" : "Enabled");
