				   ...
				   u8 sub_sec, u8 sub_nsec)
{
	...
	/* Write Sub-nanosecond Increment Value */
				   ...
				   u8 *sub_sec, u8 *sub_nsec)
{
	...
	if (ret < 0) {
		LOG_CRIT("ERROR: SUB_SECOND_INCREMENT_L read fails\n");
		return ret;
	}
...
static int _gpy2xx_ptp_get_addend(struct gpy211_device *phy, u32 *addend)
{
	...
	if (ret < 0) {
		LOG_CRIT("ERROR: MAC_TIMESTAMP_ADDEND_L read fails\n");
		return ret;
	}
	...
	if (ret < 0) {
		LOG_CRIT("ERROR: MAC_TIMESTAMP_ADDEND_H read fails\n");
		return ret;
	}
		      ...
		      struct gpy211_ptp_ctrl *ptp_cfg)
{
	...
	if (phy->id.family == ID_P31G) {
		ret = SMDIO_WRITE(phy, GMACL_EQOS_MAC_GMACL_MAC_TIMESTAMP_CONTROL_L, 0x3C01);
	} else if (phy->id.family == ID_P34X) {
		ret = SMDIO_WRITE(phy, 0x00A31A00u, 0x3C01);
	}
...
int gpy2xx_ptp_disable(struct gpy211_device *phy)
{
	...
	if (phy->id.family == ID_P31G) {
		ret = SMDIO_WRITE(phy, GMACL_EQOS_MAC_GMACL_MAC_TIMESTAMP_CONTROL_L, 0x0);
	} else if (phy->id.family == ID_P34X) {
		ret = SMDIO_WRITE(phy, 0x00A31A00u, 0x0);
	}
	...
	if (phy->id.family == ID_P31G) {
		...
		ret = SMDIO_WRITE(phy, 0x00A31A04u, 0x0);
	}
...
int gpy2xx_ptp_aux_cfg(struct gpy211_device *phy, struct gpy211_aux_cfg *aux_cfg)
{
	...
	ret = PHY_MBOX_HWWR(phy, GMACF_EQOS_MAC_GMACF_MAC_AUXILIARY_CONTROL_L, _aux_ctrl);
...
int gpy2xx_ptp_getauxts(struct gpy211_device *phy, struct timespec64 *ts)
{
	...
	ret = PHY_MBOX_HWRD(phy, GMACF_EQOS_MAC_GMACF_MAC_TIMESTAMP_STATUS_L, &_timestamp_ctrl);
	...
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_AUXILIARY_TIMESTAMP_SECONDS_L);
	...
	/* Get Seconds Part of the Auxiliary Time */
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_AUXILIARY_TIMESTAMP_SECONDS_H);
	...
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_AUXILIARY_TIMESTAMP_NANOSECONDS_L);
	...
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_AUXILIARY_TIMESTAMP_NANOSECONDS_H);
...
int gpy2xx_ptp_auxfifostat(struct gpy211_device *phy, struct gpy211_aux_fifo_stat *fifo_status)
{
	...
	ret = PHY_MBOX_HWRD(phy, GMACF_EQOS_MAC_GMACF_MAC_TIMESTAMP_STATUS_L, &_data0);
	...
	ret = PHY_MBOX_HWRD(phy, GMACF_EQOS_MAC_GMACF_MAC_TIMESTAMP_STATUS_H, &_data1);
...
int gpy2xx_ptp_resetauxfifo(struct gpy211_device *phy)
{
	...
	ret = PHY_MBOX_HWRD(phy, GMACF_EQOS_MAC_GMACF_MAC_AUXILIARY_CONTROL_L, &_data0);
	...
	ret = PHY_MBOX_HWWR(phy, GMACF_EQOS_MAC_GMACF_MAC_AUXILIARY_CONTROL_L, _data0);
