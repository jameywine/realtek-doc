...
/** \defgroup GPY211_P34X_USXGMII USXGMII_REACH APIs
	\brief Group of functional APIs for USXGMII Reach configuration only for P34X.
...
/** \brief 10 Gbps (used by \b speed in \ref gpy211_link) */
...
/** \addtogroup GPY211_ANEG_STATU */
/**@{*/
/** \brief Unknown aneg process complete status*/
#define	ANEG_PROCESS_COMPLETE_STATUS_UNKNOWN	0xFF
/** @endcond */
/**@}*/ /* GPY211_LINK_SPEED */
...
#define LED_BRLEVEL_MAX     ED_BRLVL_2LVL
/** \endcond */
...
struct gpy211_usxgmii_pcs_pma_linkstatus {

	/** \brief PMA RLU.
		Valid 0 - link down / 1 - link up */
	int usxgmii_pma_latched_linksts;

	/** \brief PMA RLU.
		Valid 0 - link down / 1 - link up */
	int usxgmii_pma_linksts;

	/** \brief PCS RLU.
		Valid 0 - link down / 1 - link up */
	int usxgmii_pcs_latched_linksts;

	/** \brief PCS RLU.
		Valid 0 - link down / 1 - link up */
	int usxgmii_pcs_linksts;
...
struct gpy211_usxgmii_mii_linkstatus {

	/** \brief phy address info for user reference
			to indicate the link status from which phy mii port*/
	int phy_addr;

	/** \brief LINK_STS port 0 to 3.
		Valid 0 - link down / 1 - link up */
	int usxgmii_mii_latched_linksts_status[3];

	/** \brief LINK_STS port 0 to 3.
		Valid 0 - link down / 1 - link up */
	int usxgmii_mii_linksts_status[3];
...
struct gpy211_usxgmii_aneg_linkstatus {

	/** \brief phy address info for user reference
			to indicate the link status from which phy mii port*/
	int phy_addr;

	/** \brief Transmit Configuration (for port 0 to 3) to be used during
			the Clause 37 auto-negotiation in the USXGMII mode.
			- 1: Configures the xpcs as the PHY side SGMII/QSGMII/USXGMII.
			- 0: Configures the xpcs as the MAC side SGMII/QSGMII/USXGMII.*/
	int usxgmii_tx_config[4];

	/** \brief aneg enable/disable for port 0 to 3.
		Valid 0 - disabled / 1 - enabled */
	int usxgmii_aneg_enabled[4];

	/** \brief AN_CMPL for port 0 to 3.
		Valid 0 - Aneg process not complete / 1 - Aneg process not complete */
	int usxgmii_aneg_complete_status[4];

	/** \brief LINK_STS for port 0 to 3.
		Valid 0 - link down / 1 - link up */
	int usxgmii_aneg_linksts_status[4];

	/** \brief linkspeed for port 0 to 3. */
	int usxgmii_aneg_linkspeed[4];

	/** \brief duplex for port 0 to 3.
		Valid 0 - Half duplex / 1 - Full duplex */
	int usxgmii_aneg_duplex[4];

	/** \brief force linkspeed enable/disable for port 0 to 3.
		applicable only when xpcs configued as usxgmii-macside
		Valid 0x0 - disabled / 0x1 - enabled / 0xff - not applicable */
	int usxgmii_force_mode[4];
...
struct gpy211_usxgmii_aneg_config {

	/** \brief phy address info for user reference
			to indicate which phy mii port is configured*/
	int phy_addr;

	/** \brief aneg enable/disable for port 0 to 3.
		Valid 1 - disabled / 0 - enabled */
	int usxgmii_aneg_disable[3];

	/** \brief aneg enable/disable for port 0 to 3.
		Valid 0 - disabled / 1 - enabled */
	int usxgmii_restart_aneg[3];

	/** \brief aneg enable/disable for port 0 to 3.
		Valid 0 - disabled / 1 - enabled */
	int usxgmii_force_linkspeed_enable[3];

	/** \brief linkspeed for port 0 to 3. */
	int usxgmii_force_linkspeed[3];

	/** \brief duplex for port 0 to 3.
		Valid 0 - Half duplex / 1 - Full duplex */
	int usxgmii_force_duplex[3];
...
struct gpy211_usxgmii_tx_bert_config {
	...
	enum gpy211_tx_bert_mode tx_bert_mode;

	/** \brief Square Wave ControlWhen square wave test pattern generation is enabled
	in SR PCS 10GBASE-R Test Pattern Control Register, this field indicates the
	number of bit times for which Ones and Zeros should be driven on the
	serial lines (square wave period) */
...
struct gpy211_usxgmii_rx_bert_config {
	...
	enum gpy211_rx_bert_mode rx_bert_mode;
...
struct gpy211_usxgmii_loopback_config {

	/** \brief Tx-to-Rx Serial Loopback in the PHY.
	Valid 0 - disabled / 1 - enabled*/
	int tx_to_rx_serial_loopback_en;

	/** \brief Rx-to-Tx parallel Loopback in DWC_xpcs.
	Valid 0 - disabled / 1 - enabled*/
	int rx_to_tx_parallel_loopback_en;
...
struct gpy211_usxgmii_vr_reset_status {

	/** \brief VR_RST.
			Valid 1 - reset complete/successfull / 0 -  reset not complete/unsuccessfull*/
	int usxgmii_vr_reset_successful;
};
...
struct gpy211_usxgmii_alignmentmarker {

	/** \brief AM_COUNT.
			Valid 0x00 - 0x7FFF */
	u16 usxgmii_am_count;
};
...
/**@{*/
/**
	\brief  This API configures the RX/TX settings in the USXGMII module only for P34X.

	 The Universal Serial 10GE Media Independent Interface (USXGMII) IP core implements an
	 ...
	 P34X provides 2 options for the serdes data interface
...
/**
	...
	\details This API gets the USXGMII REACH setting.
