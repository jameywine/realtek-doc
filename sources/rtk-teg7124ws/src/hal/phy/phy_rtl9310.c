...
#if defined(CONFIG_SDK_PHY_CUST1)
int32
magic_number_get(char *pMagicNumber)
{
    FILE *pFile = NULL;
    char buff1[255]="";
    int loc = 0, file_length = 0;

    pFile = fopen("/dev/mtd1", "r");
    if(pFile != NULL)
    {
        fseek( pFile, 0, SEEK_END );
        file_length = ftell(pFile);
        fseek( pFile, 0, SEEK_SET );
        while(loc < file_length)
        {
            fscanf(pFile, "%255s", buff1);
            loc = loc + strlen(buff1) + 1;
            if( strlen(buff1) != 0 )
            {
                fseek( pFile, loc, SEEK_SET );
                sscanf(buff1,"magic_number=%s",pMagicNumber);
                if( strlen(pMagicNumber) != 0)
                    break;
            }
        }
        fclose(pFile);
...
_phy_rtl9310_sdsTxParam_set(uint32 unit, uint32 sds, rtk_sds_eyeParam_t param, uint32 tx_para_idx)
{
    ...
    evenSds = asds - (asds % 2);
#if defined(CONFIG_SDK_PHY_CUST1)
    magic_number_get(magic_number);

    /* ECS5512F(L3)), SX12F, TL2-F7120 */
    if(strcmp(magic_number,"0x83028006") != 0 && strcmp(magic_number,"0x00051001") != 0 && strcmp(magic_number,"0x60521003") != 0)
    {
