...
/* Starry@20230110 for Copper manual 1G Config -S*/
int32
phy_8224_senoa_autoNegoEnable_get(uint32 unit, rtk_port_t port, rtk_enable_t *pEnable)
{
  int32        ret = 0;
...
int32
phy_8224_senoa_autoNegoEnable_set(uint32 unit, rtk_port_t port, rtk_enable_t enable)
{
  int32        ret = 0;
  ...
  if (enable == ENABLED)
  {
    ...
    ret = phy_common_c45_autoNegoEnable_set(unit, port, enable);
...
int32
phy_8224_senoa_speed_get(uint32 unit, rtk_port_t port, uint32 *pSpeed)
{
  int32        ret = 0;
...
int32
phy_8224_senoa_speed_set(uint32 unit, rtk_port_t port, uint32 speed)
{
  int32        cfg_autoneg;
  ...
  if (speed == PORT_SPEED_100M || speed == PORT_SPEED_10M)
  {
    ret = phy_common_c45_autoNegoEnable_set(unit, port, cfg_autoneg);
    ret = phy_common_c45_speed_set( unit,  port, speed);
  } else {
    ret = phy_8224_autoNegoAbility_set( unit,  port, &cfg_advertising);
    ret = phy_common_c45_autoNegoEnable_set(unit, port, cfg_autoneg);
  }
...
/* Starry@20230110 for Copper manual 1G Config -E*/
/* Function Name:
...
phy_8224drv_mapperInit(rt_phydrv_t *pPhydrv)
{
    ...
    pPhydrv->fPhydrv_autoNegoEnable_get             = phy_8224_senoa_autoNegoEnable_get; /* phy_common_c45_autoNegoEnable_get;*/
    pPhydrv->fPhydrv_autoNegoEnable_set             = phy_8224_senoa_autoNegoEnable_set; /* phy_common_c45_autoNegoEnable_set;*/
    ...
    pPhydrv->fPhydrv_speed_get 			            = phy_8224_senoa_speed_get; /* phy_common_c45_speed_get;*/
    pPhydrv->fPhydrv_speed_set 			            = phy_8224_senoa_speed_set; /* phy_common_c45_speed_set; */
