...
phyp_senao_port_mapping_t senao_phy_mapping_info[RTK_MAX_PORT_PER_UNIT];
...
/* Copper manual 100M/1G/2.5G/5G/10G Config -S */
int32
phy_8264_senoa_autoNegoEnable_get(uint32 unit, rtk_port_t port, rtk_enable_t *pEnable)
{
  int32        ret = 0;
  
  if(senao_phy_mapping_info[port].force_auto == 1)
  {
    *pEnable = DISABLED;
    ret = RT_ERR_OK;
  } else {
    ret = phy_common_c45_autoNegoEnable_get( unit,  port, pEnable);
  }
...
int32
phy_8264_senoa_autoNegoEnable_set(uint32 unit, rtk_port_t port, rtk_enable_t enable)
{
  int32        ret = 0;

  if (enable == ENABLED)
  {
    senao_phy_mapping_info[port].force_auto = 0;
    ret = phy_826xb_autoNegoEnable_set(unit, port, enable);
    
  }
  else if (enable == DISABLED)
  {
    senao_phy_mapping_info[port].force_auto = 1;
    ret = RT_ERR_OK;
  }
...
int32
phy_8264_senoa_speed_get(uint32 unit, rtk_port_t port, uint32 *pSpeed)
{
  int32        ret = 0;

  if(senao_phy_mapping_info[port].force_auto == 1)
  {
    *pSpeed = senao_phy_mapping_info[port].speed;
    ret = RT_ERR_OK;
  } else {
    ret = phy_common_c45_speed_get( unit,  port, pSpeed);
  }
...
int32
phy_8264_senoa_speed_set(uint32 unit, rtk_port_t port, uint32 speed)
{
  int32        cfg_autoneg;
  rtk_port_phy_ability_t  cfg_advertising={0};
  int32        ret = 0;


  if(senao_phy_mapping_info[port].force_auto == 0)
  {
    return RT_ERR_FAILED;
  }
  ...
  {
   case PORT_SPEED_100M:
     cfg_advertising.Full_100 = 1;
     break;
   case PORT_SPEED_1000M:
     cfg_advertising.Full_1000 = 1;
     break;
   case PORT_SPEED_2_5G:
     cfg_advertising.adv_2_5G = 1;
     break;
   case PORT_SPEED_5G:
     cfg_advertising.adv_5G = 1;
     break;
   case PORT_SPEED_10G:
     cfg_advertising.adv_10GBase_T = 1;
     break;
   default:
     return RT_ERR_FAILED;
  }
  ...
  if (speed == PORT_SPEED_100M)
  {
    ret = phy_826xb_autoNegoEnable_set(unit, port, cfg_autoneg);
    ret = phy_826xb_speed_set( unit,  port, speed);
  } else {
    ret = phy_826xb_autoNegoAbility_set( unit,  port, &cfg_advertising);
    ret = phy_826xb_autoNegoEnable_set(unit, port, cfg_autoneg);
  }
  ...
  senao_phy_mapping_info[port].speed = speed;
  return ret;
}
/* Copper manual 100M/1G/2.5G/5G/10G Config -E */
...
phy_826XBdrv_mapperInit(rt_phydrv_t *pPhydrv)
{
    ...
    pPhydrv->fPhydrv_autoNegoEnable_get            = phy_8264_senoa_autoNegoEnable_get;	/* phy_common_c45_autoNegoEnable_get */
    pPhydrv->fPhydrv_autoNegoEnable_set            = phy_8264_senoa_autoNegoEnable_set;	/* phy_826xb_autoNegoEnable_set */
    ...
    pPhydrv->fPhydrv_speed_get                     = phy_8264_senoa_speed_get;			/* phy_common_c45_speed_get */
    pPhydrv->fPhydrv_speed_set                     = phy_8264_senoa_speed_set;			/* phy_826xb_speed_set */
