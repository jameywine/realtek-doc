 ...
 */
int32
phy_100MportWorkaround_set(uint32 unit, rtk_port_t port, rtk_enable_t enable)
{
    hal_control_t   *pHalCtrl;
    int32           ret;
    ...
    MIIM_SEM_LOCK(unit);
    ret = pHalCtrl->pPhy_ctrl[port]->pPhydrv->fPhydrv_100MportWorkaround_set(unit, port, enable);
    MIIM_SEM_UNLOCK(unit);
...
_phy_swMacPollPhyStatus_get(uint32 unit, rtk_port_t port, rtk_port_swMacPollPhyStatus_t *pphyStatus)
{
    ...
    {
        ret = phy_bcm50998_swMacPollPhyStatus_get(unit, port, pphyStatus);
    }
    else
#endif
#if defined(CONFIG_SDK_PHY_GPY2xx)
    ...
    {
        ret = phy_gpy2xx_swMacPollPhyStatus_get(unit, port, pphyStatus);
    }
    else
#endif
    {
