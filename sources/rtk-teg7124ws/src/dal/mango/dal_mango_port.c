...
int32 _dal_mango_port_swMacPollPhyEnable_get(uint32 unit, rtk_port_t port, rtk_enable_t *penable);
//int32 _dal_mango_port_swChgSerdesModeEnable_get(uint32 unit, rtk_port_t port, rtk_enable_t *penable);
...
dal_mango_port_link_get(uint32 unit, rtk_port_t port, rtk_port_linkStatus_t *pStatus)
{
    int32   ret;
    uint32  val;
/*SDK366 remove workaround, need to check*/
#if 1   /*10G-PHY84898, 100M workaround*/
#if defined(CONFIG_SDK_PHY_CUST1) || defined(CONFIG_SDK_PHY_GPY2xx)
        /* note the status per port */
    static uint8 pre_Status[RTK_MAX_NUM_OF_PORTS] = {PORT_LINKUP};
#endif    
#endif
...
/*SDK366 remove workaround, need to check*/
        ...
        {
            if (pre_Status[port] != *pStatus)
            {
                if (ret == RT_ERR_OK && HWP_PHY_MODEL_BY_PORT(unit, port) == RTK_PHYTYPE_CUST1)
                {
                    if ((ret = dal_phy_100MportWorkaround_set(unit, port, ENABLED)) != RT_ERR_OK)
                    {
...
/* Function Name:
 *      _dal_mango_port_macForceMode_set
 * Description:
 ...
 */
void
_dal_mango_port_macForceMode_set(uint32 unit, rtk_port_t port,
                    ...
                    rtk_port_swMacPollPhyStatus_t *pphyStatus)
{
    ...
    if ((ret = reg_array_read(unit, MANGO_MAC_FORCE_MODE_CTRLr, port, REG_ARRAY_INDEX_NONE, &reg_data)) != RT_ERR_OK)
    {
    ...
    #if 0
    val = 1;
    ret = reg_field_set(unit, MANGO_MAC_FORCE_MODE_CTRLr, MANGO_MAC_FORCE_ENf, &val, &reg_data);
    RT_LOG(LOG_INFO, (MOD_DAL|MOD_PORT), "unit=%u, port=%u, MAC_FORCE_EN=%u data=0x%X ret=%x", unit, port, val, reg_data, ret);
    #endif

    val = 1;
    reg_field_set(unit, MANGO_MAC_FORCE_MODE_CTRLr, MANGO_SMI_FORCE_FC_ENf, &val, &reg_data);
    ...
    reg_field_set(unit, MANGO_MAC_FORCE_MODE_CTRLr, MANGO_SMI_FORCE_LINK_ENf, &val, &reg_data);
    ...
    reg_field_set(unit, MANGO_MAC_FORCE_MODE_CTRLr, MANGO_SMI_DUP_SELf, &val, &reg_data);
    ...
    reg_field_set(unit, MANGO_MAC_FORCE_MODE_CTRLr, MANGO_SMI_TX_PAUSE_ENf, &val, &reg_data);
    ...
    reg_field_set(unit, MANGO_MAC_FORCE_MODE_CTRLr, MANGO_SMI_RX_PAUSE_ENf, &val, &reg_data);
    ...
    reg_field_set(unit, MANGO_MAC_FORCE_MODE_CTRLr, LONGAN_MEDIA_SELf, &val, &reg_data);
...
#define FOR_AQ_DEBUG_PURPOSE_5G_2dot5G  1 /* this is request by AQ for their debug purpose, should remove later */
    ...
    reg_field_set(unit, MANGO_MAC_FORCE_MODE_CTRLr, MANGO_SMI_SPD_SELf, &val, &reg_data);
    ...
    reg_array_write(unit, MANGO_MAC_FORCE_MODE_CTRLr, port, REG_ARRAY_INDEX_NONE, &reg_data);

}

/* Function Name:
 *      _dal_mango_port_swMacPollPhyEnable_get
 * Description:
 ...
 */
int32
_dal_mango_port_swMacPollPhyEnable_get(uint32 unit, rtk_port_t port, rtk_enable_t *penable)
{
#if 0 /*Bruce need to check.*/
    /* MAC-POLL-PHY */
 ...
 */
int32
_dal_mango_port_swMacPollPhy(uint32 unit, rtk_port_t port)
{
    rtk_enable_t    enable;
    int32           ret;
    ...
    if (_dal_mango_port_swMacPollPhyEnable_get(unit, port, &enable) != RT_ERR_OK)
    {
    ...
    /* MAC Force Mode */
    _dal_mango_port_macForceMode_set(unit, port, &pMac_info[unit]->swMacPollPhyStatus[port], &phyStatus);
