...
dal_phy_init(uint32 unit)
{
    ...
    HWP_PORT_TRAVS_EXCEPT_CPU(unit, port)
    {
...
#if defined(CONFIG_SDK_PHY_CUST1) || defined(CONFIG_SDK_PHY_CUST2) || defined(CONFIG_SDK_PHY_CUST3) || defined(CONFIG_SDK_PHY_CUST4) || defined(CONFIG_SDK_PHY_CUST5) || defined(CONFIG_SDK_PHY_GPY2xx)
        ...
        if ((HWP_PHY_MODEL_BY_PORT(unit, port) == RTK_PHYTYPE_CUST1)
            ...
            || (HWP_PHY_MODEL_BY_PORT(unit, port) == RTK_PHYTYPE_GPY2xx))
        {
            continue;
        }
#endif
...
dal_phy_portLinkChange_process(int32 unit, rtk_port_t port, rtk_port_linkStatus_t linkSts)
{
    ...
    if (HWP_9310_FAMILY_ID(unit))
    {
        if (HWP_SERDES_PORT(unit, port))
        {
            phy_rtl9310_linkChange_process(unit, port, linkSts);
        }
    }
...
/* Function Name:
 *      dal_phy_100MportWorkaround_set
 * Description:
 *      Set PHY interface status of the specific port
 * Input:
 *      unit          - unit id
 ...
 */
int32
dal_phy_100MportWorkaround_set(uint32 unit, rtk_port_t port, rtk_enable_t enable)
{
    int32   ret;
    ...
    ret = phy_100MportWorkaround_set(unit, port, enable);
