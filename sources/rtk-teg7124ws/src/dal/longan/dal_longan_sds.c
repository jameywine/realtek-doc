...
#if defined(CONFIG_SDK_PHY_CUST1)
int32 dal_testmode_rtl9302c_bcm50998_8x2_5G_4FibXGE_eye_param_init(uint32 unit);
int32 dal_testmode_rtl9302b_2x8218d_bcm50998e_4XGE_eye_param_init(uint32 unit);
int32 dal_rtl9302c_bcm50998_8x2_5G_4FibXGE_eye_param_init(uint32 unit);
int32 dal_rtl9302b_2x8218d_bcm50998e_4XGE_eye_param_init(uint32 unit);
#endif
...
ERR:
    ...
    if (mode == RTK_SDS_TESTMODE_PRBS31)
    {
        if(phy_board_type == RTL9302C_BCM50998_BOARD_TYPE_1)
        {
            if(dal_testmode_rtl9302c_bcm50998_8x2_5G_4FibXGE_eye_param_init(unit)!= RT_ERR_OK)
            {
                return RT_ERR_FAILED;
            }
        }
        ...
        {
            if(dal_testmode_rtl9302b_2x8218d_bcm50998e_4XGE_eye_param_init(unit)!= RT_ERR_OK)
            {
                return RT_ERR_FAILED;
            }
        }
...
#if 0
    if(RTL9302C_CHIP_ID == HWP_CHIP_ID(unit))
    {
        if (mode == RTK_SDS_TESTMODE_PRBS31)
        {
            /* 20200603, Bruce add for SFP serdes tunning -----------
            ...
            =====P9====
            serdes set 6 eye-param pre-amp 1
            serdes set 6 eye-param main-amp 9
            serdes set 6 eye-param post-amp 0
            serdes set id 6 page 0x2f reg 24 data 0x8e88
            
            =====P10====
            serdes set 7 eye-param pre-amp 1
            serdes set 7 eye-param main-amp 8
            serdes set 7 eye-param post-amp 0
            serdes set id 7 page 0x2f reg 24 data 0x8e88
            
            =====P11====
            serdes set 8 eye-param pre-amp 1
            serdes set 8 eye-param main-amp 9
            serdes set 8 eye-param post-amp 1
            serdes set id 8 page 0x2f reg 24 data 0x8e88
            
            =====P12====
            serdes set 9 eye-param pre-amp 1
            serdes set 9 eye-param main-amp 7
            serdes set 9 eye-param post-amp 0
            serdes set id 9 page 0x2f reg 24 data 0x8e88 
            -----------------------------------------------------*/ 
    
            /*Set serdes 6 eye-param*/
            osal_memset(&param, 0, sizeof(rtk_sds_eyeParam_t));
            if(rtk_sds_eyeParam_get(unit, 6, &param) != RT_ERR_OK)
                return RT_ERR_FAILED;
            param.pre_amp = 1;
            param.pre_en = ENABLED;
            param.main_amp = 9;
            param.post_amp = 0;
            param.post_en = ENABLED;
            if(rtk_sds_eyeParam_set(unit, 6, param) != RT_ERR_OK)
                return RT_ERR_FAILED;
            /*serdes set id 6 page 0x2f reg 24 data 0x8e88*/
            hal_serdes_reg_set(unit, 6, 0x2f, 24, 0x8e88);
            /*----------------------------------------------------*/
        
            /*Set serdes 7 eye-param*/
            osal_memset(&param, 0, sizeof(rtk_sds_eyeParam_t));
            if(rtk_sds_eyeParam_get(unit, 7, &param) != RT_ERR_OK)
                return RT_ERR_FAILED;
            param.pre_amp = 1;
            param.pre_en = ENABLED;
            param.main_amp = 8;
            param.post_amp = 0;
            param.post_en = ENABLED;
            if(rtk_sds_eyeParam_set(unit, 7, param) != RT_ERR_OK)
                return RT_ERR_FAILED;
            /*serdes set id 7 page 0x2f reg 24 data 0x8e88*/
            hal_serdes_reg_set(unit, 7, 0x2f, 24, 0x8e88);
            /*----------------------------------------------------*/
        
            /*Set serdes 8 eye-param*/
            osal_memset(&param, 0, sizeof(rtk_sds_eyeParam_t));
            if(rtk_sds_eyeParam_get(unit, 8, &param) != RT_ERR_OK)
                return RT_ERR_FAILED;
            param.pre_amp = 1;
            param.pre_en = ENABLED;
            param.main_amp = 9;
            param.post_amp = 1;
            param.post_en = ENABLED;
            if(rtk_sds_eyeParam_set(unit, 8, param) != RT_ERR_OK)
                return RT_ERR_FAILED;
            /*serdes set id 8 page 0x2f reg 24 data 0x8e88*/
            hal_serdes_reg_set(unit, 8, 0x2f, 24, 0x8e88);
            /*----------------------------------------------------*/
        
            /*Set serdes 9 eye-param*/
            osal_memset(&param, 0, sizeof(rtk_sds_eyeParam_t));
            if(rtk_sds_eyeParam_get(unit, 9, &param) != RT_ERR_OK)
                return RT_ERR_FAILED;
            param.pre_amp = 1;
            param.pre_en = ENABLED;
            param.main_amp = 7;
            param.post_amp = 0;
            param.post_en = ENABLED;
            if(rtk_sds_eyeParam_set(unit, 9, param) != RT_ERR_OK)
                return RT_ERR_FAILED;
            /*serdes set id 9 page 0x2f reg 24 data 0x8e88*/
            hal_serdes_reg_set(unit, 9, 0x2f, 24, 0x8e88);
            /*----------------------------------------------------*/
        }
    }
#endif
...
dal_longan_sds_init(uint32 unit)
{
    ...
    if (HWP_UNIT_VALID_LOCAL(unit))
    {
        ...
        if(phy_board_type == RTL9302C_BCM50998_BOARD_TYPE_1)
        {
            if(dal_rtl9302c_bcm50998_8x2_5G_4FibXGE_eye_param_init(unit)!= RT_ERR_OK)
            {
                return RT_ERR_FAILED;
            }
        }
        ...
        {
            if(dal_rtl9302b_2x8218d_bcm50998e_4XGE_eye_param_init(unit)!= RT_ERR_OK)
            {
                return RT_ERR_FAILED;
            }
	}
        ...
        if(RTL9302C_CHIP_ID == HWP_CHIP_ID(unit))
        {
            /*Set serdes 2 eye-param*/
            osal_memset(&param, 0, sizeof(rtk_sds_eyeParam_t));
            if(rtk_sds_eyeParam_get(unit, 2, &param) != RT_ERR_OK)
                return RT_ERR_FAILED;	
            param.pre_amp = 0;
            param.main_amp = 9;
            param.post_amp = 0;
            if(rtk_sds_eyeParam_set(unit, 2, param) != RT_ERR_OK)
                return RT_ERR_FAILED;
    
            /*Set serdes 3 eye-param*/
            osal_memset(&param, 0, sizeof(rtk_sds_eyeParam_t));
            if(rtk_sds_eyeParam_get(unit, 3, &param) != RT_ERR_OK)
                return RT_ERR_FAILED;
            param.pre_amp = 0;
            param.main_amp = 12;
            param.post_amp = 0;
            if(rtk_sds_eyeParam_set(unit, 3, param) != RT_ERR_OK)
                return RT_ERR_FAILED;
...
#if defined(CONFIG_SDK_PHY_CUST1)
int32
dal_rtl9302c_bcm50998_8x2_5G_4FibXGE_eye_param_init(uint32 unit)
{
...
int32
dal_rtl9302b_2x8218d_bcm50998e_4XGE_eye_param_init(uint32 unit)
{
    rtk_sds_eyeParam_t    param;
    /*Add for SFP serdes tunning -----------

    Serdes 2/3/4/5/6/7/8/9
    Serdes 2    : port 1-8
    Serdes 3    : port 9-16
    Serdes 2/3  : port 17-24
    Serdes 6    : port 25
    Serdes 7    : port 26
    Serdes 8    : port 27
    Serdes 9    : port 28
    ...
    =====P1-P8====
    serdes set 2 eye-param pre-state enable
    serdes set 2 eye-param pre-amp 5
    serdes set 2 eye-param main-amp 30
    serdes set 2 eye-param post-state enable
    serdes set 2 eye-param post-amp 5

    =====P9-P16====
    serdes set 3 eye-param pre-state enable
    serdes set 3 eye-param pre-amp 0
    serdes set 3 eye-param main-amp 20
    serdes set 3 eye-param post-state enable
    serdes set 3 eye-param post-amp 0

    =====P17-P24====
    serdes set 4 eye-param pre-state enable
    serdes set 4 eye-param pre-amp 0
    serdes set 4 eye-param main-amp 12
    serdes set 4 eye-param post-state enable
    serdes set 4 eye-param post-amp 0
    serdes set 5 eye-param pre-state enable
    serdes set 5 eye-param pre-amp 0
    serdes set 5 eye-param main-amp 12
    serdes set 5 eye-param post-state enable
    serdes set 5 eye-param post-amp 0

    =====P9====
    serdes set 6 eye-param pre-amp 0
    serdes set 6 eye-param main-amp 9
    serdes set 6 eye-param post-amp 0
    serdes set id 6 page 0x2f reg 24 data 0x8e88

    =====P10====
    serdes set 7 eye-param pre-state enable
    serdes set 7 eye-param pre-amp 1
    serdes set 7 eye-param main-amp 9
    serdes set 7 eye-param post-amp 0
    serdes set id 7 page 0x2f reg 24 data 0x0e88

    =====P11====
    serdes set 8 eye-param pre-state enable
    serdes set 8 eye-param post-state enable
    serdes set 8 eye-param pre-amp 2
    serdes set 8 eye-param main-amp 10
    serdes set 8 eye-param post-amp 2
    serdes set id 8 page 0x2f reg 24 data 0x8e88
    ...
    serdes set 8 eye-param post-state enable
    serdes set 9 eye-param pre-amp 1
    serdes set 9 eye-param main-amp 10
    serdes set 9 eye-param post-amp 2
    serdes set id 9 page 0x2f reg 24 data 0x8e88
    -----------------------------------------------------*/
    ...
    /*Set serdes 4 eye-param*/
    osal_memset(&param, 0, sizeof(rtk_sds_eyeParam_t));
    if(rtk_sds_eyeParam_get(unit, 4, &param) != RT_ERR_OK)
        return RT_ERR_FAILED;
    param.pre_amp = 0;
    param.pre_en = ENABLED;
    param.main_amp = 12;
    param.post_amp = 0;
    param.post_en = ENABLED;
    if(rtk_sds_eyeParam_set(unit, 4, param) != RT_ERR_OK)
        return RT_ERR_FAILED;

    /*Set serdes 5 eye-param*/
    osal_memset(&param, 0, sizeof(rtk_sds_eyeParam_t));
    if(rtk_sds_eyeParam_get(unit, 5, &param) != RT_ERR_OK)
        return RT_ERR_FAILED;
    param.pre_amp = 0;
    param.pre_en = ENABLED;
    param.main_amp = 12;
    param.post_amp = 0;
    param.post_en = ENABLED;
    if(rtk_sds_eyeParam_set(unit, 5, param) != RT_ERR_OK)
        return RT_ERR_FAILED;
    ...
    if(rtk_sds_eyeParam_set(unit, 7, param) != RT_ERR_OK)
        return RT_ERR_FAILED;
    /*serdes set id 7 page 0x2f reg 24 data 0x0e88*/
    hal_serdes_reg_set(unit, 7, 0x2f, 24, 0x0e88);
    /*----------------------------------------------------*/
...
int32
dal_testmode_rtl9302c_bcm50998_8x2_5G_4FibXGE_eye_param_init(uint32 unit)
{
    rtk_sds_eyeParam_t    param;
    /* 20200603, Bruce add for SFP serdes tunning -----------
...
int32
dal_testmode_rtl9302b_2x8218d_bcm50998e_4XGE_eye_param_init(uint32 unit)
{
    rtk_sds_eyeParam_t    param;
    /*Add for SFP serdes tunning -----------
