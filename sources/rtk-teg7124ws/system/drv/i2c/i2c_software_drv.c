...
void senao_time_udelay(uint32 usec);
#define CLK_DURATION(clk)               do { uint32 i; for (i = 0; i <= clk; i++) {senao_time_udelay(0);}; } while(0)
...
int32 _general_sequential_nbits_dev_read(uint8 *pRdata, uint32 dev)
{
    ...
    *pRdata = 0;
    ...
    for(loop_idx = 0; loop_idx < software_i2c_data_width[dev]; loop_idx++)
    {
/*Byte to  Byte for Read*/
#if defined(CONFIG_SDK_KERNEL_LINUX_USER_MODE)
        if (software_i2c_data_width[dev] == 12) 
            CLK_DURATION(20); /* Broadcom PoE */        
        else 
            CLK_DURATION(2); /*Microsemi PoE*/
#else
        if (software_i2c_data_width[dev] == 12) 
            CLK_DURATION(20); /* Broadcom PoE */        
        else
            CLK_DURATION(10); /*Microsemi PoE*/
#endif
...
// Starry@20211007 Add Read/Write i2c for MCU upgrade -S
int32 _general_sequential_nbits_dev_for_poe_mcu_read(uint8 *pRdata, uint32 dev, uint32 devaddr,uint32 DataLen)
{
    uint32 ack = 0;
    uint32 slave = 0;
    uint32 rdata;
    ...
    // slave = software_i2c_CHIPID[dev] & 0xff;
    slave = devaddr  & 0xff;
    //RT_INIT_MSG("SENAO:%s, %04d dev=0x%x, addr=%x len=0x%x\n",__FUNCTION__,__LINE__, dev, slave, DataLen);
    ...
    *pRdata = 0;
    ...
    /* Reg Data Field*/
    for(loop_idx = 0; loop_idx < DataLen; loop_idx++)
    { 
      /*Byte to  Byte for Read*/
#if defined(CONFIG_SDK_KERNEL_LINUX_USER_MODE)
       CLK_DURATION(20); /* RTK PoE */        
#else
       CLK_DURATION(20); /* RTK PoE */        
#endif
...
int32 drv_software_i2c_sequential_nbyte_read(uint32 devaddr_len, uint8 *pRdata, uint32 dev)
{
    uint32  unit = HWP_MY_UNIT_ID();
    int32   ret = RT_ERR_FAILED;
    int32   devaddr = (devaddr_len >> 16) & 0xFF;
    int32   DataLen = devaddr_len  & 0xFF;

    // Starry
    //RT_INIT_MSG("SENAO:%s, %04d dev=0x%x, addr_len=0x%x\n",__FUNCTION__,__LINE__, dev, devaddr_len);
    ...
    if (RT_ERR_OK != ret)
    {
        goto software_i2c_general_sequential_nbyte_read_end;
    }
    ...
    ret = _general_sequential_nbits_dev_for_poe_mcu_read(pRdata, dev, devaddr, DataLen);
    if (RT_ERR_OK != ret)
    {
        goto software_i2c_general_sequential_nbyte_read_end;
    }
...
software_i2c_general_sequential_nbyte_read_end:

    return ret;
} /* end of drv_software_i2c_read */

int32 _general_nbits_dev_for_poe_mcu_write(uint8 *pWdata, uint32 dev, uint32 devaddr,uint32 DataLen)
{
    uint32 ack = 0;
    uint32 slave = 0;
    uint32 loop_idx;
    ...
    //RT_LOG(LOG_FATAL_ERR, MOD_ALL, "Write add: %02X ", slave);
    //RT_INIT_MSG("SENAO:%s, %04d dev=0x%x, addr=%x len=0x%x, WD[0]=0x%x\n",__FUNCTION__,__LINE__, dev, devaddr, DataLen, pWdata[0]);
    ...
    if (0 != ack)
    {
        RT_LOG(LOG_DEBUG, MOD_ALL, "[Write]devid=0x%x   addr=0x%x  noack 0x%x", dev, devaddr, ack);
        return RT_ERR_FAILED;
    }
    ...
    for(loop_idx = 0; loop_idx < DataLen; loop_idx++)
    {
        ...
        if (0 != ack)
        {
            RT_LOG(LOG_DEBUG, MOD_ALL, "[Read]devid=0x%x   addr=0x%x  noack 0x%x", dev, devaddr, ack);
            return RT_ERR_FAILED;
        }
     }
...
int32 drv_software_i2c_nbyte_write(uint32 devaddr_len, uint8 *pWdata, uint32 dev)
{
    uint32  unit = HWP_MY_UNIT_ID();
    int32 ret = RT_ERR_FAILED;
    int32   devaddr = (devaddr_len >> 16) & 0xFF;
    ...
    if (RT_ERR_OK != ret)
    {
        goto software_i2c_general_nbyte_write_end;
    }
    ...
    ret = _general_nbits_dev_for_poe_mcu_write(pWdata, dev, devaddr, DataLen);
    if (RT_ERR_OK != ret)
    {
        RT_LOG(LOG_DEBUG, MOD_GENERAL, "SOFTWARE I2C write fail!");
        _general_nbits_dev_stop(dev);
...
software_i2c_general_nbyte_write_end:


    return ret;
}
// Starry@20211007 Add Read/Write i2c for MCU upgrade -E
