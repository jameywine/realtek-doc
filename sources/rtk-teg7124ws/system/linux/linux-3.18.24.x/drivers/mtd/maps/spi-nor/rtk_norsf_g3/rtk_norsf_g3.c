...
unsigned long nor_spi_map_size = 0;
...
extern struct mtd_partition rtk_sdk_parts_for_rescue[RTK_MTD_PARTITION_NUM_RESCUE];
#ifdef CONFIG_SENAO_FLASH_LAYOUT_TYPE5_JFFS2_CFG_RECOVERY
extern struct mtd_partition rtk_sdk_recovery_config_parts[1];
int spi_mtd_struct_init_flag = 0;
#endif /*CONFIG_SENAO_FLASH_LAYOUT_TYPE5_JFFS2_CFG_RECOVERY*/
...
__init static int init_luna_nor_spi_map(void) {
    ...
    if (!luna_nor_spi_mtd) {
        ...
        nor_spi_map_size = luna_nor_spi_map.size;
        ...
        if (luna_nor_spi_mtd) {
            ...
            printk("SPI NOR Size (0x%x)\n", luna_nor_spi_map.size);
            luna_nor_spi_mtd->owner = THIS_MODULE;
#ifdef CONFIG_SENAO_FLASH_LAYOUT_TYPE5_JFFS2_CFG_RECOVERY
            spi_mtd_struct_init_flag = 1;
#endif /*CONFIG_SENAO_FLASH_LAYOUT_TYPE5_JFFS2_CFG_RECOVERY*/
            if(nr_parts <= 0) {       
                printk("MTD partitions obtained from built-in array\n");
#ifdef CONFIG_EPS_FLASH_LAYOUT_RESCUE_NONE
                printk("built-in array size is %d\n", ARRAY_SIZE(rtk_sdk_parts));
                add_mtd_partitions(luna_nor_spi_mtd, rtk_sdk_parts, ARRAY_SIZE(rtk_sdk_parts));
#else
                if (luna_nor_spi_map.size == 0x2000000){
                    /*FLASH_SIZE_32MB*/
                    printk("built-in array size is %d\n", ARRAY_SIZE(rtk_sdk_parts_for_rescue));
                    add_mtd_partitions(luna_nor_spi_mtd, rtk_sdk_parts_for_rescue, ARRAY_SIZE(rtk_sdk_parts_for_rescue));
                }
                else if (luna_nor_spi_map.size == 0x1000000){
                    /*FLASH_SIZE_16MB*/
                    printk("built-in array size is %d\n", ARRAY_SIZE(rtk_sdk_parts_for_rescue));
                    add_mtd_partitions(luna_nor_spi_mtd, rtk_sdk_parts_for_rescue, ARRAY_SIZE(rtk_sdk_parts_for_rescue));
                }
...
#endif /*CONFIG_EPS_FLASH_LAYOUT_RESCUE_NONE*/
            } else {
...
#ifdef CONFIG_SENAO_FLASH_LAYOUT_TYPE5_JFFS2_CFG_RECOVERY
void add_recovery_mtd_part(void)
{
    /*Only 2MB NOR-flash needs*/
    if (spi_mtd_struct_init_flag && luna_nor_spi_map.size != 0x2000000 && luna_nor_spi_map.size != 0x1000000)
        add_mtd_partitions(luna_nor_spi_mtd, rtk_sdk_recovery_config_parts, 1);
}
#endif /*CONFIG_SENAO_FLASH_LAYOUT_TYPE5_JFFS2_CFG_RECOVERY*/
