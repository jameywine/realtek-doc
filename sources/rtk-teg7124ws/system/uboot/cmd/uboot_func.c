...
void rtk_network_on(void)
{
    ...
    HWP_PORT_TRAVS(unit, port)
    {
        if (HWP_PHY_MODEL_BY_PORT(unit, port) == RTK_PHYTYPE_RTL8226B){
            sdsId = HWP_PORT_SDSID(unit, port);
            if (sdsId == HWP_NONE)
            {
                continue;
            }
/*            osal_printf("\n%s %d unit %u port %u sds=%u mode=sgmii %d\n", __func__, __LINE__, unit, port, sdsId, RTK_MII_SGMII);*/
            rtk_mac_sdsMode_set(sdsId, RTK_MII_SGMII);
        }
    }
}
 ...
 *      fiber_port_link_up
 * Description:
 *      Enable fiber ports networking functionality and configure hardware to be able to send and receive packets.
 * Input:
 *      None
 * Output:
 *      None
 ...
 *      None
 */
void fiber_port_link_up(void)
{
    char *BoardModel;

    BoardModel = getenv ("boardmodel");

    if(strcmp(BoardModel,"RTL9313_12XFIB10G1G_DEMO") == 0)
    {
        rtk_extGpio_device_init(uboot_cmd_unit, 2);
        rtk_extGpio_pin_set(uboot_cmd_unit,10,0);
        rtk_extGpio_pin_set(uboot_cmd_unit,13,0);
        rtk_extGpio_pin_set(uboot_cmd_unit,22,0);
        rtk_extGpio_pin_set(uboot_cmd_unit,26,0);
        rtk_port_10gMedia_set(0, 0, PORT_10GMEDIA_FIBER_1G);
        rtk_port_10gMedia_set(0, 8, PORT_10GMEDIA_FIBER_1G);
        rtk_port_10gMedia_set(0, 16, PORT_10GMEDIA_FIBER_1G);
        rtk_port_10gMedia_set(0, 24, PORT_10GMEDIA_FIBER_1G);
        rtk_extGpio_device_init(uboot_cmd_unit, 3);
        rtk_extGpio_pin_set(uboot_cmd_unit,0,0);
        rtk_extGpio_pin_set(uboot_cmd_unit,3,0);
        rtk_extGpio_pin_set(uboot_cmd_unit,6,0);
        rtk_extGpio_pin_set(uboot_cmd_unit,9,0);
        rtk_extGpio_pin_set(uboot_cmd_unit,12,0);
        rtk_extGpio_pin_set(uboot_cmd_unit,21,0);
        rtk_extGpio_pin_set(uboot_cmd_unit,24,0);
        rtk_extGpio_pin_set(uboot_cmd_unit,27,0);
        rtk_port_10gMedia_set(0, 32, PORT_10GMEDIA_FIBER_1G);
        rtk_port_10gMedia_set(0, 40, PORT_10GMEDIA_FIBER_1G);
        rtk_port_10gMedia_set(0, 48, PORT_10GMEDIA_FIBER_1G);
        rtk_port_10gMedia_set(0, 50, PORT_10GMEDIA_FIBER_1G);
        rtk_port_10gMedia_set(0, 52, PORT_10GMEDIA_FIBER_1G);
        rtk_port_10gMedia_set(0, 53, PORT_10GMEDIA_FIBER_1G);
        rtk_port_10gMedia_set(0, 54, PORT_10GMEDIA_FIBER_1G);
        rtk_port_10gMedia_set(0, 55, PORT_10GMEDIA_FIBER_1G);
    }
}
