...
#ifdef __BOOTLOADER__
#define BROADMODEL_MAPPING_MAX_NUM                  3
static hwp_idmapping_t     hwp_broadmodel_id_mapping[BROADMODEL_MAPPING_MAX_NUM] =
{
    [0] = { 
            .broadmodel = "RTL9311_6x8218D_6XGE_DEMO",
            .id     = HWP_RTL9311_6x8218D_6XGE_DEMO,
    },
    [1] = { 
            .broadmodel = "RTL9311_6x8218E_6XGE_DEMO",
            .id     = HWP_RTL9311_6x8218E_6XGE_DEMO,
    },
    [2] = { 
            .broadmodel = "RTL9301_3x8218D_4XGE",
...
hwp_probe(void)
{
    ...
    // Starry@20210622 suspend gpio find hwp -S
#if defined(__BOOTLOADER__) 
    { // if hwp file is wrong, loader have lock, can use command:loady upgrade nor flash 
      int inputc=0;
      char env_str[] = "5";
      udelay(400000);
      if (tstc()) {
        inputc = getc();
        if (inputc == 0x16)  { /* ctrl + v*/
          RT_INIT_MSG("\n  key another(ctrl+x)...");
          udelay(2000000);
          if (tstc()) {
            inputc = getc();
            if (inputc == 0x18)  { /*ctrl + x*/
              setenv ("bootdelay", env_str);
              hwp_probeFailure_handler();
              return RT_ERR_FAILED;
            }
          }
    ...
    // Starry@20210622 suspend gpio find hwp -E
 ...
 *      hwp_reprobe
 * Description:
 *      Re-Probe what hardware profile this board belongs to
 * Input:
 *      None
 * Output:
 *      None
...
hwp_reprobe(void)
{
    ...
    forcedhwp.type = HWP_ID_TYPE_STRING;
    for ( boardCnt = 0; boardCnt < BROADMODEL_MAPPING_MAX_NUM; boardCnt++ )
    {
        if ( board_model && (  0 == strcmp( board_model, hwp_broadmodel_id_mapping[boardCnt].broadmodel ) ) )
        {
            forcedhwp.id = hwp_broadmodel_id_mapping[boardCnt].id;
            osal_memcpy(forcedhwp.name, board_model, HWP_PROFILE_NAME_LEN_MAX);
        }
    }
    hwp_forceProfile(&forcedhwp);
    RT_INIT_MSG("  Hardware-profile forcedhwp ");
    RT_INIT_MSG("\n");
